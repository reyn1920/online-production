...
try:
    ...
    from backend.content.animate_avatar import (AnimateAvatar, AnimationConfig,
                                                AnimationQuality)
    from backend.services.avatar_engines import (AvatarEngineManager, AvatarRequest,
                                                 AvatarResponse)
...
class VoiceStyleManager:
    ...
    def get_voice_settings(
        self, style: str, accent: str, emotion: str
    ) -> Dict[str, Any]:
        ...
        accent_mods = self.accent_modifiers.get(
            accent, self.accent_modifiers["neutral"]
        )
    ...
    async def generate_avatar_from_paste(
        ...
        try:
            ...
            if config.custom_template:
                template_config = self.template_manager.get_template(
    ...
    async def batch_generate_paste_avatars():
        ...
        try:
            ...
            config = PasteAvatarConfig(
                ...
                quality=data.get(
                    "quality", "medium"
                ),  # Use medium for batch to save time
            )
