...
class ProductionDeployment:
    ...
    def validate_environment(self) -> bool:
        ...
        if os.getenv('ENVIRONMENT') != 'production':
            self.log_step("ENVIRONMENT must be set to 'production' for production deployment", "ERROR")
            return False
        
        if os.getenv('NODE_ENV') != 'production':
            self.log_step("NODE_ENV must be set to 'production' for production deployment", "ERROR")
            return False
        ...
        try:
            ...
            if current_branch != 'main':
                self.log_step(f"Production deployment must be from 'main' branch, currently on '{current_branch}'", "ERROR")
                return False
    ...
    def run_security_checks(self) -> bool:
        ...
        # Run Base44 Guard
        try:
            subprocess.run(['./tools/run_base44_guard.zsh'], check=True, cwd=self.project_root)
        ...
        try:
            ...
            for line in result.stdout.lower().split('\n'):
                for pattern in secret_patterns:
                    if pattern in line and not any(safe in line for safe in ['remove', 'fix', 'clean']):
                        self.log_step(f"Potential secret detected in recent commit: {line}", "WARNING")
        except subprocess.CalledProcessError:
    ...
    def build_application(self) -> bool:
        ...
        try:
            ...
            subprocess.run(['npm', 'install', '--production'], check=True, cwd=self.project_root)
            ...
            for file in essential_files:
                if not (dist_path / file).exists():
                    self.log_step(f"Essential file '{file}' not found in build output", "ERROR")
                    return False
    ...
    def deploy_to_netlify(self) -> Optional[str]:
        ...
        try:
            ...
            cmd = [
                ...
                '--prod',
                '--dir', 'dist',
                '--message', f'Production deployment {self.start_time.isoformat()}'
            ]
            ...
            if deploy_url:
                ...
                self.log_step("Deployment completed but URL not found in output", "WARNING")
    ...
    def run_health_checks(self, base_url: str) -> bool:
        ...
        for endpoint in self.health_endpoints:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.log_step(f"Health check failed: {endpoint} (status: {response.status_code})", "WARNING")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log_step(f"Main page check failed (status: {response.status_code})", "ERROR")
        ...
        if failed_checks:
            ...
            return len(failed_checks) < len(self.health_endpoints) / 2  # Allow some failures
    ...
    def create_deployment_report(self, deploy_url: Optional[str], success: bool):
        ...
        # Save report
        report_path = self.project_root / 'deployment_reports' / f"deployment_{int(self.start_time.timestamp())}.json"
    ...
    def deploy(self) -> bool:
        ...
        try:
            ...
            if not self.validate_environment():
                self.log_step("Environment validation failed - deployment aborted", "ERROR")
                return False
            ...
            if not self.run_health_checks(deploy_url):
                self.log_step("Health checks failed - deployment may have issues", "WARNING")
            ...
            self.log_step(f"Production deployment completed successfully: {deploy_url}", "SUCCESS")
            return True
