...
class EnvironmentValidator:
            ...
            if result.returncode == 0:
                ...
                if version_parts >= required_parts:
                    ...
                    config_result = subprocess.run(
                        ['npm', 'config', 'list'], capture_output=True, text=True, timeout=10)
    ...
    def _validate_netlify_cli(self):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                auth_result = subprocess.run(['netlify', 'status'],
                                             capture_output=True, text=True, timeout=10)
    ...
    def _validate_environment_variables(self):
        ...
        try:
            ...
            if len(present_vars) >= len(required_vars) // 2:  # At least half present
                status = ValidationStatus.PASSED if not missing_vars else ValidationStatus.WARNING
                message = "Environment variables configured" if not missing_vars else f"Some environment variables missing: {
                    ', '.join(missing_vars)}"
    ...
    def fix_issues(self, auto_fix: bool = False) -> Dict[str, any]:
        ...
        for result in self.results:
            if result.status in [ValidationStatus.FAILED,
                                 ValidationStatus.WARNING] and result.fix_command:
                try:
                    if auto_fix:
                    ...
                    logger.error(f"Failed to fix {result.name}: {str(e)}")
        ...
        return {
            ...
            "manual_fixes_required": len([r for r in self.results if r.fix_command and not auto_fix])
        }
