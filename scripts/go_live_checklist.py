...
class GoLiveValidator:
        ...
        if workflow_path.exists():
            ...
            for tool in security_tools:
                if tool.lower() in workflow_content.lower():
                    self.add_check(f"Security Tool: {tool}", "PASS", f"Security tool configured: {tool}")
                else:
                    self.add_check(f"Security Tool: {tool}", "WARN", f"Security tool not found: {tool}", critical=False)
        ...
        if package_json_path.exists():
            ...
            for script in required_scripts:
                if script in package_data.get('scripts', {}):
                    self.add_check(f"NPM Script: {script}", "PASS", f"Required script configured: {script}")
                else:
                    self.add_check(f"NPM Script: {script}", "FAIL", f"Required script missing: {script}")
                    build_ok = False
        else:
    ...
    def check_health_monitoring(self) -> bool:
        ...
        for endpoint in health_endpoints:
            ...
            if endpoint_path.exists():
                self.add_check(f"Health Endpoint: {endpoint}", "PASS", f"Health monitoring component exists: {endpoint}")
            else:
                self.add_check(f"Health Endpoint: {endpoint}", "WARN", f"Health monitoring component missing: {endpoint}", critical=False)
    ...
    def check_secrets_management(self) -> bool:
        ...
        try:
            ...
            result = subprocess.run(['grep', '-r', '-i', '--include=*.py', 
                                   '|'.join(dangerous_patterns), str(self.project_root)],
            ...
            if result.returncode == 0 and result.stdout.strip():
                ...
                for file_path in secret_files:
                    self.add_check(f"Secret Scan: {file_path}", "WARN", 
                                 f"Potential hardcoded secrets detected in: {file_path}", critical=False)
            else:
    ...
    def check_deployment_readiness(self) -> bool:
        ...
        try:
            ...
            if current_branch == 'main':
                ...
                self.add_check("Git Branch", "FAIL", f"Not on main branch (currently on: {current_branch})")
                readiness_ok = False
        except subprocess.CalledProcessError:
    ...
    def run_all_checks(self) -> Dict[str, Any]:
        ...
        critical_failures = [check for check in self.checks 
                           if check['status'] == 'FAIL' and check['critical']]
        ...
        warnings = [check for check in self.checks 
