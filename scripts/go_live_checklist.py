...
class GoLiveValidator:
    ...
    def check_environment_setup(self) -> bool:
        ...
        for file in required_files:
            ...
            if file_path.exists():
                ...
                self.add_check(
                    f"File: {file}", "FAIL", f"Required file missing: {file}"
                )
        ...
        for var in required_env_vars:
            if os.getenv(var):
                self.add_check(
                    f"Env: {var}", "PASS", f"Environment variable set: {var}"
                )
            else:
                self.add_check(
                    f"Env: {var}", "FAIL", f"Environment variable missing: {var}"
                )
                env_vars_set = False
    ...
    def check_security_configuration(self) -> bool:
        ...
        for file in security_files:
            ...
            if file_path.exists():
                self.add_check(
                    f"Security: {file}", "PASS", f"Security component exists: {file}"
                )
            else:
                self.add_check(
                    f"Security: {file}", "FAIL", f"Security component missing: {file}"
                )
                security_ok = False
    ...
    def check_ci_cd_pipeline(self) -> bool:
        ...
        if "workflow_dispatch" in workflow_content:
            self.add_check(
                "Manual Deployment", "PASS", "Manual deployment trigger configured"
            )
        else:
            self.add_check(
                "Manual Deployment", "FAIL", "Manual deployment trigger missing"
            )
            pipeline_ok = False
    ...
    def check_build_system(self) -> bool:
        ...
        if netlify_config_path.exists():
            self.add_check(
                "Netlify Config", "PASS", "Netlify configuration file exists"
            )
        else:
            self.add_check(
                "Netlify Config", "FAIL", "Netlify configuration file missing"
            )
            build_ok = False
    ...
    def check_secrets_management(self) -> bool:
        ...
        if gitignore_path.exists():
            ...
            if ".env" in gitignore_content:
                self.add_check(
                    "Gitignore .env", "PASS", ".env files are ignored by git"
                )
            else:
                self.add_check(
                    "Gitignore .env", "FAIL", ".env files not ignored by git"
                )
                secrets_ok = False
    ...
    def check_deployment_readiness(self) -> bool:
        ...
        if deploy_script_path.exists():
            ...
            self.add_check(
                "Deployment Script", "FAIL", "Production deployment script missing"
            )
            readiness_ok = False
