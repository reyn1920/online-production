...
class SecretsCLI:
    ...
    def delete_secret(self, key_name: str, confirm: bool = False) -> None:
        ...
        try:
            if not confirm:
                response = input(f"Are you sure you want to delete secret '{key_name}'? (y/N): ")
    ...
    def export_secrets(self, export_path: str, format_type: str = 'json') -> None:
        ...
        try:
            with self._get_secret_store() as store:
                ...
                print(f"⚠️  WARNING: Exported file contains unencrypted secrets. Handle with care!")
    ...
    def import_secrets(self, import_path: str, format_type: str = 'json', overwrite: bool = False) -> None:
        """
        ...
        try:
            ...
            with self._get_secret_store() as store:
                ...
                print(f"✓ Import completed: {imported_count} secrets imported, {skipped_count} skipped")
        ...
        while True:
            try:
                ...
                if cmd in ['quit', 'exit', 'q']:
                    ...
                    keys_input = input("Enter secret keys to delete (comma-separated): ")
...
Environment Variables:
  ...
  TRAE_SECRETS_DB: Path to secrets database (optional)
    ...
    add_parser.add_argument('secret_value', nargs='?', help='Secret value (will prompt if not provided)')
    ...
    get_parser.add_argument('--hide-value', action='store_true', help='Hide the secret value in output')
    ...
    # Batch add command
    batch_add_parser = subparsers.add_parser('batch-add', help='Add multiple secrets from JSON file')
    batch_add_parser.add_argument('secrets_file', help='JSON file containing secrets')
    
    # Batch delete command
    batch_delete_parser = subparsers.add_parser('batch-delete', help='Delete multiple secrets')
    batch_delete_parser.add_argument('key_names', nargs='+', help='Secret keys to delete')
    ...
    config_parser.add_argument('value', nargs='?', help='Value to set (omit to show current value)')
