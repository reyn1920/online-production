...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/load-test.log'),
    ...
    async def run(self) -> Dict[str, Any]:
        ...
        async with aiohttp.ClientSession(connector=connector) as session:
            ...
            for user_id in range(self.config.concurrent_users):
                ...
                delay = (user_id * self.config.ramp_up_seconds) / \
                    self.config.concurrent_users
                task = asyncio.create_task(
                    self.delayed_user_simulation(
                        user_id, session, delay))
                tasks.append(task)
    ...
    async def user_simulation(self, user_id: int, session: aiohttp.ClientSession):
        ...
        content_requests = [
            ...
            },
            {
                ...
                'data': {
                    'campaign_type': 'email',
                    'target_audience': random.choice(['developers', 'marketers', 'executives']),
...
class SpikeLoadTest(LoadTestScenario):
    ...
    async def user_simulation(self, user_id: int, session: aiohttp.ClientSession):
        ...
        recovery_duration = self.config.duration_seconds - normal_duration - spike_duration

        phases = [
            ('normal', normal_duration, self.config.request_delay_ms),
            ('spike', spike_duration, self.config.request_delay_ms // 5),  # 5x more requests
        ...
        for phase_name, duration, delay in phases:
            ...
            while time.time() < phase_end:
                ...
                if endpoint == '/api/content/generate':
                    result = await self.make_request(session, endpoint, 'POST', {
                        'type': 'quick_content',
...
class ScalingValidationTest:
    ...
    def analyze_scaling_events(self, baseline: Dict, post_test: Dict) -> Dict[str, Any]:
        ...
        if 'system_cpu_usage_percent' in baseline and 'system_cpu_usage_percent' in post_test:
            ...
            if baseline_cpu and post_cpu:
                try:
                    baseline_val = float(
                        baseline_cpu[0]['value'][1]) if baseline_cpu else 0
