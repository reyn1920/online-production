...
class NewsDrivenContentTrigger:
    ...
    def __init__(self, config_path: str = "rss_feeds_example.json"):
        ...
        self.political_keywords = [
            'biden', 'trump', 'congress', 'senate', 'house', 'election', 'vote', 'voting',
            'democrat', 'republican', 'politics', 'political', 'government', 'federal',
            'supreme court', 'legislation', 'bill', 'law', 'policy', 'campaign',
            'scandal', 'investigation', 'impeachment', 'corruption', 'controversy',
            'debate', 'primary', 'caucus', 'poll', 'polling', 'approval rating'
        ]
    ...
    def _calculate_urgency_score(self, articles: List[NewsArticle], trend_data: Optional[TrendData]) -> float:
        ...
        now = datetime.now()
        avg_age_hours = sum([(now - article.published).total_seconds() / 3600 for article in articles]) / len(articles)
        urgency_factors['recency'] = max(0, 1 - (avg_age_hours / 24))  # Decay over 24 hours
        ...
        avg_sentiment = sum([abs(article.sentiment_score) for article in articles]) / len(articles)
        ...
        total_content = " ".join([f"{article.title} {article.content}" for article in articles]).lower()
        for keyword in self.political_keywords:
            if keyword in total_content:
                political_keyword_count += 1
        
        urgency_factors['political_relevance'] = min(1.0, political_keyword_count / len(self.political_keywords))
    ...
    def _create_content_trigger(self, articles: List[NewsArticle], trend_data: Optional[TrendData]) -> ContentTrigger:
    ...
    def _store_trigger(self, trigger: ContentTrigger) -> bool:
        ...
        try:
            ...
            logger.info(f"Content trigger stored: {trigger.trigger_id} - {trigger.topic}")
            return True
    ...
    async def _trigger_content_creation(self, trigger: ContentTrigger) -> bool:
        ...
        try:
            ...
            plan = await planner._create_protected_right_perspective_plan(
                ...
                priority=planner.TaskPriority.HIGH
            )
            ...
            if result.get('success', False):
                ...
                logger.info(f"Content creation successful for trigger: {trigger.trigger_id}")
                return True
            else:
                logger.error(f"Content creation failed for trigger: {trigger.trigger_id}")
                return False
    ...
    async def _monitor_political_news(self):
        ...
        try:
            ...
            for feed_config in self.rss_engine.feeds:
                if feed_config.get('category') == 'politics' and feed_config.get('active', True):
            ...
            for topic, articles in topic_groups.items():
                if len(articles) >= 2:  # Require at least 2 articles for a trigger
                    ...
                    if trigger.urgency_score >= self.urgency_threshold:
                        ...
                        logger.info(f"Topic '{topic}' urgency score {trigger.urgency_score:.3f} below threshold {self.urgency_threshold}")
    ...
    def _group_articles_by_topic(self, articles: List[NewsArticle]) -> Dict[str, List[NewsArticle]]:
        ...
        for article in articles:
            ...
            if article.keywords:
                ...
                primary_keyword = next((word for word in title_words if len(word) > 4), "general")
    ...
    async def run_continuous_monitoring(self):
        ...
        self.running = True
        logger.info(f"Starting continuous political news monitoring (interval: {self.monitoring_interval} minutes)")
