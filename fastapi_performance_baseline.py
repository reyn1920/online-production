...
class MacBookAirM1Benchmark:
    ...
    def check_hey_installation(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                print("❌ Failed to install hey. Please install manually: brew install hey")
                return False
    
    def run_hey_benchmark(self, endpoint: str, duration: str = "30s", concurrency: int = 50) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                return {
                    ...
                    "success_rate": (1 - data.get("summary", {}).get("errorRate", 0)) * 100,
    ...
    async def async_benchmark(self, endpoint: str, concurrent_users: int = 50, duration_seconds: int = 30) -> Dict[str, Any]:
    ...
    def analyze_async_results(self, results: List[Dict], duration: float, endpoint: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "p95_latency_ms": round(statistics.quantiles(durations, n=20)[18] * 1000, 2) if len(durations) > 20 else 0,
            "p99_latency_ms": round(statistics.quantiles(durations, n=100)[98] * 1000, 2) if len(durations) > 100 else 0,
    ...
    def check_nginx_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            checks = {
                "worker_processes_2": "worker_processes 2" in config or "worker_processes auto" in config,
    ...
    def print_results(self, results: Dict[str, Any]):
        ...
        print(f"Successful:         {results.get('successful_requests', results.get('total_requests', 0)):,}")
    ...
    def print_performance_analysis(self, results: List[Dict[str, Any]]):
        ...
        for result in results:
            ...
            if rps >= rps_target['target']:
                print(f"  ✅ RPS: {rps:,.0f} (Excellent - above {rps_target['target']:,} target)")
            elif rps >= rps_target['min']:
                print(f"  ⚠️  RPS: {rps:,.0f} (Good - above {rps_target['min']:,} minimum)")
            else:
            ...
            if p95 <= latency_target['target']:
                print(f"  ✅ Latency: {p95:.1f}ms (Excellent - under {latency_target['target']}ms target)")
            elif p95 <= latency_target['max']:
                print(f"  ⚠️  Latency: {p95:.1f}ms (Acceptable - under {latency_target['max']}ms max)")
            else:
                print(f"  ❌ Latency: {p95:.1f}ms (Above {latency_target['max']}ms maximum)")
