...
def _cleanup_expired_states():
    ...
    expired_keys = [k for k, v in OAUTH_STATES.items() if current_time - v.get("created_at", 0) > 600]
    ...
    try:
        ...
        if token_response.status_code == 200:
            ...
            if token_info.get("data"):
                ...
                logger.info(f"TikTok OAuth successful for user: {token_info['data'].get('open_id')}")
                ...
                redirect_uri = state_data.get("redirect_uri", f"{base_url}/oauth/success")
            ...
            logger.error(f"TikTok token request failed: {token_response.status_code} - {token_response.text}")
...
async def instagram_oauth_callback(
    ...
    try:
        ...
        if token_response.status_code == 200:
            ...
            logger.error(f"Instagram token request failed: {token_response.status_code} - {token_response.text}")
            raise HTTPException(status_code=400, detail="Failed to exchange code for token")
...
async def oauth_status():
    ...
    status = {
        "tiktok": {
            "configured": bool(os.getenv("TIKTOK_CLIENT_ID") and os.getenv("TIKTOK_CLIENT_SECRET")),
            "client_id_set": bool(os.getenv("TIKTOK_CLIENT_ID")),
            "client_secret_set": bool(os.getenv("TIKTOK_CLIENT_SECRET"))
        },
        "instagram": {
            "configured": bool(os.getenv("INSTAGRAM_CLIENT_ID") and os.getenv("INSTAGRAM_CLIENT_SECRET")),
