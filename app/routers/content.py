...
try:
    from content_agent.main import ContentAgent, ContentConfig, ContentRequest, ContentResponse
except ImportError:
    try:
...
try:
    from backend.agents.specialized_agents import ContentAgent as SpecializedContentAgent
except ImportError:
...
async def content_interface(request: Request):
    ...
    return templates.TemplateResponse("content.html", {
        ...
        "content_types": [
            "article", "blog_post", "social_media", "email",
            "video_script", "product_description", "newsletter"
        ]
    })
...
async def get_trending_topics():
    ...
    if not agent or not hasattr(agent, 'news_watcher'):
        ...
        fallback_topics = [
            {"title": "AI and Machine Learning Trends", "category": "technology", "score": 0.9},
            {"title": "Sustainable Business Practices", "category": "business", "score": 0.8},
            {"title": "Remote Work Productivity", "category": "workplace", "score": 0.7},
            {"title": "Digital Marketing Strategies", "category": "marketing", "score": 0.6},
            {"title": "Health and Wellness Tips", "category": "lifestyle", "score": 0.5}
        ]
...
async def download_content(job_id: str):
    ...
    job_info = content_jobs[job_id]
    if job_info["status"] != "completed" or not job_info.get(
                ...
                "content": f"Generated {
                    ...
                    request.tone} tone. The content would be approximately {
                        request.target_length} words long and tailored for {
                            request.audience} audience.", "title": f"{
                                request.topic} - {
                                    request.type.title()}", "metadata": {
                    "word_count": request.target_length, "tone": request.tone, "type": request.type}}
