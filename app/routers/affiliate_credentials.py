...
class AffiliateCredentialsService:
    ...
    def _get_encryption_key(self) -> Optional[bytes]:
        ...
        logger.warning(
            "Using default encryption key - set AFFILIATE_ENCRYPTION_KEY in production"
        )
    ...
    def _get_program_category(self, platform_name: str) -> str:
        ...
        if any(keyword in platform_lower for keyword in ["clickbank", "cb"]):
            ...
            keyword in platform_lower for keyword in ["finance", "trading", "crypto"]
        ):
            return "Finance"
        else:
            return "General"
    ...
    def _calculate_stats(self, credentials: List[Dict]) -> Dict[str, Any]:
        ...
        if total > 0:
            ...
            inactive_ratio = (
                sum(1 for c in credentials if c["status"] == "inactive") / total
            )
    ...
    def delete_credential(self, credential_id: str) -> Dict[str, Any]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute(
                    "DELETE FROM affiliate_dashboards WHERE id = ?", (credential_id,)
                )
                conn.commit()
