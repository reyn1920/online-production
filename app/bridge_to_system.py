...
try:
    ...
    from backend.agents.base_agents import BaseAgent, PlannerAgent, ExecutorAgent, AuditorAgent, AgentCapability
    from backend.agents.specialized_agents import SystemAgent, ResearchAgent, ContentAgent, MarketingAgent, QAAgent
    from utils.logger import get_logger, TraeLogger
...
class SystemBridge:
    ...
    def _load_monetization_settings(self):
        ...
        try:
            ...
            try:
                ...
                ''')
                cursor.execute('SELECT feature_name, enabled FROM monetization_settings')
                ...
                self.logger.info(f"Loaded {len(results)} monetization settings from database")
            except Exception as db_error:
                self.logger.warning(f"Could not load from database, using defaults: {db_error}")
    ...
    def trigger_workflow(self, workflow_type: str, parameters: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Triggered {workflow_type} workflow with task ID {task_id}")
    ...
    def toggle_monetization_feature(self, feature: str, enabled: bool) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                ''')
                cursor.execute('''
                    INSERT OR REPLACE INTO monetization_settings (feature_name, enabled, updated_at)
                    VALUES (?, ?, ?)
                ''', (feature, enabled, datetime.now(timezone.utc).isoformat()))
                conn.commit()
                conn.close()
                self.logger.info(f"Monetization feature {feature} persisted to database")
            except Exception as db_error:
                self.logger.error(f"Failed to persist monetization setting: {db_error}")
            
            # Log the change
            self.logger.info(f"Monetization feature {feature} {'enabled' if enabled else 'disabled'}")
    ...
    def get_channel_status(self) -> Dict[str, ChannelMetrics]:
        ...
        try:
            ...
            try:
                ...
                ''')
                cursor.execute('SELECT platform, status, followers, content_count, engagement_rate, last_updated FROM channels')
                results = cursor.fetchall()
                conn.close()
                ...
                for row in results:
                    channels[row[0].lower()] = ChannelMetrics(
                        ...
                        last_updated=datetime.fromisoformat(row[5]) if row[5] else datetime.now(timezone.utc)
                    )
                    
            except Exception as db_error:
                self.logger.warning(f"Could not fetch channel data from database: {db_error}")
    ...
    def update_channel_metrics(self, platform: str, metrics: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            cursor.execute('''
                ...
                (platform, status, followers, content_count, engagement_rate, last_updated)
                VALUES (?, ?, ?, ?, ?, ?)
            ''', (platform, metrics.get('status', 'active'), 
                  metrics.get('followers', 0), metrics.get('content_count', 0),
                  metrics.get('engagement_rate', 0.0), datetime.now(timezone.utc).isoformat()))
            conn.commit()
            conn.close()
    ...
    def _get_system_health_score(self) -> float:
        ...
        try:
            ...
            component_score = sum(health.components.values()) / len(health.components) * 100
        ...
        try:
            if self.task_manager:
                self.task_manager.add_task(
                    ...
                    payload={'description': description, 'timestamp': datetime.now(timezone.utc).isoformat()},
    ...
    def _get_revenue_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if results:
                ...
                self.logger.info(f"Retrieved revenue data from database: {len(results)} sources")
                return revenue_data
    ...
    def _get_monetization_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if results:
                ...
                self.logger.info(f"Retrieved performance metrics from database: {len(results)} metrics")
                return performance_data
                
        except Exception as e:
            self.logger.warning(f"Could not fetch performance metrics from database: {e}")
        
        # Fallback to calculated/placeholder data
        revenue_data = self._get_revenue_data()
        total_revenue = sum(v for k, v in revenue_data.items() if isinstance(v, (int, float)))
