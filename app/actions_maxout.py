...
@dashboard_action(name="Synthesize bundles v3",
                  doc="Add-only ingest of incoming artifacts")
    ...
    try:
        ...
        if latest.exists():
            ...
            if dirs and (dirs[-1] / "synthesis_manifest.json").exists():
                manifest = json.loads(
                    (dirs[-1] / "synthesis_manifest.json").read_text())
...
@dashboard_action(name="Get release manifest",
                  doc="Returns latest synthesis manifest",
                  method="POST")
