...
@dashboard_action(name="Get release manifest", doc="Returns latest synthesis manifest", method="POST")
def get_release_manifest(payload: Dict[str, Any]|None=None) -> Dict[str,Any]:
    try:
        ...
        if not latest.exists(): return {"status":"ok","manifest":{}}
        dirs = sorted([d for d in latest.iterdir() if d.is_dir()])
        if not dirs: return {"status":"ok","manifest":{}}
