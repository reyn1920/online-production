        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                if not cursor.fetchone():
                    logger.info(
                        "Cloud software table not found. Creating from schema..."
                    )
        ...
        for software in software_list:
            if software.status == "active":
                ...
                self.update_software_status(
                    software.software_name, software.status, health_status
                )
...
def main():
    ...
    # Save report to file
    report_path = (
        f"cloud_software_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
