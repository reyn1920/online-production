...
class APIConfigManager:
    ...
    def validate_api_key(self, api_key: str) -> Dict[str, Any]:
        ...
        if not env_value:
            validation_result["warnings"].append(
                f"Environment variable {config.env_var} not set"
            )
        elif env_value != self.get_api_key(api_key):
            validation_result["warnings"].append(
                "Environment variable differs from stored value"
            )
    ...
    def deploy_to_environment(
        ...
        try:
            ...
            if dry_run:
                ...
                logger.info(
                    f"Deployed configuration to {environment}: {env_config.config_file}"
                )
        ...
        for api_key, config in self.api_configs.items():
            ...
            if not include_secrets:
                config_data["current_value"] = (
                    "***REDACTED***" if config.current_value else None
                )
    ...
    def generate_status_report(self) -> Dict[str, Any]:
        ...
        for env_name in self.environments.keys():
            deployed_apis = [
                c
                for c in self.api_configs.values()
                if env_name in c.deployment_environments
            ]
            env_stats[env_name] = {
    ...
    def show_status_report(self):
        ...
        for env, stats in report["environment_deployment"].items():
            print(
                f"  {env}: {stats['configured_apis']}/{stats['total_apis']} configured"
            )
    ...
    def interactive_export_configs(self):
        ...
        try:
            ...
            filename = (
                f"api_configs_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
