...
class ProcessMonitor:
    ...
    def detect_initialization_loop(self, process_name: str, log_patterns: List[str]) -> bool:
            ...
            for proc in psutil.process_iter(['pid', 'name', 'cmdline', 'create_time']):
                try:
                    if process_name in ' '.join(proc.info['cmdline'] or []):
                        ...
                        while (self.process_history[process_name] and 
                               self.process_history[process_name][0] < cutoff_time):
                        ...
                        if len(self.process_history[process_name]) >= 3:
                            logger.warning(f"🔄 Initialization loop detected for {process_name}")
                            logger.warning(f"Process restarted {len(self.process_history[process_name])} times in {self.loop_detection_window}s")
                            return True
    ...
    def kill_stuck_process(self, process_name: str) -> bool:
        ...
        try:
            ...
            for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
                try:
                    if process_name in ' '.join(proc.info['cmdline'] or []):
                        logger.info(f"🔪 Killing stuck process: {process_name} (PID: {proc.info['pid']})")
                        proc.kill()
                        killed = True
                except (psutil.NoSuchProcess, psutil.AccessDenied):
                    continue
...
class SelfHealingSystem:
    ...
    async def perform_healing_check(self) -> Dict[str, Any]:
        ...
        if self.process_monitor.detect_initialization_loop('main.py', ['Agent', 'initialized']):
            healing_report['issues_detected'].append('initialization_loop_main_py')
            
            # Apply healing if cooldown has passed
            if (current_time - self.last_healing_action.get('kill_stuck_main_server', 0) > 
        ...
        all_unhealthy = all(not result.get('healthy', False) for result in service_results.values())
        
        if all_unhealthy:
            healing_report['issues_detected'].append('all_services_unhealthy')
            
            # Start emergency server if cooldown has passed
            if (current_time - self.last_healing_action.get('start_emergency_server', 0) > 
                self.healing_cooldown):
