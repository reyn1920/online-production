...
class WebAIClient:
    ...
    async def chat_completion(self, 
                            messages: List[Dict], 
                            platform: WebAIPlatform,
                            model: str = None,
                            **kwargs) -> WebAIResponse:
        ...
        try:
            ...
            response_content = await self._send_web_request(
                platform, prompt, session_id
            )
    ...
    async def _send_web_request(self, 
                              platform: WebAIPlatform, 
                              prompt: str, 
                              session_id: str) -> str:
        ...
        try:
            ...
            if session_id not in puppeteer.sessions:
                await puppeteer.navigate_to_platform(
                    platform.value, 
                    config['url']
                )
            
            # Send message and get response
            response = await puppeteer.send_message(
                session_id, 
                prompt, 
                platform.value
            )
