...
class RouteLL_Client:
    ...
    def _estimate_credit_cost(self, messages: List[Dict], model: str = None) -> int:
        ...
        if any(
            unlimited_model.lower() in model.lower()
            for unlimited_model in unlimited_models
        ):
            return 0
        ...
        if any(
            ...
            for high_cost_model in high_cost_models
        ):
        ...
        try:
            ...
            if stream:
                return self._handle_streaming_response(
                    url, payload, estimated_cost, start_time
                )
            else:
    ...
    def optimize_for_credits(self) -> Dict:
        ...
        if self.credit_usage.warning_level in ["warning", "critical"]:
            ...
            recommendations.append(
                "Enable response caching to avoid duplicate requests"
            )

        unlimited_models = self.config["credit_system"]["unlimited_models"]
        recommendations.append(
            f"Available unlimited models: {', '.join(unlimited_models)}"
        )
