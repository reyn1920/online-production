...
class RouteLL_Client:
    ...
    def _setup_session(self):
        ...
        if not self.api_key:
            raise ValueError(f"API key not found in environment variable: {self.config['api_settings']['api_key_env']}")
    ...
    def check_credits(self) -> CreditUsage:
        ...
        try:
            ...
            usage_percentage = (self.credit_usage.used_credits / self.credit_usage.total_credits) * 100
            ...
            if usage_percentage >= 95:
                ...
                if self.status in [RouteLL_Status.CREDITS_LOW, RouteLL_Status.CREDITS_EXHAUSTED]:
                    self.status = RouteLL_Status.ACTIVE
            
            self.logger.info(f"Credit check: {self.credit_usage.remaining_credits} remaining ({usage_percentage:.1f}% used)")
    ...
    def _can_make_request(self, estimated_cost: int) -> bool:
        ...
        if self.credit_usage.daily_usage + estimated_cost > daily_limit:
            self.logger.warning(f"Daily usage limit would be exceeded: {self.credit_usage.daily_usage + estimated_cost} > {daily_limit}")
            return False
        
        if self.credit_usage.hourly_usage + estimated_cost > hourly_limit:
            self.logger.warning(f"Hourly usage limit would be exceeded: {self.credit_usage.hourly_usage + estimated_cost} > {hourly_limit}")
            return False
    ...
    def chat_completion(self, 
                      ...
                      model: str = None,
                      stream: bool = False,
                      **kwargs) -> Union[APIResponse, Iterator[APIResponse]]:
    ...
    def _handle_regular_response(self, url: str, payload: Dict, estimated_cost: int, start_time: float) -> APIResponse:
        ...
        if response.status_code == 200:
            ...
            self.logger.info(f"Successful request: {estimated_cost} credits used, {response_time_ms}ms")
    ...
    def _handle_streaming_response(self, url: str, payload: Dict, estimated_cost: int, start_time: float) -> Iterator[APIResponse]:
    ...
    def optimize_for_credits(self) -> Dict:
        ...
        return {
            "current_usage_percentage": (self.credit_usage.used_credits / self.credit_usage.total_credits) * 100,
