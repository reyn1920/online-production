...
from content_automation_pipeline import ContentAutomationPipeline, ContentPriority, ContentFormat
...
class AutomationController:
        ...
        try:
            ...
            if self.config['content_pipeline_enabled']:
                self.content_thread = threading.Thread(
                    target=self._run_content_pipeline,
                    daemon=True
                )
    ...
    def _collect_performance_metrics(self):
        ...
        try:
            ...
            if 'pending_opportunities' in pipeline_status:
                cursor.execute(
                    "INSERT INTO system_metrics (component, metric_name, metric_value) VALUES (?, ?, ?)",
                    ('content_pipeline', 'pending_opportunities', pipeline_status['pending_opportunities'])
                )
            
            if 'recent_projects_24h' in pipeline_status:
                cursor.execute(
                    "INSERT INTO system_metrics (component, metric_name, metric_value) VALUES (?, ?, ?)",
                    ('content_pipeline', 'projects_24h', pipeline_status['recent_projects_24h'])
                )
    ...
    def _log_error(self, component: str, error_type: str, message: str, stack_trace: str = None):
        ...
        try:
            ...
            cursor.execute("""
                INSERT INTO error_log (component, error_type, error_message, stack_trace)
                VALUES (?, ?, ?, ?)
    ...
    def get_automation_status(self) -> AutomationStatus:
        ...
        try:
            ...
            return AutomationStatus(
                ...
                rss_monitoring_active=hasattr(self.rss_engine, 'running') and self.rss_engine.running,
        ...
        def start_automation_api():
            ...
            try:
                if not self.running:
                    ...
                    return jsonify({'message': 'Automation already running', 'success': True})
            except Exception as e:
        ...
        def trigger_content_generation():
            ...
            try:
                ...
                for opportunity in opportunities[:limit]:
                    projects = self.content_pipeline.generate_content_from_opportunity(opportunity)
    ...
    def cleanup(self):
        ...
        try:
            ...
            cursor.execute(
                "DELETE FROM system_metrics WHERE timestamp < datetime('now', '-{} days')".format(retention_days)
            )
            cursor.execute(
                "DELETE FROM error_log WHERE timestamp < datetime('now', '-{} days') AND resolved = TRUE".format(retention_days)
            )
