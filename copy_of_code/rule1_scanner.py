...
class Rule1DeepScanner:
    ...
    def __init__(self, 
        ...
        self.supported_extensions = {
            ...
            '.log', '.rst', '.tex', '.csv', '.sql', '.sh', '.bat'
        }
        
        self.logger.info(f"Rule1DeepScanner initialized with {len(self.forbidden_terms)} forbidden terms")
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.scan_results_db) as conn:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_enforcement_file_path ON enforcement_history(file_path)")
            conn.commit()
    ...
    def scan_directory(self, directory: Path, recursive: bool = True, 
                      file_patterns: Optional[List[str]] = None) -> List[ScanResult]:
        ...
        self.logger.info(f"Directory scan completed: {len(results)} files, {total_violations} violations")
        
        return results
    
    def get_scan_history(self, file_path: Optional[str] = None, 
    ...
    def add_forbidden_term(self, term_id: str, category: str, patterns: List[str], 
                          severity: str = "medium", replacement: str = "[CONTENT_REMOVED]",
...
class Rule1Enforcer:
        ...
        try:
            ...
            for violation in scan_result.violations:
                ...
                if term_id in self.scanner.forbidden_terms:
                    replacement = self.scanner.forbidden_terms[term_id].get('replacement', '[CONTENT_REMOVED]')
                    ...
                    if count > 0:
                        ...
                        self.logger.debug(f"Replaced {count} instances of pattern '{pattern}' in {file_path}")
    ...
    def enforce_directory(self, directory: Path, recursive: bool = True, 
                         file_patterns: Optional[List[str]] = None,
                         create_backups: bool = True) -> List[EnforcementResult]:
        ...
        self.logger.info(f"Directory enforcement completed: {len(results)} files, {total_replacements} replacements")
    ...
    def rollback_file(self, file_path: Path, backup_path: Optional[Path] = None) -> bool:
        ...
        try:
            if backup_path is None:
                ...
                backups = list(self.scanner.backup_dir.glob(f"{file_path.stem}_*{file_path.suffix}.backup"))
                if not backups:
    ...
    def get_enforcement_history(self, file_path: Optional[str] = None, 
                               limit: int = 100) -> List[Dict[str, Any]]:
...
if __name__ == '__main__':
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for violation in scan_result.violations:
            print(f"  - {violation['category']}: {violation['match']} (line {violation['line_number']})")
