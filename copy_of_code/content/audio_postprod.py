...
class FFmpegAudioProcessor:
    ...
    def get_audio_info(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            audio_match = re.search(r"Stream #\d+:\d+.*?: Audio: (\w+).*?, (\d+) Hz, (\w+), .*, (\d+) kb/s", result.stderr)
            if audio_match:
                format_name, sample_rate, channel_layout, bit_rate = audio_match.groups()
    ...
    def apply_effect(self, input_path: str, output_path: str, effect: AudioEffect) -> bool:
    ...
    def mix_tracks(self, tracks: List[AudioTrack], output_path: str, config: AudioConfig) -> bool:
        ...
        try:
            ...
            for i, track in enumerate(tracks):
                ...
                # Pan adjustment
                if track.pan != 0.0:
                    track_filters.append(f"pan=stereo|c0={1-abs(track.pan)}*c0+{max(0,-track.pan)}*c1|c1={1-abs(track.pan)}*c1+{max(0,track.pan)}*c0")
                ...
                if track_filters:
                    filter_parts.append(f"[{input_index}:a]{','.join(track_filters)}[a{i}]")
                    input_index += 1
                else:
            ...
            active_tracks = [i for i, track in enumerate(tracks) if not track.mute and Path(track.source_path).exists()]
            if len(active_tracks) > 1:
                mix_inputs = ''.join([f"[a{i}]" for i in active_tracks])
                filter_parts.append(f"{mix_inputs}amix=inputs={len(active_tracks)}:duration=longest[mixed]")
                output_label = "[mixed]"
...
class AudioDucker:
    ...
    def apply_ducking(self, main_audio: str, trigger_audio: str, 
                     output_path: str, config: DuckingConfig) -> bool:
        ...
        try:
            ...
            cmd = [
                ...
                "-i", main_audio,    # Main audio to be ducked
                "-i", trigger_audio, # Trigger audio (e.g., voice)
...
class AudioPostProduction:
    ...
    def create_audio_job(self, tracks: List[AudioTrack], output_path: str,
                        job_id: Optional[str] = None,
                        config: Optional[AudioConfig] = None,
                        ducking_config: Optional[DuckingConfig] = None,
                        master_effects: Optional[List[AudioEffect]] = None) -> AudioJob:
    ...
    def _process_individual_tracks(self, job: AudioJob) -> List[AudioTrack]:
        ...
        for i, track in enumerate(job.tracks):
            ...
            if track.effects:
                ...
                for j, effect in enumerate(sorted(track.effects, key=lambda x: x.order)):
                    if not effect.enabled:
                        continue
                    
                    temp_path = str(self.temp_dir / f"{job.job_id}_track_{i}_effect_{j}.wav")
                    
                    if self.processor.apply_effect(current_path, temp_path, effect):
                        current_path = temp_path
                    else:
                        self.logger.warning(f"Failed to apply effect {effect.type.value} to track {track.name}")
    ...
    def _apply_ducking_to_tracks(self, job: AudioJob, tracks: List[AudioTrack]) -> List[AudioTrack]:
        ...
        for track in tracks:
            if track.track_type in ["music", "ambient"] and voice_tracks:
                ...
                ducked_path = str(self.temp_dir / f"{job.job_id}_{track.name}_ducked.wav")
    ...
    def _apply_master_effects(self, job: AudioJob, input_path: str) -> str:
        ...
        for i, effect in enumerate(sorted(job.master_effects, key=lambda x: x.order)):
            ...
            if self.processor.apply_effect(current_path, temp_path, effect):
                ...
                self.logger.warning(f"Failed to apply master effect: {effect.type.value}")
    ...
    def _finalize_output(self, job: AudioJob, input_path: str) -> bool:
        ...
        try:
            cmd = [
                self.processor.ffmpeg_path,
                "-i", input_path,
                "-ar", str(job.config.sample_rate),
                "-ac", str(job.config.channels)
            ]
            ...
            if job.config.normalize_loudness:
                cmd.extend(["-af", f"loudnorm=I={job.config.target_lufs}:TP={job.config.peak_limit}:LRA=11.0"])
    ...
    def _estimate_total_duration(self, tracks: List[AudioTrack]) -> float:
        ...
        for track in tracks:
            ...
            try:
                ...
                track_end = track.start_time + (track.duration or info.get("duration", 0))
    ...
    def create_voice_over_mix(self, voice_file: str, music_file: str, 
                             output_path: str, ducking_strength: float = 0.3) -> str:
        """Convenience method to create voice-over mix with ducking."""
        tracks = [
            AudioTrack(
                name="music",
                source_path=music_file,
                track_type="music",
                volume=0.7
            ),
            AudioTrack(
                name="voice",
                source_path=voice_file,
                track_type="voice",
                volume=1.0
            )
        ]
        ...
        master_effects = [
            AudioEffect(
                ...
                parameters={"threshold": -12.0, "ratio": 3.0, "attack": 0.003, "release": 0.1},
                order=1
            ),
            AudioEffect(
