...
def check_prerequisites():
    ...
    if not check_command('brew'):
        logging.error("Homebrew is not installed. It is required to install system libraries.")
        logging.info("Please install Homebrew by running the following command, then run this script again:")
...
def check_system_libraries():
    ...
    if missing_libs:
        ...
        logging.info("Please install them by running this command, then run this script again:")
...
def install_dependencies():
    ...
    logging.info(f"Step 4: Installing creative dependencies from '{REQUIREMENTS_FILE}'...")
    ...
    try:
        ...
        logging.error(f"Failed to install dependencies. pip exited with status {e.returncode}.")
        logging.error(f"Please check the error messages above. You may need to modify '{REQUIREMENTS_FILE}' to resolve version conflicts or remove incompatible packages like 'open3d'.")
        return False
