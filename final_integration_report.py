    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if isinstance(providers, list):
                ...
                active_response = requests.get('http://localhost:8000/integrations/providers/active')
                if active_response.status_code == 200:
                    active_providers = active_response.json()
                    active_count = len(active_providers) if isinstance(active_providers, list) else 0
                    print(f"  Active Providers: {active_count}")
                    print(f"  Activation Rate: {(active_count/len(providers)*100):.1f}%" if len(providers) > 0 else "  Activation Rate: 0%")
                    return True
            else:
...
def check_folder_structure():
    ...
    critical_folders = [
        'backend', 'app', 'frontend', 'assets', 'content', 'output',
        'agents', 'marketing_agent', 'content_agent', 'orchestrator',
        'tools', 'scripts', 'config', 'data'
    ]
...
def main():
    ...
    integration_score = (integration_working / integration_total * 100) if integration_total > 0 else 0
    
    print(f"  🔗 Integration Endpoints: {integration_working}/{integration_total} ({integration_score:.1f}%)")
