...
class APIManager:
    ...
    def __init__(self):
        ...
        self.supported_channels = [
            ...
            'ai_content', 'analytics', 'payment', 'storage', 'auth'
        ]
    ...
    def discover_channel_apis(self, channel: str, budget_limit: float = 25.0) -> Dict[str, Any]:
        ...
        try:
            ...
            result = self.integration_service.add_channel_and_discover(channel, budget_limit)
            
            if result.get('success'):
                self.logger.info(f"✅ Successfully discovered {result.get('apis_integrated', 0)} APIs for {channel}")
                ...
                self.logger.error(f"❌ Discovery failed for {channel}: {result.get('error')}")
            ...
            for channel in self.supported_channels:
                ...
                if result.get('success'):
                    ...
                    if 'cost_analysis' in result:
                        total_cost += result['cost_analysis'].get('total_monthly_cost', 0)
    ...
    def track_costs(self) -> Dict[str, Any]:
        ...
        try:
            ...
            cost_info = {
                ...
                'budget_status': 'within_budget' if stats.get('total_monthly_cost', 0) <= 25.0 else 'over_budget',
    ...
    def _get_cost_recommendations(self, stats: Dict[str, Any]) -> List[str]:
        ...
        if total_cost > 25.0:
            ...
            recommendations.append("💡 Consider switching to free/freemium alternatives")
        
        if paid_apis > 0:
            recommendations.append(f"💰 You have {paid_apis} paid APIs - review if all are necessary")
        
        if total_cost == 0:
            recommendations.append("🎉 All APIs are free! Great cost optimization")
        
        recommendations.append("📊 Regularly monitor API usage to avoid unexpected charges")
    ...
    def _print_discovery_summary(self, result: Dict[str, Any]):
        ...
        if 'integrated_apis' in result:
            ...
            for api in result['integrated_apis'][:3]:  # Show top 3
                status_emoji = {'free': '🟢', 'freemium': '🟡', 'paid': '🔴'}.get(api['status'], '⚪')
                print(f"     {status_emoji} {api['name']} ({api['provider']}) - Score: {api['quality_score']:.2f}")
        ...
        while True:
            ...
            try:
                ...
                if command == 'quit' or command == 'exit':
                    ...
                    if result.get('success'):
                        print(f"✅ Discovery complete! Check api_management_table.md for results")
                    else:
                        print(f"❌ Error: {result.get('error')}")
                    ...
                    if result.get('success'):
                        print(f"🔍 Search results for '{query}': {len(result.get('results', []))} found")
                    else:
                        print(f"❌ Error: {result.get('error')}")
                    ...
                    if 'error' not in stats:
                        ...
                        print(f"   • Free APIs: {stats.get('by_status', {}).get('free', 0)}")
                        print(f"   • Freemium APIs: {stats.get('by_status', {}).get('freemium', 0)}")
                        print(f"   • Paid APIs: {stats.get('by_status', {}).get('paid', 0)}")
                        print(f"   • Monthly Cost: ${stats.get('total_monthly_cost', 0):.2f}")
                        print(f"   • Auto-discovered: {stats.get('auto_discovered_count', 0)}")
                    else:
                    ...
                    if 'error' not in costs:
                        print(f"\n💰 Cost Analysis:")
                        print(f"   • Monthly Cost: ${costs.get('total_monthly_cost', 0):.2f}")
                        print(f"   • Budget Status: {costs.get('budget_status', 'unknown').replace('_', ' ').title()}")
...
Examples:
  ...
  python api_manager.py interactive
    ...
    parser.add_argument('command', nargs='?', choices=[
        'discover', 'discover-all', 'search', 'stats', 'costs', 'update', 'channels', 'interactive'
    ], help='Command to execute')
    
    parser.add_argument('target', nargs='?', help='Target for the command (e.g., channel name, search query)')
    parser.add_argument('--budget', type=float, default=25.0, help='Budget limit for API costs (default: $25)')
