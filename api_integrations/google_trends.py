...
class GoogleTrendsAPI(BaseAPI):
        ...
        try:
            ...
            for idx, search_term in enumerate(trending_df[0].tolist()):
                trending_list.append(
                    {"rank": idx + 1, "search_term": search_term, "country": pn}
                )
    ...
    async def get_related_topics(
        ...
        try:
            ...
            related_topics = await loop.run_in_executor(
                None, self.pytrends.related_topics
            )
                        ...
                        keyword_topics["top"] = [
                            {
                                "topic": row["topic_title"],
                                    "value": (
                                    int(row["value"]) if pd.notna(row["value"]) else 0
                                ),
                                    }
    ...
    async def get_related_queries(
        ...
        try:
            ...
            # Get related queries
            related_queries = await loop.run_in_executor(
    ...
    def _calculate_trend_metrics(
        self, data: List[Dict], keywords: List[str]
    ) -> Dict[str, Any]:
        """Calculate trend velocity and momentum metrics"""
        if not data or len(data) < 2:
            return {
                keyword: {"velocity": 0, "momentum": 0, "peak_value": 0}
                for keyword in keywords
            }
