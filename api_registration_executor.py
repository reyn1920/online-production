...
class APIRegistrationExecutor:
            ...
            try:
                with open(self.results_file, "r") as f:
                    ...
                    self.registration_results = [
                        APIRegistrationResult(**result) for result in data
                    ]
    ...
    async def register_by_category(self, category: str) -> List[APIRegistrationResult]:
        ...
        category_apis = {
            key: info
            for key, info in self.api_registry.items()
        ...
        logger.info(
            f"Registering for {len(category_apis)} APIs in category: {category}"
        )
    ...
    async def interactive_register_all(self):
        ...
        delay = (
            input("Delay between batches in seconds (default 1.0): ").strip() or "1.0"
        )
    ...
    async def interactive_register_by_category(self):
        ...
        categories = set(
            info.get("category", "unknown") for info in self.api_registry.values()
        )
        ...
        for i, category in enumerate(sorted(categories), 1):
            count = len(
                [
                    k
                    for k, v in self.api_registry.items()
        ...
        if api_name in self.api_registry:
            ...
            try:
                ...
                if result.success:
                    ...
                    print(
                        f"❌ Failed to register for {api_name}: {result.error_message}"
                    )
            finally:
                await self.close_session()
        else:
    ...
    def show_registration_status(self):
        ...
        print(
            f"Success Rate: {len(successful)/len(self.registration_results)*100:.1f}%"
        )
    ...
    def search_apis(self):
        ...
        if matches:
            ...
            for api_key, api_info in matches:
                print(
                    f"  - {api_key}: {api_info['name']} ({api_info.get('category', 'unknown')})"
                )
