    ...
    for item in DEFAULT_CATALOG:
        if item["id"] not in providers:
            p = Provider(
                ...
                requires_key=item.get("requires_key", item.get("needs_key", True)),
...
<body>
  ...
  <p class="note">Green = working, Purple = needs key, Red = not working. Toggle enabled, add keys, and test providers below. Use the <b>Add Provider</b> card to add new ones quickly.</p>
  ...
  <div class="grid">
    ...
    <div class="card">
      ...
      <div>
        ...
        <label>Secret Name (e.g., OPENWEATHER_KEY) <span class="note">(optional)</span><br/><input id="np_keyenv" type="text"/></label><br/><br/>
...
async function fetchProviders(){
  ...
  data.providers.forEach(p => {
    ...
    const keyCell = p.key_env ? `<input type="password" placeholder="${p.key_env}" onblur="saveKey('${p.id}', this.value)" />` : '<span class="note">none</span>';
    const enabled = `<input type="checkbox" ${p.enabled?'checked':''} onchange="toggleEnabled('${p.id}', this.checked)" />`;
...
async function toggleEnabled(id, enabled){
  await fetch('/integrations/providers/'+id, {method:'PATCH', headers:{'Content-Type':'application/json'}, body: JSON.stringify({enabled})});
  fetchProviders();
}
async function saveKey(id, key){
  if(!key) return;
  await fetch('/integrations/providers/'+id, {method:'PATCH', headers:{'Content-Type':'application/json'}, body: JSON.stringify({key})});
  fetchProviders();
}
...
async function addProvider(){
  ...
  const res = await fetch('/integrations/providers', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({id, name, category, docs_url, signup_url, key_env, requires_key})});
  if(res.ok){
...
async function loadAffiliates(){
  ...
  Object.keys(data).forEach(channel => {
    ...
    data[channel].forEach(a => {
        ...
        const idCell = a.id_env ? `<input type="text" placeholder="${a.id_env}" onblur="saveAffilId('${channel}','${a.id}', this.value)" />` : '<span class="note">n/a</span>';
        const enabled = `<input type="checkbox" ${a.enabled?'checked':''} onchange="toggleAffil('${channel}','${a.id}', this.checked)" />`;
...
async function saveAffilId(channel, id, id_value){
    ...
    await fetch(`/integrations/affiliates/${encodeURIComponent(channel)}/${id}`, {
        method:'PATCH', headers:{'Content-Type':'application/json'}, body: JSON.stringify({id_value})
    });
    loadAffiliates();
}
