...
class RateLimiter:
    ...
    def _check_credit_optimization(
    ...
    def _check_model_limits(self, model: str, now: float) -> Tuple[bool, str, float]:
        ...
        high_cost_models = self.config.get("credit_system", {}).get(
            "high_cost_models", []
        )
        ...
        if model in high_cost_models:
            ...
            model_requests = [
                ...
                and now - entry.get("timestamp", 0) < 3600
            ]

            if (
                len(model_requests) >= 100
            ):  # Max 100 requests per hour for expensive models
    ...
    def record_request(
        ...
        with self.lock:
            ...
            self._update_model_performance(
                model, response_time, success, credits_used, tokens_used
            )
    ...
    def _update_model_performance(
        ...
        if tokens_used > 0:
            ...
            perf["cost_per_token"] = (1 - alpha) * perf[
                "cost_per_token"
            ] + alpha * cost_per_token
    ...
    def _adjust_rate_limiting_strategy(self):
        ...
        recent_requests = [
            ...
            if time.time() - req.get("timestamp", 0) < 3600
        ]  # Last hour
    ...
    def get_optimal_model(
        self, task_type: str = "general", max_cost: float = None
    ) -> str:
        ...
        for model, perf in self.model_performance.items():
            ...
            speed_score = max(
                0, 1 - (perf["avg_response_time"] / 10)
            )  # Normalize to 0 - 1
            cost_score = max(0, 1 - (perf["cost_per_token"] / 0.01))  # Normalize to 0 - 1

            composite_score = (
                perf["success_rate"] * 0.4 + speed_score * 0.3 + cost_score * 0.3
            )
...
class CreditOptimizer:
    ...
    def get_budget_recommendations(self, monthly_budget: float) -> Dict:
        ...
        if model_perf:
            most_expensive = max(
                model_perf.items(), key = lambda x: x[1]["cost_per_token"]
            )
...
if __name__ == "__main__":
    ...
    optimized = optimizer.optimize_request(
        messages, {"cost": 0.8, "speed": 0.1, "quality": 0.1}
    )
