...
class CreditOptimizer:
    ...
    def get_model_recommendation(self, 
                               ...
                               speed_priority: bool = False) -> OptimizationRecommendation:
        """
        ...
        if not model_scores:
            ...
            estimated_cost = (estimated_tokens / 1000) * self.model_costs[fallback_model]
        ...
        reasoning = "; ".join(reasoning_parts) if reasoning_parts else "General optimization"
    ...
    def record_usage(self, 
        ...
        if model not in self.usage_patterns:
            ...
            pattern.avg_credits_per_request = (
                alpha * actual_credits + 
                (1 - alpha) * pattern.avg_credits_per_request
            )
            ...
            if quality_score:
                pattern.cost_efficiency = (
                    alpha * quality_score + 
                    (1 - alpha) * pattern.cost_efficiency
                )
            
            pattern.last_updated = now
        
        self.logger.debug(f"Recorded usage for {model}: {actual_credits:.4f} credits, success: {success}")
    
    def get_budget_recommendation(self, 
                                available_credits: float,
                                expected_requests: int,
                                task_distribution: Dict[str, float] = None) -> Dict:
        """
        ...
        for task_type, proportion in task_distribution.items():
            ...
            typical_messages = [{'role': 'user', 'content': 'Sample message for estimation'}]
        ...
        if total_estimated_cost > available_credits:
            recommendations['warning'] = f"Estimated cost ({total_estimated_cost:.4f}) exceeds available credits ({available_credits:.4f})"
            recommendations['suggested_reduction'] = total_estimated_cost - available_credits
    ...
    def get_optimization_analytics(self) -> Dict:
        ...
        if self.usage_patterns:
            total_credits = sum(p.avg_credits_per_request for p in self.usage_patterns.values())
            ...
            analytics['cost_insights'] = {
                ...
                'efficiency_gap': most_efficient[1].cost_efficiency - least_efficient[1].cost_efficiency
            }
        ...
        for model, pattern in self.usage_patterns.items():
            if pattern.success_rate < 0.8:
                recommendations.append(f"Consider avoiding {model} due to low success rate ({pattern.success_rate:.1%})")
            
            if pattern.cost_efficiency < 0.5:
                recommendations.append(f"Review usage of {model} - low cost efficiency ({pattern.cost_efficiency:.2f})")
        
        # Check for optimization opportunities
        if len(self.usage_patterns) > 1:
            efficiency_scores = [(model, pattern.cost_efficiency) for model, pattern in self.usage_patterns.items()]
            efficiency_scores.sort(key=lambda x: x[1], reverse=True)
            
            if len(efficiency_scores) >= 2:
                best_model = efficiency_scores[0][0]
                recommendations.append(f"Consider using {best_model} more frequently for better efficiency")
    ...
    async def cleanup(self):
        ...
        cutoff_time = datetime.now() - timedelta(hours=self.optimization_config['pattern_decay_hours'])
        
        expired_models = [
            model for model, pattern in self.usage_patterns.items()
        ...
        self.logger.info(f"Credit optimizer cleanup completed, removed {len(expired_models)} expired patterns")
...
if __name__ == "__main__":
    async def test_credit_optimizer():
        ...
        speed_rec = optimizer.get_model_recommendation(
            messages,
            speed_priority=True
        )
        ...
        for model, perf in analytics['model_performance'].items():
            print(f"   {model}: {perf['success_rate']:.1%} success, {perf['avg_credits']:.4f} credits")
