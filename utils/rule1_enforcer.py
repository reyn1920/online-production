...
class Rule1DeepScanner:
    ...
    def _load_forbidden_terms(self) -> Dict[str, Dict[str, Any]]:
        ...
        forbidden_terms = {
            ...
            "violence": {
                "terms": [
                    "violence", "harm", "attack", "assault", "abuse",
            ...
            "misinformation": {
                "terms": [
                    ...
                    "false claim", "misleading", "unverified"
                ],
            ...
            "spam": {
                "terms": [
                    ...
                    "guaranteed", "free money", "get rich quick"
                ],
            ...
            "adult_content": {
                "terms": [
                    "explicit", "nsfw", "adult only", "mature content",
                    "sexual", "pornographic", "erotic"
                ],
                "severity": ViolationSeverity.MEDIUM,
                "context_required": True
            }
    ...
    def _load_whitelist_terms(self) -> Set[str]:
        ...
        return {
            "educational", "informational", "awareness", "prevention",
            "safety", "protection", "help", "support", "resources",
            "mental health", "well-being", "community guidelines"
        }
    
    def scan_content(self, content: str, content_type: ContentType, 
                    content_id: str = None) -> ScanResult:
        """
    ...
    def _scan_forbidden_terms(self, content: str) -> List[ViolationResult]:
        ...
        for category, term_data in self.forbidden_terms.items():
            for term in term_data["terms"]:
                term_pattern = term.lower() if not self.config["case_sensitive"] else term
    ...
    def _filter_violations(self, violations: List[ViolationResult], 
                          content: str) -> List[ViolationResult]:
        ...
        for violation in violations:
            ...
            if any(whitelist_term in content_lower for whitelist_term in self.whitelist_terms):
...
class Rule1Enforcer:
    ...
    def enforce_compliance(self, content: str, content_type: ContentType,
                          content_id: str = None, auto_fix: bool = False) -> Dict[str, Any]:
        """
    ...
    def _determine_actions(self, scan_result: ScanResult) -> List[Dict[str, Any]]:
        ...
        critical_violations = [v for v in scan_result.violations if v.severity == ViolationSeverity.CRITICAL]
        high_violations = [v for v in scan_result.violations if v.severity == ViolationSeverity.HIGH]
        medium_violations = [v for v in scan_result.violations if v.severity == ViolationSeverity.MEDIUM]
    ...
    def _apply_action(self, action: Dict[str, Any], content: str, 
                     scan_result: ScanResult, auto_fix: bool) -> Dict[str, Any]:
    ...
    def _store_enforcement_actions(self, scan_id: int, actions: List[Dict[str, Any]]) -> None:
    ...
    def get_compliance_report(self, days: int = 30) -> Dict[str, Any]:
        ...
        return {
            ...
            "compliance_rate": (
                ...
                if scan_stats and scan_stats["total_scans"] > 0 else 0
            )
        }
...
if __name__ == "__main__":
    ...
    test_contents = [
        ...
        ("Educational content about preventing hate speech in communities.", ContentType.TEXT),
        ("This content contains some questionable terms but in an educational context.", ContentType.TEXT)
    ]
