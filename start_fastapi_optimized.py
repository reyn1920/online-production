...
class FastAPIOptimizer:
    ...
    def start_with_uvicorn(self, app_module):
        ...
        cmd = [
            ...
            "--access-log",
            "--loop", "auto"  # Let uvicorn choose best event loop for M1
        ]
    ...
    def start_with_gunicorn(self, app_module):
        ...
        cmd = [
            ...
            "--keep-alive", str(config["keepalive"]),
            "--timeout", str(config["timeout"]),
            "--graceful-timeout", str(config["graceful_timeout"]),
            "--access-logfile", "-",
            "--error-logfile", "-"
        ]
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Optimized FastAPI startup for MacBook Air M1"
    )
    parser.add_argument(
        "--gunicorn", 
        action="store_true", 
