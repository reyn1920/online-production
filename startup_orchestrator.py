...
class StartupOrchestrator:
    ...
    def start_service(self, service_name: str) -> bool:
        ...
        try:
            ...
            for dep in service.dependencies:
                if dep not in self.service_status or self.service_status[dep].status != 'running':
                    self.logger.warning(f"Dependency {dep} not running for service {service_name}")
            
            # Apply startup delay
            if service.startup_delay > 0:
                self.logger.info(f"Applying startup delay of {service.startup_delay}s for {service_name}")
    ...
    def stop_service(self, service_name: str) -> bool:
        ...
        try:
            if status.process and status.process.poll() is None:
                self.logger.info(f"Stopping service: {service_name} (PID: {status.pid})")
            ...
            try:
                for service_name, service in self.services.items():
                    if service_name in self.service_status:
                        ...
                        if status.status == 'running' and not self.health_check_service(service_name):
    ...
    async def start_all_services(self):
        ...
        if not integration_success:
            self.logger.warning("Integration layer failed to initialize, continuing with services...")
        ...
        while remaining_services:
            ...
            if not ready_services:
                ...
                self.logger.warning(f"Possible circular dependencies in: {remaining_services}")
            ...
            for service_name in ready_services:
                ...
                if success or not self.services[service_name].critical:
                    ...
                    self.logger.error(f"Critical service {service_name} failed to start")
                    return False
    ...
    def print_status_summary(self):
        ...
        for service_name, status in self.service_status.items():
            ...
            print(f"{status_icon} {service_name:<20} {status.status.upper():<10}{port_info}")
