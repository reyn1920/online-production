...
# Socket.IO imports with fallback
try:
    import socketio
    ...
    logging.warning("⚠️ Socket.IO not available - real-time features disabled")
...
try:
    ...
    setup_security_middleware = RateLimitMiddleware = AuthenticationMiddleware = RequestValidationMiddleware = None
...
class ErrorHandlingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: StarletteRequest, call_next):
        ...
        try:
            ...
            # Prometheus metrics
            if PROMETHEUS_AVAILABLE and hasattr(self, 'request_count') and self.request_count is not None:
    ...
    for module_name in config["modules"]:
        try:
            ...
            if isinstance(attr_name, list):
                attr_name = attr_name[0] if attr_name else "router"
            router = getattr(module, str(attr_name), None)
...
try:
    ...
    avatar_router = getattr(avatar, 'router', None)
except ImportError:
    avatar_router = None
...
try:
    ...
    content_router_new = getattr(content, 'router', None)
...
try:
    ...
    EtsyAPI = GumroadAPI = MonetizationManager = PaddleAPI = SendOwlAPI = None
    logger.warning(f"⚠️ Monetization platform APIs import failed: {e}")

# Financial router import
try:
    import app.routers.financial as financial_module
    financial_router = getattr(financial_module, 'router', None)
...
try:
    ...
    dashboard_router = getattr(dashboard_module, 'router', None)
...
try:
    import routers.comprehensive_dashboard as comp_dashboard_module
    comprehensive_dashboard_router = getattr(comp_dashboard_module, 'router', None)
...
try:
    ...
    auth_manager = getattr(auth_module, 'auth_manager', None)
...
if SOCKETIO_AVAILABLE and sio:
    ...
    logger.info("⚠️ Socket.IO not available - event handlers skipped")
...
try:
    ...
    logger.warning("⚠️ Production middleware config not found, using fallback")
