...
class SyntaxErrorFixer:
    def __init__(self, dry_run=False, backup=True, exclude_dirs=None):
        ...
        self.exclude_dirs_set = {"venv", "venv_stable", ".venv", ".venv_base44", "__pycache__", "site-packages", ".git", "node_modules", "dist", "build"}
        self.skip_patterns = os.getenv("BASE44_SKIP_GLOBS", "venv*,.venv*,**/site-packages/**,**/__pycache__/**").split(",")
    ...
    def should_exclude_file(self, file_path: Path) -> bool:
        ...
        if any(part in self.exclude_dirs_set for part in path_parts) or "site-packages" in path_parts:
            return True
            
        # Check glob patterns
        if any(fnmatch(path_str, pat) for pat in self.skip_patterns):
            return True
            
        # Legacy exclusions
        exclude_patterns = [
            '.trae', '.pytest_cache', '.ruff_cache', 'test_output', 'test_results'
        ] + self.exclude_dirs
