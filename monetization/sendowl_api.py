...
class SendOwlAPI(BaseMonetizationAPI):
    ...
    def create_discount_code(self, code: str, discount_percent: float, product_ids: List[str] = None) -> Dict[str, Any]:
    ...
    def _upload_digital_files(self, product_id: str, file_paths: List[str]):
        ...
        for file_path in file_paths:
            try:
                ...
                logger.info(f"Would upload digital file {file_path} to SendOwl product {product_id}")
            except Exception as e:
                logger.error(f"Failed to upload digital file {file_path}: {e}")
    
    def create_bundle(self, bundle_name: str, product_ids: List[str], bundle_price: float) -> Dict[str, Any]:
