...
class EtsyAPI(BaseMonetizationAPI):
        ...
        try:
            ...
            if product.preview_images:
                self._upload_listing_images(
                    listing["listing_id"], product.preview_images
                )
    ...
    def get_sales_data(
        ...
        try:
            ...
            total_revenue = sum(
                float(receipt.get("grandtotal", 0)) for receipt in receipts
            )
    ...
    def _upload_digital_files(self, listing_id: str, file_paths: List[str]):
        ...
        for file_path in file_paths:
            try:
                ...
                logger.info(
                    f"Would upload digital file {file_path} to listing {listing_id}"
                )
            except Exception as e:
