...
from .base_monetization import BaseMonetizationAPI, Product, ProductResponse, ProductCreationError
...
class GumroadAPI(BaseMonetizationAPI):
    ...
    def create_product(self, product: Product) -> ProductResponse:
        ...
        try:
            product_data = {
                ...
                'file_info': {},
                'preview_url': product.preview_images[0] if product.preview_images else None,
            ...
            if product_response.get('success'):
                ...
                if product.digital_files:
                    self._upload_digital_files(product_info['id'], product.digital_files)
    ...
    def create_discount_code(self, product_id: str, code: str, discount_percent: float, max_uses: int = 100) -> Dict[str, Any]:
        ...
        try:
            ...
            response = self._make_request(
                'POST',
                f'/products/{product_id}/offer_codes',
                data=discount_data
            )
    ...
    def _upload_digital_files(self, product_id: str, file_paths: List[str]):
        ...
        for file_path in file_paths:
            try:
                ...
                logger.info(f"Would upload digital file {file_path} to Gumroad product {product_id}")
            except Exception as e:
