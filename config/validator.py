...
class ConfigValidator:
    ...
    def _validate_host(self, value: str) -> Tuple[bool, str]:
        ...
        if value in valid_hosts or re.match(
    ...
    def validate_configuration(self) -> Dict[str, Any]:
        ...
        if validation_summary["valid"]:
            logger.info(
                f"✅ Configuration validation passed ({len(self.results)} checks)"
            )
        else:
            logger.error(
    ...
    def _validate_production_requirements(self, errors: List[str], warnings: List[str]):
        ...
        if secret_key and len(secret_key) < 64:
            warnings.append(
                "Consider using a longer SECRET_KEY (64+ characters) for production"
            )
