...
try:
    ...
    logger.warning(
        "Stability SDK not available - Stability AI image generation disabled"
    )
    client = None
    generation = None
import re
...
logger.add(
    "logs / content_agent.log", rotation="100 MB", retention="30 days", level="DEBUG"
)
...
class ContentItem(Base):
    ...
    )  # article, video, podcast, social_post, ebook
    content = Column(Text)
...
class ContentRequest(BaseModel):
    ...
    publishing_channels: List[str] = Field(
        default=[], description="Channels to publish to"
    )
    schedule_time: Optional[datetime] = Field(
        None, description="Schedule publication time"
    )
...
class BulkContentRequest(BaseModel):
    ...
    base_template: Optional[str] = Field(
        None, description="Base template for all content"
    )
    common_settings: ContentRequest = Field(
        ..., description="Common settings for all content"
    )
...
class ContentGenerator:
    ...
    def _build_content_prompt(self, request: ContentRequest) -> str:
        ...
        if request.keywords:
            prompt_parts.append(
                f"Include these keywords naturally: {', '.join(request.keywords)}"
            )
    ...
    async def _generate_with_anthropic(
        self, prompt: str, request: ContentRequest
    ) -> str:
        ...
        try:
            analysis = {
                "readability": self._analyze_readability(content),
                    ...
                    "estimated_reading_time": len(content.split())
...
class ContentPublisher:
    ...
    def _initialize_cms_clients(self) -> Dict[str, Any]:
        ...
        # WordPress
        if all(
            [config.WORDPRESS_URL, config.WORDPRESS_USERNAME, config.WORDPRESS_PASSWORD]
        ):
            try:
    ...
    async def publish_content(
        self, content_id: str, channels: List[str]
    ) -> Dict[str, Any]:
    ...
    async def _publish_to_social(
        self, content_item: ContentItem, channel: str
    ) -> Dict[str, Any]:
    ...
    async def create_content(self, request: ContentRequest) -> ContentResponse:
        ...
        try:
            ...
            estimated_engagement = self._estimate_engagement(
                seo_analysis, request.content_type
            )
        ...
        try:
            ...
            for item in content_items:
                content_by_type[item.content_type] = (
                    content_by_type.get(item.content_type, 0) + 1
                )
            ...
            engagement_metrics = {
                ...
                / max(total_content, 1),
                    ...
                    "draft_content": len(
                    [item for item in content_items if item.status == "draft"]
                ),
                    }

            # Top performing content
            top_performing = sorted(
                content_items, key = lambda x: x.seo_score or 0, reverse = True
            )[:5]
            ...
            for model in ai_model_performance:
                model_items = [
                    item for item in content_items if item.ai_model_used == model
                ]
                avg_score = sum(item.seo_score or 0 for item in model_items) / len(
                    model_items
                )
...
async def bulk_create_content(
    request: BulkContentRequest, background_tasks: BackgroundTasks
):
...
async def publish_content(
    content_id: str, channels: List[str], background_tasks: BackgroundTasks
):
    """Publish content to specified channels"""
...
async def get_content_file(
    content_id: str, file_type: str, db: Session = Depends(get_db)
):
...
async def generate_image(
    prompt: str, style: str = "realistic", size: str = "1024x1024"
):
...
async def generate_video(
    script: str, images: List[str], audio_path: Optional[str] = None
):
        ...
        for item in due_items:
            try:
                ...
                if results.get(item.channel, {}).get("success"):
                    ...
                    item.status = "failed"
                    item.error_message = results.get(item.channel, {}).get(
                        "error", "Unknown error"
                    )
...
async def update_content_metrics():
    ...
    try:
        ...
        published_content = (
            db.query(ContentItem).filter(ContentItem.status == "published").all()
        )
