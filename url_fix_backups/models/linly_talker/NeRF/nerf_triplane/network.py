...
class NeRFNetwork(NeRFRenderer):
    ...
    def __init__(
        ...
        self.sigma_net = MLP(
            self.in_dim + self.audio_dim + self.eye_dim,
                1 + self.geo_feat_dim,
                self.hidden_dim,
                self.num_layers,
                )
        ...
        self.color_net = MLP(
            self.in_dim_dir + self.geo_feat_dim + self.individual_dim,
                3,
    ...
    def forward_torso(self, x, poses, c = None):
        ...
        wrapped_anchor = (
            wrapped_anchor[:, :, :2]
            / wrapped_anchor[:, :, 3, None]
            / wrapped_anchor[:, :, 2, None]
        ).view(1, -1)
