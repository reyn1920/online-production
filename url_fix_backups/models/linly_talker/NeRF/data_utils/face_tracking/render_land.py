...
class Render_Face(torch.autograd.Function):
    @staticmethod
    def forward(ctx, proj_geo, texture, nbl, ori_img, is_visible, tri_inds, pixel_valid):
...
class Render_RGB(nn.Module):
    ...
    def forward(self, proj_geo, texture, nbl, ori_img, is_visible, tri_inds, pixel_valid):
        return Render_Face.apply(proj_geo, texture, nbl, ori_img, is_visible, tri_inds, pixel_valid)
...
#         )
        ...
        (render,) = render_util.render_mesh(proj_geo, ori_img, ori_size, texture, nbl, self.tris)
