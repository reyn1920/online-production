...
class APIStatusDashboard:
    ...
    def update_system_metrics(self):
        ...
        inactive_apis = sum(
            1 for s in self.api_statuses.values() if s.status == "inactive"
        )
        error_apis = sum(1 for s in self.api_statuses.values() if s.status == "error")

        # Calculate average response time
        response_times = [
            s.response_time for s in self.api_statuses.values() if s.response_time
        ]
        avg_response_time = (
        ...
        overall_health_score = (
            int(sum(health_scores) / len(health_scores)) if health_scores else 0
        )
...
<body>
    ...
    <script>
        ...
        function updateDashboard(data) {
            ...
            document.getElementById('daily - cost').textContent = `$${metrics.total_daily_cost.toFixed(2)}`;
