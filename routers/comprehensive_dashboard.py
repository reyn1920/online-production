        ...
        if system_monitor:
            try:
                ...
                metrics = {
                    ...
                    "active_connections": getattr(system_metrics, 'active_connections', 0),
                ...
                logger.warning(f"Advanced system monitoring failed, falling back to basic: {e}")
    ...
    try:
        ...
        if performance_monitor:
            try:
                ...
                logger.warning(f"Performance monitor failed, falling back to basic: {e}")
        ...
        for proc in psutil.process_iter(['pid', 'name', 'cpu_percent', 'memory_info', 'status']):
            try:
                ...
                if any(keyword in proc_name.lower() for keyword in 
                      ['python', 'node', 'nginx', 'apache', 'redis', 'postgres', 'mysql', 'mongodb']):
                    
                    # Determine service status
                    status = 'running' if proc_info['status'] == psutil.STATUS_RUNNING else 'stopped'
...
async def dashboard_health_check():
    ...
    try:
        ...
        if performance_monitor:
            try:
                ...
                return {
                    ...
                    "uptime_seconds": health_status.get('uptime_seconds', time.time() - start_time),
                    "active_websocket_connections": len(manager.active_connections),
        ...
        if cpu_percent > 80 or memory_percent > 80:
            ...
            if cpu_percent > 80:
                alerts.append({"type": "high_cpu", "severity": "warning", "value": cpu_percent})
            if memory_percent > 80:
                alerts.append({"type": "high_memory", "severity": "warning", "value": memory_percent})
        
        if cpu_percent > 95 or memory_percent > 95:
            health_status = "critical"
            alerts = [alert for alert in alerts if alert["severity"] != "warning"]
            if cpu_percent > 95:
                alerts.append({"type": "critical_cpu", "severity": "critical", "value": cpu_percent})
            if memory_percent > 95:
                alerts.append({"type": "critical_memory", "severity": "critical", "value": memory_percent})
