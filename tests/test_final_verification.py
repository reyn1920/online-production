...
def _exists_nonempty(p, secs=15):
    t0 = time.time(); pth = pathlib.Path(p)
    while time.time() - t0 < secs:
        if pth.exists() and pth.stat().st_size > 0: return True
        time.sleep(0.25)
    return False
...
def main():
    ...
    j = r.json()
    assert ("status" in j and j["status"] in ("healthy","ok","ready")) or ("ok" in j and j["ok"] is True)

    # Actions
    r = client.get("/api/actions"); assert r.status_code == 200
    acts = r.json(); assert isinstance(acts, dict) and ("actions" in acts) and len(acts["actions"]) >= 4

    # Minimal video workflow if exposed, otherwise fall back to run_channel
    mp4 = pdf = None
    # Prefer your FastAPI video endpoint if present
    try:
        r = client.post("/api/create_video", json={"prompt":"Test video","duration":3,"style":"default"})
        if r.status_code == 200 and isinstance(r.json(), dict) and r.json().get("status") in ("completed","ok"):
    ...
    if mp4: assert _exists_nonempty(mp4, 20), f"mp4 not found: {mp4}"
    if pdf: assert _exists_nonempty(pdf, 10), f"pdf not found: {pdf}"
