            ...
            while time.time() - start_time < timeout:
                try:
                    ...
                    response = requests.get(
                        f"{self.dashboard_url}/api/health", timeout=2
                    )
                ...
                if self.live_process.poll() is not None:
                    ...
                    print(
                        f"Live system process exited early. STDOUT: {stdout}, STDERR: {stderr}"
                    )
                    return False
            ...
            if response.status_code == 200:
                ...
                for task in tasks:
                    if task.get("id") == task_id:
                        print(
                            f"Task {task_id} found in queue with status: {task.get('status')}"
                        )
                        return True
    ...
    def verify_video_file_creation(self, task_id: str) -> Path:
        ...
        for pattern in possible_patterns:
            ...
            if video_path.exists() and video_path.stat().st_size > 0:
                print(
                    f"Found video file: {video_path} ({video_path.stat().st_size} bytes)"
                )
                return video_path
        ...
        if recent_videos:
            ...
            print(
                f"Found recent video file: {latest_video} ({latest_video.stat().st_size} bytes)"
            )
            return latest_video
...
class TestTRAELiveSystemIntegrity:
    ...
    def test_01_system_startup(self, trae_live_system):
        ...
        assert (
            trae_live_system.live_process.poll() is None
    ...
    def test_03_orchestrator_status_check(self, trae_live_system):
        ...
        try:
            ...
            print(
                f"✅ Orchestrator status check passed: {len(status_data['agents'])} agents found"
            )
        except requests.exceptions.RequestException as e:
        ...
        try:
            ...
            if final_status["status"] == "completed":
                ...
                if len(video_files) > 0:
                    ...
                    print(
                        f"🎥 Video file created: {video_file} ({video_file.stat().st_size} bytes)"
                    )
                ...
                pytest.fail(
                    f"Video creation task ended with status: {final_status.get('status')}"
                )
        except TimeoutError as e:
            # For a production system, timeout should be treated as a failure
            print(f"⏱️  Task timed out after 5 minutes: {e}")
            pytest.fail(
                "Video creation task timed out - this indicates a system performance issue"
            )
        except Exception as e:
            raise e
