...
class TraeAIE2EVerification:
    ...
    def _run_system_health_tests(self):
        ...
        try:
            ...
            status = TestStatus.PASSED if cpu_usage < self.config["thresholds"]["cpu_usage_max"] else TestStatus.FAILED
            ...
            self._add_result(
                ...
                message,
                {"cpu_usage": cpu_usage, "threshold": self.config["thresholds"]["cpu_usage_max"]}
            )
        except Exception as e:
            self._add_result(
                "cpu_usage_check",
        ...
        try:
            ...
            status = TestStatus.PASSED if memory_usage < self.config["thresholds"]["memory_usage_max"] else TestStatus.FAILED
        ...
        try:
            ...
            status = TestStatus.PASSED if disk_usage < self.config["thresholds"]["disk_usage_max"] else TestStatus.FAILED
    ...
    def _run_integration_tests(self):
        ...
        try:
            ...
            status = TestStatus.PASSED if initialized_count >= total_components * 0.8 else TestStatus.FAILED
    ...
    def _run_security_tests(self):
        ...
        try:
                                ...
                                for pattern in secret_patterns:
                                    ...
                                    if matches:
                                        secrets_found.extend([
                                            {"file": file_path, "pattern": pattern, "match": match}
            ...
            message = f"Found {len(secrets_found)} potential secrets" if secrets_found else "No secrets detected"
            ...
            self._add_result(
                ...
                time.time() - start_time,
                message,
                {"secrets_found": len(secrets_found), "details": secrets_found[:5]}  # Limit details
            )
        except Exception as e:
            self._add_result(
                "secret_scanning",
    ...
    def _run_performance_tests(self):
        ...
        try:
            ...
            status = TestStatus.PASSED if response_time < self.config["thresholds"]["response_time_max"] else TestStatus.FAILED
    ...
    def _run_end_to_end_tests(self):
        ...
        try:
            ...
            status = TestStatus.PASSED if success_rate >= self.config["thresholds"]["success_rate_min"] else TestStatus.FAILED
    ...
    def _add_result(self, test_name: str, category: TestCategory, status: TestStatus, 
        ...
        for category in TestCategory:
            ...
            if category_results:
                results_by_category[category.value] = {
                    ...
                    "error": len([r for r in category_results if r.status == TestStatus.ERROR]),
