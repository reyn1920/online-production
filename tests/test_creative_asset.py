...
class CreativeAssetTester:
    ...
    def run_creative_asset_test(
        ...
        try:
            ...
            self._log_step(
                test_result, f"Processing voice '{voice}' with Speechelo Pro RPA"
            )
            audio_file = self._simulate_voice_generation(
                voice, script_content, timestamp
            )
            ...
            self.logger.info(
                f"Creative Asset Test completed successfully: {final_video}"
            )
