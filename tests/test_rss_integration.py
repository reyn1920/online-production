...
from backend.agents.base_agents import PlannerAgent, AuditorAgent
from backend.agents.specialized_agents import ResearchAgent, ContentAgent
...
class RSSIntegrationTester:
    ...
    async def test_breaking_news_watcher(self):
        ...
        try:
            ...
            self.log_test("RSS Feeds Configuration", len(feeds) > 0, f"Configured {len(feeds)} RSS feeds")
            
            # Test trending topics retrieval
            trending_topics = news_watcher.get_trending_topics()
            self.log_test("Trending Topics Retrieval", True, f"Retrieved {len(trending_topics)} topics")
            
            # Test intelligence briefing
            briefing = news_watcher.get_latest_intelligence_briefing()
            self.log_test("Intelligence Briefing", briefing is not None, "Generated intelligence briefing")
    ...
    async def test_hypocrisy_database_integration(self):
        ...
        try:
            ...
            self.log_test("HypocrisyDatabaseManager Initialization", True, "Successfully initialized")
            ...
            self.log_test("Store Hypocrisy Finding", finding_id is not None, f"Stored finding with ID: {finding_id}")
            
            # Test retrieving content opportunities
            opportunities = hypocrisy_db.get_content_opportunities(limit=5)
            self.log_test("Retrieve Content Opportunities", len(opportunities) > 0, f"Retrieved {len(opportunities)} opportunities")
    ...
    async def test_planner_agent_rss_integration(self):
        ...
        try:
            ...
            self.log_test("PlannerAgent Initialization", True, "Successfully initialized with RSS intelligence")
            
            # Test RSS intelligence retrieval
            rss_intelligence = planner._get_rss_intelligence()
            self.log_test("RSS Intelligence Retrieval", rss_intelligence is not None, f"Retrieved intelligence: {list(rss_intelligence.keys())}")
            
            # Test trending topics integration
            trending_topics = rss_intelligence.get('trending_topics', [])
            self.log_test("Trending Topics Integration", len(trending_topics) >= 0, f"Found {len(trending_topics)} trending topics")
            
            # Test content opportunities identification
            content_opportunities = rss_intelligence.get('content_opportunities', [])
            self.log_test("Content Opportunities", len(content_opportunities) >= 0, f"Identified {len(content_opportunities)} opportunities")
            
            # Test topic momentum calculation
            topic_momentum = rss_intelligence.get('topic_momentum', {})
            self.log_test("Topic Momentum Calculation", isinstance(topic_momentum, dict), f"Calculated momentum for {len(topic_momentum)} topics")
    ...
    async def test_research_agent_integration(self):
        ...
        try:
            ...
            if hasattr(research_agent, 'research_tools') and 'news_watcher' in research_agent.research_tools:
                news_watcher = research_agent.research_tools['news_watcher']
                if hasattr(news_watcher, 'hypocrisy_db'):
                    self.log_test("Research-Hypocrisy Integration", True, "Hypocrisy database integrated in research tools")
                else:
                    self.log_test("Research-Hypocrisy Integration", False, "Hypocrisy database not found in research tools")
            else:
                self.log_test("Research-Hypocrisy Integration", False, "Breaking news watcher not found in research agent")
    ...
    async def test_task_queue_integration(self):
        ...
        try:
            ...
            self.log_test("RSS-Driven Task Creation", task_id is not None, f"Created task with ID: {task_id}")
            
            # Test task retrieval
            tasks = task_queue.get_tasks(status='pending', limit=10)
            self.log_test("Task Queue Retrieval", len(tasks) >= 0, f"Retrieved {len(tasks)} pending tasks")
    ...
    async def test_end_to_end_workflow(self):
        ...
        try:
            ...
            self.log_test("OODA Loop Integration", 'trending_topics' in market_conditions, "RSS intelligence integrated in OODA loop")
            ...
            if hasattr(planner, 'content_scheduling'):
                ...
                self.log_test("Dynamic Content Scheduling", len(scheduling_rules) > 0, f"Scheduling rules configured: {list(scheduling_rules.keys())}")
            ...
            if news_watcher and hasattr(news_watcher, 'get_hypocrisy_content_opportunities'):
                ...
                self.log_test("Hypocrisy Content Integration", len(hypocrisy_opportunities) >= 0, f"Retrieved {len(hypocrisy_opportunities)} hypocrisy opportunities")
