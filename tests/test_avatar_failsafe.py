...
class AvatarFailsafeTest:
    ...
    async def test_normal_operation(self):
        ...
        if result.status.value == 'success':
            ...
            if result.response_data and '_orchestration' in result.response_data:
                ...
                print(f"   Failover Triggered: {orch_info.get('failover_triggered', False)}")
        else:
        ...
        if result.status.value == 'success':
            ...
            if result.response_data and '_orchestration' in result.response_data:
                ...
                if result.api_used.api_name == 'talking-heads-fallback':
                    ...
                    print(f"   ⚠️  Expected talking-heads-fallback, got {result.api_used.api_name}")
        else:
            print(f"❌ FAILOVER FAILED: {result.error_message}")
        
        return result.status.value == 'success' and result.api_used and result.api_used.api_name == 'talking-heads-fallback'
    ...
    async def test_capability_query(self):
        ...
        for i, api in enumerate(apis, 1):
            print(f"   {i}. {api.api_name} (Priority: {api.priority}, Status: {api.status})")
    ...
    async def test_priority_ordering(self):
        ...
        priority_correct = (primary_priority is not None and 
                          fallback_priority is not None and 
                          primary_priority < fallback_priority)
        ...
        if priority_correct:
            print(f"✅ Priority ordering correct: Primary({primary_priority}) < Fallback({fallback_priority})")
            return True
        else:
            print(f"❌ Priority ordering incorrect: Primary({primary_priority}) vs Fallback({fallback_priority})")
            return False
