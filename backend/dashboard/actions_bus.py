...
def _seed_roadmap_if_missing():
    if not ROADMAP.exists() or ROADMAP.stat().st_size == 0:
        ...
        with ROADMAP.open("w", newline="") as f:
            w = csv.writer(f)
            w.writerow(["channel"])
            for c in ["DEMO_CAPABILITY_REEL", "TEST_CHANNEL", "QUICK_START", "ADVANCED_FEATURES"]:
                w.writerow([c])
...
def act_maxout_synthesize(payload: dict) -> dict:
    ...
    out = {"timestamp": time.strftime("%Y%m%dT%H%M%SZ", time.gmtime()), "ingested": items, "notes": "Add-only v3 synthesis"}
    (ASSETS / "releases" / "v3").mkdir(parents=True, exist_ok=True)
...
def act_maxout_run_one_channel(payload: dict) -> dict:
    ...
    try:
        ...
        if hasattr(ce, "capability_reel"):
            res = ce.capability_reel(channel=channel, minutes=minutes, avatars=avatars,
                                     fresh=fresh, produce_examples=produce_examples, random_seed=random_seed)
    ...
    try:
        r = requests.post(f"{BACKEND_URL}/api/capability_reel", json={
            "channel": channel,
            "minutes": minutes,
            "avatars": avatars,
            "fresh": fresh,
        ...
        return {
            ...
            "csv_path": str(ROADMAP.relative_to(ASSETS.parent)),  # "assets/incoming/channel_roadmaps_10.csv"
