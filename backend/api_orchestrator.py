...
class APIOrchestrator:
    ...
    def __init__(self, db_path: str = "right_perspective.db", 
        ...
        self.logger.info(f"API Orchestrator initialized with {len(self.api_endpoints)} endpoints")
    ...
    def _check_endpoint_health(self, endpoint: APIEndpoint) -> None:
        ...
        health_url = endpoint.configuration.get('health_check_url') if endpoint.configuration else None
        if not health_url:
        ...
        try:
            ...
            with self.api_lock:
                if endpoint.api_name in self.api_endpoints:
                    ...
                    self.api_endpoints[endpoint.api_name].last_health_check = datetime.now()
                    self.api_endpoints[endpoint.api_name].average_response_time = response_time
            
            # Update database
            self._update_endpoint_health(endpoint.api_name, health_status, response_time)
            
            self.logger.debug(f"Health check for {endpoint.api_name}: {health_status} ({response_time:.0f}ms)")
    ...
    def _update_endpoint_health(self, api_name: str, health_status: str, response_time: float) -> None:
    ...
    def select_best_api(self, api_type: str = None, exclude_apis: List[str] = None) -> Optional[APIEndpoint]:
        ...
        with self.api_lock:
            ...
            available_endpoints = [
                endpoint for endpoint in self.api_endpoints.values()
                if (endpoint.status == 'active' and 
                    endpoint.api_name not in exclude_apis and
                    endpoint.health_status in ['healthy', 'degraded'] and
                    self._check_rate_limits(endpoint))
            ]
            ...
            if self.failover_strategy == FailoverStrategy.PRIORITY_BASED:
                ...
                return min(available_endpoints, key=lambda x: self.request_counts.get(x.api_name, 0))
    ...
    async def make_request(self, request: APIRequest, api_name: str = None) -> APIResponse:
        ...
        for attempt in range(max_retries):
            ...
            if not endpoint:
                if attempt < max_retries - 1:
                    self.logger.warning(f"No available API endpoint, retrying in 5 seconds (attempt {attempt + 1}/{max_retries})")
                    await asyncio.sleep(5)
                    continue
                else:
                    raise Exception("No healthy API endpoints available after all retries")
            ...
            try:
                ...
                self._update_request_metrics(endpoint.api_name, True, response.response_time_ms)
                ...
                if attempt < max_retries - 1:
                    ...
                    raise Exception(f"All API requests failed after {max_retries} attempts: {e}")
    ...
    async def _execute_request(self, endpoint: APIEndpoint, request: APIRequest) -> APIResponse:
        ...
        try:
            ...
            if request.method.upper() == 'GET':
                ...
                response = requests.post(url, headers=headers, data=request.body, timeout=request.timeout)
            elif request.method.upper() == 'PUT':
                response = requests.put(url, headers=headers, data=request.body, timeout=request.timeout)
    ...
    def _update_request_metrics(self, api_name: str, success: bool, response_time_ms: int) -> None:
                ...
                with self.api_lock:
                    ...
                    if api_name in self.performance_history:
                        ...
                        if len(self.performance_history[api_name]) > 100:
                            self.performance_history[api_name] = self.performance_history[api_name][-100:]
                
        except Exception as e:
            self.logger.error(f"Failed to update request metrics for {api_name}: {e}")
    
    def _log_api_request(self, endpoint: APIEndpoint, request: APIRequest, 
                        response: Optional[requests.Response], response_time_ms: int, 
    ...
    def get_api_status(self) -> Dict[str, Any]:
        ...
        with self.api_lock:
            status = {
                ...
                'healthy_endpoints': len([e for e in self.api_endpoints.values() if e.health_status == 'healthy']),
                'degraded_endpoints': len([e for e in self.api_endpoints.values() if e.health_status == 'degraded']),
                'unhealthy_endpoints': len([e for e in self.api_endpoints.values() if e.health_status == 'unhealthy']),
                'endpoints': {}
            }
...
async def make_api_request(endpoint: str, method: str = "GET", 
                          ...
                          api_name: str = None, orchestrator: APIOrchestrator = None) -> APIResponse:
