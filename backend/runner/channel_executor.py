...
class ChannelExecutor:
    ...
    def __init__(self):
        ...
        for dir_path in [self.outputs_dir, self.videos_dir, self.pdfs_dir, self.audio_dir]:
            ...
            with open(roadmap_path, 'r', encoding='utf-8') as f:
                ...
                for row in reader:
                    ...
                    channel_data = {
                        ...
                        'upload_frequency': row.get('upload_frequency', 'weekly').strip(),
                        'monetization_strategy': row.get('monetization_strategy', '').strip(),
                        'voice_style': row.get('voice_style', 'professional').strip(),
                        'avatar_type': row.get('avatar_type', 'realistic').strip(),
                        'background_music': row.get('background_music', 'ambient').strip(),
    ...
    async def execute_channel(self, channel_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Generate thumbnail
            thumbnail_result = await self._generate_thumbnail(channel_data, video_result)
    ...
    async def _generate_video(self, channel_data: Dict[str, Any], script: str) -> Dict[str, Any]:
        """Generate video using Hollywood pipeline"""
        channel_id = channel_data['channel_id']
        ...
        video_params = {
            ...
            'video_length': channel_data['video_length'],
            'output_path': str(self.videos_dir / f"{channel_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.mp4")
        }
    ...
    async def _generate_pdf_product(self, channel_data: Dict[str, Any], script: str) -> Dict[str, Any]:
        ...
        output_path = self.pdfs_dir / f"{channel_id}_{product_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
    ...
    async def _generate_thumbnail(self, channel_data: Dict[str, Any], video_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        thumbnail_path = str(self.outputs_dir / "thumbnails" / f"{Path(video_path).stem}_thumb.jpg")
    ...
    async def execute_roadmap(self, roadmap_path: str = "channel_roadmaps_10.csv", 
                            channel_filter: Optional[str] = None) -> Dict[str, Any]:
    ...
    def get_execution_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            'total_video_size': sum(f.stat().st_size for f in self.videos_dir.glob('*.mp4')),
            'total_pdf_size': sum(f.stat().st_size for f in self.pdfs_dir.glob('*.pdf')),
            'latest_outputs': {
                'videos': [f.name for f in sorted(self.videos_dir.glob('*.mp4'), 
                                                key=lambda x: x.stat().st_mtime, reverse=True)[:5]],
                'pdfs': [f.name for f in sorted(self.pdfs_dir.glob('*.pdf'), 
                                              key=lambda x: x.stat().st_mtime, reverse=True)[:5]]
            }
        }
...
def run_channel_once(csv_path: str, video_dir: str = "outputs/videos", pdf_dir: str = "outputs/pdfs") -> Dict[str, Any]:
