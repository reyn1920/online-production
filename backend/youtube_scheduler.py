...
class YouTubeScheduler:
    ...
    async def schedule_video(self, video_data: Dict[str, Any], 
                            preferred_time: Optional[datetime] = None,
                            priority: SchedulePriority = SchedulePriority.MEDIUM) -> ScheduledVideo:
        """Schedule a video for optimal upload timing."""
        try:
            self.logger.info(f"Scheduling video: {video_data.get('title', 'Unknown')[:50]}...")
            ...
            if optimization:
                ...
                scheduled_video.estimated_views = int(optimization.audience_reach_increase * 1000)
            ...
            self.logger.info(f"Video scheduled for {scheduled_video.scheduled_time} with score {scheduled_video.optimal_score:.1f}")
    ...
    async def _optimize_schedule_time(self, video: ScheduledVideo) -> Optional[ScheduleOptimization]:
        ...
        try:
            ...
            engagement_boost = audience_reach_increase * 0.8  # Engagement correlates with reach
            ...
            optimization = ScheduleOptimization(
                ...
                timezone_breakdown=self._calculate_timezone_breakdown(best_time, audience_insights),
    ...
    def _generate_candidate_times(self, preferred_time: datetime, 
                                 priority: SchedulePriority, 
                                 content_type: ContentType) -> List[datetime]:
        ...
        if priority == SchedulePriority.BREAKING_NEWS:
            ...
            for day in range(look_ahead_days):
                ...
                for hour in optimal_hours:
                    candidate_time = day_base.replace(hour=hour, minute=0, second=0, microsecond=0)
    ...
    async def _score_schedule_time(self, schedule_time: datetime, 
                                  video: ScheduledVideo, 
                                  audience_insights: List[AudienceInsight]) -> float:
        """Score a potential schedule time based on multiple factors."""
        try:
            total_score = 0.0
            
            # Timezone optimization score
            timezone_score = self._calculate_timezone_score(schedule_time, audience_insights)
            total_score += timezone_score * self.config['optimization']['timezone_weight']
            
            # Engagement prediction score
            engagement_score = self._predict_engagement_score(schedule_time, video)
            total_score += engagement_score * self.config['optimization']['engagement_weight']
            
            # Competition avoidance score
            competition_score = await self._calculate_competition_score(schedule_time, video.channel_id)
            total_score += competition_score * self.config['optimization']['competition_weight']
            ...
            conflict_penalty = await self._calculate_conflict_penalty(schedule_time, video.channel_id)
    ...
    def _calculate_timezone_score(self, schedule_time: datetime, 
                                 audience_insights: List[AudienceInsight]) -> float:
        ...
        for insight in audience_insights:
            try:
                ...
                self.logger.error(f"Error calculating timezone score for {insight.timezone}: {e}")
                continue
        
        return total_score / total_weight if total_weight > 0 else 50.0
    
    def _predict_engagement_score(self, schedule_time: datetime, video: ScheduledVideo) -> float:
    ...
    def _extract_time_features(self, schedule_time: datetime, video: ScheduledVideo) -> List[float]:
    ...
    async def _calculate_competition_score(self, schedule_time: datetime, channel_id: str) -> float:
    ...
    def _calculate_day_score(self, schedule_time: datetime, content_type: ContentType) -> float:
    ...
    async def _calculate_conflict_penalty(self, schedule_time: datetime, channel_id: str) -> float:
    ...
    async def _upload_video(self, video: ScheduledVideo):
        ...
        try:
            ...
            if result and result.get('success'):
                ...
                self.logger.info(f"Video uploaded successfully: {result.get('video_id')}")
            else:
    ...
    async def _handle_upload_failure(self, video: ScheduledVideo):
        ...
        try:
            ...
            if video.retry_count < max_retries:
                ...
                video.scheduled_time = datetime.now() + timedelta(minutes=30)  # Retry in 30 minutes
                self.logger.info(f"Rescheduling video for retry {video.retry_count}/{max_retries}")
            else:
                # Mark as failed
                video.status = ScheduleStatus.FAILED
                self.logger.error(f"Video failed after {max_retries} attempts: {video.title}")
    ...
    def _generate_optimization_reasoning(self, original_time: datetime, 
                                       optimized_time: datetime,
        ...
        if time_diff.total_seconds() > 0:
            reasoning.append(f"Moved {time_diff.total_seconds() / 3600:.1f} hours later for better audience reach")
        elif time_diff.total_seconds() < 0:
            reasoning.append(f"Moved {abs(time_diff.total_seconds()) / 3600:.1f} hours earlier to avoid competition")
        ...
        if audience_insights:
            ...
            reasoning.append(f"Optimized for {primary_tz} audience at {local_time.strftime('%I:%M %p')}")
        
        # Add content-type reasoning
        reasoning.append(f"Scheduled during optimal hours for {content_type.value} content")
        
        return reasoning
    
    def _calculate_timezone_breakdown(self, schedule_time: datetime, 
                                    audience_insights: List[AudienceInsight]) -> Dict[str, float]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                """, (
                    ...
                    json.dumps(video.tags), video.video_path, video.thumbnail_path,
                    video.content_type.value, video.priority.value, video.scheduled_time.isoformat(),
                    video.optimal_score, json.dumps(video.target_timezones),
                    video.estimated_views, video.estimated_engagement, video.status.value,
                    video.retry_count, json.dumps(video.metadata),
    ...
    async def _store_optimization(self, optimization: ScheduleOptimization, video_id: str):
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                conn.execute("""
                    ...
                    WHERE id = ?
                """, (
                    video.status.value, video.retry_count, 
                    json.dumps(video.metadata), video.updated_at.isoformat(),
                    video.id
                ))
                conn.commit()
        except Exception as e:
    ...
    def _row_to_scheduled_video(self, row_data: Dict[str, Any]) -> ScheduledVideo:
        ...
        return ScheduledVideo(
            ...
            target_timezones=json.loads(row_data['target_timezones']) if row_data['target_timezones'] else [],
