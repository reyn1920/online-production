...
class APIOpportunityFinder:
    ...
    def get_research_context(self) -> ResearchContext:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            usage_patterns = {
                row["capability"]: row["total_usage"] for row in capabilities_data
            }
            ...
            for row in capabilities_data:
                if row["api_count"] < 2:  # Less than 2 APIs for redundancy
                    capability_gaps.append(
                        f"Limited redundancy for {row['capability']}"
                    )
...
"""
    ...
    async def execute_discovery_task(self, task_id: int) -> List[APISuggestion]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.row_factory = sqlite3.Row
            cursor = conn.execute(
                "SELECT * FROM api_discovery_tasks WHERE id = ?", (task_id,)
            )
        ...
        try:
            ...
            suggestions_data = await self.research_api_opportunities(
                context, focus_area
            )
            ...
            for suggestion_data in suggestions_data:
                ...
                suggestion = APISuggestion(
                    ...
                    integration_complexity=suggestion_data.get(
                        "integration_complexity", 5
                    ),
            ...
            logger.info(
                f"Discovery task {task_id} completed with {len(suggestions)} suggestions"
            )
            return suggestions
            ...
            self.update_discovery_task(
                task_id, DiscoveryStatus.FAILED, f"Task failed: {str(e)}"
            )
            logger.error(f"Discovery task {task_id} failed: {e}")
            raise

    def get_top_suggestions(
        self, limit: int = 10, capability: str = None
    ) -> List[Dict[str, Any]]:
    ...
    async def run_automated_discovery(self, max_tasks: int = 3) -> Dict[str, Any]:
        ...
        for task_data in pending_tasks:
            try:
                ...
                logger.info(
                    f"Processed task {task_data['id']}: {len(suggestions)} suggestions"
                )
...
async def example_usage():
    ...
    async with APIOpportunityFinder() as finder:
        ...
        for suggestion in top_suggestions:
            ...
            print(
                f"  Score: {suggestion['confidence_score'] * suggestion['potential_value']:.2f}"
            )
