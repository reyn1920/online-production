...
class APIOpportunityFinder:
    ...
    def __init__(self, db_path: str = "right_perspective.db", ollama_url: str = "http://localhost:11434"):
    ...
    async def research_api_opportunities(self, context: ResearchContext, 
        ...
        prompt = f"""
You are an expert API researcher. Analyze the following context and suggest new API opportunities.
...
"""
    ...
    def create_discovery_task(self, task_name: str, capability_gap: str, 
                            search_keywords: List[str], target_domains: List[str] = None,
                            priority: int = 5) -> int:
        """Create a new API discovery task"""
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute("""
                INSERT INTO api_discovery_tasks 
                (task_name, capability_gap, search_keywords, target_domains, priority, status, assigned_agent)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (
                task_name, capability_gap, json.dumps(search_keywords),
                json.dumps(target_domains or []), priority, DiscoveryStatus.PENDING.value,
                "research_agent_v1"
            ))
            conn.commit()
            return cursor.lastrowid
    
    def update_discovery_task(self, task_id: int, status: DiscoveryStatus, 
                            progress_notes: str = None, apis_found: int = None):
        """Update discovery task status and progress"""
        with sqlite3.connect(self.db_path) as conn:
            updates = ["status = ?", "updated_at = ?", "progress_notes = ?"]
            values = [status.value, datetime.now(timezone.utc).isoformat(), progress_notes]
    ...
    def save_api_suggestion(self, suggestion: APISuggestion) -> int:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                suggestion.discovery_task_id, suggestion.api_name, suggestion.api_url,
                suggestion.capability, suggestion.description, suggestion.documentation_url,
                suggestion.pricing_model, suggestion.estimated_cost, suggestion.integration_complexity,
                suggestion.potential_value, suggestion.confidence_score, suggestion.status
            ))
            conn.commit()
            return cursor.lastrowid
    ...
    async def execute_discovery_task(self, task_id: int) -> List[APISuggestion]:
        ...
        self.update_discovery_task(task_id, DiscoveryStatus.IN_PROGRESS, 
                                 "Starting API research with LLM")
        ...
        try:
            ...
            focus_area = f"{task.capability_gap} - Keywords: {', '.join(json.loads(task.search_keywords))}"
            ...
            for suggestion_data in suggestions_data:
                ...
                suggestion = APISuggestion(
                    ...
                    confidence_score=suggestion_data.get('confidence_score', 5) / 10.0,  # Convert to 0-1
            ...
            self.update_discovery_task(
                task_id, DiscoveryStatus.COMPLETED,
            ...
            self.update_discovery_task(
                task_id, DiscoveryStatus.FAILED,
                f"Task failed: {str(e)}"
            )
    ...
    def get_top_suggestions(self, limit: int = 10, capability: str = None) -> List[Dict[str, Any]]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            query += """
                ORDER BY (s.confidence_score * s.potential_value) DESC, s.discovered_at DESC
                LIMIT ?
            """
            params.append(limit)
    ...
    def update_suggestion_status(self, suggestion_id: int, status: SuggestionStatus, 
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute("""
                ...
                WHERE id = ?
            """, (status.value, review_notes, datetime.now(timezone.utc).isoformat(), suggestion_id))
            conn.commit()
...
async def example_usage():
    ...
    async with APIOpportunityFinder() as finder:
        ...
        for suggestion in suggestions:
            ...
            print(f"  Confidence: {suggestion.confidence_score:.2f}, Value: {suggestion.potential_value}")
