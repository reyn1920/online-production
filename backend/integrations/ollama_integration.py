...
class OllamaIntegration:
    ...
    def _collect_system_metrics(self):
        ...
        try:
            ...
            if total_requests % 100 == 0 and total_requests > 0:
                logger.info(f"Performance metrics - Requests: {total_requests}, "
    ...
    def _setup_prompt_templates(self):
        ...
        self.prompt_templates = {
            PromptTemplate.STRATEGIC_BRIEF: PromptConfig(
                ...
                system_prompt="You are a senior strategic advisor with 20+ years of experience in business strategy, market analysis, and corporate planning. Provide comprehensive, actionable insights.",
                user_prompt_template="Analyze the following business data and generate a strategic brief: {data}\n\nFocus on: {focus_areas}\n\nProvide specific recommendations with implementation timelines.",
            ...
            PromptTemplate.MARKET_ANALYSIS: PromptConfig(
                ...
                system_prompt="You are a market research expert specializing in digital marketing, content creation, and online business models. Provide data-driven market insights.",
                user_prompt_template="Conduct a comprehensive market analysis for: {market_segment}\n\nData: {market_data}\n\nInclude market size, growth trends, competitive landscape, and opportunities.",
            ...
            PromptTemplate.COMPETITIVE_INTEL: PromptConfig(
                ...
                system_prompt="You are a competitive intelligence analyst with expertise in digital business models, content strategies, and market positioning.",
                user_prompt_template="Analyze competitive landscape: {competitors}\n\nOur position: {our_data}\n\nIdentify competitive advantages, threats, and strategic opportunities.",
            ...
            PromptTemplate.OPPORTUNITY_ASSESSMENT: PromptConfig(
                ...
                system_prompt="You are a business development expert specializing in identifying and evaluating new market opportunities and revenue streams.",
                user_prompt_template="Evaluate this business opportunity: {opportunity_data}\n\nAssess: market potential, risks, resource requirements, ROI projections, and implementation strategy.",
            ...
            PromptTemplate.CONTENT_STRATEGY: PromptConfig(
                ...
                system_prompt="You are a content strategist and digital marketing expert with deep knowledge of YouTube, social media, and content monetization.",
                user_prompt_template="Develop content strategy for: {platform}\n\nTarget audience: {audience}\n\nCurrent performance: {performance_data}\n\nProvide specific content recommendations and optimization strategies.",
    ...
    def _select_optimal_model(self, template: Optional[PromptTemplate], context: Optional[Dict]) -> str:
        ...
        try:
            ...
            if template:
                ...
                elif template in [PromptTemplate.MARKET_ANALYSIS, PromptTemplate.COMPETITIVE_INTEL]:
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Performance metrics
            recent_requests = len([r for r in self.request_history if r.completed_at > datetime.now() - timedelta(hours=1)])
    ...
    def _setup_ollama_database(self):
        ...
        try:
            ...
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_prompt_hash ON response_cache(prompt_hash)')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_model_name ON response_cache(model_name)')
