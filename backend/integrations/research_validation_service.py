...
try:
    from backend.engines.hypocrisy_engine import HypocrisyEngine, ValidationResult, ResearchClaim
except ImportError:
    HypocrisyEngine = None
    ValidationResult = None
    ResearchClaim = None
    logging.warning("HypocrisyEngine not available. Research validation will be limited.")
...
class ResearchValidationService:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.validation_timeout_seconds = self.config.get('validation_timeout_seconds', 30)
    ...
    async def validate_research(self, request: ResearchValidationRequest) -> ResearchValidationResponse:
        """Validate a research claim or statement"""
        start_time = datetime.now()
        request_id = f"val_{int(start_time.timestamp() * 1000)}"
        
        try:
            self.logger.info(f"Starting research validation {request_id} for: {request.content[:100]}...")
            
            # Check cache first
            if self.cache_results:
                cached_result = self._get_cached_result(request)
                if cached_result:
                    self.logger.info(f"Returning cached result for validation {request_id}")
            ...
            content_opportunities = await self._find_content_opportunities(request, validation_result)
            ...
            response = ResearchValidationResponse(
                ...
                validation_status=validation_result.fact_check_status if validation_result else 'error',
                contradictions_found=validation_result.contradictions_found if validation_result else [],
                evidence_sources=validation_result.evidence_sources if validation_result else [],
                validation_notes=validation_result.validation_notes if validation_result else 'Validation failed',
            ...
            self.logger.info(f"Completed validation {request_id} in {processing_time}ms")
            return response
    ...
    async def _perform_validation(self, request: ResearchValidationRequest) -> Optional[ValidationResult]:
    ...
    def _generate_recommendations(self, 
        ...
        try:
            if not validation_result:
                recommendations.append("Manual fact-checking required - automated validation unavailable")
                recommendations.append("Verify claims through multiple independent sources")
                return recommendations
            
            if validation_result.is_valid:
                recommendations.append("Claim appears to be valid based on available evidence")
                if validation_result.confidence_score < 0.8:
                    recommendations.append("Consider gathering additional supporting evidence")
                ...
                recommendations.append("Review contradictory sources before publication")
                
                if validation_result.contradictions_found:
                    recommendations.append(f"Investigate {len(validation_result.contradictions_found)} contradictions identified")
            ...
            if request.priority == 'high':
                ...
                recommendations.append("Consider real-time fact-checking before publication")
            
            # Content-type specific recommendations
            if request.validation_type == 'statement' and request.author:
                recommendations.append(f"Cross-reference with {request.author}'s historical statements")
    ...
    async def _find_content_opportunities(self, 
                                        request: ResearchValidationRequest, 
                                        validation_result: Optional[ValidationResult]) -> List[Dict[str, Any]]:
                ...
                for opp in engine_opportunities:
                    if opp.get('subject_name') == request.author:
                        content_opp = {
                            ...
                            'description': opp.get('analysis_notes', 'Hypocrisy analysis'),
                            'confidence': opp.get('confidence_score', 0.5),
                            'potential_impact': 'high' if opp.get('public_impact_score', 5) > 7 else 'medium',
    ...
    def _get_cached_result(self, request: ResearchValidationRequest) -> Optional[ResearchValidationResponse]:
    ...
    def _cache_result(self, request: ResearchValidationRequest, response: ResearchValidationResponse) -> None:
    ...
    async def batch_validate(self, requests: List[ResearchValidationRequest]) -> List[ResearchValidationResponse]:
...
async def validate_claim(content: str, 
                        author: Optional[str] = None, 
                        source: Optional[str] = None) -> ResearchValidationResponse:
...
async def validate_statement(content: str, 
                           author: str, 
                           source: Optional[str] = None, 
                           context: Optional[str] = None) -> ResearchValidationResponse:
