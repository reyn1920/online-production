...
try:
    ...
    class Agent:
        """Fallback Agent class for when CrewAI is not available."""

        def __init__(self, role: str = None, goal: str = None, backstory: str = None, **kwargs):
            self.role = role or "Generic Agent"
            self.goal = goal or "Complete assigned tasks"
            self.backstory = backstory or "A helpful AI agent"
        ...
        def execute(self, task):
            return f"Agent {self.role} would execute: {task}"

    class Task:
        """Fallback Task class for when CrewAI is not available."""
...
#         ):
    ...
    class Crew:
        """Fallback Crew class for when CrewAI is not available."""

        def __init__(self, agents: list = None, tasks: list = None, process=None, **kwargs):
            self.agents = agents or []
            self.tasks = tasks or []
            self.process = process or "sequential"
            self.verbose = kwargs.get("verbose", True)
            self.memory = kwargs.get("memory", True)
        ...
        def kickoff(self, inputs: dict = None):
            ...
            for task in self.tasks:
                result = task.execute() if hasattr(task, "execute") else str(task)
                results.append(result)
            return {"results": results, "status": "completed"}

    class Process:
        """Process types for CrewAI fallback."""

        sequential = "sequential"
        hierarchical = "hierarchical"

    class BaseTool:
        """Fallback BaseTool class for when CrewAI is not available."""

        def __init__(self, name: str = None, description: str = None, **kwargs):
            self.name = name or "Generic Tool"
            self.description = description or "A helpful tool"

        def _run(self, *args, **kwargs):
            return f"Tool {self.name} executed with args: {args}, kwargs: {kwargs}"
