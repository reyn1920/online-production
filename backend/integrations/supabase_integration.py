...
class SupabaseIntegration:
    ...
    def __init__(self, secrets_db_path: str = 'data/secrets.sqlite'):
        ...
        if not SUPABASE_AVAILABLE:
            self.logger.warning("Supabase not installed. Install with: pip install supabase")
            self.client = None
            return
        ...
        try:
            with self.secret_store as store:
                config = SupabaseConfig(
                    ...
                    service_role_key=store.get_secret('SUPABASE_SERVICE_ROLE_KEY') or os.getenv('SUPABASE_SERVICE_ROLE_KEY'),
                    jwt_secret=store.get_secret('SUPABASE_JWT_SECRET') or os.getenv('SUPABASE_JWT_SECRET'),
                    db_password=store.get_secret('SUPABASE_DB_PASSWORD') or os.getenv('SUPABASE_DB_PASSWORD'),
                    storage_bucket=store.get_secret('SUPABASE_STORAGE_BUCKET') or 'trae-ai-storage'
                )
    ...
    async def get_user_profile(self, user_id: str) -> Optional[UserProfile]:
        ...
        try:
            response = self.client.table(TableName.USERS.value).select('*').eq('id', user_id).execute()
    ...
    async def store_credential(self, 
                             ...
                             credential_type: CredentialType,
                             description: Optional[str] = None,
                             expires_at: Optional[datetime] = None) -> Optional[SecureCredential]:
        ...
        try:
            ...
            response = self.client.table(TableName.CREDENTIALS.value).insert(credential_data).execute()
    ...
    async def get_credential(self, user_id: str, credential_id: str) -> Optional[str]:
        ...
        try:
            response = self.client.table(TableName.CREDENTIALS.value).select('*').eq('id', credential_id).eq('user_id', user_id).execute()
            ...
            if response.data:
                ...
                decrypted_value = self._decrypt_credential(credential_data['encrypted_value'])
    ...
    async def list_user_credentials(self, user_id: str) -> List[SecureCredential]:
        ...
        try:
            response = self.client.table(TableName.CREDENTIALS.value).select('id, user_id, name, credential_type, description, expires_at, created_at, updated_at').eq('user_id', user_id).execute()
    ...
    async def store_workflow_state(self, workflow_state: WorkflowState) -> bool:
        ...
        try:
            state_data = {
                ...
                'created_at': workflow_state.created_at.isoformat() if workflow_state.created_at else datetime.now().isoformat(),
                'updated_at': datetime.now().isoformat(),
    ...
    async def get_workflow_state(self, user_id: str, workflow_id: str) -> Optional[WorkflowState]:
        ...
        try:
            response = self.client.table(TableName.WORKFLOWS.value).select('*').eq('id', workflow_id).eq('user_id', user_id).execute()
    ...
    async def store_execution_record(self, execution: ExecutionRecord) -> bool:
        ...
        try:
            ...
            response = self.client.table(TableName.EXECUTIONS.value).upsert(execution_data).execute()
    ...
    async def get_execution_record(self, user_id: str, execution_id: str) -> Optional[ExecutionRecord]:
        ...
        try:
            response = self.client.table(TableName.EXECUTIONS.value).select('*').eq('id', execution_id).eq('user_id', user_id).execute()
            ...
            if response.data:
                ...
                execution = ExecutionRecord(
                    ...
                    finished_at=datetime.fromisoformat(exec_data['finished_at']) if exec_data['finished_at'] else None,
    ...
    async def upload_file(self, 
        ...
        try:
            ...
            if response:
                # Get public URL
                public_url = self.client.storage.from_(self.config.storage_bucket).get_public_url(storage_path)
    ...
    async def subscribe_to_changes(self, table: str, callback: callable, filter_conditions: Optional[Dict] = None):
        ...
        if health['status'] == 'healthy':
            ...
            if profile:
                ...
                if credential:
                    ...
                    decrypted_value = await supabase.get_credential(profile.id, credential.id)
