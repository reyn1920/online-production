...
def _fetch_petfinder_animals(limit: int, api_key: str, secret: str, animal_type: str = "dog") -> Dict[str, Any]:
    ...
    if not token:
        return {
            "success": False,
            "error": "Failed to get Petfinder OAuth token"
        }
        ...
        try:
            ...
            if provider_key == "thecatapi":
                if pet_type in ["cat", "random"]:
                    ...
                    result = {"success": False, "error": f"TheCatAPI doesn't support {pet_type}"}
            elif provider_key == "thedogapi":
                if pet_type in ["dog", "random"]:
                    result = _fetch_thedogapi_images(limit, creds.get("api_key"))
                else:
                    result = {"success": False, "error": f"TheDogAPI doesn't support {pet_type}"}
                ...
                result = _fetch_petfinder_animals(limit, creds["api_key"], creds["secret"], animal_type)
            else:
            ...
            if result["success"]:
                ...
                if attempt < max_retries:
                    try:
                        ...
                        rotate_resp = requests.post(f"{BASE}/integrations/rotate?category=pets", timeout=10)
