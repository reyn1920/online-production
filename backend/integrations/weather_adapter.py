...
def _report_usage(provider_key: str, success: bool, error: Optional[str] = None, 
                 took_ms: Optional[int] = None, quota_remaining: Optional[int] = None):
        ...
        if daily.get("time"):
            for i, date in enumerate(daily["time"]):
                if i < 5:  # Limit to 5 days
                    day_weather_code = daily.get("weather_code", [])[i] if i < len(daily.get("weather_code", [])) else 0
...
def _fetch_openweathermap_weather(lat: float, lon: float, location: str, api_key: str) -> Dict[str, Any]:
    ...
    if forecast_data.get("list"):
        ...
        for date, day_data in list(daily_forecasts.items())[:5]:  # Limit to 5 days
            ...
            weather_data["forecast"].append({
                ...
                "weather_code": day_data["weather_codes"][0] if day_data["weather_codes"] else None,
                "description": day_data["descriptions"][0].title() if day_data["descriptions"] else "Unknown",
                "precipitation_sum": day_data["precipitation"],
                "precipitation_probability": None,  # Not available in this endpoint
                "wind_speed_max": max(day_data["wind_speeds"]) if day_data["wind_speeds"] else None,
...
def fetch_weather(location: str, lat: Optional[float] = None, lon: Optional[float] = None, max_retries: int = 1) -> Dict[str, Any]:
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if active.get("needs_key", False) and not creds:
                ...
                _report_usage(provider_key, False, error_msg, int((time.time() - start_time) * 1000))
            ...
            if provider_key == "open-meteo":
                ...
                result = {"success": False, "error": f"No adapter implementation for {provider_key}"}
            ...
            if result["success"]:
                ...
                if attempt < max_retries:
                    try:
                        ...
                        rotate_resp = requests.post(f"{BASE}/integrations/rotate?category=weather", timeout=10)
...
def get_weather(location: str, lat: Optional[float] = None, lon: Optional[float] = None) -> Optional[Dict[str, Any]]:
