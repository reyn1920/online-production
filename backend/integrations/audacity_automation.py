...
try:
    ...
    print("Warning: Audio processing libraries not available. Install with: pip install librosa soundfile numpy")
...
class AudacityAutomation:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if not AUDIO_LIBS_AVAILABLE:
                self.logger.warning("Audio processing libraries not available. Some features may be limited.")
            
            # Test Audacity scripting
            if await self._test_audacity_scripting():
                self.logger.info("Audacity scripting available")
            else:
                self.logger.warning("Audacity scripting not available. Enable mod-script-pipe in Audacity.")
    ...
    def _normalize_audio(self, input_files: List[str], params: Dict[str, Any]) -> List[str]:
    ...
    def _reduce_noise(self, input_files: List[str], params: Dict[str, Any]) -> List[str]:
    ...
    def _compress_audio(self, input_files: List[str], params: Dict[str, Any]) -> List[str]:
    ...
    async def create_podcast_episode(self, episode: PodcastEpisode) -> str:
        ...
        try:
            ...
            operations = [
                ...
                {'type': 'noise_reduction', 'params': {'level': NoiseReductionLevel.MODERATE}},
            ...
            if len(input_files) > 1:
                operations.append({
                    'type': 'fade',
                    'params': {'fade_in': 0.5, 'fade_out': 1.0}
                })
            
            # Generate output path
            safe_title = "".join(c for c in episode.title if c.isalnum() or c in (' ', '-', '_')).rstrip()
            output_path = str(self.temp_dir / f"{safe_title}.{episode.output_format.value}")
    ...
    async def batch_process_voice_files(self, 
                                      input_dir: str, 
                                      output_dir: str,
                                      preset: str = "podcast_voice") -> Dict[str, Any]:
