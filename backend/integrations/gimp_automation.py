    ...
    async def create_design_from_template(self, spec: DesignSpec) -> str:
        ...
        try:
            ...
            if IMAGE_LIBS_AVAILABLE:
                ...
                # Create a minimal image using PIL as fallback
                if IMAGE_LIBS_AVAILABLE:
                    ...
                    # Add basic text if title is provided
                    if spec.title:
                        draw = ImageDraw.Draw(img)
                        # Use default font and center the text
                        bbox = draw.textbbox((0, 0), spec.title)
                        text_width = bbox[2] - bbox[0]
                        text_height = bbox[3] - bbox[1]
                        x = (width - text_width) // 2
                        y = (height - text_height) // 2
                        draw.text((x, y), spec.title, fill=spec.text_color)
