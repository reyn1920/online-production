...
class OBSAutomation:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if not self._is_obs_running():
                self.logger.warning("OBS Studio is not running. Please start OBS Studio.")
                return False
    ...
    async def _load_scenes(self):
        ...
        try:
            ...
            for scene_data in scenes_data.get("scenes", []):
                ...
                items_data = items_response.get("responseData", {}).get("sceneItems", [])
    ...
    async def _configure_recording(self, config: RecordingConfig):
        ...
        try:
            ...
            await self._send_request(
                "SetRecordDirectory",
                {"recordDirectory": config.output_path}
            )
    ...
    async def get_stream_stats(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if stream_data.get("outputActive"):
                session.duration = stream_data.get("outputDuration", 0) // 1000000  # Convert from nanoseconds
                session.bitrate = stream_data.get("outputBytes", 0) * 8 / 1000  # Convert to kbps
            ...
            return {
                ...
                'drop_percentage': (session.dropped_frames / max(session.total_frames, 1)) * 100
            }
    ...
    async def get_recording_stats(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if record_data.get("outputActive"):
                session.duration = record_data.get("outputDuration", 0) // 1000000  # Convert from nanoseconds
    ...
    async def create_automated_workflow(self, 
                                      workflow_name: str,
                                      scenes: List[Scene],
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        # Check OBS process
        health['components']['obs_process'] = {
            'running': self._is_obs_running()
        }
        
        # Check active sessions
        health['components']['sessions'] = {
            'active_streams': len([s for s in self.stream_sessions.values() if s.status == 'live']),
