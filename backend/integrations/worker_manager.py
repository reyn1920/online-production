...
class WorkerManager:
    ...
    def __init__(self, dps: DistributedProcessingSystem):
        ...
        logger.info(f"Worker Manager initialized for {self.platform} platform")
    ...
    def start_worker(self, queue_names: List[str], concurrency: int = None) -> str:
        ...
        if concurrency is None:
            concurrency = min(self.machine_capabilities.cpu_cores, 
        ...
        try:
            ...
            logger.info(f"Starting worker {worker_id} with queues: {queue_names}")
    ...
    def _build_worker_command(self, worker_id: str, queue_names: List[str], 
    ...
    def stop_worker(self, worker_id: str, timeout: int = 30) -> bool:
        ...
        try:
            ...
            if self.platform == 'win32':
                # Windows
                subprocess.run(['taskkill', '/PID', str(worker.process_id), '/T'], 
                ...
                for worker_id, worker in list(self.workers.items()):
                    ...
                    if not self._is_process_running(worker.process_id):
                        if worker.status in ['running', 'starting']:
                            logger.warning(f"Worker {worker_id} process died unexpectedly")
                            worker.status = 'failed'
                    ...
                    # Update worker status based on heartbeat
                        ...
                        if current_time - worker.started_at > timedelta(minutes=2):
                            worker.status = 'running'  # Assume it's running if process exists
            ...
            if current_count < target_count:
                ...
                for _ in range(needed):
                    try:
                        ...
                        actions[f'scale_up_{queue_name}'] = f'Started worker {worker_id}'
                    except Exception as e:
                ...
                for worker in self.workers.values():
                    if (worker.status == 'running' and 
                        queue_name in worker.queue_names and 
                        len(workers_to_stop) < excess):
                        workers_to_stop.append(worker.worker_id)
                
                for worker_id in workers_to_stop:
                    if self.stop_worker(worker_id):
                        actions[f'scale_down_{queue_name}'] = f'Stopped worker {worker_id}'
...
def main():
    ...
    parser.add_argument('--queues', nargs='+', default=['general'],
                       help='Queue names for new workers')
    ...
    try:
        if args.action == 'start':
            ...
            if args.worker_id:
                ...
                print(f"Stop worker {args.worker_id}: {'Success' if success else 'Failed'}")
            else:
                results = manager.stop_all_workers()
                for worker_id, success in results.items():
                    print(f"Stop worker {worker_id}: {'Success' if success else 'Failed'}")
            ...
            print(f"Restart worker {args.worker_id}: {'Success' if success else 'Failed'}")
            ...
            try:
                while True:
                    time.sleep(10)
                    status = manager.get_worker_status()
                    print(f"Workers: {status['running_workers']}/{status['total_workers']} running")
