...
class UnifiedAPILayer:
    ...
    async def initialize_integrations(self):
        ...
        try:
            ...
            if hasattr(self.n8n, "health_check"):
                ...
                self.logger.info(
                    f"n8n integration: {n8n_health.get('status', 'unknown')}"
                )
            ...
            if hasattr(self.crewai, "health_check"):
                ...
                self.logger.info(
                    f"CrewAI integration: {crewai_health.get('status', 'unknown')}"
                )
    ...
    async def start_server(self):
        ...
        self.logger.info(
            f"Starting Unified API server on {self.config.host}:{self.config.port}"
        )
        await server.serve()
