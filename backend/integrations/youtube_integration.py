...
""""""
            ...
            if not os.path.exists(video_path):
                return UploadResult(status="failed", error=f"Video file not found: {video_path}")
            ...
            if response.status_code == 200:
                ...
                if rows:
                    ...
                    totals = {
                        ...
                        "averageViewDuration": sum(row[2] for row in rows if len(row) > 2)
                        / len(rows),
...
"""
        ...
        """"""
            ...
            comment_body = {"snippet": {"parentId": parent_comment_id, "textOriginal": reply_text}}

            # Post the reply
            request = self.youtube_service.comments().insert(part="snippet", body=comment_body)
...
"""
        ...
        """"""
            ...
            # Set rating to 'like'
            request = self.youtube_service.comments().setRating(id=comment_id, rating="like")
...
"""
            ...
            channel_request = self.youtube_service.channels().list(part="contentDetails", mine=True)
