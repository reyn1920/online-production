...
def _fetch_newsapi_articles(query: str, limit: int, api_key: str, category: Optional[str] = None) -> Dict[str, Any]:
...
def fetch_news(query: Optional[str] = None, limit: int = 10, category: Optional[str] = None, max_retries: int = 1) -> Dict[str, Any]:
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if provider_key == "newsapi" and "api_key" in creds:
                result = _fetch_newsapi_articles(query or "", limit, creds["api_key"], category)
            ...
            if result["success"]:
                ...
                if attempt < max_retries:
                    try:
                        ...
                        rotate_resp = requests.post(f"{BASE}/integrations/rotate?category=news", timeout=10)
...
def get_top_headlines(limit: int = 10, category: Optional[str] = None) -> List[Dict[str, Any]]:
