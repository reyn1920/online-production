...
""""""
    ...
    try:
        ...
        if provider_key == "newsapi" and not creds.get("api_key"):
            secret_key = get_secret("newsapi_api_key")
            if secret_key:
                creds["api_key"] = secret_key
        elif provider_key == "guardian" and not creds.get("api_key"):
            secret_key = get_secret("guardian_api_key")
            if secret_key:
                creds["api_key"] = secret_key
        elif provider_key == "nytimes" and not creds.get("api_key"):
            secret_key = get_secret("nytimes_api_key")
            if secret_key:
                creds["api_key"] = secret_key
    except ImportError:
