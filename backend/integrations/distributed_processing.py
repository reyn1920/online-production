...
class DistributedProcessingSystem:
    ...
    def __init__(self, broker_url: str = None, result_backend: str = None):
        ...
        # Set up Celery app
        self.broker_url = broker_url or self.config.get(
    ...
    def submit_task(self, task: ProcessingTask) -> str:
        ...
        try:
            ...
            logger.info(
                f"Submitted task {task.task_id} to worker {best_worker.worker_id}"
            )
            return task.task_id
    ...
    def _find_best_worker(self, task: ProcessingTask) -> Optional[WorkerCapabilities]:
        ...
        for worker in self.workers.values():
            ...
            if not all(
                cap in worker.specialized_software for cap in task.required_capabilities
            ):
                continue
        ...
        suitable_workers.sort(
            key = lambda w: (w.current_load, -w.cpu_cores, -w.memory_gb)
        )
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        active_workers = len(
            [
                ...
                if w.last_heartbeat
                and datetime.now() - w.last_heartbeat < timedelta(minutes = 2)
            ]
        )
    ...
    def _execute_video_render(
        self, celery_task, task_data: Dict[str, Any]
    ) -> Dict[str, Any]:
