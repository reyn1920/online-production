...
class TwitterIntegration:
    ...
    def __init__(self, secrets_db_path: str = 'data/secrets.sqlite'):
        ...
        self.logger.info("Twitter integration initialized and authenticated successfully")
    ...
    def _load_credentials(self) -> TwitterCredentials:
        ...
        try:
            ...
            access_token_secret = self.secret_store.get_secret('TWITTER_ACCESS_TOKEN_SECRET')
            
            if not all([api_key, api_secret, access_token, access_token_secret]):
                raise AuthenticationError("Missing Twitter API credentials in secret store")
    ...
    def _generate_oauth_signature(self, method: str, url: str, params: Dict[str, str]) -> str:
        ...
        # Create signing key
        signing_key = f"{urllib.parse.quote(self.credentials.api_secret, safe='')}&{urllib.parse.quote(self.credentials.access_token_secret, safe='')}"
    ...
    def _create_oauth_header(self, method: str, url: str, params: Dict[str, str] = None) -> str:
        """
    ...
    def _check_rate_limit(self, endpoint: str) -> None:
        ...
        if endpoint in self.rate_limits:
            ...
            if rate_limit.remaining <= 0 and datetime.now() < rate_limit.reset_time:
                ...
                raise RateLimitError(f"Rate limit exceeded for {endpoint}. Reset in {wait_time:.0f}s")
        
        self.last_request_time = current_time
    
    def _update_rate_limit_info(self, response: requests.Response, endpoint: str) -> None:
        """
        ...
        if 'x-rate-limit-limit' in headers:
            ...
            self.logger.debug(f"Rate limit updated for {endpoint}: {remaining}/{limit} remaining")
    
    def _make_request(self, method: str, endpoint: str, params: Dict = None, 
                     data: Dict = None, files: Dict = None) -> requests.Response:
        """
        ...
        # Create OAuth header
        auth_header = self._create_oauth_header(method, url, params if method == 'GET' else data)
        
        headers = {
            'Authorization': auth_header,
            'User-Agent': 'TRAE.AI/1.0'
        }
    ...
    def post_tweet(self, tweet_data: TweetData) -> Dict[str, Any]:
        ...
        try:
            ...
            if tweet_data.hashtags:
                hashtags = ' '.join([f"#{tag.lstrip('#')}" for tag in tweet_data.hashtags if tag.strip()])
                if hashtags:
            ...
            if tweet_data.reply_to_id:
                data['reply'] = {
                    'in_reply_to_tweet_id': tweet_data.reply_to_id
                }
    ...
    def _upload_media(self, media_urls: List[str]) -> List[str]:
        ...
        for media_url in media_urls:
            try:
                ...
                files = {
                    'media': ('media', media_response.content, content_type)
                }
                ...
                if upload_response.status_code == 200:
                    ...
                    self.logger.error(f"Media upload failed: {upload_response.status_code} - {upload_response.text}")
    ...
    def search_tweets(self, query: str, max_results: int = 10, 
                     tweet_fields: List[str] = None) -> List[SearchResult]:
        """
            ...
            for tweet in tweets:
                try:
                    ...
                    if not all(key in tweet for key in ['id', 'text', 'author_id']):
                        self.logger.warning(f"Skipping tweet with missing required fields: {tweet.get('id', 'unknown')}")
                        continue
                    ...
                    if 'created_at' in tweet:
                        try:
                            ...
                            self.logger.warning(f"Invalid date format in tweet {tweet['id']}: {e}")
                    ...
                    result = SearchResult(
                        ...
                        relevance_score=self._calculate_relevance_score(tweet, sanitized_query)
                    )
                    results.append(result)
                    
                except Exception as tweet_error:
                    self.logger.warning(f"Error processing tweet {tweet.get('id', 'unknown')}: {tweet_error}")
                    continue
            
            self.logger.info(f"Successfully found {len(results)} tweets for query: {sanitized_query}")
            return results
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Twitter connection test successful for @{username} (ID: {user_id})")
            return True
