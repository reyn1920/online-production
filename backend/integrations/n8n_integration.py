...
class N8nIntegration:
    ...
    def _load_credentials(self) -> Dict[str, str]:
        ...
        try:
            with self.secret_store as store:
                ...
                missing_creds = [k for k, v in credentials.items() if not v and k != 'username' and k != 'password']
                if missing_creds:
    ...
    async def get_workflow(self, workflow_id: str) -> Optional[N8nWorkflow]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.logger.error(f"Failed to get workflow {workflow_id}: HTTP {response.status_code}")
                return None
    ...
    async def get_execution(self, execution_id: str) -> Optional[WorkflowExecution]:
        ...
        try:
            ...
            if execution_id in self.executions_cache:
                ...
                if cached_execution.status in [WorkflowStatus.SUCCESS, WorkflowStatus.ERROR, WorkflowStatus.CANCELED]:
            ...
            if response.status_code == 200:
                ...
                self.logger.error(f"Failed to get execution {execution_id}: HTTP {response.status_code}")
                return None
    ...
    async def create_webhook_workflow(self, 
                                    name: str,
                                    webhook_path: str,
                                    agent_tasks: List[Dict[str, Any]]) -> Optional[N8nWorkflow]:
    ...
    def _parse_workflow(self, workflow_data: Dict[str, Any]) -> N8nWorkflow:
        ...
        return N8nWorkflow(
            ...
            created_at=datetime.fromisoformat(workflow_data.get('createdAt', datetime.now().isoformat())),
            updated_at=datetime.fromisoformat(workflow_data.get('updatedAt', datetime.now().isoformat())),
    ...
    def _parse_execution(self, execution_data: Dict[str, Any]) -> WorkflowExecution:
        ...
        return WorkflowExecution(
            ...
            started_at=datetime.fromisoformat(execution_data.get('startedAt', datetime.now().isoformat())),
            finished_at=datetime.fromisoformat(execution_data['finishedAt']) if execution_data.get('finishedAt') else None,
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                conn.execute("""
                    ...
                    (id, name, active, nodes, connections, settings, created_at, updated_at, tags)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    workflow.id,
                    workflow.name,
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                conn.execute("""
                    ...
                    (id, workflow_id, status, started_at, finished_at, data, error, mode, retry_count)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    execution.id,
    ...
    async def _monitor_execution(self, execution_id: str):
        ...
        try:
            while True:
                ...
                if execution.status in [WorkflowStatus.SUCCESS, WorkflowStatus.ERROR, WorkflowStatus.CANCELED]:
