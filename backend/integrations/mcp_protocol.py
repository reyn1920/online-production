...
class MCPServer:
    ...
    def __init__(self, 
        ...
        self.info = MCPServerInfo(
            name=name,
            version=version,
            capabilities=capabilities or [MCPCapability.TOOLS, MCPCapability.RESOURCES, MCPCapability.CONTEXT],
    ...
    async def _handle_get_context(self, client_id: str, message: MCPMessage) -> MCPMessage:
        ...
        if context.expires_at and context.expires_at < datetime.now():
            ...
            return MCPMessage(
                ...
                error={
                    "code": -32602,
                    "message": f"Context expired: {context_id}"
                }
            )
...
class MCPClient:
    ...
    async def connect(self, uri: str) -> bool:
        ...
        try:
            ...
            if init_response and "result" in init_response:
                ...
                self.logger.info(f"Connected to MCP server: {self.server_info.get('serverInfo', {}).get('name', 'Unknown')}")
                return True
    ...
    async def set_context(self, 
        ...
        return response and "result" in response and response["result"].get("created", False)
...
if __name__ == "__main__":
    async def test_mcp_protocol():
        # Create MCP server
        server = MCPServer("TRAE-AI-MCP", capabilities=[
            MCPCapability.TOOLS,
        ...
        print("To connect a client, use: client = MCPClient() and await client.connect('ws://localhost:8765')")
