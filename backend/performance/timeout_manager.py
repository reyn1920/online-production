...
class TimeoutManager:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.response_times = {
            timeout_type: [] for timeout_type in TimeoutType
        }
    ...
    async def timeout_context(self, 
        ...
        operation_id = operation_id or f"{operation_type.value}_{int(time.time() * 1000)}"
        ...
        try:
            ...
            performance_level = self._classify_performance(duration, config.performance_target)
    ...
    def _record_performance_metric(self, metric: PerformanceMetric):
        ...
        if len(self.response_times[metric.operation_type]) > 1000:
            self.response_times[metric.operation_type] = self.response_times[metric.operation_type][-1000:]
        ...
        if metric.performance_level in [PerformanceLevel.SLOW, PerformanceLevel.CRITICAL]:
            audit_logger.log_security_event(
                ...
                severity=AuditLevel.WARNING if metric.performance_level == PerformanceLevel.SLOW else AuditLevel.ERROR,
                additional_data={
    ...
    async def chatgpt_api_call(self, 
                              url: str,
                              data: Dict[str, Any],
    ...
    def get_performance_stats(self, operation_type: Optional[TimeoutType] = None) -> Dict[str, Any]:
        ...
        compliance_status = 'compliant' if stats['p95'] <= self.config['performance_target_p95'] else 'violation'
    ...
    def generate_timeout_compliance_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            'compliance_summary': {
                ...
                'rule_10_performance_compliance': self.get_performance_stats().get('compliance_status') == 'compliant',
    ...
    def _generate_timeout_recommendations(self) -> List[str]:
        ...
        if stats.get('compliance_status') == 'violation':
            recommendations.append(f"P95 response time ({stats['p95']:.2f}s) exceeds target (5s)")
        
        cb_status = self.get_circuit_breaker_status()
        open_breakers = [op for op, status in cb_status.items() if status['state'] == 'open']
        if open_breakers:
        ...
        if long_running:
            recommendations.append(f"{len(long_running)} operations running longer than 30s")
