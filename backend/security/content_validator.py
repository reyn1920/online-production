...
class ProductionContentValidator:
    ...
    def validate_text_input(self, content: str, content_type: str = 'general') -> ValidationResult:
        ...
        if len(content) > max_length:
            result.is_valid = False
            result.errors.append(f"Content exceeds maximum length of {max_length} characters")
    ...
    def validate_html_content(self, content: str) -> ValidationResult:
        ...
        try:
            ...
            if sanitized != content:
                result.warnings.append("HTML content was sanitized - some elements were removed")
                ...
                if isinstance(obj, dict):
                    for key, value in obj.items():
                        ...
                        if key in dangerous_keys:
                            result.errors.append(f"Dangerous key detected: {current_path}")
                        ...
                        if isinstance(value, str):
                            ...
                            for pattern in self.dangerous_patterns:
                                if re.search(pattern, value, re.IGNORECASE):
                                    result.warnings.append(f"Suspicious pattern in {current_path}")
                                    result.risk_score += 20
    ...
    def validate_batch_content(self, content_items: List[Dict[str, Any]]) -> Dict[str, ValidationResult]:
