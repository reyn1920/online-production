...
class SecurityScanner:
    ...
    def __init__(self, base_dir: str = "."):
        ...
        self.scannable_extensions = {
            ...
            '.cs', '.cpp', '.c', '.h', '.hpp', '.sh', '.bash', '.zsh', '.ps1',
            '.sql', '.json', '.yaml', '.yml', '.xml', '.env', '.config', '.ini'
        }
    ...
    def scan_secrets(self, target_path: Optional[str] = None) -> ScanResult:
        ...
        try:
            ...
            logger.info(f"Secrets scan completed: {len(findings)} findings in {duration:.2f}s")
    ...
    def scan_vulnerabilities(self, target_path: Optional[str] = None) -> ScanResult:
        ...
        try:
                    ...
                    for rule_id, rule_config in self.vulnerability_patterns.items():
                        ...
                        for match in matches:
                            ...
                            finding = SecurityFinding(
                                ...
                                line_number=line_number,
                                rule_id=rule_id,
                                remediation=self._get_vulnerability_remediation(rule_id),
                                confidence='medium',
            ...
            self.scan_history.append(result)
            logger.info(f"Vulnerability scan completed: {len(findings)} findings in {duration:.2f}s")
        ...
        try:
            for file_path in target.rglob('*'):
                ...
                try:
                    ...
                    if (mode & 0o111) and file_path.suffix in {'.txt', '.md', '.json', '.yaml', '.yml'}:
                        finding = SecurityFinding(
                            severity='low',
                            category='permissions',
                    ...
                    if file_path.suffix in {'.sh', '.py', '.pl', '.rb'} and (mode & 0o022):  # Group/other writable
                        finding = SecurityFinding(
                            severity='medium',
                            category='permissions',
            ...
            self.scan_history.append(result)
            logger.info(f"Permissions scan completed: {len(findings)} findings in {duration:.2f}s")
    ...
    def comprehensive_scan(self, target_path: Optional[str] = None) -> Dict[str, ScanResult]:
        ...
        logger.info(f"Comprehensive scan completed: {total_findings} total findings, {critical_findings} critical")
    ...
    def _get_secret_remediation(self, rule_id: str) -> str:
        ...
        remediations = {
            ...
            'aws_access_key': 'Rotate AWS credentials immediately and use IAM roles or environment variables',
            'aws_secret_key': 'Rotate AWS credentials immediately and use IAM roles or environment variables',
            'github_token': 'Revoke GitHub token and use GitHub Actions secrets or environment variables',
            'private_key': 'Remove private key from code and use secure key management',
            'jwt_token': 'Remove JWT token from code and generate tokens dynamically',
            'database_url': 'Move database URL to environment variable or configuration file',
            'password': 'Remove hardcoded password and use secure authentication methods'
        }
        
        return remediations.get(rule_id, 'Remove sensitive data from code and use secure storage')
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Security Scanner - Comprehensive security analysis')
    parser.add_argument('--base-dir', default='.', help='Base directory to scan (default: current)')
    parser.add_argument('--scan-type', choices=['secrets', 'vulnerabilities', 'permissions', 'all'],
