...
from backend.integrations.youtube_integration import YouTubeIntegration, VideoMetadata, VideoPrivacy, VideoCategory
...
from content_automation_pipeline import ContentAutomationPipeline, ContentOpportunity, ContentProject
...
class YouTubeAutomationOrchestrator:
                ...
                with open(channels_config_path, 'r') as f:
                    ...
                    for channel_data in data.get('channels', []):
                        channel = YouTubeChannel(
                            ...
                            content_strategy=ContentStrategy(channel_data.get('content_strategy', 'mixed')),
                            scheduling_strategy=SchedulingStrategy(channel_data.get('scheduling_strategy', 'optimal_timing')),
                            upload_frequency=channel_data.get('upload_frequency', 3),
                            optimal_times=channel_data.get('optimal_times', ['10:00', '14:00', '18:00']),
    ...
    async def _generate_content_opportunity(self, channel: YouTubeChannel) -> Optional[Dict[str, Any]]:
    ...
    async def create_manual_video(self, channel_id: str, topic: str, 
    ...
    async def _create_video_project(self, channel: YouTubeChannel, 
    ...
    async def _optimize_video_seo(self, project: Dict[str, Any], 
    ...
    async def _schedule_video_upload(self, channel: YouTubeChannel, 
