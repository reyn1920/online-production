...
class AlertManager:
    ...
    def _setup_notification_channels(self):
        ...
        if self.config['webhook_endpoints']:
            self.notification_channels[NotificationChannel.WEBHOOK] = {
                'enabled': True,
                'config': {
                    'endpoints': [url.strip() for url in self.config['webhook_endpoints'] if url.strip()]
                }
            }
    ...
    def _monitoring_loop(self):
        ...
        while self.monitoring_active:
            try:
                # Collect metrics
                if time.time() - self.last_metric_collection >= self.config['metric_collection_interval']:
    ...
    def _collect_metrics(self):
        ...
        try:
            ...
            if self.config['enable_system_monitoring']:
                ...
                self._store_metric('system.cpu_percent', system_metrics.cpu_percent, current_time)
                self._store_metric('system.memory_percent', system_metrics.memory_percent, current_time)
                self._store_metric('system.disk_percent', system_metrics.disk_percent, current_time)
                self._store_metric('system.process_count', system_metrics.process_count, current_time)
                self._store_metric('system.uptime_seconds', system_metrics.uptime_seconds, current_time)
                
                if system_metrics.load_average:
                    self._store_metric('system.load_average_1m', system_metrics.load_average[0], current_time)
                    if len(system_metrics.load_average) > 1:
                        self._store_metric('system.load_average_5m', system_metrics.load_average[1], current_time)
                    if len(system_metrics.load_average) > 2:
                        self._store_metric('system.load_average_15m', system_metrics.load_average[2], current_time)
            
            # Collect application metrics
            if self.config['enable_application_monitoring']:
                app_metrics = self._collect_application_metrics()
                if app_metrics:
                    self._store_metric('app.request_count', app_metrics.request_count, current_time)
                    self._store_metric('app.error_count', app_metrics.error_count, current_time)
                    self._store_metric('app.response_time_ms', app_metrics.response_time_ms, current_time)
                    self._store_metric('app.active_connections', app_metrics.active_connections, current_time)
                    self._store_metric('app.queue_size', app_metrics.queue_size, current_time)
                    self._store_metric('app.cache_hit_rate', app_metrics.cache_hit_rate, current_time)
                    self._store_metric('app.database_connections', app_metrics.database_connections, current_time)
            
            # Collect health metrics
            health_status = health_monitor.get_system_health()
            self._store_metric('health.overall_score', health_status.overall_health_score, current_time)
            self._store_metric('health.availability', 100.0 if health_status.status == HealthStatus.HEALTHY else 0.0, current_time)
            
            # Collect compliance metrics
            if self.config['enable_compliance_monitoring']:
                compliance_report = compliance_monitor.get_compliance_report()
                self._store_metric('compliance.score', compliance_report['overall_compliance_score'], current_time)
                self._store_metric('compliance.violations', len(compliance_report['violations']), current_time)
    ...
    def _collect_system_metrics(self) -> SystemMetrics:
        ...
        try:
            ...
            return SystemMetrics(
                ...
                network_io={}, process_count=0, load_average=[],
    ...
    def _evaluate_rule_condition(self, rule: AlertRule, current_time: float) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Error evaluating condition for rule {rule.rule_id}: {str(e)}")
            return False
    
    def _build_evaluation_context(self, rule: AlertRule, current_time: float) -> Dict[str, Any]:
        ...
        if 'system.memory_percent' in self.metrics_buffer:
            context['memory_percent'] = self._get_latest_metric_value('system.memory_percent')
        
        if 'system.disk_percent' in self.metrics_buffer:
            context['disk_percent'] = self._get_latest_metric_value('system.disk_percent')
        
        if 'app.response_time_ms' in self.metrics_buffer:
            context['response_time_ms'] = self._get_latest_metric_value('app.response_time_ms')
        
        if 'health.availability' in self.metrics_buffer:
            context['availability'] = self._get_latest_metric_value('health.availability')
        
        if 'compliance.score' in self.metrics_buffer:
            context['compliance_score'] = self._get_latest_metric_value('compliance.score')
        ...
        context['error_rate'] = (error_count / request_count * 100) if request_count > 0 else 0
    ...
    def _check_condition_duration(self, rule: AlertRule, condition_met: bool, current_time: float) -> bool:
            ...
            try:
                if channel in self.notification_channels and self.notification_channels[channel]['enabled']:
                    ...
                    if not success:
                        ...
                        self.logger.warning(f"Failed to send notification via {channel.value} for alert {alert.alert_id}")
                
            except Exception as e:
                self.logger.error(f"Error sending notification via {channel.value}: {str(e)}")
...
"""
    ...
    def _process_escalations(self):
        ...
        for alert in list(self.active_alerts.values()):
            ...
            alert_age_minutes = (current_time - datetime.fromisoformat(alert.created_at).timestamp()) / 60
            
            for escalation in rule.escalation_rules:
                if (alert_age_minutes >= escalation['after_minutes'] and 
                    alert.escalation_level < len(rule.escalation_rules)):
                    ...
                    for channel_name in escalation['channels']:
                        try:
                            ...
                            self.logger.warning(f"Unknown notification channel in escalation: {channel_name}")
    ...
    def _cleanup_old_data(self):
        ...
        current_time = time.time()
        cutoff_time = current_time - (self.config['alert_history_retention_days'] * 24 * 3600)
        
        # Cleanup old alert history
        while (self.alert_history and 
               datetime.fromisoformat(self.alert_history[0].created_at).timestamp() < cutoff_time):
    ...
    def get_monitoring_report(self) -> Dict[str, Any]:
        ...
        recent_alerts = [
            ...
            if datetime.fromisoformat(alert.created_at).timestamp() > current_time - 24*3600
        ]
        ...
        return {
            ...
            'monitoring_status': {
                ...
                'uptime_hours': uptime_seconds / 3600,
                'last_metric_collection': datetime.fromtimestamp(self.last_metric_collection).isoformat()
            },
            ...
            'performance_metrics': {
                ...
                'total_metric_points': sum(len(buffer) for buffer in self.metrics_buffer.values()),
...
def create_custom_alert(title: str, description: str, severity: AlertSeverity, category: AlertCategory = AlertCategory.APPLICATION) -> str:
...
def get_system_health_summary() -> Dict[str, Any]:
    ...
    return {
        ...
        'memory_percent': alert_manager._get_latest_metric_value('system.memory_percent'),
