...
class HealthMonitor:
    ...
    def _register_default_health_checks(self):
        ...
        self.register_health_check('internet_connectivity', self._check_internet_connectivity)
        ...
        for endpoint in self.config['system_endpoints']:
            ...
            self.register_health_check(
                endpoint_name, 
                lambda url=endpoint: self._check_http_endpoint(url)
            )
    ...
    async def _check_disk_usage(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if usage_percent > self.config['disk_usage_threshold']:
                status = HealthStatus.CRITICAL if usage_percent > 95 else HealthStatus.WARNING
    ...
    async def _check_critical_processes(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for process in critical_processes:
                if not any(process in running_proc for running_proc in running_processes):
                    missing_processes.append(process)
            
            status = HealthStatus.HEALTHY if not missing_processes else HealthStatus.CRITICAL
    ...
    async def _trigger_alert(self, rule: AlertRule, metric: SystemMetric):
        ...
        audit_logger.log_security_event(
            ...
            severity=AuditLevel.ERROR if rule.severity in [AlertSeverity.CRITICAL, AlertSeverity.EMERGENCY] else AuditLevel.WARNING,
            additional_data={
                'alert_id': alert_id,
                'rule_id': rule.rule_id,
                'metric_name': metric.name,
    ...
    async def _monitoring_loop(self):
        ...
        while self.monitoring_active:
            try:
                ...
                if current_time - last_metric_collection >= self.config['metric_collection_interval']:
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        if any(check['status'] == HealthStatus.CRITICAL.value for check in recent_checks.values()):
            overall_status = HealthStatus.CRITICAL
        elif any(check['status'] == HealthStatus.WARNING.value for check in recent_checks.values()):
            overall_status = HealthStatus.WARNING
        elif any(check['status'] == HealthStatus.UNKNOWN.value for check in recent_checks.values()):
    ...
    def get_monitoring_report(self) -> Dict[str, Any]:
        ...
        alert_summary = {
            ...
            'total_alerts_24h': len([
                ...
                if datetime.fromisoformat(alert.timestamp) > datetime.utcnow() - timedelta(hours=24)
            ]),
            'critical_alerts_24h': len([
                alert for alert in self.alert_history
                if (datetime.fromisoformat(alert.timestamp) > datetime.utcnow() - timedelta(hours=24) and
