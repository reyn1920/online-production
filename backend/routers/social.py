...
async def post_content(request: PostRequest):
    ...
    try:
        ...
        if platform == "instagram":
            ...
            if request.media_url:
                result = client.create_pin(request.content, request.media_url, request.extra.get("board_id", ""))
            else:
                result = {"ok": False, "error": "Pinterest requires media_url"}
        elif platform == "reddit":
            subreddit = request.extra.get("subreddit", "test")
            result = client.submit_post(subreddit, request.content[:300], request.content, request.media_url)
