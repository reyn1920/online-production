...
async def stripe_webhook(request: Request, stripe_signature: Optional[str] = Header(None, alias="stripe-signature")):
    ...
    try:
        ...
        if stripe_secret and not verify_stripe_signature(payload, stripe_signature or "", stripe_secret):
...
async def handle_successful_payment(provider: str, payment_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "amount": payment_data.get('amount') or payment_data.get('amount_with_breakdown', {}).get('gross_amount', {}).get('value'),
...
async def handle_subscription_created(provider: str, subscription_data: Dict[str, Any]) -> Dict[str, Any]:
...
async def handle_subscription_cancelled(provider: str, subscription_data: Dict[str, Any]) -> Dict[str, Any]:
