...
SOFTWARE_REGISTRY = {
    ...
    "blender": {
        ...
        "capabilities": ["3d_modeling", "animation", "rendering", "video_editing", "scripting"],
    ...
    "davinci_resolve": {
        ...
        "capabilities": ["video_editing", "color_grading", "audio_editing", "vfx", "rendering"],
        "integration_type": "creative",
    ...
    "gimp": {
        ...
        "capabilities": ["image_editing", "photo_manipulation", "graphics_design", "scripting"],
        "integration_type": "creative",
    ...
    "inkscape": {
        ...
        "capabilities": ["vector_graphics", "svg_editing", "illustration", "logo_design"],
        "integration_type": "creative",
    ...
    "xcode": {
        ...
        "capabilities": ["ios_development", "macos_development", "swift", "objective_c", "debugging"],
    ...
    "cursor": {
        ...
        "capabilities": ["code_editing", "ai_assistance", "debugging", "git_integration"],
        "integration_type": "development",
        "file_formats": [".*"]
    },
    "sublime_text": {
    ...
    "github_desktop": {
        ...
        "capabilities": ["git_gui", "repository_management", "branch_management", "pull_requests"],
        "integration_type": "development",
        "file_formats": []
    },
    ...
    "ollama": {
        ...
        "capabilities": ["local_llm", "model_management", "api_server", "chat_interface"],
        "integration_type": "ai",
        "file_formats": []
    },
    "jan": {
        ...
        "capabilities": ["local_ai", "model_management", "chat_interface", "privacy_focused"],
        "integration_type": "ai",
        "file_formats": []
    },
    ...
    "docker": {
        ...
        "capabilities": ["containerization", "deployment", "microservices", "orchestration"],
    ...
    "ffmpeg": {
        ...
        "capabilities": ["video_conversion", "audio_conversion", "streaming", "codec_support"],
    ...
    "imagemagick": {
        ...
        "capabilities": ["image_conversion", "image_manipulation", "batch_processing", "format_support"],
        "integration_type": "media",
    ...
    "node": {
        ...
        "capabilities": ["javascript_runtime", "npm_packages", "web_development", "api_development"],
        "integration_type": "development",
    ...
    "python": {
        ...
        "capabilities": ["scripting", "data_science", "machine_learning", "web_development"],
        "integration_type": "development",
...
async def check_software_availability(software_key: str) -> bool:
    ...
    if "cli_path" in software:
        return os.path.exists(software["cli_path"]) or shutil.which(software["cli_path"].split('/')[-1]) is not None
...
async def execute_software_command(request: CommandRequest):
    ...
    if executable.endswith(".app"):
        ...
        result = await execute_command("open", command_args[1:], request.working_directory)
    else:
        # Direct CLI execution
        result = await execute_command(executable, [request.command] + request.args, request.working_directory)
    
    return result

@router.post("/blender/render")
async def blender_render(background_tasks: BackgroundTasks, blend_file: str, output_path: str, frame_range: Optional[str] = None):
...
async def imagemagick_convert(input_file: str, output_file: str, options: List[str] = None):
...
async def git_operation(operation: str, args: List[str] = None, repository_path: str = None):
    ...
    result = await execute_command(SOFTWARE_REGISTRY["git"]["cli_path"], command_args, repository_path)
    return result
...
async def docker_operation(operation: str, args: List[str] = None):
    ...
    result = await execute_command(SOFTWARE_REGISTRY["docker"]["cli_path"], command_args)
    return result
...
async def ollama_chat(model: str, prompt: str, system_prompt: Optional[str] = None):
    ...
    result = await execute_command(SOFTWARE_REGISTRY["ollama"]["cli_path"], ["run", model, prompt])
    return result
    ...
    for step in task.steps:
        ...
        try:
            ...
            if software.get("cli_path"):
                result = await execute_command(software["cli_path"], [command] + args, step.get("working_directory"))
            else:
                # For GUI apps, use open command
                result = await execute_command("open", ["-a", software["path"]], step.get("working_directory"))
...
async def get_creative_software():
    ...
    creative_software = {k: v for k, v in SOFTWARE_REGISTRY.items() if v["integration_type"] == "creative"}
...
async def get_development_software():
    ...
    dev_software = {k: v for k, v in SOFTWARE_REGISTRY.items() if v["integration_type"] == "development"}
...
async def get_ai_software():
    ...
    ai_software = {k: v for k, v in SOFTWARE_REGISTRY.items() if v["integration_type"] == "ai"}
...
async def creative_pipeline(input_files: List[str], output_directory: str, pipeline_type: str):
        ...
        for input_file in input_files:
            ...
            if input_file.endswith('.blend'):
                blender_result = await blender_render(
                    None, input_file, 
                    f"{output_directory}/blender_output.mov"
                )
