...
from backend.pipelines.resolve_handoff import (get_resolve_path, set_resolve_path,
...
async def get_davinci_status():
    ...
    try:
        ...
        if voice_integration:
            ...
            status["supported_formats"] = list(
                voice_integration.supported_formats.keys()
            )
...
async def create_project(project_data: ResolveProjectCreate):
    ...
    if not resolve_integration:
        raise HTTPException(
            status_code=503, detail="DaVinci Resolve integration not available"
        )
    ...
    try:
        summary = {
            ...
            "voice_features": (
                {
                    "available_voices": (
                        voice_integration.get_available_voices()
                        if voice_integration
                        else []
                    ),
                    "supported_formats": (
