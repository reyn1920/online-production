...
try:
    from backend.database.chat_db import chat_db, create_conversation, add_message, get_messages, get_conversations
except ImportError:
    # Fallback for different import paths
    try:
        from database.chat_db import chat_db, create_conversation, add_message, get_messages, get_conversations
    except ImportError:
...
class ConnectionManager:
    ...
    async def broadcast_to_room(self, message: str, room_id: str, exclude_connection: Optional[str] = None):
...
rooms: Dict[str, ChatRoom] = {
    "general": ChatRoom(id="general", name="General Chat")
}

# AI Integration Functions
async def get_ai_response(message: str, provider: str = "openai", context: Optional[Dict] = None) -> str:
    ...
    async with httpx.AsyncClient() as client:
        response = await client.post(
            ...
            json={
                "model": "gpt-3.5-turbo",
                "messages": [
                    {"role": "system", "content": "You are a helpful assistant integrated into TRAE.AI production system. Be concise and helpful."},
...
async def get_integration_data(integration_type: str, query: str, parameters: Optional[Dict] = None) -> Dict[str, Any]:
...
async def get_weather_data(query: str, parameters: Optional[Dict] = None) -> Dict[str, Any]:
    """Get weather data"""
...
async def get_news_data(query: str, parameters: Optional[Dict] = None) -> Dict[str, Any]:
    """Get news data"""
...
async def get_image_data(query: str, parameters: Optional[Dict] = None) -> Dict[str, Any]:
    """Get image data"""
...
async def get_pet_data(query: str, parameters: Optional[Dict] = None) -> Dict[str, Any]:
    ...
    try:
        async with httpx.AsyncClient() as client:
            response = await client.get(f"http://localhost:8000/pets/search?animal={query}&limit=3")
...
async def websocket_endpoint(websocket: WebSocket, user_id: str, room_id: str = Query("general")):
        ...
        while True:
            ...
            if message_type == "message":
                ...
                if chat_db and conversation_id:
                    add_message(conversation_id, "user", content, "text", {
                        "room_id": room_id,
                        "user_id": user_id
                    })
                ...
                await manager.broadcast_to_room(json.dumps(broadcast_data), room_id, exclude_connection=connection_id)
                ...
                if content_lower.startswith("/ai ") or content_lower.startswith("@ai "):
                    ...
                    if "articles" in news_data:
                        ...
                        response = f"News error: {news_data.get('error', 'Unknown error')}"
                    response_type = "integration_response"
                    ...
                    if "images" in image_data:
                        ...
                        response = f"Image error: {image_data.get('error', 'Unknown error')}"
                    response_type = "integration_response"
                    ...
                    if "pets" in pet_data:
                        ...
                        response = f"Pet error: {pet_data.get('error', 'Unknown error')}"
                    response_type = "integration_response"
                ...
                if response:
                    ...
                    if chat_db and conversation_id:
                        add_message(conversation_id, "assistant", response, response_type, {
                            "room_id": room_id,
                            "command": content.split()[0] if content.startswith("/") else None
                        })
...
async def get_conversation_messages(user_id: str, conversation_id: str, limit: int = 100):
