...
try:
    ...
    logging.warning(
        "HypocrisyDatabaseManager not available. Hypocrisy engine will be limited."
    )
...
class HypocrisyEngine:
    ...
    async def _find_contradictions(self, claim: ResearchClaim) -> List[Dict[str, Any]]:
        ...
        if not self.db_manager:
            self.logger.warning(
                "Database manager not available for contradiction search"
            )
    ...
    def _generate_validation_notes(
        ...
        notes.append(
            f"Claim validation completed with {confidence_score:.2f} confidence."
        )
        ...
        if contradictions:
            ...
            for i, contradiction in enumerate(contradictions[:3], 1):
                ...
                notes.append(
                    f"     Similarity: {contradiction.get('similarity_score', 0):.2f}"
                )
        else:
    ...
    def _extract_keywords(self, text: str) -> List[str]:
        ...
        try:
            ...
            for word, pos in blob.tags:
                if (
                    pos in ["NN", "NNS", "NNP", "NNPS", "JJ", "JJR", "JJS"]
                    and len(word) > 3
                ):
    ...
    async def detect_hypocrisy(
        ...
        try:
            ...
            for finding in existing_findings:
                ...
                ):  # Some similarity but potentially contradictory
                    ...
                    if contradiction_score > 0.6:  # Significant contradiction
                        ...
                        hypocrisy_finding = HypocrisyFinding(
                            ...
                            statement_1 = finding["statement_1"],
                                ...
                                severity_score = max(
                                1, min(10, int(contradiction_score * 10))
                            ),
                        ...
                        if finding_id:
                            self.logger.info(
                                f"Detected and stored hypocrisy finding {finding_id}"
                            )
    ...
    def get_content_opportunities(
        ...
        try:
            ...
            for opp in opportunities:
                ...
                enhanced_opp["content_potential_score"] = (
                    self._calculate_content_potential(opp)
                )
            ...
            enhanced_opportunities.sort(
                key = lambda x: x["content_potential_score"], reverse = True
            )
    ...
    def _calculate_content_potential(self, opportunity: Dict[str, Any]) -> float:
        ...
        try:
            ...
            subject_boost = (
                0.1 if opportunity.get("subject_type") == "politician" else 0.0
            )
    ...
    def _suggest_content_angles(self, opportunity: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            if contradiction_type == "temporal":
                ...
                angles.append(
                    f"{subject_name} Says Different Things to Different Audiences"
                )
                angles.append(
                    f"Context Matters: {subject_name}'s Contradictory Messages"
                )
