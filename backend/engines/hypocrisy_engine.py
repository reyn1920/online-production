...
class HypocrisyEngine:
    ...
    async def validate_research_claim(self, claim: ResearchClaim) -> ValidationResult:
        ...
        try:
            ...
            is_valid = len(contradictions) == 0 and confidence_score >= self.min_confidence_threshold
            ...
            self.logger.info(f"Validation completed. Valid: {is_valid}, Confidence: {confidence_score:.2f}")
            return result
    ...
    def _calculate_validation_confidence(self, 
        ...
        try:
            ...
            confidence = base_confidence + evidence_bonus + author_bonus + recency_bonus - contradiction_penalty
    ...
    def _generate_validation_notes(self, 
        ...
        if contradictions:
            ...
            for i, contradiction in enumerate(contradictions[:3], 1):
                notes.append(f"  {i}. Contradictory statement from {contradiction.get('source', 'unknown source')}")
    ...
    def _calculate_semantic_similarity(self, text1: str, text2: str) -> float:
        ...
        try:
            ...
            # Extract key terms
            terms1 = set([word.lower() for word, pos in blob1.tags 
                         if pos in ['NN', 'NNS', 'NNP', 'NNPS', 'VB', 'VBD', 'VBG', 'VBN', 'VBP', 'VBZ']])
            terms2 = set([word.lower() for word, pos in blob2.tags 
                         if pos in ['NN', 'NNS', 'NNP', 'NNPS', 'VB', 'VBD', 'VBG', 'VBN', 'VBP', 'VBZ']])
    ...
    async def detect_hypocrisy(self, 
                             subject_name: str, 
                             new_statement: str, 
                             source: str, 
                             context: Optional[str] = None) -> Optional[HypocrisyFinding]:
        ...
        try:
            ...
            existing_findings = self.db_manager.search_findings(
                ...
                if similarity_score > 0.3:  # Some similarity but potentially contradictory
                    ...
                    if contradiction_score > 0.6:  # Significant contradiction
                        ...
                        hypocrisy_finding = HypocrisyFinding(
                            ...
                            context_2=context,
                            date_1=datetime.fromisoformat(finding['date_1']) if finding.get('date_1') else None,
                            date_2=datetime.now(),
    ...
    def _analyze_contradiction(self, statement1: str, statement2: str) -> float:
        ...
        try:
            ...
            contradiction_indicators = [
                ...
                ('approve', 'disapprove'), ('favor', 'against'), ('for', 'against')
            ]
    ...
    def get_content_opportunities(self, limit: int = 10, min_confidence: float = 0.7) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            enhanced_opportunities.sort(
                key=lambda x: x['content_potential_score'], 
                reverse=True
            )
