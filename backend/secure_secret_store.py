...
class SecretStore:
    ...
    def _encrypt_value(self, value: str, method: EncryptionMethod = EncryptionMethod.FERNET) -> Tuple[bytes, bytes, Optional[bytes]]:
    ...
    def _decrypt_value(self, encrypted_value: bytes, salt: bytes, nonce: Optional[bytes], method: EncryptionMethod) -> str:
    ...
    def _log_access(self, secret_id: str, action: str, success: bool = True, error_message: Optional[str] = None):
    ...
    def store_secret(self,
                    ...
                    description: str = "",
                    rotation_interval: Optional[int] = None) -> str:
        ...
        with self._lock:
            ...
            try:
                ...
                with self._database_transaction() as conn:
                    ...
                    """, (
                        secret_id, name, secret_type.value, access_level.value, encryption_method.value,
                        encrypted_value, salt, nonce, checksum, metadata.created_at, metadata.updated_at,
                        expires_at, json.dumps(tags or []), description, rotation_interval, True
                    ))
    ...
    def get_secret(self, name: str, access_level_required: AccessLevel = AccessLevel.INTERNAL) -> Optional[str]:
            ...
            try:
                with self._database_transaction() as conn:
                    ...
                    if secret_access_level.value > access_level_required.value:
                        self._log_access(row['secret_id'], "GET", False, "Insufficient access level")
                        logger.warning(f"Access denied for secret '{name}': insufficient access level")
                        return None
            ...
            try:
                with self._database_transaction() as conn:
                    ...
                    conn.execute("""
                        ...
                        WHERE secret_id = ?
                    """, (
                        encrypted_value, salt, nonce, new_checksum, datetime.now(), row['secret_id']
                    ))
                    
                    # Log rotation
                    conn.execute(
                        "INSERT INTO key_rotation_log (secret_id, old_checksum, new_checksum, rotation_reason, timestamp) VALUES (?, ?, ?, ?, ?)",
                        (row['secret_id'], old_checksum, new_checksum, "Manual update", datetime.now())
                    )
    ...
    def list_secrets(self, secret_type: Optional[SecretType] = None, access_level: Optional[AccessLevel] = None) -> List[Dict[str, Any]]:
    ...
    def get_access_log(self, secret_name: Optional[str] = None, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        try:
            with self._database_transaction() as conn:
                if secret_name:
                    # Get secret_id first
                    row = conn.execute(
                        "SELECT secret_id FROM secrets WHERE name = ?",
                        (secret_name,)
                    ).fetchone()
...
if __name__ == "__main__":
    ...
    for entry in log_entries:
        print(f"- {entry['timestamp']}: {entry['action']} - {'SUCCESS' if entry['success'] else 'FAILED'}")
