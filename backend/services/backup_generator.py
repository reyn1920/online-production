...
class TemplateManager:
    ...
    def get_best_template(
        self, category: str, mode: GenerationMode
    ) -> Optional[BackupTemplate]:
        ...
        if cache_key in self.cache:
            ...
            if cached_model.expires_at > datetime.now():
                ...
                if not cached_model.model_data and cached_model.file_path:
                    cached_model.model_data = self._load_model_data(
                        cached_model.file_path
                    )
...
class BackupGenerator:
        ...
        for strategy in self.strategy_priority:
            try:
                result = await self._try_strategy(
                    strategy, model_type, parameters, mode
                )
                if result.success:
    ...
    async def _try_template_based(
        ...
        if template:
            model_data = self.template_manager.generate_from_template(
                template, parameters
            )
    ...
    async def _try_emergency_response(
        ...
        model_data = self.emergency_generator.generate_emergency_response(
            model_type, parameters
        )
    ...
    def _calculate_cache_hit_rate(self) -> float:
        ...
        total_access = sum(
            model.access_count for model in self.model_cache.cache.values()
        )
        if total_access == 0:
            return 0.0
...
def cache_model(
    ...
    return backup_generator.model_cache.cache_model(
        model_type, parameters, model_data, ttl_hours
    )
