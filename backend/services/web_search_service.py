...
Integrates web search capabilities to automatically discover and evaluate APIs for marketing channels.
...
class WebSearchService:
    ...
    def __init__(self):
        ...
        self.quality_indicators = {
            'positive': [
                'free tier', 'free plan', 'no cost', 'open source',
                'generous limits', 'good documentation', 'active community',
                'reliable uptime', 'fast response', 'easy integration'
            ],
            'negative': [
                'expensive', 'limited free', 'poor documentation',
                'unreliable', 'slow response', 'complex setup',
                'deprecated', 'shutting down'
            ]
        }
    
    def search_apis_for_channel(self, channel: str, max_results: int = 10) -> List[APICandidate]:
        """Search for APIs suitable for a specific marketing channel."""
        try:
            search_queries = self.api_search_patterns.get(channel, [f"{channel} API free alternatives"])
            all_candidates = []
    ...
    def _perform_web_search(self, query: str, max_results: int = 5) -> List[SearchResult]:
    ...
    def _extract_api_candidates(self, search_results: List[SearchResult], channel: str) -> List[APICandidate]:
        """Extract API candidates from search results."""
        candidates = []
        ...
        for result in search_results:
            try:
                ...
                pricing_model = self._determine_pricing_model(result.snippet, result.api_indicators)
    ...
    def _extract_api_name(self, title: str) -> str:
        ...
        name = re.sub(r'\s*-\s*(API|Free|Tier|Plan|Pricing).*$', '', title, flags=re.IGNORECASE)
    ...
    def _calculate_quality_score(self, result: SearchResult) -> float:
        ...
        for indicator in result.api_indicators:
            if any(pos in indicator.lower() for pos in self.quality_indicators['positive']):
                score += 0.1
            if any(neg in indicator.lower() for neg in self.quality_indicators['negative']):
                score -= 0.2
        ...
        well_known = ['google', 'microsoft', 'amazon', 'github', 'mailgun', 'sendgrid', 'twilio']
        if any(provider in result.url.lower() for provider in well_known):
            score += 0.15
        
        return min(1.0, max(0.0, score))
    
    def _deduplicate_candidates(self, candidates: List[APICandidate]) -> List[APICandidate]:
    ...
    def _rank_candidates(self, candidates: List[APICandidate]) -> List[APICandidate]:
        ...
        def ranking_key(candidate):
            ...
            pricing_score = {'free': 3, 'freemium': 2, 'paid': 1}[candidate.pricing_model]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--action', choices=['search', 'trending', 'evaluate'], required=True)
    ...
    if args.action == 'search' and args.channel:
        ...
        for candidate in candidates[:5]:
            print(f"  {candidate.name} - {candidate.pricing_model} - Score: {candidate.quality_score:.2f}")
