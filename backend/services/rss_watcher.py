...
class RSSWatcherService:
    ...
    def _calculate_urgency_score(self, article: NewsArticle, feed_config: Dict) -> float:
        ...
        high_priority_keywords = [
            ...
            'scandal', 'crisis', 'emergency', 'investigation', 'leaked'
        ]
        ...
        logger.debug(f"Urgency calculation for '{article.title[:50]}...': {urgency_factors} -> {urgency_score:.3f}")
    ...
    def _is_duplicate_item(self, feed_name: str, article: NewsArticle) -> bool:
        ...
        try:
            ...
            if count == 0:
                # Store new item
                cursor.execute("""
                    INSERT INTO rss_feed_items (feed_name, item_url, item_title, item_hash)
                    VALUES (?, ?, ?, ?)
...
"""
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.post(
                    f"{self.api_base_url}/api/create_video",
                    json=payload
                )
                ...
                if response.status_code == 200:
                    ...
                    logger.info(f"Video creation initiated: {task_id} for trigger {trigger.trigger_id}")
                    return task_id
                else:
            ...
            for feed_config in self.rss_engine.feeds:
                ...
                try:
                    ...
                    for article in articles:
                        ...
                        # Calculate urgency
                        urgency_score = self._calculate_urgency_score(article, feed_config)
                        
                        # Only create trigger if urgency meets threshold
                        if urgency_score >= self.min_urgency_threshold:
                            # Create trigger
                            trigger_id = f"rss_{int(time.time())}_{hash(article.url) % 10000}"
                            ...
                            logger.info(f"Created video trigger: {trigger.title[:50]}... (urgency: {urgency_score:.3f})")
                        
                except Exception as e:
                    logger.error(f"Error processing feed {feed_name}: {e}")
                    continue
            
            logger.info(f"RSS processing complete. Created {triggers_created} video triggers.")
    ...
    async def run_continuous_monitoring(self):
        ...
        self.running = True
        logger.info(f"Starting RSS watcher (interval: {self.monitoring_interval} seconds)")
    ...
    def start_monitoring(self, monitoring_interval: int = None, min_urgency_threshold: float = None,
                        include_affiliates: bool = None, video_duration: int = None):
        ...
        logger.info(f"RSS monitoring started (interval: {self.monitoring_interval}s, threshold: {self.min_urgency_threshold})")
        return {
            "status": "started",
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        for trigger in triggers:
            print(f"- {trigger['title'][:50]}... (urgency: {trigger['urgency_score']:.3f})")
