...
from .web_search_service import WebSearchService, APICandidate
...
class AutoDiscoveryService:
    ...
    def add_channel(self, channel_name: str, custom_profile: Optional[ChannelProfile] = None) -> bool:
        ...
        try:
            ...
            if profile.auto_discovery_enabled:
                self.logger.info(f"Running initial API discovery for new channel: {channel_name}")
                discovery_result = self.discover_apis_for_channel(channel_name)
                
                if discovery_result:
                    self.logger.info(f"Discovered {len(discovery_result.discovered_apis)} APIs for {channel_name}")
                    return True
    ...
    def discover_apis_for_channel(self, channel_name: str) -> Optional[DiscoveryResult]:
        ...
        try:
            ...
            discovery_result = DiscoveryResult(
                ...
                next_discovery_date=datetime.now() + timedelta(days=profile.discovery_frequency_days)
            )
            ...
            self.logger.info(f"Discovery completed for {channel_name}: {len(recommended_apis)} recommended APIs")
    ...
    def run_scheduled_discovery(self) -> Dict[str, DiscoveryResult]:
        ...
        try:
            ...
            self.logger.info(f"Running scheduled discovery for {len(due_channels)} channels")
    ...
    def _filter_and_rank_apis(self, apis: List[APICandidate], profile: ChannelProfile) -> List[APICandidate]:
        ...
        for api in apis:
            ...
            if profile.preferred_pricing == 'free' and api.pricing_model != 'free':
                continue
            elif profile.preferred_pricing == 'freemium' and api.pricing_model == 'paid':
                continue
    ...
    def _calculate_preference_score(self, api: APICandidate, profile: ChannelProfile) -> float:
        ...
        score += pricing_scores.get(profile.preferred_pricing, {}).get(api.pricing_model, 0.0)
        
        return min(1.0, score)
    
    def _analyze_costs(self, apis: List[APICandidate], budget_limit: float) -> Dict[str, Any]:
        """Analyze costs of recommended APIs."""
        analysis = {
            'total_free_apis': len([api for api in apis if api.pricing_model == 'free']),
            'total_freemium_apis': len([api for api in apis if api.pricing_model == 'freemium']),
            'total_paid_apis': len([api for api in apis if api.pricing_model == 'paid']),
        ...
        for api in apis:
            if api.pricing_model == 'paid' and api.cost_estimate:
                ...
                import re
                cost_match = re.search(r'\$([0-9,]+(?:\.[0-9]{2})?)', api.cost_estimate or '')
                if cost_match:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                """, (
                    ...
                    profile.last_discovery_run.isoformat() if profile.last_discovery_run else None,
    ...
    def _get_channel_profile(self, channel_name: str) -> Optional[ChannelProfile]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute(
                    "SELECT * FROM channel_profiles WHERE name = ?",
                    (channel_name,)
                )
                ...
                if row:
                    return ChannelProfile(
                        ...
                        last_discovery_run=datetime.fromisoformat(row[7]) if row[7] else None,
    ...
    def get_discovery_stats(self) -> Dict[str, Any]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                # Active channels
                cursor.execute("SELECT COUNT(*) FROM channel_profiles WHERE auto_discovery_enabled = 1")
                ...
                return {
                    ...
                    'discovery_rate': recent_discoveries / 7 if recent_discoveries > 0 else 0
                }
...
if __name__ == "__main__":
    ...
    parser.add_argument('--action', choices=['add-channel', 'discover', 'scheduled', 'stats', 'recommendations'], required=True)
    parser.add_argument('--channel', help="Channel name")
    parser.add_argument('--budget', type=float, help="Budget limit for channel")
    parser.add_argument('--pricing', choices=['free', 'freemium', 'paid'], help="Preferred pricing model")
    ...
    if args.action == 'add-channel' and args.channel:
        ...
        print(f"Channel '{args.channel}' {'added successfully' if success else 'failed to add'}")
        ...
        if result:
            ...
            print(f"Estimated Monthly Cost: ${result.cost_analysis.get('estimated_monthly_cost', 0):.2f}")
            print(f"Within Budget: {result.cost_analysis.get('within_budget', 'Unknown')}")
            
            print("\nTop Recommendations:")
            for api in result.recommended_apis[:3]:
                print(f"  {api.name} ({api.provider}) - {api.pricing_model} - Score: {api.quality_score:.2f}")
        ...
        for rec in recommendations:
            print(f"  {rec['api_name']} ({rec['provider']}) - {rec['pricing_model']} - Score: {rec['quality_score']:.2f}")
