...
class MetricsCollector:
    ...
    def get_metric_stats(self, metric_name: str, time_window: int = 300) -> Dict[str, Any]:
...
class AlertManager:
    ...
    def add_alert_rule(self, metric_name: str, threshold: float, 
                      ...
                      time_window: int = 300, min_samples: int = 3):
        """Add an alert rule"""
...
class AutoScaler:
    ...
    def add_scaling_rule(self, resource_type: str, scale_up_threshold: float,
                        scale_down_threshold: float, metric_name: str,
    ...
    def evaluate_scaling(self, current_capacities: Dict[str, int]) -> List[ScalingRecommendation]:
    ...
    def _evaluate_resource_scaling(self, resource_type: str, rule: Dict[str, Any],
                                 current_capacity: int) -> Optional[ScalingRecommendation]:
        ...
        if current_value > scale_up_threshold and current_capacity < rule['max_capacity']:
            # Scale up
            action = ScalingAction.SCALE_UP
            recommended_capacity = min(
                int(current_capacity * rule['scaling_factor']),
                rule['max_capacity']
            )
            confidence = min((current_value - scale_up_threshold) / scale_up_threshold, 1.0)
            reasoning = f"Metric {rule['metric_name']} ({current_value:.2f}) exceeds scale-up threshold ({scale_up_threshold})"
        
        elif current_value < scale_down_threshold and current_capacity > rule['min_capacity']:
            # Scale down
            ...
            confidence = min((scale_down_threshold - current_value) / scale_down_threshold, 1.0)
    ...
    def _estimate_scaling_impact(self, current_capacity: int, 
                               recommended_capacity: int, 
...
class PerformanceMonitor:
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_scaling_timestamp ON scaling_recommendations(timestamp)")
    ...
    def record_model_generation_metric(self, generation_time_ms: float, 
                                     success: bool, model_type: str):
    ...
    def record_api_request_metric(self, endpoint: str, response_time_ms: float, 
                                status_code: int):
        ...
        recent_requests = len([
            m for m in self.metrics_collector.metrics["api.requests_total"]
    ...
    def get_scaling_recommendations(self, current_capacities: Dict[str, int]) -> List[ScalingRecommendation]:
    ...
    def generate_performance_report(self, start_time: float, 
    ...
    def _generate_metrics_summary(self, start_time: float, 
    ...
    def _identify_bottlenecks(self, start_time: float, 
...
def record_model_generation(generation_time_ms: float, success: bool, model_type: str = "default"):
    """Record model generation performance"""
    performance_monitor.record_model_generation_metric(generation_time_ms, success, model_type)

def record_api_request(endpoint: str, response_time_ms: float, status_code: int):
    """Record API request performance"""
    performance_monitor.record_api_request_metric(endpoint, response_time_ms, status_code)

def get_current_performance_status() -> Dict[str, Any]:
    """Get current performance status"""
    return {
        'active_alerts': len(performance_monitor.get_active_alerts()),
        'cpu_usage': performance_monitor.metrics_collector.get_metric_stats("system.cpu.usage_percent"),
        'memory_usage': performance_monitor.metrics_collector.get_metric_stats("system.memory.usage_percent"),
        'generation_latency': performance_monitor.metrics_collector.get_metric_stats("model.generation.latency_ms")
    }
