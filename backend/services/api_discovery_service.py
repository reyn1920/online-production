...
class APIDiscoveryService:
    ...
    async def search_and_analyze(self, query: str, channel: str) -> List[APICandidate]:
        ...
        for result in search_results:
            try:
                ...
                logger.error(f"Error analyzing candidate {result.get('url', 'unknown')}: {e}")
        ...
        if 'affiliate' in query.lower():
            results.extend([
                ...
                },
                {
                    ...
                    'cost': 'free',
                    'features': ['merchant data', 'commission tracking', 'reporting']
                }
            ])
        
        return results
    
    async def analyze_api_candidate(self, result: Dict, channel: str) -> Optional[APICandidate]:
    ...
    def calculate_api_score(self, cost_model: str, features: List[str], 
                          description: str, url: str, channel: str) -> float:
        ...
        trusted_domains = [
            'developers.google.com', 'developer.twitter.com', 'developers.facebook.com',
            'github.com', 'rapidapi.com', 'sendgrid.com', 'mailchimp.com'
        ]
    ...
    def determine_category(self, name: str, description: str) -> str:
        ...
        categories = {
            'social': ['social', 'twitter', 'facebook', 'instagram', 'tiktok', 'youtube'],
            ...
            for candidate in candidates:
                try:
                    conn.execute("""
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """, (
                        candidate.name, candidate.url, candidate.signup_url,
                        candidate.category, candidate.cost_model, candidate.description,
                        json.dumps(candidate.features), candidate.rate_limits,
                        candidate.documentation_url, candidate.score,
                        candidate.discovered_at, candidate.channel
                    ))
    ...
    async with APIDiscoveryService() as service:
        if args.list_free:
            ...
            for api in free_apis[:args.top]:
                print(f"  • {api['name']} ({api['cost_model']}) - Score: {api['score']:.1f}")
            ...
            for candidate in candidates[:args.top]:
                print(f"  • {candidate.name} ({candidate.cost_model}) - Score: {candidate.score:.1f}")
