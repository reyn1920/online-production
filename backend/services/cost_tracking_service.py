...
Manages API costs, tracks usage, and provides budget controls for the marketing automation system.
...
class CostTrackingService:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                conn.execute("""
                    ...
                    VALUES (?, 
                        COALESCE((SELECT requests_made FROM api_usage WHERE api_name = ?), 0) + ?,
                        COALESCE((SELECT monthly_cost FROM api_usage WHERE api_name = ?), 0) + ?,
                        ?, ?)
                """, (api_name, api_name, requests_count, api_name, cost, datetime.now(), datetime.now()))
                ...
                conn.execute("""
                    ...
                    VALUES (?, ?, 
                        COALESCE((SELECT requests_count FROM cost_history WHERE api_name = ? AND date = ?), 0) + ?,
                        COALESCE((SELECT cost_amount FROM cost_history WHERE api_name = ? AND date = ?), 0) + ?)
                """, (api_name, today, api_name, today, requests_count, api_name, today, cost))
    ...
    def get_free_alternatives(self, service_category: str) -> List[Dict[str, Any]]:
        ...
        free_alternatives = {
            'ai_language': [
                {'name': 'Hugging Face Transformers', 'type': 'free', 'limit': '1000 requests/month'},
                {'name': 'Ollama Local', 'type': 'free', 'limit': 'unlimited (local)'},
                {'name': 'OpenAI Free Tier', 'type': 'freemium', 'limit': '$5 credit'}
            ],
            'social_media': [
                {'name': 'YouTube Data API', 'type': 'free', 'limit': '10000 units/day'},
                {'name': 'Twitter Basic', 'type': 'free', 'limit': '500k tweets/month'},
                {'name': 'Instagram Basic Display', 'type': 'free', 'limit': '200 requests/hour'}
            ],
            'email': [
                {'name': 'Mailgun Free', 'type': 'free', 'limit': '5000 emails/month'},
                {'name': 'SendGrid Free', 'type': 'free', 'limit': '100 emails/day'},
                {'name': 'SMTP.js', 'type': 'free', 'limit': 'unlimited (self-hosted)'}
            ],
            'analytics': [
                {'name': 'Google Analytics', 'type': 'free', 'limit': '10M hits/month'},
                {'name': 'Plausible Community', 'type': 'free', 'limit': 'unlimited (self-hosted)'},
                {'name': 'Matomo Cloud Free', 'type': 'free', 'limit': '30k actions/month'}
            ]
        }
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?)
                """, (alert_id, api_name, threshold_type, threshold_value, datetime.now()))
                
            self.logger.info(f"Budget alert set for {api_name}: {threshold_type} >= {threshold_value}")
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                for alert_id, threshold_type, threshold_value in cursor.fetchall():
                    ...
                    if should_trigger:
                        ...
                        self.logger.warning(f"Budget alert triggered for {api_name}: {threshold_type} reached {current_value} (threshold: {threshold_value})")
                        
                        # Disable API if cost threshold exceeded
                        if threshold_type == 'cost' and current_value >= threshold_value:
                            self.disable_api(api_name, f"Cost threshold exceeded: ${current_value:.2f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--action', choices=['track', 'report', 'alert', 'disable', 'enable'], required=True)
    ...
    parser.add_argument('--threshold-type', choices=['cost', 'usage', 'percentage'], help="Alert threshold type")
