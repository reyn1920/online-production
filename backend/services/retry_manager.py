...
class FailureClassifier:
    ...
    def classify_exception(exception: Exception) -> FailureType:
        ...
        elif "500" in exception_message or "502" in exception_message or \
             "503" in exception_message or "504" in exception_message:
            return FailureType.SERVER_ERROR
        elif "400" in exception_message or "401" in exception_message or \
             "403" in exception_message or "404" in exception_message:
        ...
        with self.lock:
            if self.state == CircuitState.CLOSED:
                ...
                if self.last_failure_time and \
                   (datetime.now() - self.last_failure_time).total_seconds() * 1000 > self.config.timeout_ms:
...
class AdaptiveRetryCalculator:
    ...
    def calculate_adaptive_delay(self, service_name: str, base_delay_ms: int, 
        ...
        with self.lock:
            ...
            success_rate = sum(self.success_rates[service_name]) / len(self.success_rates[service_name])
            avg_response_time = sum(self.avg_response_times[service_name]) / len(self.avg_response_times[service_name])
...
class AdvancedRetryManager:
    ...
    async def execute_with_retry(self, 
        ...
        for attempt in range(1, config.max_attempts + 1):
            ...
            try:
                ...
                logger.warning(f"Attempt {attempt} failed for {service_name}: {e}. Retrying in {delay_ms}ms")
    ...
    def _calculate_delay(self, config: RetryConfig, attempt: int, 
                       failure_type: FailureType, service_name: str) -> int:
    ...
    def _log_attempt(self, service_name: str, operation_id: str, 
                    attempt: RetryAttempt, success: bool):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                if stats and stats[0] > 0:
                    total_attempts, successful_attempts, avg_response_time, avg_delay, max_attempts = stats
                ...
                if service_name in self.bulkheads:
                    bulkhead_info = {
                        'current_utilization_percent': self.bulkheads[service_name].get_utilization(),
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            return {
                ...
                'high_utilization_bulkheads': sum(1 for bh in self.bulkheads.values() 
                                                 if bh.get_utilization() > 80)
            }
...
def with_retry(service_name: str, config: RetryConfig = None, 
              circuit_config: CircuitBreakerConfig = None):
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            result = await retry_manager.execute_with_retry(
                func, service_name, config=config, 
                circuit_config=circuit_config, *args, **kwargs
            )
