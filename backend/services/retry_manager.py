...
class CircuitBreaker:
    ...
    def record_failure(self):
        ...
        with self.lock:
            ...
            if self.state == CircuitState.HALF_OPEN:
                ...
                self.success_count = 0
                logger.warning(
                    f"Circuit breaker '{self.name}' opened after half - open failure"
                )
            elif self.state == CircuitState.CLOSED:
    ...
    def get_state_info(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            failure_rate = (
                (recent_failures / recent_calls * 100) if recent_calls > 0 else 0
            )
...
class AdaptiveRetryCalculator:
    ...
    def calculate_adaptive_delay(
        ...
        with self.lock:
            ...
            success_multiplier = (
                2.0 - success_rate
            )  # Lower success rate = higher multiplier
...
class AdvancedRetryManager:
    ...
    def get_or_create_bulkhead(
        self, name: str, max_concurrent: int = 10
    ) -> BulkheadIsolation:
        ...
        for attempt in range(1, config.max_attempts + 1):
            ...
            if config.bulkhead_enabled:
                if not await bulkhead.acquire():
                    ...
                    return RetryResult(
                        success = False,
                            error = Exception(
                            f"Bulkhead capacity exceeded for {service_name}"
                        ),
                            total_attempts = attempt - 1,
                            total_time_ms = int((time.time() - start_time) * 1000),
                            attempts = attempts,
            ...
            try:
                ...
                if config.adaptive_enabled:
                    self.adaptive_calculator.update_metrics(
                        service_name, True, response_time_ms
                    )
                ...
                if config.adaptive_enabled:
                    self.adaptive_calculator.update_metrics(
                        service_name, False, response_time_ms
                    )
                ...
                if not self.failure_classifier.is_retryable(failure_type, config):
                    logger.info(
                        f"Non - retryable failure for {service_name}: {failure_type.value}"
                    )
                    return RetryResult(
                        success = False,
                            error = e,
                ...
                delay_ms = self._calculate_delay(
                    config, attempt, failure_type, service_name
                )
    ...
    def _calculate_delay(
        ...
        failure_multiplier = self.failure_classifier.get_retry_delay_multiplier(
            failure_type
        )
                ...
                if service_name in self.bulkheads:
                    bulkhead_info = {
                        "current_utilization_percent": self.bulkheads[
                            ...
                            "max_concurrent_calls": self.bulkheads[
                            service_name
