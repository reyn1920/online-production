...
class PetCareAPIManager:
    ...
    async def get_dog_breeds(self, limit: int = 20) -> APIResponse:
        ...
        if response.rate_limit_remaining is not None:
            ...
            self._update_rate_limit(
                "dog_api", response.rate_limit_remaining, reset_time
            )
    ...
    async def get_cat_breeds(self, limit: int = 20) -> APIResponse:
        ...
        if response.rate_limit_remaining is not None:
            reset_time = datetime.now() + timedelta(hours = 1)
            self._update_rate_limit(
                "cat_api", response.rate_limit_remaining, reset_time
            )
    ...
    async def search_adoptable_pets(
        ...
        if not self.petfinder_key or not self.petfinder_secret:
            return APIResponse(
                success = False, error="Petfinder API credentials not configured"
            )
        ...
        try:
            ...
            async with self.session.post(token_url, data = token_data) as token_response:
                if token_response.status != 200:
                    return APIResponse(
                        success = False, error="Failed to authenticate with Petfinder"
                    )
            ...
            if response.rate_limit_remaining is not None:
                reset_time = datetime.now() + timedelta(hours = 1)
                self._update_rate_limit(
                    "petfinder", response.rate_limit_remaining, reset_time
                )
        ...
        return {
            "ebird": {
                "configured": bool(self.ebird_token),
                    "rate_limited": (
                    not self._check_rate_limit("ebird")
                    if "ebird" in self._rate_limits
                    else False
                ),
                    },
                "dog_api": {
