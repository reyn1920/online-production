...
class PetCareAPIManager:
    ...
    async def get_bird_observations(self, region_code: str = 'US', 
                                   days_back: int = 7) -> APIResponse:
        ...
        headers = {
            'X-eBirdApiToken': self.ebird_token
        }
        params = {
            'back': days_back,
            'maxResults': 50
    ...
    async def search_adoptable_pets(self, animal_type: str = 'dog', 
                                   location: str = '10001', 
        ...
        try:
            ...
            async with self.session.post(token_url, data=token_data) as token_response:
                ...
                if not access_token:
                    return APIResponse(success=False, error="No access token received from Petfinder")
    ...
    def get_api_status(self) -> Dict[str, Any]:
        ...
        return {
            'ebird': {
                ...
                'rate_limited': not self._check_rate_limit('ebird') if 'ebird' in self._rate_limits else False
            },
            'dog_api': {
                'configured': bool(self.dog_api_key),
                'rate_limited': not self._check_rate_limit('dog_api') if 'dog_api' in self._rate_limits else False
            },
            'cat_api': {
                'configured': bool(self.cat_api_key),
                'rate_limited': not self._check_rate_limit('cat_api') if 'cat_api' in self._rate_limits else False
            },
            'petfinder': {
                'configured': bool(self.petfinder_key and self.petfinder_secret),
                'rate_limited': not self._check_rate_limit('petfinder') if 'petfinder' in self._rate_limits else False
            }
        }
