...
from .automated_model_generator import AutomatedModelGenerator, ModelRequest, ModelResponse
...
class FailureInjector:
    ...
    async def inject_failure(self, failure_type: FailureType, duration_seconds: int = 30):
...
class StressTester:
    ...
    async def run_test(self, config: TestConfig) -> TestResult:
        ...
        logger.info(f"Starting stress test: {config.test_name} ({config.test_type.value})")
        ...
        try:
            ...
            logger.info(f"Test {config.test_name} completed with {result.success_rate:.2f}% success rate")
        
        return result
    
    async def _run_load_test(self, config: TestConfig, result: TestResult):
        """Run load test with gradual ramp-up"""
        logger.info(f"Running load test: {config.concurrent_users} users, {config.requests_per_second} RPS")
        ...
        for scheduled_time, model_type in request_schedule:
            task = asyncio.create_task(
                self._execute_scheduled_request(scheduled_time, model_type, config, result)
            )
            tasks.append(task)
    ...
    async def _run_failover_test(self, config: TestConfig, result: TestResult):
        ...
        request_task = asyncio.create_task(self._run_continuous_requests(config, result))
    ...
    async def _run_reliability_test(self, config: TestConfig, result: TestResult):
        ...
        for i in range(1000):  # 1000 requests for statistical significance
            ...
            try:
                response = await self.model_generator.generate_model_with_full_redundancy(request)
                ...
                if not response.success:
                    logger.error(f"RELIABILITY FAILURE: Request {request_id} failed: {response.error}")
                
            except Exception as e:
                logger.error(f"RELIABILITY FAILURE: Request {request_id} exception: {e}")
        ...
        if result.success_rate >= config.target_success_rate:
            ...
            logger.info(f"✅ RELIABILITY TEST PASSED: {result.success_rate:.2f}% success rate")
        else:
            result.reliability_score = result.success_rate
            logger.error(f"❌ RELIABILITY TEST FAILED: {result.success_rate:.2f}% success rate (target: {config.target_success_rate}%)")
    ...
    async def _execute_scheduled_request(self, scheduled_time: float, model_type: str, 
                                       config: TestConfig, result: TestResult) -> RequestMetrics:
        """Execute a request at the scheduled time"""
        # Wait for scheduled time
        await asyncio.sleep(scheduled_time)
        
        request_id = f"{config.test_name}_{int(time.time() * 1000)}_{random.randint(1000, 9999)}"
    ...
    async def _inject_delayed_failure(self, failure_type: FailureType, 
                                    delay_seconds: int, duration_seconds: int):
        """Inject failure after delay"""
        await asyncio.sleep(delay_seconds)
        await self.failure_injector.inject_failure(failure_type, duration_seconds)
    
    def _update_result_with_metrics(self, result: TestResult, metrics: List[RequestMetrics]):
        ...
        # Errors
        result.errors = [m.error for m in metrics if m.error and m.error not in result.errors]
    ...
    def _calculate_final_metrics(self, result: TestResult):
        ...
        reliability_factors = [
            ...
            min(result.avg_quality_score / 10.0, 1.0) if result.avg_quality_score > 0 else 0.5,  # Quality (30%)
            1.0 if result.avg_response_time_ms < 10000 else max(0, 1.0 - (result.avg_response_time_ms - 10000) / 20000),  # Response time (20%)
            1.0 if result.failover_events == 0 else max(0, 1.0 - result.failover_events / 10)  # Failover stability (10%)
        ]
        
        weights = [0.4, 0.3, 0.2, 0.1]
        result.reliability_score = sum(factor * weight for factor, weight in zip(reliability_factors, weights)) * 100
    ...
    async def _store_test_result(self, result: TestResult):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                """, (
                    result.test_name, result.test_type.value, result.status.value,
                    result.start_time, result.end_time, result.total_requests,
                    result.successful_requests, result.failed_requests, result.success_rate,
                    result.avg_response_time_ms, result.p95_response_time_ms, result.p99_response_time_ms,
                    result.throughput_rps, result.avg_quality_score, result.failover_events,
                    result.recovery_time_ms, result.reliability_score,
                    json.dumps(result.resource_usage), json.dumps(result.errors),
    ...
    async def _store_request_metric(self, test_name: str, metric: RequestMetrics):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                """, (
                    test_name, metric.request_id, metric.model_type,
                    metric.start_time, metric.end_time, metric.success,
                    metric.response_time_ms, metric.backend_used, metric.quality_score,
                    metric.error, metric.retry_count, metric.failover_count
                ))
                conn.commit()
    ...
    def generate_report(self, test_names: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                passed_tests = sum(1 for r in results if r[12] >= 99.0)  # reliability_score >= 99%
                avg_success_rate = statistics.mean([r[9] for r in results]) if results else 0
                avg_reliability_score = statistics.mean([r[17] for r in results]) if results else 0
                ...
                if avg_success_rate < 100.0:
                    report['recommendations'].append(
                        "Success rate below 100% - investigate failed requests and improve redundancy"
                    )
                
                if avg_reliability_score < 99.0:
                    report['recommendations'].append(
                        "Reliability score below 99% - review system architecture and failover mechanisms"
                    )
...
async def run_comprehensive_reliability_test() -> Dict[str, Any]:
    ...
    for config in test_configs:
        ...
        # Log result
        if result.success_rate >= config.target_success_rate:
            logger.info(f"✅ {config.test_name} PASSED: {result.success_rate:.2f}% success rate")
        else:
            logger.error(f"❌ {config.test_name} FAILED: {result.success_rate:.2f}% success rate")
    
    # Generate comprehensive report
    report = stress_tester.generate_report([config.test_name for config in test_configs])
    ...
    report['overall_assessment'] = {
        ...
        'recommendation': 'System ready for production' if all_passed else 'Address failures before production deployment'
    }
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        print(f"- Average Reliability Score: {report['summary']['avg_reliability_score']:.2f}")
        
        print(f"\nTest Results:")
        for test in report['test_results']:
            status_icon = "✅" if test['success_rate'] >= 100.0 else "❌"
            print(f"{status_icon} {test['test_name']}: {test['success_rate']:.2f}% success, {test['reliability_score']:.2f} reliability")
