...
class NewsScraperForRightPerspective:
    def __init__(self, db_path: str = "conservative_research.db"):
        ...
        self.target_keywords = [
            ...
            'democrat', 'democratic', 'liberal', 'progressive',
            # Policy flip-flop keywords
            'border wall', 'immigration', 'tariffs', 'trade war',
            'russia collusion', 'steele dossier', 'fake news',
            # Conservative talking points
            'hypocrisy', 'flip-flop', 'contradiction', 'lies',
            'double standard', 'mainstream media bias'
        ]
    ...
    def calculate_relevance_score(self, title: str, content: str) -> float:
        ...
        high_value_keywords = ['hypocrisy', 'flip-flop', 'double standard', 'lies', 'fake news']
    ...
    def scrape_article_content(self, url: str, content_selector: str) -> Optional[str]:
        ...
        try:
            ...
            selectors = [content_selector, 'p', '.content p', '.article p', '.post-content p']
    ...
    def scrape_source(self, source_name: str, source_config: Dict, source_type: str) -> List[Dict]:
                ...
                for link in article_links[:20]:  # Limit to 20 articles per section
                    try:
                        ...
                        if relevance_score >= 1.0:  # Only scrape relevant articles
                            # Scrape full content
                            content = self.scrape_article_content(article_url, source_config['content_selector'])
                            ...
                            if content:
                                ...
                                articles.append({
                                    ...
                                    'right_perspective_potential': relevance_score >= 3.0
                                })
    ...
    def generate_content_summary(self) -> Dict:
        ...
        cursor.execute('SELECT COUNT(*) FROM scraped_articles WHERE right_perspective_potential = TRUE')
        high_potential = cursor.fetchone()[0]
        
        cursor.execute('SELECT source_type, COUNT(*) FROM scraped_articles GROUP BY source_type')
        ...
        return {
            ...
            'potential_percentage': round((high_potential / total_articles * 100), 2) if total_articles > 0 else 0
        }
...
def main():
    ...
    for i, article in enumerate(high_potential, 1):
        print(f"{i}. [{article['source']}] {article['title']} (Score: {article['relevance_score']})")
    ...
    print(f"High-potential articles: {summary['high_potential_articles']} ({summary['potential_percentage']}%)")
