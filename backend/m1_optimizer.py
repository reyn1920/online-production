...
class M1PerformanceOptimizer:
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            "thermal_throttle_threshold": 80,  # °C
    ...
    def get_system_metrics(self) -> M1SystemMetrics:
        ...
        try:
            ...
            return M1SystemMetrics(
                ...
                background_tasks=sum(1 for t in self.active_tasks.values() 
        ...
        with self.task_lock:
            ...
            for i, task in enumerate(self.task_queue):
                if self._can_execute_task(task, current_metrics):
                    return self.task_queue.popleft() if i == 0 else self.task_queue.pop(i)
            
            return None
    
    def _can_execute_task(self, task: OptimizationTask, metrics: M1SystemMetrics) -> bool:
        """Check if a task can be executed given current system state."""
        # Check memory requirements
        available_memory = (100 - metrics.memory_percent) * psutil.virtual_memory().total / (100 * 1024**2)
        ...
        if (task.priority == TaskPriority.BACKGROUND and 
            metrics.background_tasks >= self.config["background_task_limit"]):
            return False
    ...
    def _execute_task(self, task: OptimizationTask):
        ...
        logger.info(f"Executing task {task.id}: {task.name} on {executor._thread_name_prefix} cores")
    ...
    def _choose_executor(self, task: OptimizationTask) -> ThreadPoolExecutor:
        ...
        if task.core_preference == CoreType.PERFORMANCE:
            ...
            if task.cpu_intensive or task.priority in [TaskPriority.CRITICAL, TaskPriority.HIGH]:
                return self.performance_executor
            else:
                return self.efficiency_executor
    ...
    def _update_throttle_level(self, metrics: M1SystemMetrics):
        ...
        if (metrics.thermal_state == "critical" or 
            metrics.memory_pressure == "critical" or
            metrics.cpu_percent > 95):
            self.current_throttle_level = min(5, self.current_throttle_level + 1)
        
        elif (metrics.thermal_state == "serious" or 
              metrics.memory_pressure == "warn" or
        ...
        elif (metrics.thermal_state == "nominal" and 
              metrics.memory_pressure == "normal" and
        ...
        if self.current_throttle_level != old_level:
            ...
            logger.info(f"Throttle level changed: {old_level} -> {self.current_throttle_level}")
    ...
    def _apply_throttle_level(self):
        ...
        logger.debug(f"Applied throttle level {self.current_throttle_level}: "
