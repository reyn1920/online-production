        ...
        <script>
            async function createVideo() {
                ...
                try {
                    const response = await fetch('/api/create_video', {
                        ...
                        body: JSON.stringify({prompt: prompt, include_affiliates: true})
                    });
                    const result = await response.json();
                    document.getElementById('videoResult').innerHTML = JSON.stringify(result, null, 2);
                } catch (error) {
            ...
            async function setSetting() {
                ...
                try {
                    ...
                    const result = await response.json();
                    document.getElementById('settingsResult').innerHTML = JSON.stringify(result, null, 2);
                } catch (error) {
            ...
            async function toggleAffiliateEmbed() {
                ...
                await fetch('/api/settings', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({key: 'affiliate_embed_enabled', value: newValue})
                });
...
async def generate_campaign_content(campaign_id: str, request: dict):
    ...
    try:
        ...
        if content_type == "social_post":
            ...
            generated_content = f"Exclusive Update: Your {audience or 'valued'} insights await - Don't miss out!"
