...
class TaskQueue:
    ...
    def initialize_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_workers_agent_type ON agent_workers (agent_type)")
    ...
    def submit_task(self, task_type: str, agent_type: str, payload: Dict[str, Any],
    ...
    def register_agent(self, agent_type: str, agent_instance: Any, 
                      ...
                      max_concurrent_tasks: int = 1) -> str:
                    ...
                    if not self.priority_queues[priority].empty():
                        try:
                            ...
                            if datetime.fromtimestamp(scheduled_time) <= datetime.now():
                                ...
                                self.priority_queues[priority].put((scheduled_time, task_id))
    ...
    def _execute_task(self, task_id: str, worker_id: str, agent_instance: Any):
        ...
        try:
            ...
            self._log_task_execution(task_id, worker_id, 'completed', 
    ...
    def _handle_task_failure(self, task_id: str, worker_id: str, error_message: str):
        ...
        if retry_count <= task['max_retries']:
            ...
            self._log_task_execution(task_id, worker_id, 'failed_retry_scheduled',
                                   {'error': error_message, 'retry_delay': retry_delay})
            
            self.logger.warning(f"Task failed, scheduling retry {retry_count}/{task['max_retries']}: {task_id}")
        else:
            # Move to dead letter queue
            self._update_task_status(task_id, TaskStatus.DEAD_LETTER)
            self._log_task_execution(task_id, worker_id, 'moved_to_dead_letter',
                                   {'error': error_message})
    ...
    def _save_task(self, task: Task):
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                task.task_id, task.task_type, task.agent_type, task.priority.value,
                json.dumps(task.payload), json.dumps(task.dependencies),
    ...
    def _update_task_retry(self, task_id: str, retry_count: int, error_message: str, retry_time: datetime):
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute("""
                ...
                WHERE task_id = ?
            """, (
                retry_count, error_message, retry_time.isoformat(),
    ...
    def _save_agent_worker(self, worker: AgentWorker):
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                worker.worker_id, worker.agent_type, worker.status,
                worker.current_task_id, worker.tasks_completed,
                worker.last_heartbeat.isoformat(),
                json.dumps(worker.capabilities),
                worker.max_concurrent_tasks, worker.current_load
            ))
    
    def _log_task_execution(self, task_id: str, worker_id: str, action: str, details: Dict[str, Any] = None):
    ...
    def _remove_offline_worker(self, worker_id: str):
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
                "DELETE FROM agent_workers WHERE worker_id = ?",
                (worker_id,)
            )
...
if __name__ == "__main__":
    ...
    try:
        for _ in range(30):
            ...
            print(f"Queue metrics: {metrics.pending_tasks} pending, {metrics.completed_tasks} completed")
            time.sleep(1)
