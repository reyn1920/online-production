...
try:
    ...
    print(
        "Warning: APIDiscoveryService not found. Make sure the service is properly installed."
    )
...
def get_channel_apis(channel):
    ...
    try:
        ...
        if not APIDiscoveryService:
            return jsonify(
                {"success": True, "apis": get_mock_channel_apis(channel, limit)}
            )
    ...
    try:
        ...
        with sqlite3.connect(db_path) as conn:
            ...
            avg_cursor = conn.execute(
                "SELECT AVG(score) as avg_score FROM discovered_apis"
            )
