...
class TaskQueueManager:
    ...
    def _serialize_safe(obj: Any) -> str:
        ...
        try:
            ...
            return json.dumps({"error": f"Serialization failed: {str(e)}", "repr": str(obj)})
    
    def __init__(self, db_path: str = "data/trae_master.db", secret_store: Optional[SecretStore] = None):
        """
        ...
        try:
            with self._get_db_connection() as conn:
                ...
                if row:
                    ...
                    self.update_task_status(task['id'], TaskStatus.IN_PROGRESS, agent_id)
        ...
        try:
            ...
            if shutil.which('ffmpeg'):
                ...
                cmd = [
                    ...
                    '-vf', f'drawtext=text="{script_content[:50]}...":fontcolor=white:fontsize=48:x=(w-text_w)/2:y=(h-text_h)/2',
                    '-c:v', 'libx264',
                    '-pix_fmt', 'yuv420p',
                    '-r', '30',
                    output_path
                ...
                if result.returncode == 0 and Path(output_path).exists():
                    self.logger.info(f"FFmpeg video created successfully: {output_path}")
                    return True
                else:
        ...
        try:
            ...
            if 'script_content' in payload and 'output_path' in payload:
                ...
                if success:
                    ...
                    self.update_task_status(task_id, TaskStatus.FAILED, error_message="Video creation failed")
                    return False
