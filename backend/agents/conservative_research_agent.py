...
class ConservativeResearchAgent:
    ...
    def load_documented_examples(self):
        ...
        for example in self.documented_examples:
            example_id = hashlib.md5(f"{example['politician']}_{example['title']}".encode()).hexdigest()
            
            # Check if example already exists
            cursor.execute('SELECT id FROM hypocrisy_examples WHERE id = ?', (example_id,))
            if cursor.fetchone():
                continue
    ...
    def extract_title(self, element) -> Optional[str]:
        ...
        title_element = element.find('a') or element.find('h1') or element.find('h2') or element.find('h3')
        if title_element:
    ...
    def is_relevant_article(self, title: str) -> bool:
        ...
        relevant_keywords = [
            'democrat', 'democratic', 'biden', 'harris', 'pelosi', 'schumer', 'aoc',
            'obama', 'clinton', 'schiff', 'nadler', 'waters',
            'hypocrisy', 'flip-flop', 'contradiction', 'lies', 'false claim',
            'border', 'immigration', 'wall', 'fence', 'security',
            'russia', 'collusion', 'investigation', 'mueller', 'durham',
            'economy', 'tariff', 'trade', 'inflation', 'gas prices',
            'covid', 'lockdown', 'mandate', 'vaccine',
            'crime', 'defund police', 'bail reform',
            'election', 'voting', 'fraud', 'integrity'
        ]
    ...
    def calculate_hypocrisy_score(self, content: str) -> float:
        ...
        high_value_patterns = [
            r'flip.?flop', r'contradiction', r'hypocrisy', r'hypocrite',
            r'said one thing.*did another', r'changed position',
            r'voted for.*now opposes', r'previously supported.*now against'
        ]
        ...
        medium_value_keywords = [
            'false claim', 'misleading', 'inaccurate', 'debunked',
            'fact check', 'pants on fire', 'four pinocchios'
        ]
        ...
        politicians = [
            ...
            'schiff', 'nadler', 'waters', 'clinton', 'obama'
        ]
    ...
    def get_hypocrisy_examples(self, category: str = None, politician: str = None, 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--action', choices=['research', 'examples', 'weekly'], 
