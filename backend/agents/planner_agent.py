...
class PlannerAgent(BaseAgent):
    ...
    def create_strategy(self, name: str, objectives: List[str], 
                       initial_tactics: Dict[str, Any]) -> StrategyPlan:
    ...
    def process_performance_feedback(self, strategy_id: str, 
        ...
        try:
            ...
            if metrics.overall_score() < self.refinement_trigger_threshold:
                self.logger.info(f"Performance below threshold ({metrics.overall_score():.2f}), triggering refinement")
    ...
    def _refine_strategy(self, strategy_id: str, current_metrics: StrategyMetrics) -> bool:
        ...
        try:
            ...
            insights = self._generate_refinement_insights(strategy, current_metrics, historical_metrics)
            ...
            refined_strategy.confidence_score = min(0.95, refined_strategy.confidence_score + self.learning_rate)
            
            self._save_strategy(refined_strategy)
            
            # Log strategic insight
            self._log_strategic_insight("strategy_refinement", 
    ...
    def _generate_refinement_insights(self, strategy: StrategyPlan, 
                                    current_metrics: StrategyMetrics,
                                    historical_metrics: List[StrategyMetrics]) -> Dict[str, Any]:
        ...
        if historical_metrics:
            avg_content = sum(m.content_performance for m in historical_metrics) / len(historical_metrics)
            avg_marketing = sum(m.marketing_roi for m in historical_metrics) / len(historical_metrics)
            avg_engagement = sum(m.audience_engagement for m in historical_metrics) / len(historical_metrics)
    ...
    def _load_strategy(self, strategy_id: str) -> Optional[StrategyPlan]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
                "SELECT * FROM strategy_plans WHERE strategy_id = ?",
                (strategy_id,)
            )
            row = cursor.fetchone()
