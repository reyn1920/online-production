...
from backend.integrations.twitter_integration import TwitterIntegration, TweetData, TweetType, SearchResult
...
class TwitterEngagementAgent:
        ...
        return [
            TopicProfile(
                ...
                keywords=["AI", "machine learning", "neural network", "deep learning", 
                         "artificial intelligence", "ML", "algorithm", "data science"],
                priority_weight=1.0,
            ...
            TopicProfile(
                ...
                keywords=["programming", "coding", "developer", "software", "python", 
                         "javascript", "react", "nodejs", "github", "opensource"],
                priority_weight=0.9,
            ...
            TopicProfile(
                ...
                keywords=["youtube", "content creator", "video", "streaming", "creator", 
                         "youtuber", "content", "channel", "subscriber"],
                priority_weight=0.8,
            ...
            TopicProfile(
                ...
                keywords=["technology", "tech", "innovation", "startup", "digital", 
                         "automation", "future", "trends", "disruption"],
                priority_weight=0.7,
    ...
    async def discover_engagement_opportunities(self, max_opportunities: int = 20) -> List[EngagementOpportunity]:
        """
                ...
                for query in search_queries[:3]:  # Limit queries per topic
                    try:
                        ...
                        for result in results:
                            ...
                            if opportunity and opportunity.confidence_score >= self.min_confidence_score:
    ...
    async def _analyze_engagement_potential(self, search_result: SearchResult, 
                                    profile: TopicProfile) -> Optional[EngagementOpportunity]:
        """
        ...
        try:
            ...
            suggested_reply, confidence, reasoning = await self._generate_contextual_reply(
                context, profile
            )
    ...
    def _build_conversation_context(self, search_result: SearchResult, 
                                  profile: TopicProfile) -> Optional[ConversationContext]:
        """
    ...
    async def _generate_contextual_reply(self, context: ConversationContext, 
                                 profile: TopicProfile) -> Tuple[str, float, str]:
        """
        ...
        try:
            ...
            response = await self.ollama.query_llm(
                prompt=prompt,
            ...
            confidence, reasoning = self._evaluate_reply_quality(
                reply_text, context, profile
            )
    ...
    def _get_style_guidance(self, engagement_style: str, sentiment: SentimentType) -> str:
        ...
        """
        style_map = {
            "educational_helpful": "Be informative and educational. Share knowledge or resources.",
            "technical_supportive": "Offer technical insights or solutions. Be precise and helpful.",
            "encouraging_collaborative": "Be supportive and encouraging. Suggest collaboration.",
            "insightful_forward_thinking": "Share insights about trends and future implications."
        }
    ...
    def _evaluate_reply_quality(self, reply_text: str, context: ConversationContext, 
                              profile: TopicProfile) -> Tuple[float, str]:
        """
        ...
        topic_matches = sum(1 for keyword in profile.keywords 
        ...
        if '?' in context.tweet_text and any(word in reply_text.lower() 
                                           for word in ['yes', 'no', 'try', 'use', 'check']):
            confidence += 0.15
            reasoning_parts.append("responds to question")
        
        # Check for value-adding words
        value_words = ['help', 'try', 'check', 'consider', 'suggest', 'recommend', 'tip']
    ...
    def _calculate_priority(self, context: ConversationContext, 
                         profile: TopicProfile) -> ConversationPriority:
        """
    ...
    def execute_engagement(self, opportunity_id: str) -> EngagementResult:
        ...
        try:
            ...
            # Prepare reply tweet
            reply_text = f"@{opportunity.conversation.author_username} {opportunity.suggested_reply}"
    ...
    def _record_user_interaction(self, username: str, interaction_type: EngagementType, 
                               tweet_id: str, success: bool) -> None:
        """
    ...
    async def search_and_engage(self, keywords: List[str], max_engagements: int = 5) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            opportunities = await self.discover_engagement_opportunities(max_opportunities=max_engagements * 2)
