...
from .web_automation_tools import (
    WebAutomationAgent, AutomationTarget, AutomationAction,
    ActionType, StealthLevel
)
...
class FinancialAgent(BaseAgent):
    ...
    def __init__(self, agent_id: str = "financial_agent", name: str = "Financial Agent", config: Dict[str, Any] = None):
        super().__init__(agent_id, name)
        self.config = config or {}
        self.agent_type = "financial"
        self.analysis_interval = self.config.get('analysis_interval', 3600)  # 1 hour
        self.reallocation_threshold = self.config.get('reallocation_threshold', 0.15)  # 15% improvement
        ...
        self.allocation_strategy = AllocationStrategy(self.config.get('strategy', 'profit_maximization'))
        ...
        self.resource_costs = self.config.get('resource_costs', {
                ...
                for channel in channels_data:
                    ...
                    if not performance['meets_threshold']:
                        recommendations.append({
                            ...
                            'action': 'reduce_allocation' if performance['roi'] < 0 else 'optimize_costs'
                        })
        ...
        try:
            ...
            for channel in channels:
                ...
                if financials:
                    ...
                    if financials.status in [ChannelStatus.HIGHLY_PROFITABLE, ChannelStatus.PROFITABLE]:
                        analysis_results['profitable_channels'] += 1
                    elif financials.status in [ChannelStatus.UNDERPERFORMING, ChannelStatus.LOSS_MAKING]:
            ...
            logger.info(f"Profitability analysis completed: {analysis_results['profitable_channels']}/{analysis_results['channels_analyzed']} channels profitable")
    ...
    async def _calculate_channel_financials(self, channel: Dict[str, Any]) -> Optional[ChannelFinancials]:
        ...
        try:
            ...
            logger.error(f"Failed to calculate financials for channel {channel.get('id', 'unknown')}: {e}")
            return None
            ...
            for opportunity in opportunities:
                if opportunity['expected_improvement'] >= self.reallocation_threshold:
                    ...
                    if reallocation['status'] == 'success':
                        ...
                        optimization_results['expected_profit_increase'] += reallocation['expected_profit_increase']
                        optimization_results['resources_optimized'].append(reallocation['resource_type'])
                        optimization_results['affected_channels'].extend(reallocation['affected_channels'])
            
            logger.info(f"Resource optimization completed: {optimization_results['reallocations_made']} reallocations made")
    ...
    async def _verify_affiliate_payouts(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for program in affiliate_programs:
                ...
                verification_results['payouts_verified'] += program_results.get('payouts_verified', 0)
                verification_results['discrepancies_found'] += program_results.get('discrepancies_found', 0)
                verification_results['total_discrepancy_amount'] += program_results.get('discrepancy_amount', 0.0)
            ...
            logger.info(f"Payout verification completed: {verification_results['discrepancies_found']} discrepancies found")
            ...
            if login_result['status'] == 'success':
                ...
                for expected in expected_payouts:
                    ...
                    if actual_payout:
                        ...
                        if abs(expected_amount - actual_amount) > 0.01:  # Allow for small rounding differences
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'last_analysis': self.financial_metrics.last_calculated.isoformat() if self.financial_metrics.last_calculated else None
        }
    ...
    async def _validate_rephrase_accuracy(self, original: str, rephrased: str) -> bool:
        ...
        financial_terms = ['financial', 'profit', 'revenue', 'cost', 'roi', 'allocation', 'budget']
        
        original_lower = original.lower()
        rephrased_lower = rephrased.lower()
        
        # Check if any financial terms from original are in rephrased
        for term in financial_terms:
            if term in original_lower and term in rephrased_lower:
                return True
        
        # If no specific financial terms, check general similarity
