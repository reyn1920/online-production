...
class YouTubeEngagementAgent:
    ...
    def _load_topic_profiles(self) -> Dict[str, TopicProfile]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                for row in rows:
                    topic, keywords_json, priority_weight, engagement_style, max_daily, current_count, last_reset = row
    ...
    def _create_default_topic_profiles(self) -> Dict[str, TopicProfile]:
        ...
        default_profiles = {
            ...
            'content_creation': TopicProfile(
                ...
                keywords=['content', 'creation', 'video', 'editing', 'production', 'creative', 'tutorial'],
                priority_weight=0.9,
            ...
            'business_growth': TopicProfile(
                ...
                keywords=['business', 'growth', 'marketing', 'strategy', 'entrepreneurship', 'startup'],
                priority_weight=0.8,
            ...
            'community_support': TopicProfile(
                ...
                keywords=['help', 'support', 'question', 'problem', 'issue', 'assistance'],
                priority_weight=1.2,
    ...
    async def monitor_video_comments(self, video_id: str) -> List[EngagementOpportunity]:
        ...
        try:
            ...
            self.logger.info(f"Found {len(opportunities)} engagement opportunities for video {video_id}")
    ...
    def _create_comment_context(self, comment_data: Dict[str, Any], video_title: str, video_category: str) -> CommentContext:
        ...
        return CommentContext(
            ...
            video_id=comment_data.get('snippet', {}).get('videoId', comment_data.get('video_id', '')),
    ...
    def _analyze_sentiment(self, text: str) -> SentimentType:
        ...
        try:
            ...
            positive_words = ['great', 'awesome', 'amazing', 'love', 'excellent', 'fantastic', 'helpful', 'thanks']
            negative_words = ['bad', 'terrible', 'hate', 'awful', 'worst', 'useless', 'boring', 'stupid']
    ...
    async def _analyze_comment_for_engagement(self, context: CommentContext) -> Optional[EngagementOpportunity]:
        ...
        try:
            ...
            reply, confidence, reasoning = await self._generate_contextual_reply(context, best_profile)
            ...
            # Create opportunity
            opportunity_id = hashlib.md5(f"{context.comment_id}_{datetime.now().isoformat()}".encode()).hexdigest()
    ...
    def _find_best_topic_profile(self, context: CommentContext) -> Optional[TopicProfile]:
        ...
        for profile in self.topic_profiles.values():
            ...
            matched_keywords = [kw for kw in context.topic_keywords if kw.lower() in [pk.lower() for pk in profile.keywords]]
            if matched_keywords:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute('''
                    ...
                    WHERE topic = ?
                ''', (profile.current_daily_count, profile.last_reset_date.isoformat(), profile.topic))
                conn.commit()
        except Exception as e:
            self.logger.error(f"Failed to update topic profile: {e}")
    
    async def _generate_contextual_reply(self, context: CommentContext, profile: TopicProfile) -> Tuple[str, float, str]:
...
Reply:"""
    ...
    def _evaluate_reply_quality(self, reply_text: str, context: CommentContext, profile: TopicProfile) -> Tuple[float, str]:
        ...
        # Quality indicators
        if '?' in context.comment_text and any(word in reply_text.lower() for word in ['yes', 'no', 'here', 'try', 'can']):
            score += 0.2
            reasons.append("Addresses question")
        
        # Avoid generic responses
        generic_phrases = ['thanks for watching', 'please subscribe', 'check out my other videos']
    ...
    def _calculate_priority(self, context: CommentContext, profile: TopicProfile) -> CommentPriority:
    ...
    def is_action_allowed(self, action: str) -> bool:
        ...
        try:
            ...
            config_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'config', 'state.json')
    ...
    async def _execute_engagement(self, opportunity_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'message': 'Engagement executed successfully' if success else f'Engagement failed: {error_message}',
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    hashlib.md5(f"{result.opportunity_id}_{datetime.now().isoformat()}".encode()).hexdigest(),
    ...
    def get_engagement_stats(self) -> Dict[str, Any]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute("SELECT COUNT(*) FROM engagement_opportunities WHERE status = 'pending'")
                ...
                if stats['total_opportunities'] > 0:
                    stats['success_rate'] = stats['completed_engagements'] / stats['total_opportunities']
