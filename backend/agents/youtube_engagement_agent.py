...
class YouTubeEngagementAgent:
    ...
    async def monitor_video_comments(
        ...
        try:
            ...
            for comment_data in comments:
                ...
                context = self._create_comment_context(
                    comment_data, video_title, video_category
                )
    ...
    async def _fetch_video_comments(self, video_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.info(
                f"Fetched {len(comment_data)} comments for video {video_id}"
            )
            return comment_data
    ...
    def _create_comment_context(
        ...
        if "snippet" in comment_data and "topLevelComment" in comment_data["snippet"]:
            ...
            author_name = comment_data.get(
                "authorDisplayName", comment_data.get("author", "")
            )
            published_at = comment_data.get(
                "publishedAt", comment_data.get("published_at", "")
            )
            like_count = comment_data.get(
                "likeCount", comment_data.get("like_count", 0)
            )
            reply_count = comment_data.get(
                "totalReplyCount", comment_data.get("reply_count", 0)
            )
            comment_id = comment_data.get("id", "")
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                for row in rows:
                    ...
                    opportunity["keywords_matched"] = json.loads(
                        opportunity["keywords_matched"]
                    )
                    opportunities.append(opportunity)
            ...
            try:
                if engagement_type == "reply":
                    ...
                    if api_result.get("status") == "success":
                        ...
                        self.logger.info(
                            f"Successfully posted reply to comment {comment_id}"
                        )
                    else:
    ...
    def get_engagement_stats(self) -> Dict[str, Any]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute(
                    "SELECT COUNT(*) FROM engagement_results WHERE status = 'posted'"
                )
