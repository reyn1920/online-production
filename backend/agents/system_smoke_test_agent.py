...
from .base_agents import BaseAgent, AgentCapability, AgentStatus
...
try:
    ...
    def dashboard_action(name=None, description=None, category=None, requires_auth=False):
        def decorator(func):
            return func
        return decorator
...
class SystemSmokeTestAgent(BaseAgent):
    ...
    def _run_test(self, test_id: str, name: str, test_func: Callable, 
                  severity: TestSeverity = TestSeverity.CRITICAL) -> TestResult:
    ...
    def _test_dashboard_port_configuration(self) -> tuple[bool, str, Dict[str, Any]]:
        ...
        if actual_port == expected_port:
            return True, f"Dashboard port correctly configured: {actual_port}", {"port": actual_port}
        else:
            return False, f"Dashboard port misconfigured. Expected: {expected_port}, Actual: {actual_port}", {
    ...
    def _test_secrets_availability(self) -> tuple[bool, str, Dict[str, Any]]:
        ...
        if not missing_secrets:
            return True, f"All {len(available_secrets)} required secrets are available", {
                "available_secrets": available_secrets,
                "total_count": len(available_secrets)
            }
        else:
            return False, f"Missing {len(missing_secrets)} required secrets: {', '.join(missing_secrets)}", {
    ...
    def _test_disk_space_availability(self) -> tuple[bool, str, Dict[str, Any]]:
        ...
        try:
            ...
            if free_gb >= required_gb:
                return True, f"Sufficient disk space available: {free_gb:.2f}GB free", {
                ...
                return False, f"Insufficient disk space. Required: {required_gb}GB, Available: {free_gb:.2f}GB", {
                    "free_gb": round(free_gb, 2),
                    "required_gb": required_gb,
    ...
    def _test_ffmpeg_installation(self) -> tuple[bool, str, Dict[str, Any]]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                return True, f"FFmpeg is installed and accessible: {version_line}", {
                    "version_output": version_line,
                    "return_code": result.returncode
                }
            else:
                return False, f"FFmpeg command failed with return code: {result.returncode}", {
                    "return_code": result.returncode,
    ...
    def _test_directory_permissions(self) -> tuple[bool, str, Dict[str, Any]]:
        ...
        if not failed_dirs:
            return True, f"All {len(critical_dirs)} critical directories are writable", {
                "test_results": test_results,
                "directories_tested": critical_dirs
            }
        else:
            return False, f"Write permission failed for {len(failed_dirs)} directories: {', '.join(failed_dirs)}", {
                "test_results": test_results,
        ...
        try:
            ...
            if response.status_code == 200:
                try:
                    ...
                    return True, "Dashboard health endpoint is responding correctly", {
                        "status_code": response.status_code,
                        "response_time_ms": int(response.elapsed.total_seconds() * 1000),
                        "health_data": health_data
                    }
                except json.JSONDecodeError:
                    return True, "Dashboard health endpoint is responding (non-JSON)", {
                        "status_code": response.status_code,
                        "response_time_ms": int(response.elapsed.total_seconds() * 1000)
                    }
            else:
                return False, f"Dashboard health endpoint returned status {response.status_code}", {
                    "status_code": response.status_code,
                    "response_text": response.text[:200]
                }
                
        except ConnectionError:
            return False, f"Cannot connect to dashboard at {self.base_url}", {
                "url": f"{self.base_url}/api/health",
                "error": "connection_refused"
            }
        except Timeout:
            return False, f"Dashboard health check timed out after {self.health_check_timeout}s", {
    ...
    def _test_orchestrator_health_endpoint(self) -> tuple[bool, str, Dict[str, Any]]:
        ...
        try:
            ...
            if response.status_code == 200:
                try:
                    health_data = response.json()
                    return True, "Orchestrator health endpoint is responding correctly", {
                        "status_code": response.status_code,
                        "response_time_ms": int(response.elapsed.total_seconds() * 1000),
                        "health_data": health_data
                    }
                except json.JSONDecodeError:
                    return True, "Orchestrator health endpoint is responding (non-JSON)", {
                        "status_code": response.status_code,
                        "response_time_ms": int(response.elapsed.total_seconds() * 1000)
                    }
            else:
                return False, f"Orchestrator health endpoint returned status {response.status_code}", {
                    "status_code": response.status_code,
                    "response_text": response.text[:200]
                }
                
        except ConnectionError:
            return False, f"Cannot connect to orchestrator at {self.orchestrator_url}", {
                "url": f"{self.orchestrator_url}/api/health",
                "error": "connection_refused"
            }
        except Timeout:
            return False, f"Orchestrator health check timed out after {self.health_check_timeout}s", {
                "timeout_seconds": self.health_check_timeout
            }
        except Exception as e:
            return False, f"Orchestrator health check failed: {str(e)}", {"error": str(e)}
    ...
    def _test_video_creation_workflow(self) -> tuple[bool, str, Dict[str, Any]]:
        ...
        return True, f"Complete video creation workflow successful in {total_duration}ms", {
            "task_id": task_id,
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                try:
                    ...
                    if task_id:
                        return True, f"Video task created successfully: {task_id}", {
                            "task_id": task_id,
                            "status_code": response.status_code,
                            "response_data": response_data
                        }
                    else:
                        return False, "Video task created but no task ID returned", {
                            "status_code": response.status_code,
                            "response_data": response_data
                        }
                        
                except json.JSONDecodeError:
                    return False, "Video task endpoint returned invalid JSON", {
                        "status_code": response.status_code,
                        "response_text": response.text[:200]
                    }
            else:
                return False, f"Video task creation failed with status {response.status_code}", {
                    "status_code": response.status_code,
                    "response_text": response.text[:200]
                }
    ...
    def _poll_task_until_completion(self, task_id: str) -> tuple[bool, str, Dict[str, Any]]:
                ...
                if response.status_code == 200:
                    try:
                        ...
                        if isinstance(tasks_data, list) and tasks_data:
                            ...
                            return False, "Invalid task data format received", {
                        ...
                        if current_status == TaskStatus.COMPLETED:
                            ...
                            return True, f"Task completed successfully after {poll_count} polls in {duration}ms", {
                                "task_id": task_id,
                            ...
                            return False, f"Task failed with status: {current_status}", {
                                "task_id": task_id,
                                "final_status": current_status,
                                "poll_count": poll_count,
                                "task_data": task_data
                        ...
                        return False, "Task status endpoint returned invalid JSON", {
                            "status_code": response.status_code
                        }
                else:
                    return False, f"Task status check failed with status {response.status_code}", {
                        "status_code": response.status_code
                    }
                    
            except Exception as e:
                return False, f"Error polling task status: {str(e)}", {
                    "error": str(e),
                    "poll_count": poll_count
                }
        
        # Timeout reached
        return False, f"Task polling timed out after {self.max_task_wait_time}s. Last status: {last_status}", {
            "task_id": task_id,
    ...
    def _verify_video_output_file(self, task_id: str) -> tuple[bool, str, Dict[str, Any]]:
        ...
        if not outputs_dir.exists():
            return False, "Outputs directory does not exist", {
        ...
        if not found_files:
            return False, "No video output files found", {
                "outputs_dir": str(outputs_dir),
        ...
        if file_size == 0:
            return False, f"Video output file is empty: {most_recent_file.name}", {
                "file_path": str(most_recent_file),
                "file_size": file_size
            }
        
        return True, f"Video output file verified: {most_recent_file.name} ({file_size} bytes)", {
            "file_path": str(most_recent_file),
            "file_size": file_size,
    ...
    async def run_complete_smoke_test(self) -> SmokeTestReport:
        ...
        test_suite = [
            # Pre-flight infrastructure checks
            ("port_config", "Dashboard Port Configuration", self._test_dashboard_port_configuration, TestSeverity.CRITICAL),
            ("secrets_check", "Required Secrets Availability", self._test_secrets_availability, TestSeverity.CRITICAL),
            ("disk_space", "Disk Space Availability", self._test_disk_space_availability, TestSeverity.WARNING),
            ("ffmpeg_install", "FFmpeg Installation", self._test_ffmpeg_installation, TestSeverity.CRITICAL),
            ("dir_permissions", "Directory Write Permissions", self._test_directory_permissions, TestSeverity.CRITICAL),
            
            # Live system health checks
            ("dashboard_health", "Dashboard Health Endpoint", self._test_dashboard_health_endpoint, TestSeverity.CRITICAL),
            ("orchestrator_health", "Orchestrator Health Endpoint", self._test_orchestrator_health_endpoint, TestSeverity.WARNING),
            
            # End-to-end workflow testing
            ("video_workflow", "Complete Video Creation Workflow", self._test_video_creation_workflow, TestSeverity.CRITICAL),
        ]
        ...
        critical_failures = sum(1 for r in test_results if r.status == TestStatus.FAILED and r.severity == TestSeverity.CRITICAL)
        
        # Determine overall status
        if critical_failures > 0:
            overall_status = TestStatus.FAILED
        elif failed_tests > 0:
            overall_status = TestStatus.FAILED  # Any failure is considered overall failure
        else:
        ...
        if overall_status == TestStatus.PASSED:
            self.logger.info(f"✅ ALL TESTS PASSED. The TRAE.AI system is 100% live and fully operational.")
        else:
            self.logger.error(f"❌ SMOKE TEST FAILED. {critical_failures} critical failures, {failed_tests} total failures.")
    ...
    def get_test_summary(self) -> Dict[str, Any]:
        ...
        if not self.current_test_run:
            return {"status": "no_test_run", "message": "No smoke test has been executed yet"}
    ...
    async def _execute_with_monitoring(self, task_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _validate_rephrase_accuracy(self, original: str, rephrased: str, context: Dict[str, Any]) -> bool:
