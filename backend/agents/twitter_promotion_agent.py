...
from backend.integrations.twitter_integration import TwitterIntegration, TweetData, TweetType
...
class TwitterPromotionAgent:
    ...
    def create_promotion_campaign(self, video_metadata: VideoMetadata, 
                                strategy: PromotionStrategy = PromotionStrategy.IMMEDIATE) -> str:
        """
        ...
        try:
            ...
            self.logger.info(f"Created promotion campaign {campaign_id} for video {video_metadata.video_id}")
            return campaign_id
        ...
        try:
            ...
            for hashtag in candidates:
                if hashtag:
                    ...
                    hashtag_scores.append((hashtag, analysis.relevance_score + analysis.trend_score))
            
            # Sort by score and select top hashtags
            hashtag_scores.sort(key=lambda x: x[1], reverse=True)
            selected_hashtags = [f"#{hashtag}" for hashtag, _ in hashtag_scores[:self.max_hashtags]]
    ...
    def _analyze_hashtag_performance(self, hashtag: str) -> HashtagAnalysis:
        ...
        try:
            ...
            cursor.execute(
                "SELECT * FROM hashtag_performance WHERE hashtag = ?",
                (hashtag,)
            )
    ...
    def _calculate_optimal_posting_time(self, strategy: PromotionStrategy) -> Optional[datetime]:
        ...
        if strategy == PromotionStrategy.IMMEDIATE:
            ...
            next_optimal_hours = [h for h in self.optimal_posting_hours if h > current_hour]
    ...
    def execute_promotion(self, campaign_id: str) -> bool:
        ...
        try:
            ...
            if tweet_id:
                ...
                self.logger.info(f"Successfully posted promotion for campaign {campaign_id}")
                return True
            else:
                # Mark as failed
    ...
    def get_campaign_analytics(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'top_hashtags': [
                    {'hashtag': row[0], 'usage_count': row[1], 'engagement_rate': row[2]}
...
if __name__ == "__main__":
    ...
    try:
        campaign_id = agent.create_promotion_campaign(video, PromotionStrategy.IMMEDIATE)
