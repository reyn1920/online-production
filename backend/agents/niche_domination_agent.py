...
class ProactiveNicheDominationAgent(BaseAgent):
    ...
    def __init__(self, agent_id: str = "niche_domination_agent", name: str = "Niche Domination Agent", config: Dict[str, Any] = None):
        super().__init__(agent_id, name)
        self.config = config or {}
        self.db_path = self.config.get('db_path', 'right_perspective.db')
        self.ollama_client = OllamaIntegration(self.config.get('ollama_config', {'endpoint': 'http://localhost:11434'}))
        # Fix: Extract ollama_url as string from config to prevent rstrip() error on dict
        ollama_url = _as_str(self.config.get('ollama_url', 'http://localhost:11434'), 'http://localhost:11434')
        ...
        self.growth_threshold = self.config.get('growth_threshold', 0.20)  # 20% monthly growth
        self.opportunity_score_threshold = self.config.get('opportunity_threshold', 0.75)
    ...
    def _init_database(self):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                # Create indexes
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_growth_metrics_channel_niche ON growth_metrics(channel, niche)")
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_opportunities_score ON niche_opportunities(opportunity_score DESC)")
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_expansions_status ON niche_expansions(status)")
    ...
    def analyze_growth_opportunities(self) -> List[NicheOpportunity]:
        ...
        all_opportunities = (
            high_performers + market_gaps + trending_opportunities + 
            competitor_weaknesses + seasonal_opportunities
        )
        ...
        self.logger.info(f"Identified {len(qualified_opportunities)} qualified opportunities")
    ...
    def _collect_current_metrics(self) -> List[GrowthMetrics]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                """.format(self.analysis_window_days))
        ...
        for metric in metrics:
            ...
            if performance_score >= 0.7:
                ...
                for channel_type in ChannelType:
                    if channel_type.value != metric.channel:  # Don't expand to same channel
                        opportunity = NicheOpportunity(
                            niche_name=f"{metric.niche}_expansion_{channel_type.value}",
                            channel_type=channel_type,
                            opportunity_score=performance_score * 0.9,  # Slight discount for expansion
                            trigger_type=ExpansionTrigger.HIGH_ENGAGEMENT,
                            market_size=self._estimate_market_size(metric.niche, channel_type),
                            competition_level=1.0 - metric.market_saturation,
                            entry_difficulty=self._calculate_entry_difficulty(channel_type),
                            revenue_potential=metric.revenue_per_view * 0.8,  # Conservative estimate
                            content_requirements=self._get_content_requirements(metric.niche, channel_type),
                            recommended_strategy=self._generate_expansion_strategy(metric, channel_type),
    ...
    def execute_niche_expansion(self, opportunity: NicheOpportunity) -> bool:
        ...
        try:
            ...
            initial_content = self._generate_initial_content(opportunity, content_strategy)
            
            # 3. Set up channel/platform presence
            channel_setup = self._setup_channel_presence(opportunity)
            
            # 4. Deploy initial content
            deployment_success = self._deploy_initial_content(opportunity, initial_content)
            
            # 5. Set up monitoring and optimization
            monitoring_setup = self._setup_expansion_monitoring(opportunity)
            
            if all([content_strategy, initial_content, channel_setup, deployment_success, monitoring_setup]):
...
"""
        ...
        try:
            ...
            for trend in trending_searches[0][:10]:  # Top 10 trends
                ...
                if not interest_data.empty:
                    ...
                    trends.append({
                        'topic': trend,
                        'search_volume': int(avg_interest * 1000),  # Estimate volume
    ...
    def _fetch_twitter_trends(self) -> List[Dict]:
        ...
        try:
            ...
            with SecretStore() as store:
                ...
                trending_topics = api.get_place_trends(1)[0]['trends']  # WOEID 1 = worldwide
                ...
                for video in data.get('items', []):
                    ...
                    for word in title_words + tags:
                        if len(word) > 3:  # Filter short words
                            topic_counts[word] = topic_counts.get(word, 0) + int(stats.get('viewCount', 0))
                
                # Convert to trend format
                for topic, view_count in sorted(topic_counts.items(), key=lambda x: x[1], reverse=True)[:10]:
                    trends.append({
                        'topic': topic.title(),
            ...
            for subreddit in subreddits:
                ...
                for post in data['data']['children'][:5]:  # Top 5 from each
                    ...
                    trends.append({
                        ...
                        'search_volume': post_data['score'] * 10,  # Estimate volume from score
                        'competition': 0.6,
                        'best_channel': 'blog',
    ...
    def _get_seasonal_trends(self, month: int) -> List[Dict]:
        ...
        for topic in seasonal_topics:
            opportunities.append({
                ...
                'content_calendar': [f'{topic} guide', f'{topic} tips', f'{topic} reviews'],
    ...
    def _get_content_requirements(self, niche: str, channel_type: ChannelType) -> List[str]:
        ...
        channel_requirements = {
            ChannelType.YOUTUBE: ['video content', 'thumbnails', 'descriptions', 'tags'],
    ...
    def _generate_expansion_strategy(self, metric: GrowthMetrics, channel_type: ChannelType) -> str:
        ...
        try:
            ...
            for i in range(content_count):
                content = self.content_generator.generate_content(
                    ...
                    content_type=opportunity.content_requirements[0] if opportunity.content_requirements else 'article'
                )
                if not content:
                    return False
            return True
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute("""
                    ...
                    GROUP BY status
                """)
                opportunity_summary = {row['status']: {'count': row['count'], 'avg_score': row['avg_score']} 
                                     for row in cursor.fetchall()}
    ...
    async def _validate_rephrase_accuracy(self, original_task: Dict[str, Any], rephrased: str, context) -> bool:
        ...
        niche_terms = ['niche', 'market', 'growth', 'expansion', 'opportunity', 'domination', 'competition']
