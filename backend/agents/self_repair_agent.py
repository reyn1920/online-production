...
from .safe_mode_manager import SafeModeManager, EnvironmentSnapshot
...
class ProgressiveSelfRepairAgent(BaseAgent):
    ...
    def handle_component_failure(self, component_name: str, error_message: str, 
        ...
        try:
            ...
            if not snapshot:
                self.logger.warning("Failed to create pre-repair snapshot - proceeding without Safe Mode")
            else:
            ...
            if repair_tier == RepairTier.RESTART:
                ...
                success = self._execute_tier3_repair(component_name, error_message, error_context, repair_history)
            
            repair_duration = time.time() - repair_start_time
            
            # Enhanced post-repair validation and stability verification
            if success:
                post_repair_validation = self._perform_post_repair_validation(component_name, repair_duration)
                
                if not post_repair_validation['stable']:
                    self.logger.warning(f"Post-repair validation failed: {post_repair_validation['issues']}")
                    ...
                    if snapshot:
                        ...
                        if rollback_result.success:
                            ...
                            self._log_rollback_event(component_name, snapshot.snapshot_id, "validation_failure")
                        else:
                            self.logger.error(f"Automatic rollback failed: {rollback_result.error_message}")
                            self._log_rollback_event(component_name, snapshot.snapshot_id, "rollback_failed")
            ...
            if snapshot:
                try:
                    ...
                    if rollback_result.success:
                        ...
                        self._log_rollback_event(component_name, snapshot.snapshot_id, "exception_recovery")
                    else:
                        self.logger.error(f"Emergency rollback failed: {rollback_result.error_message}")
    ...
    def _determine_repair_tier(self, component_name: str, repair_history: List[Dict]) -> RepairTier:
        ...
        recent_attempts = [
            attempt for attempt in repair_history 
    ...
    def _execute_tier1_repair(self, component_name: str, error_message: str, 
                             error_context: Optional[Dict]) -> bool:
        ...
        self._log_repair_attempt(RepairAttempt(
    ...
    def _execute_tier2_repair(self, component_name: str, error_message: str, 
                             error_context: Optional[Dict]) -> bool:
    ...
    def _execute_tier3_repair(self, component_name: str, error_message: str, 
                             error_context: Optional[Dict], repair_history: List[Dict]) -> bool:
        """Tier 3: AI-powered research and novel solution generation."""
        start_time = time.time()
        ...
        try:
            ...
            if ai_response and ai_response.get('response'):
                ...
                if executable_code:
                    ...
                    success = False
                    execution_details = f"No executable code found in AI response: {solution[:200]}..."
            else:
                success = False
    ...
    def _generate_ai_repair_prompt(self, component_name: str, error_message: str, 
                                  repair_history: List[Dict]) -> str:
        ...
        if repair_history:
            failed_repairs = [f"Tier {r['repair_tier']}: {r['repair_action']} - {r['outcome']}" 
                            for r in repair_history[-10:]]  # Last 10 attempts
            history_summary = ", ".join(failed_repairs)
        else:
            history_summary = "None"
        
        # Check if this is a creative component
        is_creative = component_name in ['linly_talker', 'blender_compositor', 'creative_pipeline']
...
"""
            ...
            if any(keyword in code for keyword in ['import ', 'def ', 'class ', 'print(']):
                ...
                if is_creative:
                    ...
                    try:
                        ...
                        cmd = ['bash', '-c', f'source activate_creative.sh && python {temp_file}']
    ...
    def _check_dependencies(self, component_name: str) -> List[str]:
        ...
        elif component_name in ['linly_talker', 'blender_compositor', 'creative_pipeline']:
            ...
            if not os.path.exists(creative_env_path):
                ...
                try:
                    result = subprocess.run([
                        f'{creative_env_path}/bin/python', '-c', 
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                """, (
                    ...
                    }) if attempt.execution_details or attempt.error_context else None,
                    int(attempt.duration) if attempt.duration else None,
                    snapshot_id
                ))
                conn.commit()
                self.logger.info(f"Logged repair attempt for {attempt.component_name}: {attempt.repair_action}")
        except Exception as e:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                if existing:
                    ...
                    cursor.execute("""
                        ...
                        WHERE component_name = ?
                    """, (status.value, consecutive_failures, total_failures, status.value, component_name))
                else:
                    # Insert new record
                    cursor.execute("""
                    ...
                    """, (
                        component_name, status.value,
                        0 if status == ComponentStatus.HEALTHY else 1,
                        0 if status == ComponentStatus.HEALTHY else 1,
                        datetime.now().isoformat() if status != ComponentStatus.HEALTHY else None
                    ))
    ...
    def _perform_pre_repair_validation(self, component_name: str, error_message: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if component_name in ['database', 'ollama_service']:
                ...
                if self._is_component_in_use(component_name):
                    validation_result['risk_level'] = 'high'
                    validation_result['recommendations'].append('Consider scheduling repair during low usage')
            ...
            if any(pattern in error_message.lower() for pattern in dangerous_patterns):
                validation_result['risk_level'] = 'high'
                validation_result['recommendations'].append('High-risk error detected - proceed with caution')
            
            self.logger.info(f"Pre-repair validation for {component_name}: {validation_result['risk_level']} risk")
            return validation_result
    ...
    def _perform_post_repair_validation(self, component_name: str, repair_duration: float) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                validation_result['stable'] = False
                validation_result['issues'].append(f'Database integrity compromised: {str(db_error)}')
            ...
            if component_name == 'ollama_service':
                try:
                    ...
                    if response.status_code != 200:
                        validation_result['stable'] = False
                        validation_result['issues'].append('Ollama service not responding correctly')
                except Exception as ollama_error:
                    validation_result['stable'] = False
                    validation_result['issues'].append(f'Ollama connectivity failed: {str(ollama_error)}')
            
            elif component_name in ['linly_talker', 'blender_compositor', 'creative_pipeline']:
                # Validate creative environment
                try:
                    result = subprocess.run([
                        'venv_creative/bin/python', '-c', 
                    ...
                    validation_result['stable'] = False
                    validation_result['issues'].append(f'Creative environment error: {str(creative_error)}')
            
            # Performance impact assessment
            if repair_duration > 60:
                validation_result['performance_impact'] = 'high'
                validation_result['recommendations'].append('Long repair duration may indicate underlying issues')
            ...
            memory_usage = psutil.virtual_memory().percent
            if memory_usage > 85:
                validation_result['issues'].append(f'High memory usage after repair: {memory_usage}%')
            ...
            self.logger.info(f"Post-repair validation for {component_name}: {'STABLE' if validation_result['stable'] else 'UNSTABLE'}")
            return validation_result
    ...
    def _log_rollback_event(self, component_name: str, snapshot_id: str, rollback_reason: str):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute("""
                    INSERT INTO rollback_log (component_name, snapshot_id, rollback_reason)
                    VALUES (?, ?, ?)
    ...
    async def _execute_with_monitoring(self, task: Dict[str, Any], context) -> Dict[str, Any]:
        ...
        try:
            ...
            if task_type == 'repair_component':
                ...
                success = self.handle_component_failure(component_name, error_message, error_context)
                return {
                    'success': success,
                    'result': {'component_repaired': success, 'component': component_name}
                }
    ...
    async def _generate_diagnostic_report(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            report = {
                'Initial Goal': 'Fix all startup errors and prepare system for live deployment',
                'Analysis Log': self._generate_analysis_log(repair_history),
                'Point of Failure': self._identify_point_of_failure(failed_repairs, critical_components),
                'Current Hypothesis': self._generate_hypothesis(failed_repairs, critical_components),
    ...
    def _generate_analysis_log(self, repair_history: List[Dict]) -> List[str]:
        ...
        if repair_history:
            ...
            for i, component in enumerate(components_attempted, 2):
                ...
                if latest_repair:
                    log_steps.append(f"{i}. Attempted {latest_repair['repair_action']} for {component}")
        else:
            log_steps.append("1. No repair attempts found in system logs")
        
        return log_steps
    
    def _identify_point_of_failure(self, failed_repairs: List[Dict], critical_components: List[Dict]) -> str:
    ...
    def _generate_hypothesis(self, failed_repairs: List[Dict], critical_components: List[Dict]) -> str:
        ...
        if failed_repairs:
            ...
            if failure_types.get('dependency', 0) > 0:
                return "Root cause appears to be missing dependencies or import errors. The system may be missing required packages or have incorrect module paths."
            elif failure_types.get('implementation', 0) > 0:
                return "Root cause appears to be implementation errors - missing methods or attributes in classes. The codebase may have incomplete implementations or interface mismatches."
            elif failure_types.get('connectivity', 0) > 0:
                return "Root cause appears to be connectivity issues - services may not be running or network connections are failing."
            else:
                return "Root cause is unclear from error patterns. Manual investigation of the specific error contexts is required."
        else:
            return "No failed repairs to analyze. System may have configuration or initialization issues that prevent proper error detection."
