...
try:
    from enhanced_web_scraping_tools import (
        EnhancedWebScraper, ScrapingConfig, ScrapingMethod, 
        ExtractionRule, ScrapingResult, DataFormat
    )
except ImportError:
...
class APIDiscoveryEngine:
        ...
        try:
            ...
            for entry in entries[:self.config.max_apis_per_source]:
                try:
                    ...
                    if (self.config.categories_filter and 
                        category not in self.config.categories_filter):
                        continue
                    ...
                    is_free = auth_str in ['', 'no', 'none'] or 'free' in entry.get('Description', '').lower()
    ...
    async def validate_apis(self, apis: List[APIInfo]) -> List[APIInfo]:
        ...
        for result in validated_apis:
            if isinstance(result, APIInfo) and result.validation_score >= self.config.min_validation_score:
...
'''
    ...
    def save_discovered_apis(self, apis: List[APIInfo], filename: str = "discovered_apis.json") -> None:
    ...
    def search_apis(self, query: str) -> List[APIInfo]:
        ...
        for api in self.discovered_apis.values():
            if (query_lower in api.name.lower() or 
                ...
                any(query_lower in tag for tag in api.tags)):
                results.append(api)
    ...
    async def run_full_discovery(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'categories': {category.value: len(self.get_apis_by_category(category)) 
...
if __name__ == "__main__":
    async def main():
        ...
        config = APIDiscoveryConfig(
            ...
            categories_filter=[APICategory.WEATHER, APICategory.NEWS, APICategory.FINANCE],
            free_only=True
        )
