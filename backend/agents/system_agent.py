...
class SystemHealth:
    ...
    def overall_health_score(self) -> float:
        ...
        # Service health
        service_health = sum(self.service_status.values()) / max(1, len(self.service_status))
...
class SystemAgent(BaseAgent):
    ...
    def __init__(self, db_path: str = "data/right_perspective.db", agent_id: str = "SystemAgent", name: str = "System Management Agent", main_loop=None):
        ...
        self.watchdog_threshold = int(os.getenv('WATCHDOG_THRESHOLD_SECONDS', '300'))  # 5 minutes default
        self.watchdog_check_interval = int(os.getenv('WATCHDOG_CHECK_INTERVAL', '60'))  # 1 minute default
    ...
    def start_monitoring(self):
        ...
        self.repair_thread = threading.Thread(target=self._repair_processor, daemon=True)
        ...
        if self.watchdog_enabled:
            ...
            self.logger.info(f"Watchdog system enabled (threshold: {self.watchdog_threshold}s)")
    ...
    @dashboard_action("Get System Health", "Retrieve current system health metrics and status")
    def get_system_health(self) -> SystemHealth:
        ...
        for agent_name, last_heartbeat in self.agent_heartbeats.items():
            ...
            agent_status[agent_name] = time_since_heartbeat < (self.heartbeat_interval * 2)
        
        # Service status
        service_status = {}
        for service_name, check_command in self.registered_services.items():
            try:
                result = subprocess.run(check_command, shell=True, capture_output=True, timeout=10)
    ...
    @dashboard_action("Run System Diagnostics", "Perform comprehensive system health diagnostics")
    ...
    def execute_repair(self, diagnostic: DiagnosticResult) -> RepairAction:
        ...
        action_id = f"repair_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{diagnostic.component}"
        ...
        try:
            ...
            self.logger.info(f"Repair action {action_id}: {'SUCCESS' if success else 'FAILED'}")
        ...
        try:
            ...
            for agent_name, last_heartbeat in self.agent_heartbeats.items():
                ...
                if time_since_heartbeat > (self.heartbeat_interval * 3):
                    self.logger.warning(f"Agent {agent_name} missed heartbeat ({time_since_heartbeat:.1f}s)")
    ...
    def _restart_service(self, service_name: str) -> bool:
        ...
        try:
            ...
            if sys.platform == "darwin":
                subprocess.run(["launchctl", "kickstart", "-k", f"gui/501/{service_name}"], check=True)
            else:
        ...
        try:
            ...
            for temp_dir in temp_dirs:
                if os.path.exists(temp_dir):
                    subprocess.run(["find", temp_dir, "-type", "f", "-atime", "+7", "-delete"], check=True)
            
            # Clean logs older than 30 days
            log_dirs = ["/var/log", "./logs"]
            for log_dir in log_dirs:
                if os.path.exists(log_dir):
                    subprocess.run(["find", log_dir, "-name", "*.log", "-mtime", "+30", "-delete"], check=True)
    ...
    def _investigate_system_stall(self) -> bool:
        ...
        try:
            self.logger.warning("Investigating system stall - gathering diagnostic information")
            
            # Get current system metrics
            health = self.get_system_health()
            
            # Log detailed system state
            self.logger.info(f"System Health Score: {health.overall_health_score():.2f}")
            self.logger.info(f"CPU Usage: {health.cpu_usage:.1f}%")
            self.logger.info(f"Memory Usage: {health.memory_usage:.1f}%")
            self.logger.info(f"Active Agents: {sum(health.agent_status.values())}/{len(health.agent_status)}")
            self.logger.info(f"Active Services: {sum(health.service_status.values())}/{len(health.service_status)}")
            
            # Check for stuck processes
            stuck_processes = []
            for proc in psutil.process_iter(['pid', 'name', 'status', 'cpu_times']):
                try:
                    if proc.info['status'] == 'zombie' or proc.info['cpu_times'].user > 3600:  # 1 hour CPU time
                        stuck_processes.append(f"{proc.info['name']} (PID: {proc.info['pid']})")
                except (psutil.NoSuchProcess, psutil.AccessDenied):
                    continue
            
            if stuck_processes:
                self.logger.warning(f"Potentially stuck processes: {', '.join(stuck_processes)}")
        ...
        with sqlite3.connect(self.db_path) as conn:
            for result in results:
                conn.execute("""
                    ...
                    (component, status, message, suggested_action, confidence, timestamp)
                    VALUES (?, ?, ?, ?, ?, ?)
                """, (
                    result.component,
                    result.status,
    ...
    def update_system_activity(self):
        ...
        if self.watchdog_alerts_sent > 0:
            self.logger.info("System activity detected, resetting watchdog alert counter")
            self.watchdog_alerts_sent = 0
    
    def _watchdog_monitor(self):
        """Monitor system cadence and alert when stalled beyond threshold"""
        self.logger.info(f"Watchdog monitor started (threshold: {self.watchdog_threshold}s, check interval: {self.watchdog_check_interval}s)")
        
        while self.monitoring_active:
            try:
                current_time = datetime.now()
                time_since_activity = (current_time - self.last_system_activity).total_seconds()
                ...
                if time_since_activity > self.watchdog_threshold:
                    if self.watchdog_alerts_sent < self.max_watchdog_alerts:
                        ...
                        self.logger.warning(f"Watchdog alert limit reached ({self.max_watchdog_alerts}). Suppressing further alerts until activity resumes.")
                        self.watchdog_alerts_sent += 1  # Increment to prevent repeated warnings
