...
from .research_tools import CompetitorAnalyzer
...
from .web_automation_tools import (
    ...
    StealthLevel, AutomationSequence, ActionType
)
...
class GrowthAgent(BaseAgent):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logger.info(f"GrowthAgent initialized with {self.growth_threshold}% growth threshold")
    ...
    async def process_task(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Check if growth actions are enabled
        if not self.config.get('growth_enabled', True):
            return {
                'status': 'disabled',
        ...
        try:
            ...
            for channel_id, metrics in self.active_channels.items():
                ...
                if updated_metrics:
                    ...
                    growth_rate = self._calculate_growth_rate(channel_id, updated_metrics)
                    ...
                    if growth_rate < self.growth_threshold:
                        logger.warning(f"Channel {channel_id} growth plateau detected: {growth_rate}%")
            ...
            logger.info(f"Growth monitoring completed: {results['monitored_channels']} channels")
    ...
    async def _fetch_instagram_metrics(self, channel_id: str) -> Optional[ChannelMetrics]:
        ...
        try:
            ...
            with sqlite3.connect(db_path) as conn:
                ...
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    metrics.channel_id, metrics.channel_name, metrics.platform,
                    metrics.subscribers, metrics.views_last_30d, metrics.revenue_last_30d,
                    metrics.growth_rate_mom, metrics.engagement_rate, metrics.content_frequency,
                    metrics.niche, metrics.growth_phase.value, 
                    metrics.created_at.isoformat(), metrics.last_updated.isoformat()
                ))
                conn.commit()
    ...
    def _calculate_growth_rate(self, channel_id: str, current_metrics: ChannelMetrics) -> float:
    ...
    async def _trigger_niche_expansion(self, source_channel_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not await self._check_expansion_cooldown(source_channel_id):
                return {
                    'status': 'skipped',
                    'reason': 'expansion_cooldown_active'
                }
            ...
            if not niche_opportunity:
                return {
                    'status': 'failed',
                    'reason': 'no_suitable_niche_found'
                }
            ...
            if hasattr(self, 'competitor_analyzer'):
                ...
                if opportunities:
                    best_opportunity = max(
                        opportunities,
                        key=lambda x: (x.profit_potential * (1 - x.competition_level) * x.confidence_score)
                    )
    ...
    async def _execute_day_one_blitz(self, expansion_plan: ExpansionPlan) -> Dict[str, Any]:
        ...
        try:
            ...
            affiliate_result = {"status": "success", "message": "Affiliate setup initiated"}
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'active_expansions': len([p for p in self.expansion_plans.values() if p.status == 'active']),
