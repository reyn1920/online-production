...
class BreakingNewsWatcher:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.keywords_of_interest = self.config.get('keywords', [
            'AI', 'artificial intelligence', 'machine learning', 'automation',
            'digital marketing', 'social media', 'content creation', 'YouTube',
            'affiliate marketing', 'online business', 'SaaS', 'startup'
        ])
    ...
    def _get_default_feeds(self) -> List[Dict[str, str]]:
        ...
        return [
            {'url': 'https://feeds.feedburner.com/TechCrunch', 'category': NewsCategory.TECHNOLOGY, 'name': 'TechCrunch'},
            {'url': 'https://rss.cnn.com/rss/edition.rss', 'category': NewsCategory.GENERAL, 'name': 'CNN'},
            {'url': 'https://feeds.reuters.com/reuters/businessNews', 'category': NewsCategory.BUSINESS, 'name': 'Reuters Business'},
            {'url': 'https://feeds.feedburner.com/venturebeat/SZYF', 'category': NewsCategory.TECHNOLOGY, 'name': 'VentureBeat'},
            {'url': 'https://feeds.feedburner.com/Mashable', 'category': NewsCategory.TECHNOLOGY, 'name': 'Mashable'},
            {'url': 'https://feeds.feedburner.com/socialmediaexaminer', 'category': NewsCategory.SOCIAL_MEDIA, 'name': 'Social Media Examiner'},
            {'url': 'https://feeds.feedburner.com/MarketingLand', 'category': NewsCategory.MARKETING, 'name': 'Marketing Land'},
        ]
    ...
    async def monitor_feeds(self, duration_hours: int = 24) -> List[NewsItem]:
        ...
        if not feedparser:
            self.logger.error("feedparser not available. Install with: pip install feedparser")
            return []
    ...
    async def _process_news_items(self, news_items: List[NewsItem]) -> List[NewsItem]:
        ...
        for item in news_items:
            try:
                # Extract keywords
                item.keywords = self._extract_keywords(item.title + " " + item.description)
        ...
        try:
            ...
            briefing_content = {
                ...
                'sentiment_summary': {
                    ...
                    'neutral': len([item for item in news_items if -0.1 <= item.sentiment_score <= 0.1])
                }
            }
            ...
            self.logger.info(f"Updated trend analysis with {len(keyword_counts)} keywords")
    ...
    def get_trending_keywords(self, hours: int = 24) -> Dict[str, int]:
        ...
        recent_items = [
            item for item in self.news_cache.values()
    ...
    async def scan_for_hypocrisy(self, hours_back: int = 48) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            political_items = [
                item for item in self.news_cache.values()
                if item.published > cutoff_time and 
                ('politic' in item.title.lower() or 'politic' in item.description.lower())
            ]
            ...
            for figure in public_figures:
                ...
                contradictions = await self._find_contradictory_statements(figure, political_items)
                ...
                for contradiction in contradictions:
                    ...
                    if score > 0.6:  # Only high-confidence contradictions
                        hypocrisy_finding = {
                            ...
                            'time_gap_days': (contradiction['date_2'] - contradiction['date_1']).days,
            ...
            self.logger.info(f"Discovered {len(hypocrisy_findings)} hypocrisy opportunities")
    ...
    def _extract_public_figures(self, news_items: List[NewsItem]) -> List[str]:
        ...
        political_keywords = [
            'president', 'senator', 'congressman', 'governor', 'mayor',
            'minister', 'secretary', 'representative', 'candidate'
        ]
    ...
    async def _find_contradictory_statements(self, figure: str, recent_items: List[NewsItem]) -> List[Dict[str, Any]]:
        ...
        figure_statements = []
        for item in recent_items:
            if figure.lower() in item.title.lower() or figure.lower() in item.description.lower():
        ...
        for i, stmt1 in enumerate(figure_statements):
            for stmt2 in figure_statements[i+1:]:
                ...
                common_words = self._find_common_important_words(stmt1['text'], stmt2['text'])
                ...
                if len(common_words) >= 2:  # Same topic
                    ...
                    contradiction_indicators = [
                        ...
                        ('will', 'will not'), ('should', 'should not'), ('favor', 'against')
                    ]
    ...
    def _find_common_important_words(self, text1: str, text2: str) -> List[str]:
        ...
        words1 = set(word.lower().strip('.,!?"') for word in text1.split() if len(word) > 3 and word.lower() not in stop_words)
        words2 = set(word.lower().strip('.,!?"') for word in text2.split() if len(word) > 3 and word.lower() not in stop_words)
    ...
    async def _store_hypocrisy_finding(self, finding: Dict[str, Any]) -> None:
        ...
        try:
            ...
            hypocrisy_finding = HypocrisyFinding(
                ...
                severity_score=max(1, min(10, int(finding['contradiction_score'] * 10) or 1)),  # Convert to 1-10 scale, ensure >= 1
                confidence_score=finding['contradiction_score'],  # Keep as 0.0-1.0 range
                verification_status='pending',
                evidence_links=[link for link in [finding.get('link_1', ''), finding.get('link_2', '')] if link],
    ...
    def get_hypocrisy_content_opportunities(self, limit: int = 10) -> List[Dict[str, Any]]:
        """Get unused hypocrisy findings for content creation"""
        try:
            if not self.hypocrisy_db:
                self.logger.warning("Hypocrisy database manager not available, falling back to direct SQLite")
            ...
            for opp in db_opportunities:
                opportunities.append({
                    ...
                    'topic': opp['tags'],
                    'contradiction_score': opp['severity_score'] / 10.0,  # Convert back to 0-1 scale
                    ...
                    'date_1': opp['date_1'],
                    'date_2': opp['date_2'],
                    'time_gap_days': (opp['date_2'] - opp['date_1']).days if opp['date_2'] and opp['date_1'] else 0,
    ...
    def get_latest_news(self, limit: int = 10, category: Optional[NewsCategory] = None) -> List[NewsItem]:
        ...
        try:
            ...
            for row in results:
                try:
                    ...
                    published = datetime.fromisoformat(row[3]) if row[3] else datetime.now()
                    
                    # Map category string to enum
                    category_enum = self._map_category(row[5]) if row[5] else NewsCategory.GENERAL
                    
                    # Map trend strength string to enum
                    trend_strength = TrendStrength(row[8]) if row[8] else TrendStrength.WEAK
...
class CompetitorAnalyzer:
    ...
    async def analyze_niche(self, niche_keywords: List[str], max_channels: int = 50) -> List[CompetitorChannel]:
        ...
        try:
            ...
            channel_ids = await self._search_channels_by_keywords(niche_keywords, max_channels)
            ...
            return sorted(analyzed_channels, key=lambda x: x.opportunity_score, reverse=True)
            
        except Exception as e:
            self.logger.error(f"Error analyzing niche: {e}")
            return []
    
    async def _search_channels_by_keywords(self, keywords: List[str], max_results: int) -> List[str]:
        """Search for YouTube channels by keywords using YouTube Data API"""
        from backend.secret_store import SecretStore
        
        try:
            # Get YouTube API key from secure storage
            with SecretStore(self.config.get('secrets_db', 'data/secrets.sqlite')) as store:
            ...
            for keyword in keywords[:3]:  # Limit to first 3 keywords to avoid quota exhaustion
                try:
                    # Search for channels
                    ...
                    if search_response.status_code == 200:
                        ...
                        self.logger.warning(f"YouTube search API error for keyword '{keyword}': {search_response.status_code}")
            ...
            if not channel_ids:
                self.logger.warning("No channels found through search, using fallback popular channels")
                fallback_channels = [
    ...
    def _fetch_channel_data(self, channel_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            if channel_response.status_code != 200:
                self.logger.error(f"YouTube API error for channel {channel_id}: {channel_response.status_code}")
                return None
                ...
                if video_items:
                    ...
                    if video_stats_response.status_code == 200:
                        ...
                        if video_details:
                            ...
                            engagement_rate = total_engagement / len(video_details) if video_details else 0.0
                    
                    # Calculate upload frequency based on recent videos
                    if len(video_items) >= 2:
                        from datetime import datetime
                        try:
                            latest_date = datetime.fromisoformat(video_items[0]['snippet']['publishedAt'].replace('Z', '+00:00'))
                            oldest_date = datetime.fromisoformat(video_items[-1]['snippet']['publishedAt'].replace('Z', '+00:00'))
                            days_diff = (latest_date - oldest_date).days
                            if days_diff > 0:
                                upload_frequency = (len(video_items) * 7) / days_diff  # videos per week
                        except Exception as e:
                    ...
                    if 'tech' in channel_description or 'technology' in channel_description:
                        themes.append('Technology')
                    if 'review' in channel_description:
                        themes.append('Reviews')
                    if 'education' in channel_description or 'tutorial' in channel_description:
            ...
            return {
                ...
                'upload_frequency': max(upload_frequency, 0.1),  # Minimum 0.1 videos per week
                'average_views': average_views,
    ...
    def find_content_gaps(self, channels: List[CompetitorChannel]) -> List[str]:
        ...
        potential_themes = [
            'Beginner Tutorials', 'Advanced Techniques', 'Tool Reviews',
            'Industry News', 'Behind the Scenes', 'Q&A Sessions',
            'Collaboration Videos', 'Live Streams', 'Short Form Content',
            'Educational Series', 'Case Studies', 'Interviews'
        ]
...
class MarketValidator:
    ...
    async def validate_product_idea(self, product_concept: str, target_keywords: List[str]) -> MarketOpportunity:
        ...
        try:
            ...
            trend_data = await self._analyze_market_trends(product_concept, target_keywords)
            
            # Evaluate monetization potential
            monetization_score = self._assess_monetization_potential(product_concept, search_data)
            
            # Identify target audience
            target_audience = self._identify_target_audience(product_concept, target_keywords)
            
            # Find content gaps
            content_gaps = await self._identify_content_gaps(target_keywords)
            
            # Generate product recommendations
            recommended_products = self._generate_product_recommendations(product_concept, search_data)
            
            # Calculate confidence score
            confidence_score = self._calculate_confidence_score(search_data, trend_data, monetization_score)
        ...
        try:
            ...
            with SecretStore(self.config.get('secrets_db', 'data/secrets.sqlite')) as store:
                ...
                if not all([api_key, customer_id, developer_token]):
                    self.logger.error("Google Ads API credentials not configured in secret store")
            ...
            self.logger.info(f"Retrieved search metrics for {len(keywords)} keywords: {total_volume} total volume")
    ...
    async def _analyze_market_trends(self, product_concept: str, keywords: List[str]) -> Dict[str, str]:
        ...
        try:
            ...
            try:
                ...
                self.logger.error("pytrends library not available. Install with: pip install pytrends")
            ...
            pytrends.build_payload(trend_keywords, cat=0, timeframe='today 12-m', geo='US', gprop='')
            
            # Get interest over time data
            interest_over_time = pytrends.interest_over_time()
            
            if interest_over_time.empty:
                self.logger.warning(f"No trend data available for keywords: {trend_keywords}")
                return {
                    'direction': 'stable',
                    'confidence': 'low'
                }
            ...
            recent_avg = recent_period.mean().mean()  # Average across all keywords and time
            ...
            try:
                ...
                related_count = sum(len(queries.get('top', [])) if queries.get('top') is not None else 0 
            ...
            self.logger.info(f"Trend analysis complete: {direction} trend with {confidence} confidence")
            ...
            trending_terms = ['ai', 'automation', 'saas', 'digital', 'online', 'remote', 'machine learning', 'blockchain']
            has_trending = any(term in product_concept.lower() or 
    ...
    def _assess_monetization_potential(self, product_concept: str, search_data: Dict) -> float:
        ...
        # Product type impact
        high_value_terms = ['course', 'software', 'tool', 'platform', 'service', 'consulting']
        if any(term in product_concept.lower() for term in high_value_terms):
            score += 0.4
        
        return min(score, 1.0)
    
    def _identify_target_audience(self, product_concept: str, keywords: List[str]) -> str:
        ...
        elif any(term in all_text for term in ['developer', 'programming', 'code', 'tech']):
            return "Developers & Tech Professionals"
        elif any(term in all_text for term in ['student', 'learn', 'education', 'course']):
    ...
    async def _identify_content_gaps(self, keywords: List[str]) -> List[str]:
        ...
        try:
            if not requests or not BeautifulSoup:
                self.logger.error("Required libraries not available for content gap analysis")
            ...
            for keyword in keywords[:5]:  # Limit to 5 keywords for efficiency
                try:
                    ...
                    if 'advanced' not in keyword_lower and 'expert' not in keyword_lower:
                    ...
                    self.logger.warning(f"Error analyzing content gaps for keyword '{keyword}': {e}")
                    continue
    ...
    def _generate_product_recommendations(self, product_concept: str, search_data: Dict) -> List[str]:
        """Generate recommended products based on market analysis"""
        recommendations = []
        
        # Base recommendations on search volume and competition
        if search_data['total_volume'] > 5000 and search_data['competition_level'] in ['low', 'medium']:
    ...
    def _calculate_confidence_score(self, search_data: Dict, trend_data: Dict, monetization_score: float) -> float:
...
if __name__ == "__main__":
    async def test_research_tools():
        ...
        try:
            news_items = await news_watcher.monitor_feeds(duration_hours=0.1)  # 6 minutes
        ...
        try:
            ...
            opportunity = await validator.validate_product_idea(product_concept, keywords)
...
class SEOAuditService:
    ...
    async def _perform_seo_audit(self, website_url: str, request_data: Dict) -> SEOAuditResult:
        ...
        try:
            ...
            technical_seo = await self._analyze_technical_seo(website_url, soup, response)
            
            # On-Page SEO Analysis
            on_page_seo = await self._analyze_on_page_seo(soup, request_data.get('target_keywords', []))
            
            # Content Analysis
            content_analysis = await self._analyze_content(soup)
            
            # Competitor Analysis
            competitor_analysis = await self._analyze_competitors(request_data.get('industry', ''), request_data.get('target_keywords', []))
            
            # Generate recommendations
            recommendations, priority_issues, opportunities = self._generate_recommendations(
                technical_seo, on_page_seo, content_analysis, competitor_analysis
            )
            
            # Calculate overall score
            overall_score = self._calculate_overall_score(technical_seo, on_page_seo, content_analysis)
    ...
    async def _analyze_technical_seo(self, url: str, soup: BeautifulSoup, response) -> Dict[str, Any]:
        ...
        if title_tag and title_tag.text.strip():
            if 10 <= len(title_tag.text) <= 60:
                ...
                technical_seo['meta_tags']['issues'].append(f'Title tag length ({len(title_tag.text)} chars) not optimal (10-60 chars)')
        else:
        ...
        if meta_description and meta_description.get('content', '').strip():
            ...
            if 120 <= desc_length <= 160:
                ...
                technical_seo['meta_tags']['issues'].append(f'Meta description length ({desc_length} chars) not optimal (120-160 chars)')
        else:
        ...
        if viewport_meta:
            ...
            technical_seo['mobile_friendly']['issues'].append('Missing viewport meta tag')
        ...
        if json_ld:
            ...
            technical_seo['structured_data']['issues'].append('No structured data found')
        ...
        if response_time:
            ...
            if response_seconds < 2:
                ...
                technical_seo['page_speed']['issues'].append(f'Slow response time: {response_seconds:.2f}s')
        
        return technical_seo
    
    async def _analyze_on_page_seo(self, soup: BeautifulSoup, target_keywords: List[str]) -> Dict[str, Any]:
        ...
        if len(h1_tags) == 1:
            ...
            on_page_seo['heading_structure']['issues'].append(f'Multiple H1 tags found ({len(h1_tags)})')
        ...
        if len(headings) >= 3:
            ...
            on_page_seo['heading_structure']['issues'].append('Insufficient heading structure')
        ...
        if images:
            ...
            if alt_ratio < 0.8:
                on_page_seo['image_optimization']['issues'].append(f'{len(images) - len(images_with_alt)} images missing alt text')
        else:
        ...
        if target_keywords:
            ...
            keyword_found = any(keyword.lower() in page_text for keyword in target_keywords)
            if keyword_found:
                on_page_seo['keyword_optimization']['score'] = 70
            else:
                on_page_seo['keyword_optimization']['score'] = 20
                on_page_seo['keyword_optimization']['issues'].append('Target keywords not found in content')
    ...
    async def _analyze_content(self, soup: BeautifulSoup) -> Dict[str, Any]:
        ...
        if len(words) >= 300:
            ...
            content_analysis['content_structure']['issues'].append(f'Low word count: {len(words)} words')
            ...
            try:
                ...
                if sentences:
                    ...
                    if 15 <= avg_sentence_length <= 25:
                        ...
                        content_analysis['readability']['issues'].append(f'Average sentence length: {avg_sentence_length:.1f} words')
            except Exception as e:
    ...
    async def _analyze_competitors(self, industry: str, keywords: List[str]) -> Dict[str, Any]:
        ...
        if keywords:
            competitor_analysis['market_insights'].append(f'Target keywords: {", ".join(keywords[:5])}')
    ...
    def _generate_recommendations(self, technical_seo: Dict, on_page_seo: Dict, 
                                content_analysis: Dict, competitor_analysis: Dict) -> Tuple[List[str], List[str], List[str]]:
    ...
    def _calculate_overall_score(self, technical_seo: Dict, on_page_seo: Dict, content_analysis: Dict) -> float:
        ...
        content_score = min(100, (content_analysis['word_count'] / 500) * 100) if content_analysis['word_count'] > 0 else 0
    ...
    async def _generate_pdf_report(self, audit_result: SEOAuditResult, request_data: Dict) -> str:
        ...
        for category, data in audit_result.technical_seo.items():
            status = "Good" if data['score'] >= 70 else "Needs Improvement" if data['score'] >= 40 else "Critical"
    ...
    async def _send_audit_report_email(self, request_data: Dict, report_path: str):
        ...
        try:
            ...
            # Send email
            server = smtplib.SMTP(self.smtp_config.get('smtp_server', ''), self.smtp_config.get('smtp_port', 587))
            server.starttls()
    ...
    def _get_request_data(self, request_id: str) -> Optional[Dict]:
        ...
        try:
            cursor.execute(
                "SELECT * FROM seo_audit_requests WHERE request_id = ?",
                (request_id,)
            )
            row = cursor.fetchone()
    ...
    async def _save_audit_results(self, request_id: str, audit_result: SEOAuditResult, report_path: str):
