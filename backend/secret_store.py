...
class SecretStore:
    ...
    def __init__(self, db_path: str = "data/secrets.sqlite", master_password: Optional[str] = None):
        """
    ...
    def delete_secret(self, key_name: str) -> bool:
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.execute(
                    "DELETE FROM secrets WHERE key_name = ?",
                    (key_name,)
                )
                conn.commit()
    ...
    def secret_exists(self, key_name: str) -> bool:
        ...
        # Return False gracefully if in disabled mode
        if getattr(self, '_disabled', False):
            self.logger.debug(f"SecretStore disabled - returning False for secret existence: {key_name}")
            return False
            
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.execute(
                    "SELECT 1 FROM secrets WHERE key_name = ? LIMIT 1",
                    (key_name,)
                )
