...
class HypocrisyFinding:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "evidence_links": (
                json.dumps(self.evidence_links) if self.evidence_links else None
            ),
    ...
    def store_finding(self, finding: HypocrisyFinding) -> Optional[int]:
        ...
        try:
            ...
            if finding.contradiction_type not in valid_contradiction_types:
                ...
                self.logger.warning(
                    f"Invalid contradiction_type, defaulting to 'direct'"
                )
            ...
            if finding.verification_status not in valid_verification_statuses:
                ...
                self.logger.warning(
                    f"Invalid verification_status, defaulting to 'pending'"
                )
            ...
            with self.get_connection() as conn:
                ...
                if cursor.fetchone():
                    self.logger.warning(
                        f"Duplicate hypocrisy finding for {finding.subject_name}"
                    )
                    return None
            ...
            self.logger.error(
                f"Database constraint violation when storing hypocrisy finding: {e}"
            )
            return None
        except Exception as e:
    ...
    def get_content_opportunities(
        ...
        try:
            with self.get_connection() as conn:
                ...
                self.logger.info(
                    f"Retrieved {len(opportunities)} hypocrisy content opportunities"
                )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        try:
            with self.get_connection() as conn:
                ...
                # Content usage
                cursor.execute(
                    "SELECT COUNT(*) FROM hypocrisy_tracker WHERE content_used = TRUE"
                )
            ...
            with self.get_connection() as conn:
                ...
                for row in cursor.fetchall():
                    ...
                    if finding["evidence_links"]:
                        finding["evidence_links"] = json.loads(
                            finding["evidence_links"]
                        )
                    if finding["tags"]:
                        finding["tags"] = json.loads(finding["tags"])
                    if finding["fact_check_results"]:
                        finding["fact_check_results"] = json.loads(
                            finding["fact_check_results"]
                        )
                    findings.append(finding)

                self.logger.info(
                    f"Found {len(findings)} hypocrisy findings matching criteria"
                )
                return findings
