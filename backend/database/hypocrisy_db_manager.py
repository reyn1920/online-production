...
class HypocrisyFinding:
    ...
    def __post_init__(self):
        ...
        valid_contradiction_types = {'direct', 'contextual', 'temporal', 'value', 'policy_shift', 'audience_based'}
        ...
        valid_subject_types = {'person', 'organization', 'publication', 'politician', 'celebrity', 'influencer'}
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'fact_check_results': json.dumps(self.fact_check_results) if self.fact_check_results else None,
...
class HypocrisyDatabaseManager:
    ...
    def store_finding(self, finding: HypocrisyFinding) -> Optional[int]:
        ...
        try:
            ...
            with self.get_connection() as conn:
                ...
                self.logger.info(f"Stored hypocrisy finding {finding_id} for {finding.subject_name}")
                return finding_id
    ...
    def get_content_opportunities(self, limit: int = 10, min_confidence: float = 0.6) -> List[Dict[str, Any]]:
        ...
        try:
            with self.get_connection() as conn:
                ...
                for row in cursor.fetchall():
                    opportunity = {
                        'id': row['id'],
                        'subject_name': row['subject_name'],  # Fixed: use 'subject_name' instead of 'figure'
    ...
    def update_verification_status(self, finding_id: int, status: str, reviewed_by: str = None) -> bool:
        ...
        try:
            with self.get_connection() as conn:
                ...
                if cursor.rowcount > 0:
                    conn.commit()
                    self.logger.info(f"Updated verification status for finding {finding_id} to {status}")
                    return True
                else:
                    self.logger.warning(f"Hypocrisy finding {finding_id} not found")
                    return False
    ...
    def get_recent_findings(self, hours: int = 24) -> List[Dict[str, Any]]:
        ...
        try:
            with self.get_connection() as conn:
                ...
                self.logger.info(f"Found {len(findings)} recent hypocrisy findings from last {hours} hours")
                return findings
