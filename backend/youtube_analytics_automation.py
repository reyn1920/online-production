...
class YouTubeAnalyticsAutomation:
        ...
        try:
            ...
            for video_id in video_ids:
                try:
                    ...
                    analytics_data = await self.youtube_integration.get_video_analytics(video_id)
                    
                    if analytics_data:
                        metrics = self._process_video_analytics(video_id, analytics_data)
                    ...
                    self.logger.error(f"Error collecting metrics for video {video_id}: {e}")
                    continue
    ...
    def _process_video_analytics(self, video_id: str, analytics_data: Dict[str, Any]) -> VideoMetrics:
        ...
        try:
            ...
            metrics = VideoMetrics(
                ...
                published_at=datetime.fromisoformat(analytics_data.get('published_at', datetime.now().isoformat())),
                ...
                thumbnail_impressions=analytics_data.get('thumbnail_impressions', 0),
    ...
    async def collect_channel_metrics(self, channel_ids: List[str]) -> List[ChannelMetrics]:
        """Collect comprehensive channel-level metrics."""
        try:
            self.logger.info(f"Collecting channel metrics for {len(channel_ids)} channels")
            ...
            for channel_id in channel_ids:
                try:
                    ...
                    analytics_data = await self.youtube_integration.get_channel_analytics(channel_id)
                    ...
                    self.logger.error(f"Error collecting metrics for channel {channel_id}: {e}")
                    continue
    ...
    def _process_channel_analytics(self, channel_id: str, analytics_data: Dict[str, Any]) -> ChannelMetrics:
        ...
        try:
            metrics = ChannelMetrics(
                ...
                total_watch_time_minutes=analytics_data.get('total_watch_time_minutes', 0.0),
                total_videos=analytics_data.get('total_videos', 0),
                subscriber_growth_rate=analytics_data.get('subscriber_growth_rate', 0.0),
                average_views_per_video=analytics_data.get('average_views_per_video', 0.0),
                average_engagement_rate=analytics_data.get('average_engagement_rate', 0.0),
                total_revenue=analytics_data.get('total_revenue', 0.0),
                estimated_monthly_revenue=analytics_data.get('estimated_monthly_revenue', 0.0),
    ...
    async def generate_insights(self, channel_ids: List[str]) -> List[AnalyticsInsight]:
        ...
        try:
            ...
            for channel_id in channel_ids:
                ...
                content_insights = await self._generate_content_insights(channel_id, performance_data)
                audience_insights = await self._generate_audience_insights(channel_id, performance_data)
                optimization_insights = await self._generate_optimization_insights(channel_id, performance_data)
            ...
            filtered_insights = [i for i in insights if i.confidence_level >= min_confidence]
            
            # Limit number of insights
            max_insights = self.config['insights']['max_insights_per_run']
            filtered_insights = sorted(filtered_insights, key=lambda x: x.impact_score, reverse=True)[:max_insights]
    ...
    async def _generate_content_insights(self, channel_id: str, performance_data: Dict[str, Any]) -> List[AnalyticsInsight]:
    ...
    async def _generate_audience_insights(self, channel_id: str, performance_data: Dict[str, Any]) -> List[AnalyticsInsight]:
    ...
    async def _generate_optimization_insights(self, channel_id: str, performance_data: Dict[str, Any]) -> List[AnalyticsInsight]:
    ...
    def _analyze_title_patterns(self, top_videos: List[VideoMetrics], bottom_videos: List[VideoMetrics]) -> Optional[AnalyticsInsight]:
        ...
        try:
            ...
            if abs(top_avg_length - bottom_avg_length) > 10:  # Significant difference
                ...
                return AnalyticsInsight(
                    ...
                    description=f"Top performing videos have titles averaging {optimal_length} characters, while lower performing videos average {int(bottom_avg_length)} characters.",
                    impact_score=75.0,
    ...
    def _analyze_video_length_performance(self, video_metrics: List[VideoMetrics]) -> Optional[AnalyticsInsight]:
    ...
    async def monitor_performance_alerts(self, channel_ids: List[str]) -> List[PerformanceAlert]:
    ...
    async def _check_performance_drops(self, channel_id: str) -> List[PerformanceAlert]:
        ...
        try:
            ...
            historical_avg_views = statistics.mean([m.views for m in historical_metrics])
            ...
            if recent_avg_views < historical_avg_views * (1 - drop_threshold):
                drop_percentage = ((historical_avg_views - recent_avg_views) / historical_avg_views) * 100
    ...
    async def generate_performance_report(self, channel_id: str, time_frame: TimeFrame = TimeFrame.WEEK) -> Dict[str, Any]:
        ...
        try:
            ...
            # Collect metrics
            video_metrics = await self._get_video_metrics_by_date_range(channel_id, start_date, end_date)
            ...
            report = {
                ...
                'recommendations': self._generate_report_recommendations(video_metrics, insights, alerts),
    ...
    def _calculate_summary_statistics(self, video_metrics: List[VideoMetrics]) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_engagement_rate = statistics.mean([vm.engagement_rate for vm in video_metrics])
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                conn.execute("""
                    ...
                    (video_id, title, published_at, views, watch_time_minutes, likes, dislikes,
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    metrics.video_id, metrics.title, metrics.published_at.isoformat(),
                    metrics.views, metrics.watch_time_minutes, metrics.likes, metrics.dislikes,
                    metrics.comments, metrics.shares, metrics.subscribers_gained,
                    metrics.click_through_rate, metrics.average_view_duration,
                    json.dumps(metrics.audience_retention), metrics.impressions,
                    metrics.impression_ctr, metrics.revenue, metrics.rpm,
                    json.dumps(metrics.traffic_sources), json.dumps(metrics.demographics),
                    metrics.engagement_rate, metrics.thumbnail_impressions, metrics.thumbnail_ctr,
                    metrics.end_screen_clicks, metrics.card_clicks, metrics.playlist_additions,
    ...
    def get_analytics_status(self) -> Dict[str, Any]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor = conn.execute("SELECT COUNT(*) FROM analytics_insights WHERE expires_at > datetime('now')")
    ...
    async def _get_video_metrics_by_date_range(self, channel_id: str, start_date: datetime, end_date: datetime) -> List[VideoMetrics]:
    ...
    def _generate_report_recommendations(self, video_metrics: List[VideoMetrics], 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--insights", type=str, help="Generate insights for channel ID")
    ...
    if args.collect:
        ...
        print("Use --collect, --insights, --report, --monitor, or --status with channel ID")