...
class YouTubeSEOOptimizer:
    ...
    async def research_keywords(
        ...
        try:
            ...
            if cache_key in self.keyword_cache:
                ...
                if (datetime.now() - cached_data["timestamp"]).hours < self.config[
                    "keyword_research"
    ...
    async def optimize_video_seo(
        ...
        try:
            ...
            optimized_description = self._optimize_description(
                description, top_keywords, topic
            )
    ...
    def _optimize_title(self, title: str, keywords: List[str]) -> str:
        ...
        try:
            ...
            if len(title) <= max_length and any(
                kw.lower() in title.lower() for kw in keywords[:3]
            ):
                return title
    ...
    def _optimize_description(
        ...
        try:
            ...
            # Add hashtags
            hashtags = "\n\n" + " ".join(
                [f"#{kw.replace(' ', '')}" for kw in keywords[:3]]
            )
            optimized += hashtags
    ...
    def _calculate_seo_score(
        ...
        try:
            ...
            tag_keyword_count = sum(
                ...
                if any(kw.keyword.lower() in tag.lower() for tag in tags)
            )
            ...
            total_score = (
                title_score + desc_score + tag_score + keyword_score + readability_score
            )
...
if __name__ == "__main__":
    ...
    if args.research:
        ...
        for kw in keywords[:10]:
            print(
                f"{kw.keyword}: {kw.search_volume} searches, {kw.relevance_score:.2f} relevance"
            )
