...
class RelentlessOptimizer:
    ...
    async def create_thumbnail_test(self, video_id: str, thumbnail_variants: List[str], 
                                  hypothesis: str = "New thumbnail will improve CTR") -> str:
        ...
        test = OptimizationTest(
            ...
            secondary_metrics=[OptimizationMetric.VIEW_DURATION, OptimizationMetric.ENGAGEMENT_RATE],
    ...
    async def create_title_test(self, video_id: str, title_variants: List[str],
                              hypothesis: str = "Optimized title will improve CTR") -> str:
    ...
    async def analyze_audience_retention(self, video_id: str, 
                                       retention_data: List[float]) -> RetentionAnalysis:
        """Analyze audience retention and identify optimization opportunities"""
        try:
            # Calculate retention metrics
            average_retention = statistics.mean(retention_data) if retention_data else 0.0
            ...
            for i in range(1, len(retention_data)):
                ...
                if drop > 10.0:  # 10% drop threshold
                    timestamp = (i / len(retention_data)) * 100  # Convert to percentage of video
                    drop_off_points.append((timestamp, drop))
            
            # Identify engagement peaks (retention increases)
            engagement_peaks = []
            for i in range(1, len(retention_data)):
                if retention_data[i] > retention_data[i-1] + 5.0:  # 5% increase threshold
                    timestamp = (i / len(retention_data)) * 100
                    engagement_peaks.append((timestamp, "Engagement spike detected"))
            
            # Generate optimization suggestions
            suggestions = self._generate_retention_suggestions(retention_data, drop_off_points, engagement_peaks)
    ...
    def _generate_retention_suggestions(self, retention_data: List[float], 
                                      drop_off_points: List[Tuple[float, float]],
                                      engagement_peaks: List[Tuple[float, str]]) -> List[str]:
        ...
        if avg_retention < 30:
            suggestions.append("Overall retention is low - consider stronger hook in first 15 seconds")
        elif avg_retention < 50:
            suggestions.append("Moderate retention - optimize pacing and content structure")
        
        # Drop-off point suggestions
        if drop_off_points:
            early_drops = [dp for dp in drop_off_points if dp[0] < 25]
            if early_drops:
                suggestions.append("Significant early drop-offs detected - strengthen opening hook and preview")
            
            mid_drops = [dp for dp in drop_off_points if 25 <= dp[0] <= 75]
            if mid_drops:
                suggestions.append("Mid-video drop-offs detected - improve pacing and add engagement elements")
        
        # Engagement peak suggestions
        if engagement_peaks:
            suggestions.append(f"Found {len(engagement_peaks)} engagement peaks - analyze and replicate successful elements")
        else:
            suggestions.append("No engagement peaks detected - add more interactive or surprising elements")
        
        # End retention suggestions
        if retention_data and retention_data[-1] > 70:
            suggestions.append("Strong end retention - consider longer content or stronger call-to-action")
        elif retention_data and retention_data[-1] < 20:
            suggestions.append("Weak end retention - improve conclusion and call-to-action placement")
    ...
    async def _collect_test_metrics(self, test: OptimizationTest) -> None:
        ...
        try:
            # Simulate metric collection (in real implementation, this would call YouTube Analytics API)
            for variant in test.variants:
                ...
                if variant.impressions > 0:
                    variant.engagement_score = (variant.clicks / variant.impressions) * 100
    ...
    async def _check_statistical_significance(self, test: OptimizationTest) -> bool:
        ...
        try:
            ...
            ctr_a = variant_a.clicks / variant_a.impressions if variant_a.impressions > 0 else 0
            ctr_b = variant_b.clicks / variant_b.impressions if variant_b.impressions > 0 else 0
            
            # Simple significance test (in production, use proper statistical tests)
            difference = abs(ctr_a - ctr_b)
            relative_difference = difference / max(ctr_a, ctr_b) if max(ctr_a, ctr_b) > 0 else 0
            
            # Consider significant if >10% relative difference with sufficient sample size
            is_significant = (relative_difference > 0.1 and 
                            variant_a.impressions + variant_b.impressions >= test.min_sample_size)
            ...
            self.logger.error(f"Error checking significance for test {test.test_id}: {e}")
            return False
    ...
    async def _complete_test(self, test: OptimizationTest, force: bool = False) -> None:
        ...
        try:
            ...
            if len(variant_scores) >= 2:
                improvement = (max(variant_scores) - min(variant_scores)) / min(variant_scores) * 100
            ...
            self.logger.info(f"Test {test.test_id} {status}. Winner: {best_variant.name}")
            
        except Exception as e:
            self.logger.error(f"Error completing test {test.test_id}: {e}")
    
    async def _apply_winning_variant(self, test: OptimizationTest, winner: OptimizationVariant) -> None:
        ...
        try:
            ...
            self.logger.info(f"Applied winning variant for video {test.video_id}: {winner.name}")
    ...
    async def get_optimization_report(self, video_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                if history_rows:
                    improvements = [row[5] for row in history_rows if row[5] is not None]  # improvement_percentage column
                    if improvements:
...
class RelentlessOptimizer:
    ...
    def __init__(self, db_path: str = "right_perspective.db"):
        self.logger = logging.getLogger(__name__)
        self.logger.warning("Using fallback RelentlessOptimizer - some features may be limited")
...
if __name__ == "__main__":
    async def test_relentless_optimization():
        ...
        try:
            ...
            analysis = await optimizer.analyze_audience_retention("test_video_123", retention_data)
            print(f"Retention analysis completed. Average retention: {analysis.average_view_duration:.1f}%")
        ...
        try:
            ...
            print(f"Generated optimization report with {report.get('active_tests', 0)} active tests")
        except Exception as e:
