...
class ChannelIntelligenceFeeds:
    ...
    def _initialize_extractors(self) -> Dict[ChannelType, Dict[str, Any]]:
        ...
        return {
            ChannelType.TECH: {
                "keywords": ["technology", "software", "hardware", "AI", "machine learning", 
                           "programming", "coding", "development", "innovation", "startup"],
                "relevance_boost": ["breakthrough", "revolutionary", "cutting-edge", "beta", "launch"],
                "credibility_sources": ["techcrunch.com", "arstechnica.com", "wired.com", "theverge.com"]
            },
            ChannelType.WELLNESS: {
                "keywords": ["health", "wellness", "nutrition", "fitness", "mental health", 
                           "exercise", "diet", "meditation", "yoga", "supplements"],
                "relevance_boost": ["study shows", "research", "clinical trial", "proven", "effective"],
                "credibility_sources": ["healthline.com", "webmd.com", "mayoclinic.org", "nih.gov"]
            },
            ChannelType.FINANCE: {
                "keywords": ["finance", "investment", "stock", "market", "economy", "trading", 
                           "cryptocurrency", "bitcoin", "portfolio", "retirement"],
                "relevance_boost": ["earnings", "profit", "loss", "bull market", "bear market"],
                "credibility_sources": ["bloomberg.com", "reuters.com", "wsj.com", "marketwatch.com"]
            },
            ChannelType.POLITICAL: {
                "keywords": ["politics", "government", "election", "policy", "congress", "senate", 
                           "president", "conservative", "liberal", "democrat", "republican"],
                "relevance_boost": ["breaking", "scandal", "investigation", "vote", "bill passed"],
                "credibility_sources": ["breitbart.com", "dailywire.com", "townhall.com", "foxnews.com"]
            },
            ChannelType.BUSINESS: {
                "keywords": ["business", "entrepreneur", "startup", "company", "CEO", "revenue", 
                           "growth", "strategy", "marketing", "sales"],
                "relevance_boost": ["acquisition", "merger", "IPO", "funding", "expansion"],
                "credibility_sources": ["forbes.com", "businessinsider.com", "entrepreneur.com"]
            },
            ChannelType.SCIENCE: {
                "keywords": ["science", "research", "discovery", "experiment", "study", "theory", 
                           "physics", "chemistry", "biology", "space"],
                "relevance_boost": ["breakthrough", "discovery", "published", "peer-reviewed"],
                "credibility_sources": ["nature.com", "sciencemag.org", "newscientist.com"]
            }
        }
        ...
        for channel_id, config in channels.items():
            if config.rss_feeds:
                task = asyncio.create_task(
                    self.monitor_channel_feeds(channel_id)
                )
                tasks.append(task)
    ...
    async def monitor_channel_feeds(self, channel_id: str):
        ...
        for feed_url in config.rss_feeds:
            try:
                ...
                self.logger.error(f"Error processing feed {feed_url} for {channel_id}: {e}")
    ...
    async def process_feed(self, channel_id: str, feed_url: str):
        ...
        try:
            ...
            self.logger.info(f"Processed {processed_count} entries from {feed_url} for {channel_id}")
    ...
    def _calculate_credibility_score(self, item: FeedItem, feed_url: str, 
    ...
    async def _store_feed_item(self, channel_id: str, item: FeedItem):
        ...
        if entry_type == "study" and "research" in str(config.knowledge_base_tables):
            table_name = next((t for t in config.knowledge_base_tables if "research" in t or "study" in t), table_name)
    ...
    async def get_latest_intelligence(self, channel_id: str, limit: int = 10) -> List[Dict[str, Any]]:
        """Get latest intelligence for a channel"""
        return self.protocol.get_channel_knowledge_base(channel_id)[:limit]
    
    async def search_intelligence(self, channel_id: str, query: str, limit: int = 10) -> List[Dict[str, Any]]:
