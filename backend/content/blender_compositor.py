...
class ResourceMonitor:
    ...
    def check_resource_limits(self, memory_limit_gb: float = 8.0, cpu_limit: float = 90.0) -> bool:
...
class BlenderScriptGenerator:
    ...
    def _select_optimal_template(self, config: RenderConfig, layers: List[CompositeLayer]) -> str:
    ...
    def _generate_node_cache_key(self, layers: List[CompositeLayer], mode: CompositeMode) -> str:
        """Generate cache key for node configuration."""
        layer_hash = hashlib.md5(str([(l.name, l.type, l.blend_mode) for l in layers]).encode()).hexdigest()
    ...
    def _generate_layer_cache_key(self, layer: CompositeLayer, index: int) -> str:
        ...
        layer_data = f"{layer.name}_{layer.type}_{layer.blend_mode}_{layer.opacity}_{index}"
    ...
    def _generate_layer_nodes(self, layer: CompositeLayer, index: int, y_offset: int) -> List[str]:
        ...
        if layer.scale != (1.0, 1.0) or layer.position != (0.0, 0.0) or layer.rotation != 0.0:
            lines.extend([
                f"try:",
...
class BlenderCompositor:
    ...
    def __init__(self, blender_executable: Optional[str] = None, max_workers: int = 2):
        ...
        self.executor = ThreadPoolExecutor(
            ...
            thread_name_prefix="BlenderCompositor"
        )
    ...
    def create_composite_job(self, layers: List[CompositeLayer], 
                           output_path: str,
                           job_id: Optional[str] = None,
                           config: Optional[RenderConfig] = None,
                           mode: CompositeMode = CompositeMode.FULL_COMPOSITE) -> RenderJob:
        ...
        job = RenderJob(
            ...
            metadata={
                ...
                "estimated_frames": config.frame_end - config.frame_start + 1 if config else 250
            }
        )
    ...
    def _execute_blender_script(self, job: RenderJob, script_path: str) -> bool:
        ...
        try:
            ...
            if job.config.max_memory_gb > 0:
                cmd.extend(["--", "--memory", str(int(job.config.max_memory_gb * 1024))])
    ...
    def _monitor_render_progress(self, job: RenderJob, process: subprocess.Popen) -> None:
            ...
            while process.poll() is None:
                try:
                    ...
                    if Path(checkpoint_file).exists():
                        ...
                        current_frame = checkpoint.get('last_frame', job.config.frame_start)
    ...
    def _collect_rendered_frames(self, job: RenderJob) -> List[int]:
        ...
        for frame in range(job.config.frame_start, job.config.frame_end + 1):
            frame_file = output_dir / f"{base_name}_{frame:04d}.{job.config.output_format.lower()}"
    ...
    def resume_job(self, job_id: str) -> bool:
        ...
        if job.status not in ["failed", "paused"]:
            self.logger.warning(f"Job {job_id} is not in a resumable state: {job.status}")
            return False
    ...
    def create_avatar_composite(self, avatar_video: str, background_image: str,
                              output_path: str, effects: Optional[List[str]] = None) -> str:
            ...
            for i, effect in enumerate(effects):
                if Path(effect).exists():
                    layers.append(CompositeLayer(
                        name=f"effect_{i}",
                        type="image" if effect.lower().endswith(('.png', '.jpg', '.jpeg')) else "video",
                        source_path=effect,
                        blend_mode="SCREEN",
                        opacity=0.5
                    ))
        ...
        if self.process_job(job.job_id):
            ...
            raise RuntimeError(f"Failed to create avatar composite: {job.error_message}")
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'jobs': {
                ...
                'failed': failed_jobs,
                'total_processed': self.performance_monitor['jobs_completed'] + self.performance_monitor['jobs_failed']
            },
    ...
    def setup_distributed_rendering(self, nodes: List[str]) -> bool:
        ...
        self.logger.info(f"Distributed rendering: {available_nodes}/{len(nodes)} nodes available")
