...
class ChannelPersonas:
    ...
    def adapt_content_to_persona(
        ...
        adapted_content = self._apply_writing_style(
            adapted_content, persona.writing_style
        )

        # Apply tone modifications
        adapted_content = self._apply_tone_attributes(
        ...
        for sentence in sentences:
            ...
            # If too long, consider breaking
                ...
                for i, word in enumerate(words):
                    if (
                        word.lower()
                        in ["however", "therefore", "furthermore", "moreover"]
                        and i > 8
                    ):
                        first_part = " ".join(words[:i])
