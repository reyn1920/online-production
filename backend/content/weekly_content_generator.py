...
class WeeklyContentGeneratorForRightPerspective:
    ...
    def get_recent_hypocrisy_examples(
    ...
    def create_slow_news_special(self) -> Dict:
        ...
        evergreen_examples = self.get_evergreen_content(
            selected_theme.lower().replace(" ", "_"), 5
        )
        ...
        for i, example in enumerate(evergreen_examples):
            segments.append(
                ContentSegment(
                    ...
                    evidence_urls=(
                        [example["evidence_url"]] if example["evidence_url"] else []
                    ),
                    talking_points=[
                    ...
                    humor_angle=self.generate_humor_angle(
                        example["title"], "crowder_style"
                    ),
                )
            )
    ...
    def create_weekly_content_plan(self) -> Dict:
        ...
        weekly_plan = {
            ...
            "content_strategy": (
                "recent_news" if len(recent_examples) >= 3 else "evergreen_focus"
            ),
            "episodes": [],
        }
    ...
    def save_content_plan(self, content_plan: Dict) -> int:
        ...
        logger.info(
            f"Saved weekly content plan {plan_id} for week of {content_plan['week_of']}"
        )
        return plan_id
