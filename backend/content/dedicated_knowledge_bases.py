...
class DedicatedKnowledgeBases:
    ...
    def add_knowledge_entry(self, channel_id: str, table_name: str, entry_data: Dict[str, Any]) -> str:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                entry_id, channel_id, table_name,
                ...
                json.dumps({k: v for k, v in entry_data.items() if k not in ['title', 'content', 'source']}),
    ...
    def get_knowledge_entries(self, channel_id: str, table_name: str = None, 
    ...
    def search_knowledge_base(self, channel_id: str, search_term: str, 
                            table_name: str = None, min_credibility: float = 0.0) -> List[Dict[str, Any]]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            params = [
                f"%{search_term}%", f"%{search_term}%", f"%{search_term}%",  # For relevance scoring
                channel_id, min_credibility,
                f"%{search_term}%", f"%{search_term}%", f"%{search_term}%"   # For filtering
            ]
    ...
    def get_evidence_for_topic(self, channel_id: str, topic: str, evidence_types: List[str] = None) -> Dict[str, List[Dict[str, Any]]]:
    ...
    def track_knowledge_usage(self, entry_id: str, channel_id: str, content_id: str = None, 
                            usage_context: str = None, effectiveness_score: float = None):
        """Track usage of knowledge entries for optimization"""
        usage_id = f"usage_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{hash(entry_id) % 10000}"
        
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO knowledge_usage
                (usage_id, entry_id, channel_id, content_id, usage_context, effectiveness_score, used_at)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (
                usage_id, entry_id, channel_id, content_id, usage_context, 
    ...
    def get_recommended_evidence(self, channel_id: str, content_topic: str, 
