    ...
    def _get_live_script_briefing(self, topic: str, genre: str) -> str:
        ...
        try:
            ...
            if hasattr(self.research_agent, 'get_hypocrisy_content_opportunities'):
                hypocrisy_opportunities = self.research_agent.get_hypocrisy_content_opportunities(topic)
...
"""
            ...
            live_briefing += f"• Use trending topics to add relevance and timeliness\n"
            live_briefing += f"• Leverage recent headlines for authentic dialogue and scenarios\n"
    ...
    def create_logline(self, topic: str, genre: str = "Drama", 
                      duration: int = 10, audience: str = "General", live_briefing: str = "") -> str:
        ...
        briefing_context = ""
        if live_briefing:
            briefing_context = f"\n\nCURRENT EVENTS CONTEXT:\n{live_briefing}\n\nUse this current events context to make the logline more timely and relevant."
    ...
    def create_synopsis(self, logline: str, duration: int = 10, live_briefing: str = "") -> str:
        ...
        briefing_context = ""
        if live_briefing:
            briefing_context = f"\n\nCURRENT EVENTS CONTEXT:\n{live_briefing}\n\nIncorporate relevant current events to make the synopsis more engaging and timely."
    ...
    def _parse_characters(self, character_text: str) -> List[Character]:
        ...
        if not characters:
            default_characters = [
                Character(
                    name="Alex Rivera",
                    description="A determined coffee shop owner who believes in creating community through quality beverages and genuine connections.",
                    personality="Passionate, detail-oriented, empathetic, with a strong work ethic and natural leadership qualities",
                    ...
                    motivation="To build a sustainable business that serves as a community hub and provides ethically sourced coffee",
                    arc="Learns to balance business success with personal fulfillment while building meaningful relationships"
                ),
                Character(
                    name="Jordan Chen",
                    description="A talented barista and aspiring artist who finds inspiration in the daily interactions with customers.",
                    personality="Creative, observant, introverted but warm, with a keen eye for detail and artistic expression",
                    background="Art school graduate working part-time while developing their portfolio and artistic career",
                    motivation="To support their artistic pursuits while contributing to a positive workplace environment",
                    arc="Gains confidence in their artistic abilities and learns to share their creativity with others"
                )
            ]
    ...
    def _extract_personality_traits(self, description: str) -> str:
        ...
        if any(word in description_lower for word in ['confident', 'bold', 'assertive']):
            traits.append('confident')
        if any(word in description_lower for word in ['kind', 'caring', 'compassionate']):
            traits.append('empathetic')
        if any(word in description_lower for word in ['creative', 'artistic', 'imaginative']):
            traits.append('creative')
        if any(word in description_lower for word in ['determined', 'persistent', 'driven']):
            traits.append('determined')
        if any(word in description_lower for word in ['funny', 'humorous', 'witty']):
            traits.append('humorous')
        
        if traits:
            return f"Characterized by being {', '.join(traits)}, with depth and authenticity that drives character development"
        else:
            return "Multi-dimensional with realistic strengths and flaws that create compelling character dynamics"
    ...
    def _generate_character_background(self, name: str, description: str) -> str:
        ...
        if any(word in description_lower for word in ['coffee', 'barista', 'shop']):
            return "Has experience in hospitality and customer service, with a passion for coffee culture and community building"
        elif any(word in description_lower for word in ['office', 'business', 'corporate']):
            return "Professional background in business with experience in corporate environments and team management"
        elif any(word in description_lower for word in ['artist', 'creative', 'design']):
            return "Educational background in the arts with experience in creative industries and artistic expression"
        else:
            return "Diverse life experiences that inform their worldview and approach to relationships and challenges"
    ...
    def _extract_character_motivation(self, description: str) -> str:
        ...
        if any(word in description_lower for word in ['success', 'achieve', 'goal']):
            return "Driven by the desire to achieve meaningful success while maintaining personal values and relationships"
        elif any(word in description_lower for word in ['help', 'support', 'community']):
            return "Motivated by the opportunity to make a positive impact on others and contribute to community well-being"
        elif any(word in description_lower for word in ['create', 'build', 'develop']):
            return "Inspired by the creative process and the satisfaction of building something meaningful and lasting"
        else:
            return "Seeks personal growth and authentic connections while navigating life's challenges and opportunities"
    ...
    def _generate_character_arc(self, name: str, personality: str) -> str:
        ...
        if 'confident' in personality.lower():
            ...
            return "Develops the courage to share their creative vision while learning to collaborate and accept constructive feedback"
        elif 'determined' in personality.lower():
            return "Discovers that true success includes personal fulfillment and meaningful relationships, not just achievement"
        else:
            return "Undergoes meaningful personal growth through challenges that reveal inner strength and authentic character"
    
    def create_scene_breakdown(self, synopsis: str, characters: List[Character], 
                             duration: int = 10) -> List[Scene]:
    ...
    def _create_default_scenes(self, duration: int) -> List[Scene]:
        ...
        scene_templates = [
            ...
            },
            {
                ...
                "time": "AFTERNOON",
                "description": "A modern office space with glass walls and contemporary furniture. Employees collaborate in an open workspace.",
        ...
        for i in range(num_scenes):
            ...
            scene = Scene(
                ...
                dialogue=[
                    {"character": template["characters"][0], "line": "This scene establishes the setting and mood."},
                    {"character": template["characters"][-1], "line": "The story unfolds naturally from here."}
                ],
    ...
    def _parse_scenes(self, scene_text: str, character_names: List[str], 
                     total_duration: int) -> List[Scene]:
        ...
        for i, section in enumerate(scene_sections[1:], 1):  # Skip first empty section
            ...
            time_of_day = "DAY"
            description = "A scene unfolds with careful attention to detail and atmosphere."
            extracted_actions = []
            
            for line in lines:
                line_lower = line.lower()
                if "location" in line_lower or "setting" in line_lower or "int." in line_lower or "ext." in line_lower:
                    location = line.split(':')[-1].strip() if ':' in line else line.strip()
                    if not location.startswith(('INT.', 'EXT.')):
                        location = f"INT. {location.upper()}"
                elif "time" in line_lower or "morning" in line_lower or "afternoon" in line_lower or "evening" in line_lower or "night" in line_lower:
                    time_of_day = line.split(':')[-1].strip() if ':' in line else self._extract_time_of_day(line)
                elif len(line) > 30 and not any(keyword in line_lower for keyword in ['scene', 'location', 'time', 'character']):
                    if not description or description == "A scene unfolds with careful attention to detail and atmosphere.":
                        description = line[:200]  # Use first substantial line as description
                elif len(line) > 15 and any(action_word in line_lower for action_word in ['walks', 'enters', 'sits', 'stands', 'looks', 'moves', 'opens', 'closes']):
            ...
            scene = Scene(
                ...
                duration_estimate=total_duration * 60 // max(len(scene_sections[1:]), 3)
            )
            scenes.append(scene)
        ...
        while len(scenes) < 3:
            ...
            if scene_num <= len(default_scenes):
                ...
                scenes.append(Scene(
                    ...
                    time_of_day="DAY",
                    characters=character_names[:1] if character_names else ["PROTAGONIST"],
                    description="The story continues with natural progression and development.",
                    dialogue=[
                        {"character": character_names[0] if character_names else "PROTAGONIST", 
...
"""
    ...
    def _parse_script_text(self, script_text: str) -> Tuple[List[Dict[str, str]], List[str]]:
        ...
        if not dialogue:
            ...
            if any(word in words for word in ['coffee', 'shop', 'cafe']):
                dialogue.append({"character": "BARISTA", "line": "Welcome to our coffee shop! What can I get you today?"})
                dialogue.append({"character": "CUSTOMER", "line": "I'll have a cappuccino, please."})
            elif any(word in words for word in ['office', 'work', 'meeting']):
                dialogue.append({"character": "MANAGER", "line": "Let's discuss the quarterly results in today's meeting."})
                dialogue.append({"character": "EMPLOYEE", "line": "I've prepared the presentation slides."})
            else:
                dialogue.append({"character": "PROTAGONIST", "line": "This is where our story begins."})
    ...
    def generate_full_script(self, topic: str, genre: str = "Drama", 
                           duration: int = 10, audience: str = "General") -> Script:
        ...
        try:
            ...
            logline = self.create_logline(topic, genre, duration, audience, live_briefing)
