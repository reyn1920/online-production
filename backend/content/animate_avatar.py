...
class LinlyTalkerEngine:
        ...
        try:
            ...
            if process.returncode == 0:
                ...
                if self._post_process_video(output_path, config):
                    self.logger.info("Linly-Talker generation and post-processing completed successfully")
                else:
                    self.logger.warning("Linly-Talker generation completed but post-processing failed")
                return True
            else:
    ...
    def _build_advanced_linly_command(self, source_image: str, audio_file: str, 
        ...
        if config.quality == AnimationQuality.HIGH:
            ...
            cmd.extend(["--enhancer", "RestoreFormer", "--background_enhancer", "realesrgan", "--upscale", "2"])
            if not self.gpu_available:
                cmd.append("--cpu")
...
class CacheManager:
    ...
    def _generate_cache_key(self, source_image: str, audio_file: str, config: AnimationConfig) -> str:
    ...
    def get_cached_result(self, source_image: str, audio_file: str, config: AnimationConfig) -> Optional[str]:
    ...
    def cache_result(self, source_image: str, audio_file: str, config: AnimationConfig, result_path: str) -> bool:
...
class AnimateAvatar:
        ...
        try:
            ...
            if success:
                ...
                if quality_score < job.config.quality_threshold:
                    self.logger.warning(f"Quality below threshold ({quality_score:.2f} < {job.config.quality_threshold:.2f}) for job {job_id}")
                ...
                self.logger.info(f"Animation job completed: {job_id} (quality: {quality_score:.2f})")
                return True
            else:
                job.status = "failed"
    ...
    def submit_job_async(self, source_image: str, audio_file: str, 
                        output_path: str, job_id: Optional[str] = None,
    ...
    def _preprocess_image_with_emotion(self, image_path: str, config: AnimationConfig) -> str:
        ...
        try:
            ...
            processed_path = self.temp_dir / f"processed_emotion_{config.emotion.value}_{Path(image_path).name}"
