    ...
    def _validate_inputs(self, source_image: str, audio_file: str) -> bool:
        ...
        try:
            ...
            if width < 256 or height < 256:
                self.logger.error(
                    f"Image too small (minimum 256x256): {width}x{height}"
                )
                return False
        ...
        try:
            ...
            if config.stabilize_video:
                ...
                try:
                    ...
                    self.logger.warning(
                        "Additional stabilization failed, keeping original"
                    )
...
class AnimateAvatar:
    ...
    def __init__(self, config: Optional[AnimationConfig] = None):
        ...
        if self.config.enable_caching:
            ...
            self.cache_manager = CacheManager(
                str(cache_dir), self.config.max_cache_size
            )
        else:
    ...
    def process_job(self, job_id: str) -> bool:
        ...
        try:
            ...
            if (
                ...
                processed_image = self._preprocess_image_with_emotion(
                    job.source_image, job.config
                )
            else:
    ...
    def _preprocess_image_with_emotion(
        ...
        try:
            ...
            processed_path = (
                self.temp_dir
                / f"processed_emotion_{config.emotion.value}_{Path(image_path).name}"
            )
