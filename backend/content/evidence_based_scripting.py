...
class EvidenceBasedScripting:
    ...
    verifiable facts, quotes, and counter-evidence from the right_perspective.db database.
    """
    ...
    def __init__(self, db_path: str = "./right_perspective.db"):
        ...
        self.logger.info("Evidence-Based Scripting initialized for The Right Perspective (PROTECTED)")
    ...
    async def generate_evidence_based_script(self, 
                                           topic: str, 
                                           script_template: str,
        ...
        try:
            ...
            if not evidence_data['facts'] and not evidence_data['quotes'] and not evidence_data['statistics']:
            ...
            validation_result = self._validate_evidence_integration(enhanced_script, evidence_data)
    ...
    async def _extract_topic_evidence(self, topic: str) -> Dict[str, List[Dict[str, Any]]]:
        """Extract all relevant evidence for a given topic from the database."""
        try:
            conn = sqlite3.connect(str(self.db_path))
            cursor = conn.cursor()
            
            # Search for evidence related to the topic (case-insensitive, partial matches)
            ...
            self.logger.info(f"Extracted evidence: {len(evidence_data['facts'])} facts, "
    ...
    def _remove_duplicate_evidence(self, evidence_data: Dict[str, List[Dict[str, Any]]]) -> Dict[str, List[Dict[str, Any]]]:
    ...
    def _identify_evidence_insertion_points(self, script_template: str) -> List[Dict[str, Any]]:
        ...
        patterns = [
            ...
            (r'\b(statistics show|data reveals|numbers indicate)\b', 'statistic_needed'),
    ...
    async def _integrate_evidence_into_script(self, 
                                            script_template: str,
                ...
                if insertion_point['line_number'] == i:
                    ...
                    if 'fact' in evidence_type and evidence_data['facts']:
                        ...
                        if evidence_used['quotes'] < len(evidence_data['quotes']):
                            ...
                            enhanced_lines.append(f"\n[QUOTE: \"{quote['content']}\"]")
                            if quote['source_url']:
                                enhanced_lines.append(f"[SOURCE: {quote['source_url']}]")
                            evidence_used['quotes'] += 1
                    
                    elif 'statistic' in evidence_type and evidence_data['statistics']:
                        if evidence_used['statistics'] < len(evidence_data['statistics']):
                            stat = evidence_data['statistics'][evidence_used['statistics']]
    ...
    def _validate_evidence_integration(self, enhanced_script: str, evidence_data: Dict[str, List[Dict[str, Any]]]) -> Dict[str, Any]:
        ...
        markers_found = sum(1 for marker in evidence_markers if marker in enhanced_script)
        
        if markers_found == 0:
            validation_result['valid'] = False
            validation_result['issues'].append('No evidence markers found in enhanced script')
        
        # Calculate evidence integration score
        total_evidence = len(evidence_data['facts']) + len(evidence_data['quotes']) + len(evidence_data['statistics'])
        if total_evidence > 0:
            validation_result['evidence_integration_score'] = min(markers_found / total_evidence, 1.0)
        
        # Check for minimum evidence requirements
        if validation_result['evidence_integration_score'] < 0.3:  # At least 30% of evidence should be integrated
            validation_result['valid'] = False
            validation_result['issues'].append('Insufficient evidence integration (minimum 30% required)')
        
        return validation_result
    
    def _generate_evidence_report(self, evidence_data: Dict[str, List[Dict[str, Any]]], topic: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'evidence_summary': {
                ...
                'total_evidence_items': len(evidence_data['facts']) + len(evidence_data['quotes']) + len(evidence_data['statistics'])
            },
            'source_breakdown': self._analyze_evidence_sources(evidence_data),
            'evidence_quality_score': self._calculate_evidence_quality_score(evidence_data),
            'recommendations': self._generate_evidence_recommendations(evidence_data),
            'generated_at': datetime.now().isoformat()
        }
    
    def _analyze_evidence_sources(self, evidence_data: Dict[str, List[Dict[str, Any]]]) -> Dict[str, Any]:
    ...
    def _calculate_evidence_quality_score(self, evidence_data: Dict[str, List[Dict[str, Any]]]) -> float:
    ...
    def _generate_evidence_recommendations(self, evidence_data: Dict[str, List[Dict[str, Any]]]) -> List[str]:
        ...
        if total_evidence < 3:
            recommendations.append("Consider adding more evidence to strengthen the script")
        ...
        if len(evidence_data['statistics']) == 0:
            recommendations.append("Including statistics would provide stronger factual support")
        ...
        if items_without_sources > 0:
            recommendations.append(f"{items_without_sources} evidence items lack source URLs")
        
        return recommendations
    
    async def add_evidence_to_database(self, 
                                     topic: str, 
                                     content: str, 
                                     source_url: Optional[str] = None,
                                     content_type: str = 'fact') -> bool:
        ...
        try:
            ...
            cursor.execute("""
                INSERT INTO evidence (topic, content, source_url, content_type, date_added)
            ...
            self.logger.info(f"Added evidence to database: {content_type} for topic '{topic}'")
            return True
