...
class ComplianceMonitor:
        ...
        while self.monitoring_active:
            ...
            for rule_id, rule in self.rules.items():
                ...
                if (rule.last_check is None or 
                    (current_time - rule.last_check).total_seconds() >= rule.monitoring_interval):
                    ...
                    try:
                        ...
                        audit_logger.log_security_event(
                            ...
                            severity=AuditLevel.INFO if rule.status == ComplianceStatus.COMPLIANT else AuditLevel.WARNING,
                            additional_data={
                                'rule_id': rule_id,
    ...
    async def check_rate_limiting(self) -> Dict[str, Any]:
        ...
        if self.performance_metrics['rate_limit_hits'] > self.config['alert_thresholds']['rate_limit_violations']:
            violations.append({
    ...
    async def check_error_handling(self) -> Dict[str, Any]:
        ...
        if total_requests > 0:
            error_rate = self.performance_metrics['error_rates'].get('5xx', 0) / total_requests
    ...
    async def check_webhook_security(self) -> Dict[str, Any]:
        ...
        if self.performance_metrics['webhook_failures'] > self.config['alert_thresholds']['webhook_failures']:
            violations.append({
    ...
    async def check_performance_requirements(self) -> Dict[str, Any]:
        ...
        if self.performance_metrics['api_response_times']:
            p95_response_time = sorted(self.performance_metrics['api_response_times'])[int(0.95 * len(self.performance_metrics['api_response_times']))]
    ...
    def _handle_violations(self, rule: ComplianceRule, violations: List[Dict[str, Any]]):
        """Handle compliance violations"""
        for violation in violations:
            # Log violation
            audit_logger.log_security_event(
                event_description=f"Compliance violation: {rule.title}",
                severity=AuditLevel.WARNING if rule.severity != 'critical' else AuditLevel.ERROR,
                additional_data={
    ...
    def _attempt_auto_remediation(self, rule: ComplianceRule, violation: Dict[str, Any]):
    ...
    async def _generate_periodic_reports(self):
        ...
        while self.monitoring_active:
            ...
            # Save report
            report_file = Path(f"/var/log/trae/compliance/report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
    ...
    def generate_compliance_report(self) -> Dict[str, Any]:
        ...
        compliant_rules = sum(1 for rule in self.rules.values() if rule.status == ComplianceStatus.COMPLIANT)
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        non_compliant_rules = [rule for rule in self.rules.values() if rule.status != ComplianceStatus.COMPLIANT]
    ...
    def stop_monitoring(self):
        ...
        audit_logger.log_security_event(
            event_description="Compliance monitoring stopped",
            severity=AuditLevel.INFO
        )
