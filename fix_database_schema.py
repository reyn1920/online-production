...
def fix_database_schema():
    ...
    if os.path.exists(main_db):
        ...
        try:
            ...
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='affiliate_programs'")
            if cursor.fetchone():
                ...
                if 'category' not in columns:
                    print("  ➕ Adding 'category' column to affiliate_programs table...")
                    cursor.execute("ALTER TABLE affiliate_programs ADD COLUMN category TEXT DEFAULT 'General'")
            ...
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='api_registry'")
            if cursor.fetchone():
                ...
                if 'name' not in columns:
                    ...
                    cursor.execute("UPDATE api_registry SET name = api_name WHERE name IS NULL")
            ...
            if cursor.fetchone():
                ...
                if 'name' not in columns:
                    ...
                    if 'service_name' in columns:
                        cursor.execute("UPDATE api_registry SET name = service_name WHERE name IS NULL")
