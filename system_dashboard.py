        ...
        <div class="bg - white rounded - lg shadow - md p - 6 mb - 6">
            <div class="flex items - center justify - between">
                <div>
                    ...
                    <p class="text - gray - 600 mt - 2">Monitor and manage your TRAE.AI application</p>
                </div>
    ...
    <script>
        function dashboard() {
            return {
                ...
                init() {
                    this.refreshData();
                    setInterval(() => this.refreshData(), 5000); // Auto - refresh every 5 seconds
                },

                async refreshData() {
                    try {
                        const [systemResponse, serviceResponse, processesResponse, logsResponse] = await Promise.all([
                            fetch('/api / system'),
                                fetch('/api / service'),
                                fetch('/api / processes'),
                                fetch('/api / logs')
