...
class RevenueConfig:
    ...
    def __init__(self):
        ...
        # Alert thresholds
        self.revenue_alert_threshold = float(os.getenv('REVENUE_ALERT_THRESHOLD', '1000.0'))
        self.growth_alert_threshold = float(os.getenv('GROWTH_ALERT_THRESHOLD', '0.1'))  # 10%
...
class RevenueForecaster:
    ...
    async def generate_forecast(self, historical_data: pd.DataFrame, days_ahead: int = 30, model: str = "linear") -> Dict[str, Any]:
    ...
    async def _linear_forecast(self, data: pd.DataFrame, days_ahead: int) -> Dict[str, Any]:
        ...
        forecast_dates = [data['date'].max() + timedelta(days=i) for i in range(1, days_ahead + 1)]
    ...
    async def _prophet_forecast(self, data: pd.DataFrame, days_ahead: int) -> Dict[str, Any]:
    ...
    async def _random_forest_forecast(self, data: pd.DataFrame, days_ahead: int) -> Dict[str, Any]:
        ...
        for i in range(days_ahead):
            ...
            features = [
                ...
                data['amount'].iloc[-7] if len(data) >= 7 else last_row['amount'],  # lag_7
                data['amount'].iloc[-14] if len(data) >= 14 else last_row['amount']  # lag_14
            ]
    ...
    def _generate_simple_forecast(self, data: pd.DataFrame, days_ahead: int) -> Dict[str, Any]:
        ...
        forecast_dates = [datetime.now() + timedelta(days=i) for i in range(1, days_ahead + 1)]
...
class RevenueTracker:
    ...
    def setup_routes(self):
        ...
        async def record_revenue(request: RevenueStreamRequest):
            ...
            try:
                logger.info(f"💰 Recording revenue: ${request.amount} from {request.source}")
        ...
        async def get_analytics(
            ...
            try:
                ...
                return {
                    ...
                    "source_breakdown": {k: round(v, 2) for k, v in source_breakdown.items()}
                }
        ...
        async def generate_forecast(request: ForecastRequest):
            ...
            try:
                logger.info(f"🔮 Generating {request.days_ahead}-day forecast using {request.model}")
                ...
                for forecast_point in forecast_result['forecast']:
                    forecast_record = RevenueForecast(
                        ...
                        predicted_amount=Decimal(str(forecast_point['predicted_amount'])),
                        confidence_lower=Decimal(str(forecast_point.get('confidence_lower', 0))),
                        confidence_upper=Decimal(str(forecast_point.get('confidence_upper', 0))),
                        model_used=forecast_result['model'],
                        source_filter=','.join(request.sources) if request.sources else None
                    )
        ...
        async def create_revenue_goal(request: RevenueGoalRequest):
            ...
            try:
                logger.info(f"🎯 Creating revenue goal: {request.name} - ${request.target_amount}")
        ...
        async def get_goal_progress():
            ...
            try:
                goals = self.db_session.query(RevenueGoal).filter(
                    RevenueGoal.status == 'active'
                ).all()
                ...
                return {
                    ...
                    "completed_goals": len([g for g in goal_progress if g["status"] == "completed"])
                }
        ...
        async def get_alerts():
            ...
            try:
                alerts = self.db_session.query(RevenueAlert).filter(
                    RevenueAlert.acknowledged == False
                ).order_by(RevenueAlert.triggered_at.desc()).limit(50).all()
    ...
    async def collect_all_revenue_data(self):
        ...
        try:
            ...
            youtube_data = await self.youtube_analytics.get_revenue_data(start_date, end_date)
            for data_point in youtube_data:
                if data_point['revenue'] > 0:
                    revenue_stream = RevenueStream(
                        ...
                        metadata=json.dumps({
                            'monetized_playbacks': data_point['monetized_playbacks'],
                            'cpm': data_point['cpm']
                        })
                    )
                    
                    # Check if already exists
                    existing = self.db_session.query(RevenueStream).filter(
            ...
            gumroad_data = await self.gumroad_analytics.get_sales_data(start_date, end_date)
            for sale in gumroad_data:
                ...
                # Check if already exists
                existing = self.db_session.query(RevenueStream).filter(
                    RevenueStream.transaction_id == sale['transaction_id']
                ).first()
            ...
            affiliate_data = await self.affiliate_tracker.get_affiliate_data(start_date, end_date)
    ...
    async def check_revenue_alerts(self, revenue_stream: RevenueStream):
        ...
        try:
            ...
            daily_revenue = self.db_session.query(RevenueStream).filter(
    ...
    async def generate_daily_report(self):
        ...
        try:
            ...
            report = {
                ...
                'total_revenue': round(total_revenue, 2),
                'transaction_count': transaction_count,
                'average_transaction': round(total_revenue / transaction_count if transaction_count > 0 else 0, 2),
                'source_breakdown': {k: round(v, 2) for k, v in source_breakdown.items()}
            }
            
            # Save report
            report_path = self.config.reports_dir / f"daily_report_{yesterday.isoformat()}.json"
            with open(report_path, 'w') as f:
                json.dump(report, f, indent=2)
            
            logger.info(f"✅ Daily report generated: ${total_revenue:.2f} from {transaction_count} transactions")
