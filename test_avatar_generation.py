...
async def test_avatar_generation():
    ...
    for engine_name, engine in manager.engines.items():
        ...
        try:
            info = engine.get_engine_info() if hasattr(engine, 'get_engine_info') else "No info method"
...
def main():
    ...
    if models_path.exists():
        checkpoint_files = list(models_path.glob("*.pth")) + list(models_path.glob("*.safetensors"))
