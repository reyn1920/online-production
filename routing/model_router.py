...
class ModelRouter:
    ...
    def route_request(
        ...
        self.logger.info(
            f"🎯 Routing request: task={task_type.value}, complexity={complexity:.2f}"
        )
        ...
        result = self._create_routing_result(
            best_model, messages, task_type, "Optimal selection"
        )
    ...
    def _score_candidates(
        ...
        for model_name in candidates:
            ...
            score = self._calculate_model_score(
                model, task_type, complexity, preferences
            )
    ...
    def record_request_outcome(
        ...
        if quality_rating is not None:
            perf["avg_quality"] = (1 - alpha) * perf[
                "avg_quality"
            ] + alpha * quality_rating
    ...
    def get_routing_analytics(self) -> Dict:
        ...
        # Performance metrics
        avg_routing_time = (
            sum(d["routing_time"] for d in self.routing_history) / total_decisions
        )
        avg_complexity = (
            sum(d["complexity"] for d in self.routing_history) / total_decisions
        )
