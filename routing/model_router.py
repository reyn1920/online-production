...
class ModelRouter:
    ...
    def _initialize_models(self) -> Dict[str, ModelCapability]:
        ...
        models["route-llm"] = ModelCapability(
            ...
            max_tokens=4096,
            strengths=[TaskType.GENERAL_CHAT, TaskType.SUMMARIZATION, TaskType.QUESTION_ANSWERING],
    ...
    def _load_task_patterns(self) -> Dict[TaskType, List[str]]:
        ...
        return {
            TaskType.CODE_GENERATION: [
                "write code", "function", "class", "algorithm", "debug", "programming",
                "python", "javascript", "java", "c++", "sql", "html", "css", "react"
            ],
            TaskType.MATH_REASONING: [
                "calculate", "solve", "equation", "mathematics", "algebra", "geometry",
                "statistics", "probability", "integral", "derivative", "theorem"
            ],
            TaskType.CREATIVE_WRITING: [
                "story", "poem", "creative", "narrative", "character", "plot",
                "write a", "compose", "imagine", "fiction", "novel"
            ],
            TaskType.ANALYSIS: [
            ...
            TaskType.SUMMARIZATION: [
                ...
                "main ideas", "tldr", "condensed", "abstract"
            ],
            TaskType.TRANSLATION: [
                "translate", "translation", "language", "french", "spanish",
                "german", "chinese", "japanese", "convert to"
            ],
            TaskType.QUESTION_ANSWERING: [
                "what is", "how to", "why", "when", "where", "who", "explain",
                "definition", "meaning", "help me understand"
            ]
        }
    ...
    def estimate_complexity(self, messages: List[Dict]) -> float:
        ...
        complexity_indicators = [
            ...
            any(word in content.lower() for word in ['complex', 'detailed', 'comprehensive', 'thorough']),
            content.count('?') > 2,  # Multiple questions
            any(word in content.lower() for word in ['analyze', 'compare', 'evaluate', 'explain in detail'])
        ]
    ...
    def route_request(self, messages: List[Dict], preferences: Dict = None, constraints: Dict = None) -> Dict:
        """
        ...
        self.logger.info(f"🎯 Routing request: task={task_type.value}, complexity={complexity:.2f}")
        ...
        if not candidates:
            ...
            fallback_model = self.config.get('model_routing', {}).get('fallback_model', 'route-llm')
            return self._create_routing_result(fallback_model, messages, task_type, "No suitable candidates")
        
        # Score and rank candidates
        scored_candidates = self._score_candidates(candidates, task_type, complexity, preferences, constraints)
    ...
    def _score_candidates(self, candidates: List[str], task_type: TaskType, complexity: float, 
                         preferences: Dict, constraints: Dict) -> List[Tuple[str, float]]:
    ...
    def _calculate_model_score(self, model: ModelCapability, task_type: TaskType, 
                              complexity: float, preferences: Dict) -> float:
        ...
        if complexity > 0.7 and model.tier in [ModelTier.PREMIUM, ModelTier.SPECIALIZED]:
    ...
    def _create_routing_result(self, model_name: str, messages: List[Dict], 
                              task_type: TaskType, reason: str) -> Dict:
    ...
    def record_request_outcome(self, model_name: str, task_type: TaskType, success: bool, 
                              response_time: float, quality_rating: float = None):
        ...
        self.logger.info(f"📊 Updated performance for {model_name} on {task_type.value}: "
    ...
    def suggest_model_for_task(self, task_description: str, budget_constraint: float = None) -> Dict:
...
if __name__ == "__main__":
    ...
    suggestion = router.suggest_model_for_task("Help me debug this complex algorithm", budget_constraint=0.005)
