...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('ai_ceo_system.log'),
    ...
    def start_system(self, components: Optional[List[str]] = None):
        ...
        try:
            ...
            for component_name in components_to_start:
                ...
                # Apply startup delay
                startup_delay = self.config['components'].get(component_name, {}).get('startup_delay', 0)
                if startup_delay > 0:
    ...
    def _check_system_resources(self) -> bool:
        ...
        try:
            ...
            if available_memory < 1.0:
                logger.error(f"❌ Insufficient memory: {available_memory:.1f}GB available (1GB required)")
                return False
            
            # Check available disk space (minimum 1GB)
            available_disk = psutil.disk_usage('.').free / (1024**3)
            if available_disk < 1.0:
                logger.error(f"❌ Insufficient disk space: {available_disk:.1f}GB available (1GB required)")
                return False
            
            logger.info(f"✅ System resources: {available_memory:.1f}GB RAM, {available_disk:.1f}GB disk")
            return True
    ...
    def _start_monitoring_dashboard(self) -> bool:
        ...
        try:
            ...
            dashboard_thread = threading.Thread(
                ...
                kwargs={'host': '0.0.0.0', 'port': self.config['components']['monitoring_dashboard'].get('port', 5000)},
                daemon=True
            )
            ...
            try:
                if hasattr(component, 'get_status'):
                    ...
                    if status.get('status') != 'running':
                        logger.warning(f"⚠️ Component {component_name} status: {status.get('status')}")
