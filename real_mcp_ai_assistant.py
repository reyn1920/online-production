...
#                     ];
                    ...
                    for (const selector of selectors) {{
                        ...
                        if (elements.length > 0) {{//Get the last response (most recent)
...
#             )
    ...
    time.localtime(response.timestamp))}\\n""
        ...
        if successful_responses:
            report += "\\n🎯 **Next Steps:**\\n"
            report += "1. Review the AI analysis above\\n"
            report += "2. Implement the suggested fixes\\n"
            report += "3. Test the solution in a development environment\\n"
            report += "4. Apply prevention strategies to avoid similar issues\\n"
