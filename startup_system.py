...
class SystemManager:
    def __init__(self, mode: str = "production"):
        ...
        self.apps_to_close = [
            ...
            "VLC", "QuickTime Player", "iTunes", "Music"
        ]
        
        # Essential apps to keep running
        self.essential_apps = [
            "Finder", "System Preferences", "Activity Monitor",
            "Terminal", "iTerm", "Trae", "Sublime Text",
            "Python", "Node", "uvicorn", "Ollama", "Chrome"
        ]
        ...
        try:
            ...
            for req_file in requirements_files:
                ...
                if result.returncode == 0:
                    ...
                    logger.warning(f"⚠️ Failed to install from {req_file}: {result.stderr[:200]}...")
                    continue
            else:
                # If all requirements files fail, continue anyway
                logger.warning("⚠️ Could not install dependencies, continuing with existing packages...")
            ...
            try:
                ...
                for app in self.apps_to_close:
                    if app.lower() in proc_name.lower():
                        ...
                        is_essential = any(essential.lower() in proc_name.lower() 
                ...
                for name, proc_info in self.processes.items():
                    ...
                    if process.poll() is not None:
                        ...
                        if proc_info['restart_count'] < 5:
                            ...
                            logger.info(f"✅ {name} restarted (attempt {proc_info['restart_count']})")
                        else:
        ...
        try:
            ...
            if self.process_watchdog:
                watchdog_thread = threading.Thread(
                    target=self._run_process_watchdog,
                    daemon=True
                )
