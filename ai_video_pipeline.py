...
class DaVinciResolveAutomation:
    ...
    def _initialize_resolve(self):
        ...
        try:
            ...
            print(
                "Please ensure DaVinci Resolve is installed and external scripting is enabled"
            )
            self.resolve = None
...
"""
        ...
        script_path = os.path.join(
            self.temp_script_dir, f"enhance_{int(time.time())}.py"
        )
    ...
    def enhance_video(
        ...
        script_path = self.create_enhancement_script(
            video_path, output_path, environment_type
        )
...
class AIVideoProductionPipeline:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.blender_automation = BlenderAutomation(
            self.config["blender"]["executable_path"]
        )
    ...
    def check_system_requirements(self):
        ...
        if self.blender_automation.is_available():
            ...
            print("⚠ Blender is not available")
            print(
                "  Please ensure Blender is installed and accessible via command line"
            )
    ...
    def enhance_with_blender(
        ...
        enhanced_output = os.path.join(
            self.temp_dir, f"enhanced_avatar_{int(time.time())}.mp4"
        )
    ...
    def finalize_in_resolve(
        ...
        if not self.resolve_automation.is_available():
            print(
                "⚠ DaVinci Resolve not available, copying enhanced video as final output"
            )
            final_output = os.path.join(
                self.output_dir, f"final_production_{int(time.time())}.mp4"
            )
        ...
        try:
            ...
            if additional_assets:
                media_files.extend(
                    [asset for asset in additional_assets if os.path.exists(asset)]
                )
