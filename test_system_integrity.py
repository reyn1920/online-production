...
are 100% complete, fully integrated, and working correctly in a live production environment.
...
class TestSystemStartupAndHealth:
    ...
    def test_launch_live_script_startup(self, system_test):
        ...
        assert system_test.launch_process.poll() is None, "launch_live.py process terminated unexpectedly"
        ...
        for attempt in range(max_retries):
            try:
                ...
                if response.status_code == 200:
                    ...
                    assert health_data.get("status") == "healthy", f"Health check failed: {health_data}"
                    return
...
class TestCoreToolVerification:
    ...
    def test_ollama_server_connectivity(self, system_test):
        ...
        try:
            ...
            assert response.status_code == 200, f"Ollama server returned status {response.status_code}"
    ...
    def test_initialize_core_agents(self):
        ...
        try:
            ...
            for agent_class, agent_name in agent_classes:
                ...
                assert hasattr(agent_class, 'capabilities'), f"{agent_name} missing capabilities property"
                ...
                assert issubclass(agent_class, BaseAgent), f"{agent_name} does not inherit from BaseAgent"
    ...
    def test_full_video_creation_workflow(self, system_test):
        ...
        try:
            response = requests.post(
                f"{system_test.dashboard_url}/api/tasks",
                json=task_data,
                timeout=10
            )
        ...
        while time.time() - start_time < max_wait_time:
            try:
                response = requests.get(
                    f"{system_test.dashboard_url}/api/tasks/{task_id}",
                    timeout=5
                )
        ...
        assert final_status == "completed", f"Task did not complete successfully. Final status: {final_status}"
        ...
        assert video_found, "No recent .mp4 video file found in output directories after workflow completion"
