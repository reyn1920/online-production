...
#         )
        ...
        logging.getLogger(__name__).info("Media Dashboard initialized")
...
#                             )
        ...
        return jsonify({"success": True, "workflows": workflows})
                ...
                logging.getLogger(__name__).error(f"Error getting workflows: {e}")
        return jsonify({"success": False, "error": str(e)}), 500
...
#                     )
        ...
        def handle_disconnect():
            ...
            logging.getLogger(__name__).info(f"Client {client_id} disconnected")
        ...
        def monitor_loop():
            while True:
                try:
                    ...
                    logging.getLogger(__name__).error(f"Monitoring error: {e}")
                    time.sleep(5)

        monitoring_thread = threading.Thread(target = monitor_loop, daemon = True)
        monitoring_thread.start()
        logging.getLogger(__name__).info("Monitoring thread started")
...
#                 )
    ...
    def run(self, host: str = None, port: int = None, debug: bool = None):
        ...
        logging.getLogger(__name__).info(f"Starting Media Dashboard on {host}:{port}")
...
<body class="bg - gray - 100 min - h - screen">
    ...
    <script>//Initialize Socket.IO connection
        const socket = io();//Dashboard state
        let dashboardData = {
            metrics: {},
                activeJobs: {},
                jobHistory: []
