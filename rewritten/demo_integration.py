...
Today we'll examine our key performance indicators,
market expansion strategies, and future growth opportunities.
Our AI-driven approach has revolutionized our operations.
""",
        "educational_content": """
In this lesson, we'll explore the fundamentals of artificial intelligence
and its applications in modern video production.
From automated editing to AI-generated avatars,
technology is transforming how we create content.
"""
    }
    ...
    print(f"✓ Created sample assets in: {assets_dir}")
...
def demo_basic_integration():
    ...
    # Run basic pipeline
    result = pipeline.run_complete_pipeline(script_text=script_text, environment_type="studio")
...
def demo_advanced_integration():
    ...
    for file_path in Path(assets_dir).glob("*"):
        if file_path.suffix.lower() in [".mp4", ".mov", ".avi", ".png", ".jpg"]:
            if "background" in file_path.name.lower() or "logo" in file_path.name.lower():
