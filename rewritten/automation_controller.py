    ...
    def start_automation(self):
        ...
        try:
            ...
            if self.config["rss_monitoring_enabled"]:
                self.rss_thread = threading.Thread(target=self._run_rss_monitoring, daemon=True)
    ...
    def _setup_api_routes(self):
        ...
        def get_performance_metrics():
            ...
            try:
                ...
                errors = []
                for row in cursor.fetchall():
                    errors.append({"component": row[0], "error_type": row[1], "count": row[2]})

                conn.close()

                return jsonify({"metrics": metrics, "errors": errors, "period_days": days})
