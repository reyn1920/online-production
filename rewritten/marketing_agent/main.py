...
class MarketingConfig:
    ...
    def __init__(self):
        ...
        self.google_search_console_credentials = os.getenv("GOOGLE_SEARCH_CONSOLE_CREDENTIALS")
...
class TwitterManager:
    ...
    async def post_tweet(self, content: str, media_path: Optional[str] = None) -> Dict[str, Any]:
        """Post a tweet"""
        if not self.client:
    ...
    def _get_tweet_metrics(self, tweet_id: str) -> Dict[str, Any]:
        """Get real tweet metrics"""
        try:
            if self.client:
...
class EmailMarketingManager:
    ...
    def _get_campaign_metrics(self, campaign_id: str) -> Dict[str, Any]:
        """Get real email campaign metrics"""
        try:
            if self.sendgrid_client:
                # Use SendGrid Stats API to get campaign metrics
...
class SEOManager:
    ...
    async def get_trending_keywords(self, category: str = "technology") -> List[Dict[str, Any]]:
        """Get trending keywords for content optimization"""
        try:
            return self._get_real_trending_keywords(category)
        except Exception as e:
            logger.error(f"Trending keywords fetch failed: {e}")
            return []

    def _get_real_trending_keywords(self, category: str = "technology") -> List[Dict[str, Any]]:
        """Get real trending keywords using Google Trends"""
        try:
            if self.trends_client:
                ...
                for keyword in trending_searches[0][:10]:  # Top 10 trending
                    try:
                        # Build payload for the keyword
                        self.trends_client.build_payload([keyword], cat=0, timeframe="today 3-m")
                        interest_data = self.trends_client.interest_over_time()
                            ...
                            keywords.append(
                                {
                                    ...
                                    "competition": "medium",  # Default since exact competition data requires paid API
                                    "trend": (
                                        "rising"
...
class AffiliateManager:
    ...
    async def generate_affiliate_link(
        ...
        try:
            return self._generate_real_affiliate_link(product_url, network)
        except Exception as e:
            logger.error(f"Affiliate link generation failed: {e}")
            return {"success": False, "error": str(e)}

    def _generate_real_affiliate_link(self, product_url: str, network: str) -> Dict[str, Any]:
        """Generate real affiliate links based on network"""
        try:
            if network.lower() == "amazon":
                ...
                if affiliate_id:
                    affiliate_url = f"https://www.shareasale.com/u.cfm?d=1234567&m=12345&u={affiliate_id}&urllink={product_url}"
                    return {
                        "success": True,
                        "affiliate_url": affiliate_url,
                        "commission_rate": 8.0,
            ...
            return {
                "success": False,
                "error": f"Network {network} not configured or supported",
...
class MarketingAgent:
    ...
    def setup_routes(self):
        ...
        async def generate_affiliate_link(product_url: str, network: str = "amazon"):
            ...
            try:
                result = await self.affiliate_manager.generate_affiliate_link(product_url, network)
