...
class Face_3DMM(nn.Module):
    def __init__(self, modelpath, id_dim, exp_dim, tex_dim, point_num):
        ...
        DMM_info = np.load(os.path.join(modelpath, "3DMM_info.npy"), allow_pickle=True).item()
        ...
        keys_info = np.load(os.path.join(modelpath, "keys_info.npy"), allow_pickle=True).item()
...
#         )
    ...
    def forward_geo(self, id_para, exp_para):
        id_para = id_para * self.sig_id
        exp_para = exp_para * self.sig_exp
        geometry = torch.mm(id_para, self.base_id) + torch.mm(exp_para, self.base_exp) + self.mu
