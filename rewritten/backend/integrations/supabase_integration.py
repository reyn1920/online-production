...
try:
    ...
    class Client:
        """Fallback Supabase Client for when supabase-py is not available."""

        def __init__(self, supabase_url: str, supabase_key: str, **kwargs):
            self.supabase_url = supabase_url
            self.supabase_key = supabase_key
            self.logger = setup_logger(__name__)
            self.logger.warning("Supabase client not available - using fallback")

        def table(self, table_name: str):
            return FallbackTable(table_name, self.logger)

        def storage(self):
            return FallbackStorage(self.logger)
    ...
    class FallbackTable:
        ...
        def select(self, *args):
            self.logger.warning(f"Supabase table.select not available for {self.table_name}")
            return self

        def insert(self, data):
            self.logger.warning(f"Supabase table.insert not available for {self.table_name}")
            return self

        def update(self, data):
            self.logger.warning(f"Supabase table.update not available for {self.table_name}")
            return self

        def delete(self):
            self.logger.warning(f"Supabase table.delete not available for {self.table_name}")
            return self
    ...
    class FallbackBucket:
        ...
        def upload(self, path, file_data, options=None):
            self.logger.warning(f"Supabase storage upload not available for {self.bucket_name}")
            return {"data": None, "error": "Supabase not available"}

        def download(self, path):
            self.logger.warning(f"Supabase storage download not available for {self.bucket_name}")
            return {"data": None, "error": "Supabase not available"}

    class FallbackAuth:
        """Fallback auth operations."""

        def __init__(self, logger):
            self.logger = logger

        def sign_up(self, credentials):
            self.logger.warning("Supabase auth.sign_up not available")
            return {"user": None, "error": "Supabase not available"}

        def sign_in(self, credentials):
            self.logger.warning("Supabase auth.sign_in not available")
            return {"user": None, "error": "Supabase not available"}
...
#             )
            ...
            for cred_data in response.data:
                credential = SecureCredential(**cred_data, encrypted_value="[ENCRYPTED]")
