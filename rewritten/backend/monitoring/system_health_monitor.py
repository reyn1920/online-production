            ...
            if len(self.historical_data) >= 10:
                ...
                if len(recent_values) >= 5:
                    ...
                    if failure_probability > 0.7:
                        self._generate_optimization_suggestions(metric, failure_probability)
        ...
        try:
            ...
            if metric.status in [HealthStatus.CRITICAL, HealthStatus.FAILED]:
                ...
                pattern["avg_failure_threshold"] = (
                    pattern["avg_failure_threshold"] * (pattern["failure_count"] - 1) + metric.value
            ...
            if pattern["failure_count"] > 0:
                ...
                threshold_similarity = 1.0 - abs(
                    metric.value - pattern["avg_failure_threshold"]
                ) / max(pattern["avg_failure_threshold"], 1.0)
                ...
                if pattern["last_failure_time"]:
                    ...
                    # Higher risk if recent failure or if it's been too long (indicating overdue)
