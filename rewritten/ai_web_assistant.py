...
Respond with clear, actionable advice."""
    ...
    async def _interact_with_service(self, service: str, query: str) -> str:
        ...
        if service == "abacus":
            return "Abacus AI Analysis: Based on your query, I recommend checking the data pipeline \
    and model configuration. The issue might be related to feature engineering or model validation."
        elif service == "gemini":
            return "Gemini Response: I can help you debug this code. The error suggests a schema mismatch. Try adding proper column validation \
    and ensure your database migrations are up to date."
        elif service == "chatgpt":
            return "ChatGPT Analysis: This looks like a common database schema issue. Here's what I recommend: 1) Check if the column exists, 2) Run proper migrations, 3) Add error handling for missing columns."
