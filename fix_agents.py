...
def patch_right_perspective_firewall():
    ...
    for path, txt in candidates:
        ...
        class_match = re.search(r"class\s+RightPerspectiveFirewall\s*(?:\([^)]*\))?\s*:\s*(?:\n\s+\"\"\".*?\"\"\"\s*)?", txt, re.S)
        if not class_match:
        ...
        if "_rule_cache" in txt:
            details.append({"file": str(path), "patched": False, "reason": "cache_already_present"})
            continue
        ...
        if has_init:
            ...
            if init_match:
                ...
                if "self._rule_cache" not in init_body:
                    ...
                    if cache_added:
                        ...
                        patched_any = True
                        details.append({"file": str(path), "patched": True, "reason": "added_cache_to_init"})
                    else:
                        details.append({"file": str(path), "patched": False, "reason": "could_not_locate_init_body"})
                else:
                    details.append({"file": str(path), "patched": False, "reason": "cache_already_in_init"})
        else:
            ...
            patched_any = True
            details.append({"file": str(path), "patched": True, "reason": "added_init_with_cache"})
...
def patch_content_agent():
    ...
    for path, txt in candidates:
        ...
        has_shutil_import = re.search(r"^\s*import\s+shutil\s*$|^\s*from\s+\S+\s+import\s+.*shutil", txt, re.M)
        ...
        if has_shutil_usage and not has_shutil_import:
            ...
            for i, line in enumerate(lines):
                if line.strip().startswith('import ') or line.strip().startswith('from '):
                    continue
                elif line.strip() == '' and i > 0 and (lines[i-1].strip().startswith('import ') or lines[i-1].strip().startswith('from ')):
            ...
            patched_any = True
            details.append({"file": str(path), "patched": True, "reason": "added_shutil_import"})
        else:
            reason = "no_shutil_usage" if not has_shutil_usage else "import_already_present"
...
def patch_database(db_path: Path):
    ...
    try:
        ...
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='api_discovery_tasks';")
        ...
        if table_exists:
            ...
            if task_type_col and task_type_col[3] == 1:  # NOT NULL constraint exists
                print("  📝 Recreating api_discovery_tasks table to remove NOT NULL constraint...")
                ...
                if existing_data:
                    ...
                    for row in existing_data:
                        ...
                        if len(row) >= 6:
                            cursor.execute("INSERT INTO api_discovery_tasks (id, task_type, status, created_at, updated_at, data) VALUES (?, ?, ?, ?, ?, ?);", row[:6])
                        else:
                            # Pad with defaults if row is shorter
                            padded_row = list(row) + [None] * (6 - len(row))
                            cursor.execute("INSERT INTO api_discovery_tasks (id, task_type, status, created_at, updated_at, data) VALUES (?, ?, ?, ?, ?, ?);", padded_row[:6])
