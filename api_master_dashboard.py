...
try:
    ...
    print(
        "⚠️  Required modules not found. Make sure api_registration_automation.py and api_testing_suite.py are in the same directory."
    )
    sys.exit(1)
...
class APIMasterDashboard:
    ...
    def get_dashboard_stats(self) -> Dict:
        ...
        registered_count = sum(
            1 for status in self.api_status.values() if status.registered
        )
        has_key_count = sum(1 for status in self.api_status.values() if status.has_key)
        tested_count = sum(
            1 for status in self.api_status.values() if status.last_tested
        )
        working_count = sum(
        ...
        for cost_tier in ["FREE", "FREEMIUM", "PAID"]:
            cost_apis = [
                s for s in self.api_status.values() if s.cost_tier == cost_tier
            ]
    ...
    def display_dashboard(self):
        ...
        print(
            f"✅ Registered: {stats['registered']} ({stats['registration_rate']:.1f}%)"
        )
        print(f"🔑 Has Keys: {stats['has_keys']}")
        print(f"🧪 Tested: {stats['tested']}")
        print(
            f"💚 Working: {stats['working']} ({stats['success_rate']:.1f}% success rate)"
        )
    ...
    def register_api_interactive(self, api_key: str):
        ...
        if success:
            ...
            if api_key in self.api_status:
                ...
                self.api_status[api_key].has_key = bool(
                    os.getenv(API_REGISTRY[api_key]["env_var"])
                )
                self.save_status()
    ...
    def generate_registration_plan(self):
        ...
        unregistered_apis = [
            (k, v) for k, v in self.api_status.items() if not v.registered
        ]
        ...
        try:
            while True:
                ...
                critical_apis = [
                    ...
                    if v.priority == "high" and v.has_key
                ]
