...
try:
    from backend.agents.specialized_agents import (
        SystemAgent, ResearchAgent, ContentAgent, MarketingAgent
    )
...
try:
    from backend.integrations.research_validation_service import ResearchValidationService
except ImportError as e:
...
class AutonomousOrchestrator:
    ...
    def __init__(self, main_loop=None):
        ...
        self.thread_pool = ThreadPoolExecutor(max_workers=10, thread_name_prefix="TRAE-Agent")
    ...
    def _initialize_agents(self):
        ...
        if SystemAgent:
            try:
                ...
                logger.info("✅ SystemAgent initialized: System monitoring and management")
            except Exception as e:
        ...
        if ResearchAgent:
            try:
                ...
                logger.info("✅ ResearchAgent initialized: Research and intelligence gathering")
            except Exception as e:
        ...
        if ContentAgent:
            try:
                ...
                logger.info("✅ ContentAgent initialized: Content creation and evolution")
            except Exception as e:
        ...
        if MarketingAgent:
            try:
                agent = MarketingAgent(
                    ...
                    name="Marketing & Promotion Agent"
                )
                ...
                logger.info("✅ MarketingAgent initialized: Marketing automation and strategy")
            except Exception as e:
        ...
        if FinancialAgent:
            try:
                ...
                logger.info("✅ FinancialAgent initialized: Financial analysis and trading automation")
            except Exception as e:
        ...
        if YouTubeEngagementAgent:
            try:
                agent = YouTubeEngagementAgent(
                    db_path='data/youtube_engagement.sqlite'
                )
                ...
                logger.info("✅ YouTubeEngagementAgent initialized: YouTube content engagement and optimization")
            except Exception as e:
                logger.error(f"❌ Failed to initialize YouTubeEngagementAgent: {e}")
                self.update_agent_status('YouTubeEngagementAgent', 'error', error=str(e))
        
        # Progressive Self Repair Agent
        if ProgressiveSelfRepairAgent:
            try:
                agent = ProgressiveSelfRepairAgent(
                    config={'agent_id': 'progressive_self_repair_agent', 'name': 'Self Repair Agent'}
                )
                ...
                logger.info("✅ ProgressiveSelfRepairAgent initialized: System self-repair and optimization")
            except Exception as e:
                logger.error(f"❌ Failed to initialize ProgressiveSelfRepairAgent: {e}")
                self.update_agent_status('ProgressiveSelfRepairAgent', 'error', error=str(e))
        ...
        if ProactiveNicheDominationAgent:
            try:
                ...
                logger.info("✅ ProactiveNicheDominationAgent initialized: Market niche identification and domination")
            except Exception as e:
                logger.error(f"❌ Failed to initialize ProactiveNicheDominationAgent: {e}")
                self.update_agent_status('ProactiveNicheDominationAgent', 'error', error=str(e))
        ...
        if EvolutionAgent:
            try:
                ...
                logger.info("✅ EvolutionAgent initialized: Continuous system evolution and adaptation")
            except Exception as e:
    ...
    def _initialize_news_monitoring(self):
        ...
        if NewsMonitoringService:
            try:
                ...
                logger.info("✅ News Monitoring Service initialized: Political RSS feed monitoring for Right Perspective")
            except Exception as e:
    ...
    def update_agent_status(self, agent_name: str, status: str, task_id: Optional[str] = None, error: Optional[str] = None):
    ...
    def get_system_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'active_agents': len([a for a in self.agent_states.values() if a.get('status') not in ['error', 'stopped']]),
        ...
        while self.running and not self.shutdown_event.is_set():
            try:
                ...
                if self.task_queue:
                    ...
                    if task:
                        logger.info(f"📋 {agent_name} processing task: {task.get('id', 'unknown')}")
                        with self.agent_state_lock:
                            self.update_agent_status(agent_name, 'processing', task_id=task.get('id'))
                        
                        # Process the task with timeout and defensive programming
                        try:
                            if hasattr(agent, 'process_task') and callable(getattr(agent, 'process_task', None)):
                                await asyncio.wait_for(agent.process_task(task), timeout=60.0)
                            elif hasattr(agent, 'execute') and callable(getattr(agent, 'execute', None)):
                                await asyncio.wait_for(agent.execute(task), timeout=60.0)
                            else:
                                logger.warning(f"⚠️  Agent {agent_name} has no callable process_task or execute method")
                        except asyncio.TimeoutError:
                            logger.warning(f"⏰ Task timeout for agent {agent_name}")
                        except Exception as e:
                            logger.error(f"❌ Task processing error for {agent_name}: {e}")
                        
                        agent_info['task_count'] += 1
                        logger.info(f"✅ {agent_name} completed task: {task.get('id', 'unknown')}")
                
                # Agent-specific autonomous operations with defensive programming
                if hasattr(agent, 'autonomous_cycle') and callable(getattr(agent, 'autonomous_cycle', None)):
                    try:
                        await asyncio.wait_for(agent.autonomous_cycle(), timeout=30.0)
                    except asyncio.TimeoutError:
                        logger.warning(f"⏰ Autonomous cycle timeout for agent {agent_name}")
                    except Exception as e:
    ...
    async def _get_next_task_for_agent(self, agent_name: str) -> Optional[Dict[str, Any]]:
    ...
    def _run_agent_thread(self, agent_name: str):
        ...
        try:
            ...
            if self.main_loop and not self.main_loop.is_closed():
                ...
                future = asyncio.run_coroutine_threadsafe(self._run_agent_loop(agent_name), self.main_loop)
                ...
                try:
                    ...
                    if last_heartbeat:
                        ...
                        if time_since_heartbeat > 120:  # 2 minutes
                            logger.warning(f"⚠️  {agent_name} heartbeat timeout ({time_since_heartbeat:.1f}s)")
    ...
    def control_agent(self, agent_id: str, action: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to control agent {agent_id} with action {action}: {e}")
            return False
...
def main():
    ...
    try:
        ...
        logger.error(f"❌ Database migration failed: {e} - continuing with startup but some features may not work")
    ...
    loop_thread = threading.Thread(target=run_event_loop, daemon=True, name="AsyncEventLoop")
    loop_thread.start()
    ...
    try:
        ...
        if DashboardApp:
            ...
            try:
                ...
                dashboard_port = int(os.getenv('DASHBOARD_PORT') or os.getenv('PORT') or '8080')
                logger.info(f"[dashboard] attempting to bind to http://127.0.0.1:{dashboard_port}")
                ...
                def _routes():
                    return {
                        ...
                        "routes": [{"rule": r.rule, "methods": list(r.methods)} for r in dashboard_app.app.url_map.iter_rules()]
                    }
                ...
                if getattr(e, "errno", None) in (48, 98):  # EADDRINUSE mac/linux
                    ...
                    try:
                        ...
                        logger.info(f"✅ Dashboard started on fallback port {fallback_port}")
                    except Exception as fallback_e:
                        logger.error(f"❌ Fallback port {fallback_port} also failed: {fallback_e}")
                        import traceback
        ...
        if orchestrator.news_monitoring_service:
            ...
            try:
                ...
                logger.info("✅ News Monitoring Service started - monitoring political RSS feeds")
            except Exception as e:
        ...
        logger.info(f"   • Active Agents: {stats['active_agents']}/{stats['total_agents']}")
        logger.info(f"   • Memory Usage: {stats['memory_usage']:.1f}%")
        logger.info(f"   • Database Health: {'✅' if stats['database_health'] else '❌'}")
        ...
        heartbeat_counter = 0
        while orchestrator.running and not orchestrator.shutdown_event.is_set() and not _shutdown_event.is_set():
            ...
            if heartbeat_counter % 300 == 0:
                stats = orchestrator.get_system_stats()
                logger.info(f"💓 System heartbeat - Uptime: {stats['uptime']}, Active Agents: {stats['active_agents']}")
