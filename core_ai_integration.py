...
class CoreAIIntegration:
    ...
    def __init__(self):
        ...
        self.platforms = {
            AIPlatform.CHATGPT: {
                ...
                "status": "available",
                "capabilities": ["coding", "debugging", "analysis", "writing", "problem_solving"],
                "last_used": None,
                "usage_count": 0,
            ...
            AIPlatform.GEMINI: {
                ...
                "status": "available", 
                "capabilities": ["research", "analysis", "reasoning", "multimodal", "data_processing"],
                "last_used": None,
                "usage_count": 0,
            ...
            AIPlatform.ABACUS: {
                ...
                "status": "available",
                "capabilities": ["data_science", "ml_insights", "analytics", "predictions", "optimization"],
                "last_used": None,
                "usage_count": 0,
    ...
    def get_platform_for_task(self, task_type: str) -> List[AIPlatform]:
        ...
        return self.task_mappings.get(task_type, [AIPlatform.CHATGPT, AIPlatform.GEMINI, AIPlatform.ABACUS])
    ...
    async def process_with_multiple_platforms(self, prompt: str, task_type: str = "general", platforms: Optional[List[AIPlatform]] = None) -> Dict[str, AIResponse]:
    ...
    def get_cost_analytics(self) -> Dict[str, Any]:
        ...
        for platform_enum, platform_info in self.platforms.items():
            platform_costs[platform_enum.value] = {
                ...
                "remaining_free_requests": max(0, platform_info["monthly_limit"] - platform_info["usage_count"]),
                "cost_efficiency": platform_info["total_cost"] / max(1, platform_info["usage_count"])
            }
    ...
    def _get_cost_recommendations(self, analytics: Dict[str, Any]) -> List[str]:
        ...
        for platform, data in analytics["platform_breakdown"].items():
            if data["cost_efficiency"] > 0.005:  # High cost per request
                recommendations.append(f"Consider reducing usage of {platform} - high cost per request (${data['cost_efficiency']:.4f})")
            
            if data["remaining_free_requests"] <= 5:  # Near free tier limit
                recommendations.append(f"Approaching free tier limit for {platform} - {data['remaining_free_requests']} requests remaining")
        ...
        recommendations.append("Web platforms (ChatGPT, Gemini, Abacus AI) are cost-effective - no API fees required")
        recommendations.append("Browser automation allows unlimited usage within platform free tiers")
        
        if not recommendations:
            recommendations.append("AI usage is cost-efficient - no immediate optimizations needed")
...
async def ask_ai(prompt: str, platform: AIPlatform = AIPlatform.CHATGPT, task_type: str = "general") -> AIResponse:
...
if __name__ == "__main__":
    ...
    async def demo():
        ...
        response = await ask_ai("Analyze this code for optimization opportunities", AIPlatform.CHATGPT, "code_analysis")
