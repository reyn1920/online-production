...
import httpx
from shared_utils import http_with_fallback, get_secret, gambling_enabled, Provider
...
PROVIDERS = {
    "news": [
        ...
        Provider("guardian", "The Guardian", "https://open-platform.theguardian.com", "UK news and opinion"),
        Provider("nytimes", "New York Times", "https://developer.nytimes.com", "US news of record")
    ],
    "ai": [
        Provider("arxiv", "arXiv", "https://arxiv.org", "Academic papers and preprints"),
        Provider("semanticscholar", "Semantic Scholar", "https://www.semanticscholar.org", "AI research papers")
    ],
    "wellness": [
        Provider("usda", "USDA FoodData", "https://fdc.nal.usda.gov", "Nutritional information"),
        Provider("edamam", "Edamam", "https://developer.edamam.com", "Recipe and nutrition API")
    ],
    "sports": [
        Provider("balldontlie", "Ball Don't Lie", "https://www.balldontlie.io", "NBA games and stats"),
        Provider("thesportsdb", "TheSportsDB", "https://www.thesportsdb.com", "Sports data and schedules"),
        Provider("football_data", "Football-Data.org", "https://www.football-data.org", "Soccer/football data")
    ],
    "finance": [
        Provider("finnhub", "Finnhub", "https://finnhub.io", "Stock market data"),
        Provider("alphavantage", "Alpha Vantage", "https://www.alphavantage.co", "Financial data and forex"),
    ...
    "geocoding": [
        Provider("openmeteo_geo", "Open-Meteo Geocoding", "https://open-meteo.com", "Location geocoding")
    ],
    "pets": [
        Provider("thedogapi", "TheDogAPI", "https://api.thedogapi.com", "Dog breeds and images"),
        Provider("thecatapi", "TheCatAPI", "https://api.thecatapi.com", "Cat breeds and images"),
        Provider("petfinder", "Petfinder", "https://api.petfinder.com", "Pet adoption listings")
    ],
    "pets_affiliates": [
        Provider("chewy", "Chewy", "https://www.chewy.com/affiliates", "Pet supplies affiliate program"),
        Provider("petco", "Petco", "https://www.petco.com/shop/en/petcostore/content/affiliate-program", "Pet retailer affiliate program"),
        Provider("petsmart", "PetSmart", "https://www.petsmart.com/help/affiliate-program/", "Pet store affiliate program"),
        Provider("rover", "Rover", "https://join.rover.com/affiliates/", "Pet sitting/walking affiliate program"),
        Provider("embark", "Embark Dog DNA", "https://www.embarkvet.com/affiliates/", "Dog DNA testing affiliate program"),
        Provider("barkbox", "BarkBox", "https://www.barkbox.com/affiliates", "Pet subscription box affiliate program")
    ],
    "pets_birds_data": {
    ...
    "pets_birds": [
        Provider("ebird", "eBird (Bird Sightings)", "https://api.ebird.org/v2", "Bird observation and sighting data")
    ],
    "pets_misc": [
        Provider("zoo_animal_api", "Zoo Animal API (Random Animal)", "https://zoo-animal-api.vercel.app", "Random animal facts and information"),
        Provider("dog_ceo", "Dog CEO (Images/Breeds)", "https://dog.ceo", "Dog breed images and information")
    ],
    "pets_fish": [
        Provider("fishwatch", "NOAA FishWatch (Species Info)", "https://www.fishwatch.gov", "Fish species and sustainability information")
    ],
    "pets_care": [
        Provider("vetster", "Vetster (Tele-vet)", "https://api.vetster.com", "Veterinary telemedicine services"),
        Provider("pawp", "Pawp (Tele-vet)", "https://api.pawp.com", "Online veterinary consultations"),
        Provider("airvet", "Airvet (Tele-vet)", "https://api.airvet.com", "Virtual veterinary care platform")
    ],
    "scheduling": [
        Provider("calendly", "Calendly (Scheduling)", "https://api.calendly.com", "Appointment scheduling and calendar management")
    ],
    "gambling": [
        Provider("theoddsapi", "The Odds API", "https://the-odds-api.com", "Sports betting odds"),
        Provider("apisports_odds", "API-Sports", "https://rapidapi.com/api-sports", "Sports odds via RapidAPI")
    ],
    ...
    async def do(client: httpx.AsyncClient, p):
        if p.id == "guardian":
            ...
            r = await client.get(
                ...
                params={"api-key": key, "q": topic, "order-by":"newest","page-size":10,"show-fields":"trailText,thumbnail"},
            )
            r.raise_for_status()
            js = r.json()
            return [{"title":x["webTitle"],"url":x["webUrl"],"source":"Guardian"} for x in js["response"]["results"]]
        if p.id == "newsapi":
            key = get_secret("NEWSAPI_KEY")
            r = await client.get(
                "https://newsapi.org/v2/everything",
                headers={"Authorization": key},
                params={"q": topic, "sortBy":"publishedAt","pageSize":10,"language":"en"},
            )
            r.raise_for_status()
            js = r.json()
            return [{"title":a["title"],"url":a["url"],"source":a.get("source",{}).get("name","NewsAPI")} for a in js.get("articles",[])]
        if p.id == "gdelt":
            # GDELT query: recent English articles with keyword
            r = await client.get(
                "https://api.gdeltproject.org/api/v2/doc/doc",
                params={"query": topic, "mode":"ArtList","timespan":"1d","format":"json","maxrecords":50},
            )
            r.raise_for_status()
            js = r.json()
            return [{"title":a["title"],"url":a["url"],"source":"GDELT"} for a in js.get("articles",[])]
        if p.id == "hackernews":
            # HN Search by Algolia (no key)
            r = await client.get("https://hn.algolia.com/api/v1/search", params={"query": topic, "tags": "story"})
            r.raise_for_status()
            js = r.json()
            return [{"title":h["title"],"url":h["url"] or f"https://news.ycombinator.com/item?id={h['objectID']}", "source":"HN"} for h in js.get("hits",[])]
        if p.id == "gnews":
            key = get_secret("GNEWS_KEY")
            r = await client.get("https://gnews.io/api/v4/search", params={"q": topic, "lang":"en","max":10,"token": key})
            r.raise_for_status(); js = r.json()
            return [{"title":a["title"],"url":a["url"],"source":"GNews"} for a in js.get("articles",[])]
        raise RuntimeError("unknown news provider")
    results = await http_with_fallback("news", do, prefer=["guardian","newsapi","gdelt","gnews","hackernews"])
...
async def tech_news(topic: str = Query("gpu")):
    async def do(client: httpx.AsyncClient, p):
        if p.id == "producthunt":
            ...
            q = {"query": f'query {{ posts(order: RANKING, first: 10, query: "{topic}") {{ edges {{ node {{ name url tagline }} }} }} }}'}
            r = await client.post("https://api.producthunt.com/v2/api/graphql",
                                  headers={"Authorization": f"Bearer {token}","Content-Type":"application/json"},
                                  json=q)
            r.raise_for_status(); js = r.json()
            edges = js.get("data",{}).get("posts",{}).get("edges",[])
            return [{"title":e["node"]["name"],"url":e["node"]["url"],"source":"ProductHunt"} for e in edges]
        if p.id == "github":
            r = await client.get("https://api.github.com/search/repositories",
                                 params={"q": topic, "sort":"stars","order":"desc","per_page":10},
                                 headers={"Accept":"application/vnd.github+json"})
            r.raise_for_status(); js = r.json()
            return [{"title":repo["full_name"],"url":repo["html_url"],"source":"GitHub"} for repo in js.get("items",[])]
        if p.id == "hackernews":
            r = await client.get("https://hn.algolia.com/api/v1/search", params={"query": topic, "tags": "story"})
            r.raise_for_status(); js = r.json()
            return [{"title":h["title"],"url":h["url"] or f"https://news.ycombinator.com/item?id={h['objectID']}", "source":"HN"} for h in js.get("hits",[])]
    ...
    async def do(client: httpx.AsyncClient, p):
        if p.id == "arxiv":
            ...
            for e in entries:
                ...
                if t and l:
                    out.append({"title":t.group(1).strip(), "url": l.group(1).strip(), "source":"arXiv"})
            return out
        if p.id == "paperswithcode":
            r = await client.get("https://paperswithcode.com/api/v1/search/", params={"q": q, "format":"json"})
            r.raise_for_status(); js = r.json()
            return [{"title":it["paper_title"],"url":it["paper_url"],"source":"PapersWithCode"} for it in js.get("results",[])]
        raise RuntimeError("unknown knowledge provider")
    results = await http_with_fallback("knowledge", do, prefer=["arxiv","paperswithcode","wikimedia"])
    ...
    async def do(client: httpx.AsyncClient, p):
        if p.id == "usda_fdc":
            ...
            for item in js.get("foods", []):
                out.append({"desc": item.get("description"), "brand": item.get("brandOwner"),
                            "calories": next((n["value"] for n in item.get("foodNutrients", []) if n.get("nutrientName")=="Energy"), None),
                            "source":"USDA"})
            return out
        if p.id == "openfda":
            r = await client.get("https://api.fda.gov/food/enforcement.json", params={"search": food, "limit": 5})
            # openFDA returns 404 when no results; treat as empty
            if r.status_code == 404:
                return []
            r.raise_for_status(); js = r.json()
            return [{"desc": rec.get("reason_for_recall"), "brand": rec.get("product_description"), "calories": None, "source":"openFDA"}
...
async def sports_data(sport: str = Query("basketball")):
    async def do(client: httpx.AsyncClient, p):
        if p.id == "balldontlie":
            r = await client.get("https://www.balldontlie.io/api/v1/games", params={"seasons[]": "2023", "per_page": 10})
            r.raise_for_status(); js = r.json()
            return [{"title": f"{g['home_team']['full_name']} vs {g['visitor_team']['full_name']}", "score": f"{g['home_team_score']}-{g['visitor_team_score']}", "source": "BallDontLie"} for g in js.get("data", [])]
        if p.id == "thesportsdb":
            key = get_secret("THESPORTSDB_KEY")
            r = await client.get(f"https://www.thesportsdb.com/api/v1/json/{key}/searchteams.php", params={"t": sport})
            r.raise_for_status(); js = r.json()
            return [{"title": t["strTeam"], "league": t.get("strLeague"), "source": "TheSportsDB"} for t in js.get("teams", []) or []]
        if p.id == "football_data":
            key = get_secret("FOOTBALL_DATA_KEY")
            r = await client.get("https://api.football-data.org/v4/competitions", headers={"X-Auth-Token": key})
            r.raise_for_status(); js = r.json()
            return [{"title": c["name"], "area": c.get("area", {}).get("name"), "source": "Football-Data"} for c in js.get("competitions", [])]
        raise RuntimeError("unknown sports provider")
    results = await http_with_fallback("sports", do, prefer=["balldontlie", "thesportsdb", "football_data"])
...
async def finance_data(symbol: str = Query("AAPL")):
    async def do(client: httpx.AsyncClient, p):
        if p.id == "alphavantage":
            ...
            r = await client.get("https://www.alphavantage.co/query", params={"function": "GLOBAL_QUOTE", "symbol": symbol, "apikey": key})
            r.raise_for_status(); js = r.json()
            quote = js.get("Global Quote", {})
            return [{"symbol": quote.get("01. symbol"), "price": quote.get("05. price"), "change": quote.get("09. change"), "source": "AlphaVantage"}] if quote else []
        if p.id == "finnhub":
            key = get_secret("FINNHUB_KEY")
            r = await client.get("https://finnhub.io/api/v1/quote", params={"symbol": symbol, "token": key})
            r.raise_for_status(); js = r.json()
            return [{"symbol": symbol, "price": js.get("c"), "change": js.get("d"), "source": "Finnhub"}]
        if p.id == "coingecko":
            r = await client.get("https://api.coingecko.com/api/v3/simple/price", params={"ids": symbol.lower(), "vs_currencies": "usd", "include_24hr_change": "true"})
            r.raise_for_status(); js = r.json()
            data = js.get(symbol.lower(), {})
            return [{"symbol": symbol, "price": data.get("usd"), "change": data.get("usd_24h_change"), "source": "CoinGecko"}] if data else []
        if p.id == "coincap":
            r = await client.get(f"https://api.coincap.io/v2/assets/{symbol.lower()}")
            r.raise_for_status(); js = r.json()
            asset = js.get("data", {})
            return [{"symbol": asset.get("symbol"), "price": asset.get("priceUsd"), "change": asset.get("changePercent24Hr"), "source": "CoinCap"}] if asset else []
        raise RuntimeError("unknown finance provider")
    results = await http_with_fallback("finance", do, prefer=["alphavantage", "finnhub", "coingecko", "coincap"])
...
async def weather_data(location: str = Query("New York")):
    async def do(client: httpx.AsyncClient, p):
        if p.id == "openmeteo":
            r = await client.get("https://api.open-meteo.com/v1/forecast", params={"latitude": "40.7128", "longitude": "-74.0060", "current_weather": "true"})
            r.raise_for_status(); js = r.json()
            current = js.get("current_weather", {})
            return [{"location": location, "temperature": current.get("temperature"), "windspeed": current.get("windspeed"), "source": "OpenMeteo"}]
        if p.id == "weather_gov":
            ...
            if forecast_url:
                ...
                r2.raise_for_status(); js2 = r2.json()
                periods = js2.get("properties", {}).get("periods", [])
                return [{"location": location, "forecast": p.get("detailedForecast"), "temperature": p.get("temperature"), "source": "Weather.gov"} for p in periods[:3]]
            return []
...
async def sports_standings(comp: str = Query("PL")):
    async def do(client, p):
        if p.id == "football_data":
            key = get_secret("FOOTBALL_DATA_KEY")
            r = await client.get(f"https://api.football-data.org/v4/competitions/{comp}/standings",
            ...
            for s in js.get("standings", []):
                if s.get("type") == "TOTAL":
                    for t in s.get("table", []):
                        table.append({
                            "pos": t.get("position"), "team": t["team"]["name"],
                            "played": t.get("playedGames"), "gd": t.get("goalDifference"),
                            "points": t.get("points")
                        })
            return table
        if p.id == "thesportsdb":
            ...
            out = [{"recent_match": f"{m.get('strHomeTeam')} vs {m.get('strAwayTeam')}", "score": m.get("intHomeScore"), "score2": m.get("intAwayScore")} for m in (js.get("teams") or [])]
            return out
        raise RuntimeError("unknown sports provider")
    return {"competition": comp, "table": await http_with_fallback("sports", do, prefer=["football_data","thesportsdb"])}
...
async def finance_quotes(symbols: str = Query("AAPL,BTC")):
    ...
    async def do(client, p):
        if p.id == "finnhub":
            key = get_secret("FINNHUB_KEY"); out = []
            for s in syms:
                r = await client.get("https://finnhub.io/api/v1/quote", params={"symbol": s, "token": key})
                if r.status_code == 429: r.raise_for_status()
                if r.status_code == 200:
                    q = r.json(); out.append({"symbol": s, "price": q.get("c"), "high": q.get("h"), "low": q.get("l"), "source":"Finnhub"})
            return out
        if p.id == "alphavantage":
            key = get_secret("ALPHAVANTAGE_KEY"); out=[]
            for s in syms:
                func = "GLOBAL_QUOTE" if "-" not in s and not s.endswith("USD") else "CURRENCY_EXCHANGE_RATE"
                params = {"apikey": key}
                if func == "GLOBAL_QUOTE": params.update({"function": func, "symbol": s})
                else:
                    base, quote = (s.split("-")[0], s.split("-")[1]) if "-" in s else (s.replace("USD",""), "USD")
                    params.update({"function":"CURRENCY_EXCHANGE_RATE","from_currency": base,"to_currency": quote})
                r = await client.get("https://www.alphavantage.co/query", params=params)
                r.raise_for_status(); js = r.json()
                if func == "GLOBAL_QUOTE":
                    g = js.get("Global Quote",{}); price = g.get("05. price")
                else:
                    g = js.get("Realtime Currency Exchange Rate",{}); price = g.get("5. Exchange Rate")
                out.append({"symbol": s, "price": float(price) if price else None, "source":"AlphaVantage"})
            return out
        if p.id == "coingecko":
            ...
            for s in syms:
                ...
                if r.status_code == 200:
                    js = r.json()
                    cid = next((c["id"] for c in js.get("coins",[]) if c.get("symbol","").upper()==s.upper()), None)
                    if cid: ids.append(cid)
            if not ids: return []
            r = await client.get("https://api.coingecko.com/api/v3/simple/price", params={"ids": ",".join(ids), "vs_currencies": "usd"})
            r.raise_for_status(); js = r.json()
            out=[]
        ...
        if p.id == "coincap":
            out=[]
            for s in syms:
                r = await client.get(f"https://api.coincap.io/v2/assets", params={"search": s})
                r.raise_for_status(); js = r.json()
                if js.get("data"):
                    a = js["data"][0]; out.append({"symbol": a["symbol"], "price": float(a["priceUsd"]), "source":"CoinCap"})
            return out
        raise RuntimeError("unknown finance provider")
    prefer = ["finnhub","alphavantage","coingecko","coincap"]
    return {"symbols": syms, "quotes": await http_with_fallback("finance", do, prefer=prefer)}

# --- WEATHER: geocode + forecast + US alerts
@router.get("/weather/forecast")
async def weather_forecast(city: str = Query(None), lat: float = Query(None), lon: float = Query(None)):
    async def do_geo(client, p):
        if p.id == "openmeteo_geo":
            r = await client.get("https://geocoding-api.open-meteo.com/v1/search", params={"name": city, "count": 1})
            r.raise_for_status(); js = r.json()
            if not js.get("results"): return None
            r0 = js["results"][0]; return r0["latitude"], r0["longitude"], r0.get("name")
        raise RuntimeError("unknown geocoder")

    if city and (lat is None or lon is None):
        latlon = await http_with_fallback("geocoding", do_geo, prefer=["openmeteo_geo"])
        if not latlon: raise HTTPException(404, "City not found")
        lat, lon, _ = latlon
    ...
    async def do_weather(client, p):
        ...
        if p.id == "meteostat":
            r = await client.get("https://meteostat.net/api/point/daily", params={"lat": lat, "lon": lon})
            if r.status_code == 404: return {}
            r.raise_for_status(); return r.json()
        raise RuntimeError("unknown weather provider")
    ...
    async def do_alerts(client, p):
        if p.id == "weather_gov":
            r = await client.get("https://api.weather.gov/alerts/active", params={"point": f"{lat},{lon}"})
            if r.status_code in (404, 204): return []
            r.raise_for_status(); js = r.json()
            return [{"event": f["properties"]["event"], "severity": f["properties"].get("severity"), "headline": f["properties"].get("headline")} for f in js.get("features",[])]
...
async def odds_lines(sport: str = Query("americanfootball_nfl"), regions: str = Query("us,us2"), markets: str = Query("h2h,spreads,totals")):
    if not gambling_enabled():
        raise HTTPException(status_code=403, detail="Gambling features are disabled. Enable with GAMBLING_FEATURES_ENABLED=true.")
    async def do(client, p):
        if p.id == "theoddsapi":
            key = get_secret("THEODDSAPI_KEY")
            r = await client.get("https://api.the-odds-api.com/v4/sports/{}/odds".format(sport),
                                 params={"apiKey": key, "regions": regions, "markets": markets, "oddsFormat": "american"})
            r.raise_for_status(); js = r.json()
            out=[]
            for game in js:
                out.append({
                    "commence": game.get("commence_time"), "home": game.get("home_team"), "away": game.get("away_team"),
                    "books": [{"book": b.get("bookmaker"),
                               "h2h": next((m["outcomes"] for m in b.get("markets",[]) if m["key"]=="h2h"), None),
                               "spreads": next((m["outcomes"] for m in b.get("markets",[]) if m["key"]=="spreads"), None),
                               "totals": next((m["outcomes"] for m in b.get("markets",[]) if m["key"]=="totals"), None)} for b in game.get("bookmakers",[])]
                })
            return out
        if p.id == "apisports_odds":
            key = get_secret("RAPIDAPI_KEY")
            # demo: API-FOOTBALL odds (soccer) using RapidAPI headers
            r = await client.get("https://api-football-v1.p.rapidapi.com/v3/odds",
                                 headers={"X-RapidAPI-Key": key, "X-RapidAPI-Host":"api-football-v1.p.rapidapi.com"},
                                 params={"date": "2024-08-01"}) # adjust as needed
            r.raise_for_status(); js = r.json()
            # normalize minimally
            out=[]
            for r0 in js.get("response", []):
                ...
                out.append({"commence": fixture.get("date"), "home": None, "away": None,
                            "books": [{"book": b.get("name"), "bets": [bet.get("name") for bet in b.get("bets",[])]} for b in bks]})
            return out
        raise RuntimeError("unknown odds provider")
    return {"sport": sport, "regions": regions, "markets": markets,
...
async def gambling_odds(sport: str = Query("americanfootball_nfl")):
    ...
    if not gambling_enabled():
        raise HTTPException(
            status_code=403,
            detail="Gambling features are not enabled on this instance"
        )
    
    async def do(client: httpx.AsyncClient, p):
        if p.id == "theoddsapi":
            key = get_secret("THEODDSAPI_KEY")
            r = await client.get("https://api.the-odds-api.com/v4/sports/americanfootball_nfl/odds", params={"apiKey": key, "regions": "us", "markets": "h2h"})
            r.raise_for_status(); js = r.json()
            return [{"game": f"{g['home_team']} vs {g['away_team']}", "commence_time": g.get("commence_time"), "bookmakers": len(g.get("bookmakers", [])), "source": "TheOddsAPI"} for g in js[:5]]
        raise RuntimeError("unknown gambling provider")
    results = await http_with_fallback("gambling", do, prefer=["theoddsapi"])
    return {"sport": sport, "items": results, "disclaimer": "Gambling odds are for informational purposes only. Please gamble responsibly."}
...
async def get_dog_breeds(limit: int = Query(10)):
    ...
    async def do(client: httpx.AsyncClient, p):
        if p.id == "thedogapi":
            ...
            r = await client.get(f"https://api.thedogapi.com/v1/breeds?limit={limit}", headers=headers)
            r.raise_for_status()
            js = r.json()
            return [{"id": breed.get("id"), "name": breed.get("name"), "temperament": breed.get("temperament"), "life_span": breed.get("life_span"), "source": "TheDogAPI"} for breed in js]
...
async def get_dog_images(breed_id: str = Query(None), limit: int = Query(1)):
    ...
    async def do(client: httpx.AsyncClient, p):
        if p.id == "thedogapi":
            ...
            r = await client.get("https://api.thedogapi.com/v1/images/search", params=params, headers=headers)
            r.raise_for_status()
            js = r.json()
            return [{"id": img.get("id"), "url": img.get("url"), "width": img.get("width"), "height": img.get("height"), "source": "TheDogAPI"} for img in js]
        raise RuntimeError("unknown pets provider")
    results = await http_with_fallback("pets", do, prefer=["thedogapi"])
...
async def get_cat_breeds(limit: int = Query(10)):
    ...
    async def do(client: httpx.AsyncClient, p):
        if p.id == "thecatapi":
            ...
            if key:
                headers["x-api-key"] = key
            r = await client.get(f"https://api.thecatapi.com/v1/breeds?limit={limit}", headers=headers)
            r.raise_for_status()
            js = r.json()
            return [{"id": breed.get("id"), "name": breed.get("name"), "temperament": breed.get("temperament"), "life_span": breed.get("life_span"), "source": "TheCatAPI"} for breed in js]
        raise RuntimeError("unknown pets provider")
...
async def get_cat_images(breed_id: str = Query(None), limit: int = Query(1)):
    ...
    async def do(client: httpx.AsyncClient, p):
        if p.id == "thecatapi":
            ...
            if breed_id:
                params["breed_ids"] = breed_id
            r = await client.get("https://api.thecatapi.com/v1/images/search", params=params, headers=headers)
            r.raise_for_status()
            js = r.json()
            return [{"id": img.get("id"), "url": img.get("url"), "width": img.get("width"), "height": img.get("height"), "source": "TheCatAPI"} for img in js]
        raise RuntimeError("unknown pets provider")
    results = await http_with_fallback("pets", do, prefer=["thecatapi"])
    return {"breed_id": breed_id, "limit": limit, "images": results}

@router.get("/pets/adoptions")
async def get_pet_adoptions(animal_type: str = Query("dog"), location: str = Query("10001"), limit: int = Query(20)):
    ...
    async def do(client: httpx.AsyncClient, p):
        if p.id == "petfinder":
            ...
            r = await client.get("https://api.petfinder.com/v2/animals", params=params, headers=headers)
            r.raise_for_status()
            js = r.json()
            return [{"id": animal.get("id"), "name": animal.get("name"), "type": animal.get("type"), "breed": animal.get("breeds", {}).get("primary"), "age": animal.get("age"), "gender": animal.get("gender"), "url": animal.get("url"), "source": "Petfinder"} for animal in js.get("animals", [])]
        raise RuntimeError("unknown pets provider")
    results = await http_with_fallback("pets", do, prefer=["petfinder"])
    return {"animal_type": animal_type, "location": location, "limit": limit, "adoptions": results}
    ...
    try:
        ...
        for provider in filtered_providers:
            program_info = {
                ...
                "category": "supplies" if provider.id in ["chewy", "petco", "petsmart", "barkbox"] else "services" if provider.id == "rover" else "health"
            }
...
async def get_bird_sightings(region: str = Query("US"), limit: int = Query(10)):
    ...
    async def do(client: httpx.AsyncClient, p):
        if p.id == "ebird":
            ...
            r = await client.get(f"https://api.ebird.org/v2/data/obs/{region}/recent?back=7&maxResults={limit}", headers=headers)
            r.raise_for_status()
            js = r.json()
            return [{"species": obs.get("comName"), "scientific_name": obs.get("sciName"), "location": obs.get("locName"), "date": obs.get("obsDt"), "count": obs.get("howMany"), "source": "eBird"} for obs in js]
...
async def fish_species(limit: int = Query(50, ge=1, le=500)):
    ...
    return {"provider": "static_fallback", "count": len(limited_data), "species": limited_data}
        ...
        if p.id == "zoo_animal_api":
            ...
            for _ in range(limit):
                try:
                    ...
                    r.raise_for_status()
                    js = r.json()
                    animals.append({"name": js.get("name"), "latin_name": js.get("latin_name"), "habitat": js.get("habitat"), "diet": js.get("diet"), "source": "Zoo Animal API"})
                except:
                    break
            return animals
        elif p.id == "dog_ceo":
    ...
    try:
        ...
        for provider in providers:
            service_info = {
                ...
                "url": provider.base_url,
                "description": provider.description,
                "features": ["appointment_booking", "calendar_integration", "automated_reminders"]
            }
