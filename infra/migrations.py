...
def _table_exists(conn, name: str) -> bool:
    with closing(conn.cursor()) as c:
        c.execute("SELECT 1 FROM sqlite_master WHERE type IN ('table','view') AND name=?;", (name,))
...
def _ensure_news_intelligence(conn):
    ...
    if not _table_exists(conn, "news_intelligence"):
        ...
        if _table_exists(conn, "news_articles"):
            ...
            summary_col = "COALESCE(summary, content, description, '')" if any(col in cols for col in ["summary", "content", "description"]) else "''"
            date_col = "COALESCE(created_at, published_at, updated_at, CURRENT_TIMESTAMP)" if any(col in cols for col in ["created_at", "published_at", "updated_at"]) else "CURRENT_TIMESTAMP"
            ...
            """)
    # Helpful indexes
    conn.execute("CREATE INDEX IF NOT EXISTS idx_news_intelligence_created_at ON news_intelligence(created_at DESC);")
    conn.execute("CREATE UNIQUE INDEX IF NOT EXISTS ux_news_intelligence_url ON news_intelligence(url);")
...
def _ensure_discovery_tasks(conn):
    ...
    if not _column_exists(conn, "discovery_tasks", "task_name"):
        ...
        if _column_exists(conn, "discovery_tasks", "task_type"):
            conn.execute("UPDATE discovery_tasks SET task_name = task_type WHERE task_name IS NULL;")
