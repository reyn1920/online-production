    ...
    def supervise_worker_task(
        ...
        while iteration < self.max_iterations:
            ...
            try:
                ...
                if not success:
                    print(
                        f"❌ [Guardian Supervisor] Worker reported failure on attempt {iteration}"
                    )
                    continue
            ...
            if validation_result.failed == 0 and validation_result.errors == 0:
                ...
                print(
                    f"🎉 [Guardian Supervisor] Task '{task_name}' completed successfully in {iteration} iterations."
                )
        ...
        print(
            f"⚠️ [Guardian Supervisor] Maximum iterations ({self.max_iterations}) reached without full success."
        )
...
def example_integration_workflow():
    ...
    print(
        f"   Total Tests: {validation_result['test_results']['passed'] + validation_result['test_results']['failed']}"
    )
