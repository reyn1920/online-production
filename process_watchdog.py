...
class ProcessWatchdog:
    ...
    def _monitor_service(self, service_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if service_name in self.restart_history:
                status["restart_count_last_hour"] = len(
                    self.restart_history[service_name]
                )
            ...
            if needs_restart and config.critical:
                ...
                logger.info(
                    f"ℹ️ Service {service_name} needs restart but is not critical"
                )
    ...
    def start_watchdog(self):
        ...
        logger.info(
            f"Monitoring {len(self.services)} services: {list(self.services.keys())}"
        )
