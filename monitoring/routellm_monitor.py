...
class CreditMonitor:
    ...
    def __init__(self, config_path: str = None, db_path: str = None):
        ...
        self.db_path = (
            db_path
            or "/Users / thomasbrianreynolds / online production / data / routellm_usage.db"
        )
    ...
    def record_usage(self, metric: UsageMetric):
        ...
        cursor.execute(
            ...
            (timestamp, credits_used, model_used, response_time_ms, success, user_id, request_type)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """,
            (
    ...
    def _check_usage_alerts(self):
        ...
        hourly_limit = self.config["monitoring"]["credit_tracking"][
            "hourly_usage_limit"
        ]
        ...
        # Daily usage alerts
        if daily_credits >= daily_limit and not self._recent_alert_exists(
            "DAILY_LIMIT_REACHED"
        ):
            self._trigger_alert(
                Alert(
        ...
        # Hourly usage alerts
        if hourly_credits >= hourly_limit and not self._recent_alert_exists(
    ...
    def _setup_default_alerts(self):
        ...
        def file_alert_handler(alert: Alert):
            alert_log_path = (
                "/Users / thomasbrianreynolds / online production / logs / routellm_alerts.log"
            )
    ...
    def _monitoring_loop(self):
        ...
        update_frequency = self.config["monitoring"]["credit_tracking"][
    ...
    def _generate_daily_summary_if_needed(self):
        ...
        cursor.execute(
            "SELECT COUNT(*) FROM daily_summaries WHERE date = ?", (yesterday,)
        )
        ...
        cursor.execute(
            ...
            SELECT
                COALESCE(SUM(credits_used), 0) as total_credits,
                    ...
                    COALESCE(SUM(CASE WHEN success THEN 1 ELSE 0 END) * 100.0 / COUNT(*), 0) as success_rate
            FROM usage_metrics
            WHERE DATE(timestamp) = ?
        """,
            (yesterday,),
        ...
        cursor.execute(
            ...
            (date, total_credits_used, total_requests, avg_response_time_ms, success_rate, top_models)
            VALUES (?, ?, ?, ?, ?, ?)
        """,
            (
                yesterday,
