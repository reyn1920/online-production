...
class CreditMonitor:
    ...
    def __init__(self, config_path: str = None, db_path: str = None):
        ...
        self.db_path = db_path or "/Users/thomasbrianreynolds/online production/data/routellm_usage.db"
    ...
    def _load_current_usage(self) -> Dict:
        ...
        return {
            ...
            "usage_percentage": (monthly_credits / monthly_limit) * 100 if monthly_limit > 0 else 0
        }
    ...
    def record_usage(self, metric: UsageMetric):
        ...
        self.logger.info(f"Recorded usage: {metric.credits_used} credits, model: {metric.model_used}")
    ...
    def _check_usage_alerts(self):
        ...
        if daily_credits >= daily_limit and not self._recent_alert_exists("DAILY_LIMIT_REACHED"):
            self._trigger_alert(Alert(
                ...
                timestamp=datetime.now(),
                metric_data={"daily_usage": daily_credits, "daily_limit": daily_limit}
            ))
        ...
        if hourly_credits >= hourly_limit and not self._recent_alert_exists("HOURLY_LIMIT_REACHED"):
            self._trigger_alert(Alert(
                ...
                timestamp=datetime.now(),
                metric_data={"hourly_usage": hourly_credits, "hourly_limit": hourly_limit}
            ))
