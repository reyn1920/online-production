...
#         )
    ...
    def record_model_generation(self, model_type: str, status: str, duration: float):
        ...
        self.metrics.model_generation_requests.labels(status=status, model_type=model_type).inc()

        if status == "success":
            self.metrics.model_generation_duration.labels(model_type=model_type).observe(duration)

    def record_scaling_event(self, action: str, resource_type: str):
        """Record scaling event"""
        self.metrics.scaling_events.labels(action=action, resource_type=resource_type).inc()
