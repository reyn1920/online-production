...
class ProcessWatchdog:
    ...
    def scan_processes(self) -> Dict[int, ProcessInfo]:
        ...
        try:
            for proc in psutil.process_iter(['pid', 'name', 'cmdline', 'create_time', 'cpu_percent', 'memory_percent', 'status']):
                try:
                    info = proc.info
                    if not info['cmdline']:
                        continue
    ...
    def _get_matching_rules(self, process_name: str, cmdline: List[str]) -> List[WatchdogRule]:
        ...
        for rule in self.rules.values():
            ...
            if (rule.process_pattern.lower() in process_name.lower() or 
                rule.process_pattern.lower() in cmdline_str):
    ...
    def detect_stuck_processes(self) -> List[ProcessInfo]:
        ...
        for process in self.processes.values():
            ...
            if is_stuck:
                logger.warning(f"🔒 Stuck process detected: PID {process.pid} ({process.name}) - {stuck_reason}")
        ...
        try:
            ...
            if rule.restart_command:
                ...
                if new_process.returncode is None:
                    ...
                    self.alert_queue.put(f"Process {process.name} (PID {process.pid}) restarted successfully")
                    return True
                else:
                    logger.error(f"❌ Process restart failed with exit code: {new_process.returncode}")
                    return False
            else:
    ...
    async def monitoring_cycle(self):
        ...
        try:
            ...
            for issue_type, issue_processes in resource_issues.items():
                if issue_processes:
                    logger.warning(f"⚠️ {issue_type.replace('_', ' ').title()} issues detected in {len(issue_processes)} processes")
