...
def create_basic_video(background_image_path: str, audio_path: str, output_path: str) -> bool:
    """
    ...
    Args:
        ...
        audio_path (str): Path to the audio file (mp3, wav, etc.) - can be None for silent video
    ...
    if use_audio:
        ...
        command = [
            ...
            '-i', audio_path,
            '-vf', 'scale=1920:1080,format=yuv420p',  # Standard 1080p
            '-c:v', 'libx264',
        ...
        command = [
            ...
            '-c:v', 'libx264',
            '-tune', 'stillimage',  # Optimize for static image
            '-t', '10',  # 10 seconds duration for silent video
            '-y',  # Overwrite output file if it exists
            output_path
        ]
...
def create_basic_video_with_defaults(audio_path: str, title: str = "Video", output_dir: str = None) -> str:
    """
...
def create_default_background(output_path: str) -> bool:
    ...
    command = [
        'ffmpeg',
        '-f', 'lavfi',
        '-i', 'color=c=#1e3c72:size=1920x1080:duration=1',
        '-vf', (
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print("Usage: python basic_video_generator.py <background_image> <audio_file> [output_file]")
        sys.exit(1)
