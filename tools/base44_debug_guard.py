...
#         }
    ...
    def log_fix(self, message: str, action: Optional[str] = None):
        """Log applied fix"""
...
#         }
        ...
        logger.info(f"FIX APPLIED: {message} - {action_str}")
...
#                 ] + missing_packages, check=True, capture_output=True, text=True)
    ...
    def check_syntax_errors(self) -> bool:
        """Check for Python syntax errors using Ruff"""
        logger.info("=== CHECKING SYNTAX ERRORS WITH RUFF ===")

        # Define specific first-party directories to scan
        scan_dirs = ['app', 'backend', 'scripts', 'services', 'monetization', 'orchestrator', 'tools']
        scan_paths = []
...
#             )
            ...
            if result.returncode == 0:
                logger.info("✅ Ruff check passed - no syntax errors found")
                return True
            else:
                # Log Ruff output as warnings (not blocking errors)
                if result.stdout:
                    self.log_warning("Ruff found issues", result.stdout)
                if result.stderr:
                    self.log_warning("Ruff stderr", result.stderr)

                # Don't fail the build for linting issues, just log them
                logger.info("⚠️ Ruff found linting issues but continuing...")
                return True

        except FileNotFoundError:
            self.log_warning("Ruff not found, falling back to basic syntax check")
            # Fallback to basic syntax check for critical files only
            critical_files = [self.project_root / 'main.py']
            for py_file in critical_files:
                if py_file.exists():
                    try:
                        with open(py_file, 'r', encoding='utf-8') as f:
                            compile(f.read(), py_file, 'exec')
                        logger.info(f"✓ {py_file.name} - syntax OK")
                    except SyntaxError as e:
                        self.log_error(f"Critical syntax error in {py_file}: {e}")
                        return False
            return True
        except Exception as e:
            self.log_warning(f"Error running Ruff: {e}")
            return True  # Don't fail the build for linting tool issues
