...
FORBIDDEN = [
    ...
    r"\bYOUR_API_KEY_HERE\b", r"\bREPLACE_WITH_ACTUAL\b", r"\bCHANGE_ME\b",
    r"\bfake_api_key_12345\b", r"\btest_secret_key\b", r"\bexample_password\b",
    r"\blorem ipsum dolor\b", r"\bsit amet consectetur\b",
    r"\b123-456-7890\b", r"\buser@example\.com\b", r"\bpassword123\b",
    r"\bNOT_IMPLEMENTED_YET\b", r"\bCOMING_SOON_FEATURE\b", r"\bUNDER_CONSTRUCTION\b",
    r"\bDUMMY_DATA_HERE\b", r"\bSAMPLE_CONTENT_ONLY\b", r"\bTEST_MODE_ONLY\b"
]

EXCLUDE_DIRS = {
    ".git", "node_modules", "dist", "build", ".next", ".vercel", ".cache",
    "__pycache__", ".venv", "venv", "venv_creative", ".idea", ".vscode", "env", ".env", ".pytest_cache",
    "models", "backups", "cache", "snapshots", "test-results", "test_results", "outputs", "output"
}
SCAN_EXTS = {
    ".js",".jsx",".ts",".tsx",".mjs",".cjs",
    ".json",".yml",".yaml",".md",".html",".css",".scss",
    ".py",".sh",".zsh",".toml",".ini",".conf"
}
...
def run(cmd: str, cwd: Optional[Path] = None, env: Optional[Dict[str,str]] = None, timeout: Optional[int]=None) -> Tuple[int,str,str]:
    log(f"RUN: {cmd}")
    p = subprocess.Popen(
    ...
    try:
        ...
        return 124, out, err + "\nTIMEOUT"
    if out: 
        with LOG_PATH.open("a", encoding="utf-8") as fh: fh.write(out)
    if err:
        with LOG_PATH.open("a", encoding="utf-8") as fh: fh.write(err)
...
def pm_exec(pm: str, args: str) -> str:
    if pm == "pnpm": return f"pnpm {args}"
    if pm == "yarn": return f"yarn {args}"
    return f"npm {args}"
...
def has_eslint_config(root: Path) -> bool:
    for f in [".eslintrc", ".eslintrc.json", ".eslintrc.cjs", ".eslintrc.js", ".eslintrc.yaml", ".eslintrc.yml"]:
        if (root / f).exists(): return True
    return False
...
def scan_forbidden(root: Path) -> List[Dict[str, Any]]:
    ...
    skip_files = {
        ...
        "demo_", "_demo.", ".demo.", "demo.", "Demo", "DEMO",
        "sample_", "_sample.", ".sample.", "sample.", "Sample", "SAMPLE",
        "mock_", "_mock.", ".mock.", "mock.", "Mock", "MOCK"
    }
    ...
    for path in root.rglob("*"):
        ...
        for pat in patterns:
            for m in pat.finditer(text):
                ...
                if any(tech in ctx.lower() for tech in [
                    "placeholder=", "placeholder:", "placeholder'", 'placeholder"',
                    "# check for todo", "scanning for todo", "todo_count", "todo/fixme",
                    "grep", "found $todo", "no pending todo", "todo items",
                    "mock_data:", "tests/mock_data", "mock_data/", "mock_data=",
                    "def ", "class ", "import ", "from ", "function", "method",
                    "<input", "<textarea", "placeholder=", "id=", "type=",
                    "log_info", "log_warning", "log_success", "echo", "print",
                    "# ", "// ", "/* ", "<!-- ", "config", "yaml", "json",
                    "[redacted", "redacted]", "security scan", "weak_passwords",
                    "common_passwords", "password_list", "security_check", "scanner"
                ]):
                    continue
                ...
                issues.append({"file": str(rel), "line": line_no, "hit": m.group(0), "context": ctx})
    return issues
...
def ensure_node(pm: str):
    ...
    if rc != 0: raise RuntimeError("Node not available.")
    log(f"Node OK: {out.strip()}")
    rc, out, _ = run(f"{pm} -v" if pm != "npm" else "npm -v")
    if rc != 0: raise RuntimeError(f"{pm} not available.")
...
def node_checks(root: Path, pm: str, report: Dict[str, Any]):
    if (root / "package.json").exists():
        ...
        if has_script(root, "lint"):
            ...
            if rc != 0: raise RuntimeError("Node lint failed.")
        elif has_eslint_config(root):
            rc, _, _ = run(pm_exec(pm, "exec eslint ."), cwd=root)
            report["node_lint"] = (rc == 0)
            if rc != 0: raise RuntimeError("ESLint run failed.")
        else:
        ...
        if has_tsconfig(root):
            ...
            if rc != 0: raise RuntimeError("Type check failed.")
        else:
        ...
        if has_script(root, "build"):
            ...
            if rc != 0: raise RuntimeError("Node build failed.")
        else:
...
def serve_and_probe(dist_dir: Path, port: int = 5173, duration_sec: int = 15) -> bool:
    ...
    proc = subprocess.Popen(cmd, cwd=str(dist_dir), shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    try:
        time.sleep(2)
        ok = False
...
def pass_once(root: Path, pm: str, pass_name: str) -> Dict[str, Any]:
    ...
    if issues:
        ...
        for i, issue in enumerate(issues[:10]):
            log(f"  {i+1}. {issue['file']}:{issue['line']} - '{issue['hit']}' in: {issue['context'][:100]}")
...
def main():
    ...
    final_report: Dict[str, Any] = {"root": str(ROOT), "passes": [], "status": "unknown"}
